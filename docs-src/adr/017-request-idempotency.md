...
(`.../Bases/Cqrs/IdempotencyConventionTestsBase.cs:10`), whose single `[Fact]`
...
(`MMCA.Common/Source/Presentation/MMCA.Common.API/Controllers/AuthControllerBase.cs:93-94`), which is
the canonical double-submit: a user pressing the button twice should get one account and one response.
Login (`:69-70`), refresh (`:117-118`) and revoke (`:144-145`, with the per-session revoke at
`:207-208`) are `[NonIdempotent]`, as is the OAuth code exchange
(`.../Controllers/OAuthControllerBase.cs:149-150`). Token issuance must never be replay-cached,
...
OAuth `complete` action is a `[HttpGet]` (`OAuthControllerBase.cs:87`) and is therefore outside the
...
with no key (`.../Idempotency/IdempotencyFilter.cs:132-136`), only enables request buffering when a key
