...
`MMCA.Store.Sales.IntegrationTests/Contract/ProblemDetailsContractTests.cs:15`) and three of the four
...
`MMCA.ADC.Identity.IntegrationTests/Contract/ProblemDetailsContractTests.cs:16`): **ADC Notification
has no problem-details subclass**. The versioning contract is subclassed once per repo, on ADC
...
- **Coverage is uneven by suite.** Security headers and graceful shutdown are Gateway-only, versioning
  is one host per repo, ADC Notification has no problem-details guard, and Helpdesk has none of it.
  Every one of those is an unguarded host for that contract, not a decision that the contract does not
  apply.
