...
- **Declaration order, and one constructor shape.** An app namespace sorts above `MMCA.Common.*` for
  ...
  dropping those three to `suggestion` in the **staged** `.editorconfig` (`stage.ps1:807-810`); the
  ...
  Every other analyzer stays at error, as the delta's own header says (`stage.ps1:805`). The
...
- **`mmca-module` alone cannot finish the job.** Seven edits are beyond it because `dotnet new`
  ...
  declared `copyOnly` at `stage.ps1:1077` and exempted from the smoke's rename sweeps at
  `smoke.ps1:114`), which runs
