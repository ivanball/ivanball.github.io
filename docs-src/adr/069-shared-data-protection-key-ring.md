...
`maxReplicas: 2` (`MMCA.ADC/infra/main.bicep:1177` Identity service, `:1806` UI host). Only the
Identity service runs with **no session affinity**; the UI ingress is sticky
(`MMCA.ADC/infra/main.bicep:1725-1726`), which narrows the UI window rather than closing it, since
...
- **One `DefaultAzureCredential` instance serves both sinks** (`DataProtectionExtensions.cs:68`), so
  ...
  identity with `AZURE_CLIENT_ID` on both adopting apps (`MMCA.ADC/infra/main.bicep:1097`, `:1759`).
...
- **In ADC, infrastructure provisions one private container, not a new storage account.**
  ...
  `MMCA.ADC` (`:1095-1096`, `:1757-1758`), unconditionally. No extra role assignment is needed: the
...
- **The Azure dependencies live in the Aspire package only.**
  ...
  and pinned centrally (`MMCA.Common/Directory.Packages.props:114-115`), alongside a direct
  `System.Security.Cryptography.Xml` pin that lifts that chain's transitive off a vulnerable version
  for consumers without the ASP.NET Core framework reference (`Directory.Packages.props:120`).
...
also run at `maxReplicas: 2` (`MMCA.Store/infra/main.bicep:1447` UI host, `:1038` Identity
...
- **The blob URI is gated behind a readiness flag.** `DataProtection__ApplicationName='MMCA.Store'`
  (`:1430`) and `AZURE_CLIENT_ID` (`:1432`) are unconditional, but
  `DataProtection__BlobStorageUri` is appended only when the `dataProtectionStorageReady` parameter is
  true (default `false` at `:89`, concatenated at `:1433-1435`). The flag exists because
  ...
  (`MMCA.Store/.github/workflows/deploy.yml:1039`).
