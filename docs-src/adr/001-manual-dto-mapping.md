...
## Status
Accepted. _Mechanism clarified 2026-06-26: the per-entity mappers are Riok.Mapperly source-generated (compile-time), not hand-written line by line. The decision to avoid runtime convention/reflection mapping (AutoMapper, Mapster) is unchanged._ _Mapper count refreshed 2026-08-23: 31 DTO mappers across Store + ADC._ _Revised 2026-09-11 (recount): 33 DTO mappers across Store + ADC (20 in ADC, 13 in Store)._ _Revised 2026-09-19 (recount): 35 DTO mappers across Store + ADC (22 in ADC, 13 in Store); ADC added `PartnerDTOMapper` and `SessionAssetDTOMapper`._
...
## Trade-offs
- More files (35 DTO mappers across Store + ADC: 22 in ADC, 13 in Store, plus the parallel `IEntityRequestMapper` classes). The interface's default `MapToDTOs` implementation is available to remove the batch-mapping boilerplate, though in practice each concrete mapper re-declares the identical one-line projection rather than relying on the default.
