...
- **The apps run as one shared user-assigned managed identity, referenced as `existing`.**
  ...
  (`MMCA.ADC/infra/main.bicep:985-990`, `:1165`, `:1279`, `:1393`, `:1546`, `:1646`;
  ...
  (`MMCA.ADC/infra/main.bicep:890-894`, `MMCA.Store/infra/main.bicep:630-634`).
- **The vault is referenced, not created; the deployment writes the values into it.** The template
  declares the vault as `existing` (`MMCA.ADC/infra/main.bicep:903-905`,
  `MMCA.Store/infra/main.bicep:643-645`) and then writes secret child resources: fourteen in ADC
  (`MMCA.ADC/infra/main.bicep:907-976`) and eleven in Store
...
- **Every Container App secret entry is a `keyVaultUrl` reference bound to that identity.** ADC
  Identity (`MMCA.ADC/infra/main.bicep:1004-1019`), Conference (`:1184-1191`), Engagement
  (`:1298-1302`), Notification (`:1422-1430`); Store Identity
  ...
  (`MMCA.ADC/infra/main.bicep:1563`, `:1666`; `MMCA.Store/infra/main.bicep:1058`, `:1139`).
- **Containers consume secrets only through `secretRef`.** The SQL connection string
  (`MMCA.ADC/infra/main.bicep:1038`, `MMCA.Store/infra/main.bicep:977`), the broker connection string
  (`MMCA.ADC/infra/main.bicep:1475`, `MMCA.Store/infra/main.bicep:995`), the RSA private key and its
  HS256 fallback (`MMCA.ADC/infra/main.bicep:1095`, `:1102`; `MMCA.Store/infra/main.bicep:795`,
  `:802`), SMTP (`MMCA.ADC/infra/main.bicep:1105`, `:1490`; `MMCA.Store/infra/main.bicep:805`,
  `:1014`), the OAuth client secrets (`MMCA.ADC/infra/main.bicep:1108`, `:1112`), the Anthropic key
  (`:1233`), the native-push hub connection string (`:1488`), and the two Stripe keys
...
- **An unconfigured optional secret gets a placeholder, not a missing entry.** Optional values are
  written as the literal `unused` when their parameter is empty (`MMCA.ADC/infra/main.bicep:945`,
  `:950`, `:955`, `:960`, `:965`, `:970`, `:975`; `MMCA.Store/infra/main.bicep:675`, `:680`, `:685`,
  `:690`, `:695`, `:700`), while the app-side reference is conditional (for example `hasSmtpPassword`
  at `MMCA.ADC/infra/main.bicep:1016` and `hasStripe` at `MMCA.Store/infra/main.bicep:957-960`), so
...
- **The two role assignments are bootstrapped out of band, deliberately.** The deploy identity holds
  ...
  Contributor without role-assignment-write (`MMCA.ADC/infra/main.bicep:899-902`,
  `MMCA.Store/infra/main.bicep:639-642`). It is the same least-privilege posture that keeps ADR-045's
  avatar-storage grant behind a default-false flag (`MMCA.ADC/infra/main.bicep:112-113`, `:806-810`).
...
- **SQL authentication is staged behind `useManagedIdentitySql`, and the stage is additive.** The
  ...
  `MMCA.Store/.github/workflows/deploy.yml:1009-1027`), driven by repository variables that are
...
- **A reference has one home; a literal has as many homes as it has consumers.** Three vault secrets
  ...
  (`MMCA.ADC/infra/main.bicep:1007-1008`, `:1187-1188`, `:1300-1301`, `:1425-1426`) and all three
  Store services (`MMCA.Store/infra/main.bicep:732-733`, `:862-863`, `:954-955`), and the SMTP
  password by two apps in each (`MMCA.ADC/infra/main.bicep:1016`, `:1429`;
...
- **Reuse the identity that already existed.** The user-assigned identity was introduced to pull
  ...
  (`MMCA.ADC/infra/main.bicep:881-885`, `MMCA.Store/infra/main.bicep:621-625`). Granting it Key Vault
...
- **The template alone does not stand up an environment.** The vault, the identity, its AcrPull grant
  ...
  (`MMCA.ADC/infra/main.bicep:886-888`, `:903-905`) and cannot report that a grant is missing. The
...
- **Two literal values remain in the template.** The Application Insights connection string is an
  ...
  (`MMCA.ADC/infra/main.bicep:871-877`, `MMCA.Store/infra/main.bicep:611-617`). Both are telemetry
