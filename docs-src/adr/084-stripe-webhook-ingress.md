...
## Status
Accepted (2026-08-14). Revised 2026-08-31.
...
- **One anonymous, raw-body endpoint.** `POST /Payments/webhook` is `[AllowAnonymous]`
  ...
  (`MMCA.Store/Source/Hosts/MMCA.Store.Gateway/appsettings.json:77-80`) on a `sales` cluster that
  carries no `HttpRequest` block at all, so `Version` and `VersionPolicy` stay unset
  (`appsettings.json:101-105`), unlike `catalog` (`:83-91`) and `identity` (`:92-100`) which pin
  `Version` 2 with `RequestVersionExact` (`:85-86`, `:94-95`); the reasoning is at
...
- **200 hides processing failures from Stripe by design.** A post-acceptance failure is invisible in
  ...
  `DependencyInjection.cs:34-36`). The 200 for a post-acceptance failure is itself pinned by test
