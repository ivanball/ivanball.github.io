...
- **Three provider values, one abstraction.** `MessageBusProvider` has exactly `InProcess = 0`,
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:418-421`); for either
  broker value it replaces `IMessageBus` with `BrokerMessageBus` (`:438`) and `IEventBus` with
  `BrokerEventBus` (`:444`), so the outbox becomes the only delivery channel. No application or
...
- **Local development is RabbitMQ, wired by the AppHost.** `AddMessageBroker()` provisions the
  ...
  (`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:95,118,158,183`, broker declared at `:60`)
...
- **Production is Azure Service Bus, injected by Bicep.** Each service container app receives
  ...
  ADC at `MMCA.ADC/infra/main.bicep:1099-1100` (identity, `:1000`), `:1256` (conference, `:1186`),
  `:1373` (engagement, `:1301`), `:1502` (notification, `:1416`); Store at
  `MMCA.Store/infra/main.bicep:778-779` (identity, `:706`), `:895` (catalog, `:837`), `:994` (sales,
  `:929`). The images are the same ones the AppHost runs locally; only the two environment variables
  differ.
- **One resolution order for the connection string.** `ResolveBrokerConnectionString` prefers an
  ...
  `ConnectionStrings:messaging` (`DependencyInjection.cs:512-521`). Aspire's `WithReference` and
...
- **Retry policy is identical on both transports.** Each branch of `ConfigureBrokerTransport` calls
  ...
  `ConfigureEndpoints`: RabbitMQ at `DependencyInjection.cs:556-561`, Azure Service Bus at
  `:573-578`. The values come from one settings object: `RetryLimit` 5, `RetryMinIntervalSeconds` 1,
  ...
  (`DependencyInjection.cs:531-536`).
- **Service Bus Standard tier and `Manage` rights are forced by the topology MassTransit builds.**
  Both namespaces are `Standard`/`Standard` (`MMCA.ADC/infra/main.bicep:694-705`,
  `MMCA.Store/infra/main.bicep:526-537`) because `UsingAzureServiceBus` configures a topic per
  message type plus a subscription per consumer, and Basic supports queues only
  (`MMCA.ADC/infra/main.bicep:689-690`, `MMCA.Store/infra/main.bicep:521-525`). The `app-clients`
  authorization rule carries `Send` + `Listen` + **`Manage`** (`MMCA.ADC/infra/main.bicep:713-723`,
  `MMCA.Store/infra/main.bicep:545-555`) so `ConfigureEndpoints` can provision that topology at
  startup; without `Manage` the first publish fails with an Unauthorized topology error
  (`MMCA.ADC/infra/main.bicep:707-712`). Store sources the connection string from that dedicated rule
  rather than `RootManageSharedAccessKey`, so a later move to managed identity can revoke it without
  touching the namespace root (`MMCA.Store/infra/main.bicep:129-132`).
- **Tests use the transport the tier is testing.** A per-service integration host configures no
  ...
  (`DependencyInjection.cs:418-421`). The cross-service round-trip tier runs the real broker: the
...
- **A dedicated emulator tier exists to prove the production binding.** ADC's fixture pins
  ...
  (`MMCA.ADC/Tests/Integration/MMCA.ADC.ServiceBusEmulator.IntegrationTests/Infrastructure/ServiceBusEmulatorFixture.cs:49`)
  ...
  one-hour quota (`:63-70`), which is why the tier runs in its own test process. Store carries the
...
(`MMCA.Helpdesk/Source/Hosts/MMCA.Helpdesk.Web/Program.cs:84-86`), and its AppHost provisions no
...
- **Two brokers means two behaviors to keep aligned.** Configuration parity is enforced by one code
  ...
  RabbitMQ container does not, `DependencyInjection.cs:531-536`), so a transport-specific behavior
...
- **The emulator is not Azure Service Bus.** It imposes its own quotas (the one-hour entity TTL the
  fixture works around at `ServiceBusEmulatorFixture.cs:63-70`, a throttled admin plane), so a green
...
- **`Manage` rights are broad.** The `app-clients` rule can create and delete entities in the
  ...
  declaring every topic in Bicep (`MMCA.ADC/infra/main.bicep:707-712`).
- **Provider selection is per host and silent when missing.** A service that never receives
  ...
  error (`DependencyInjection.cs:418-421`); correctness depends on auditing the AppHost and the Bicep
  env lists, the same inventory caveat ADR-021 carries for the inbox.
- **A stale comment already exists in one AppHost.** The note above ADC's Notification registration
  still says no `WithBroker()` is wired
  (`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:82-86`) while the registration calls it at
  `:95`, which is the readability cost of keeping the choice in orchestration code.
