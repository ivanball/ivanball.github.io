...
in `HttpResultExecutor.ExecuteAsync` (`MMCA.Common.UI.Services.Api`) and hands the response to
...
(`MMCA.Common.Shared.Abstractions`, `MMCA.Common.Shared.Http`, `MMCA.Common.UI.Services.Api`) and the
