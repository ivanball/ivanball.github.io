...
**Visibility:** a speaker is publicly visible only with at least one visible session (BR-49 eligible, on a Published event) or an explicit `EventSpeaker` row on a Published event (BR-239). Hidden speakers are filtered from list, paged, lookup, and junction reads, and a GET by id returns HTTP 404 for non-privileged callers, with one carve-out: the linked speaker can always read their own profile (the BR-214 self-edit form cannot load without it), and that self-read is never stored into the shared output cache. Privileged readers (`Organizer` or `ContentEditor`) see every speaker. `Email` stays redacted from public reads regardless of visibility (BR-66).
