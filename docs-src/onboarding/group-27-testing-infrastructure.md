...
- **Concept**: an i18n fitness test. `[Rubric §27, i18n]` assesses whether localization is complete and
  ...
  [ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html) (which supersedes the single-locale [ADR-011](https://ivanball.github.io/docs/adr/011-single-locale-i18n.html), `TranslationCompletenessTests.cs:4`).
- **Walkthrough**
...
- **Why it's built this way**: exercising the module's real registration order against real command/query
  ...
  making the [ADR-014](https://ivanball.github.io/docs/adr/014-cqrs-decorator-pipeline.html) ordering executable (`DecoratorPipelineOrderTests.cs:17`).
- **Where it's used**: ADC architecture-test tier; no database needed.
...
- **Concept**: `[Rubric §16, Maintainability]` and `[Rubric §32, Dependency & Supply-Chain]`, the
  lockstep-versioning rule ([ADR-016](https://ivanball.github.io/docs/adr/016-lockstep-versioning-masstransit-pin.html)) is made executable here, matching the no-phased-rollout policy
...
- **Concept**: `[Rubric §7, Microservices Readiness]` and `[Rubric §9, API & Contract Design]`, the
  ...
  the outbox pattern ([ADR-003](https://ivanball.github.io/docs/adr/003-outbox-dual-dispatch.html)) protects delivery.
- **Walkthrough**
...
- **Concept**: `[Rubric §27, i18n]`, this is the source-code companion to
  ...
  checks that visible strings actually route through resources ([ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html),
...
- **Concept**: `[Rubric §30, Compliance/Privacy/Data Governance]`, the `PiiAttribute` / soft-delete-vs-erasure
  model ([ADR-005](https://ivanball.github.io/docs/adr/005-soft-delete-vs-erasure.html)) is pinned by the base so no PII field goes unmarked.
- **Walkthrough**
...
- **Concept**: `[Rubric §8, Data Architecture]`, this keeps the polyglot-persistence extension point ([ADR-018](https://ivanball.github.io/docs/adr/018-polyglot-persistence.html)) usable.
...
- **Walkthrough**
  ...
  - `ResetDatabaseAsync()`
    ...
    load-bearing rule for the database-per-service topology ([ADR-006](https://ivanball.github.io/docs/adr/006-database-per-service.html)): a host with multiple physical
...
- **Concept introduced, exercising the real RS256/JWKS path in tests.** `[Rubric §11, Security]`
  ...
  JWKS/RS256 validation code path ([ADR-004](https://ivanball.github.io/docs/adr/004-authentication-dual-fetch.html) authentication dual-fetch, taught in
...
- **What it is**: an opt-in fitness base that builds a real `ServiceCollection` through a repo's own
  ...
  and asserts the *runtime object graph* nests the decorators in exactly the [ADR-014](https://ivanball.github.io/docs/adr/014-cqrs-decorator-pipeline.html) order.
...
- **Concept introduced, verifying a decorator chain by unwrapping the constructed graph.** The
  ...
  *how you prove it*. Scrutor's `TryDecorate` applies decorators in **reverse registration order**, so
  ...
  `[Rubric §34, Architecture Governance]` covers the fact that a decision record ([ADR-014](https://ivanball.github.io/docs/adr/014-cqrs-decorator-pipeline.html)) is enforced
...
- **Concept introduced, the disposable-database integration fixture + environment-variable overrides.**
  ...
  database-per-service routing ([ADR-006](https://ivanball.github.io/docs/adr/006-database-per-service.html)) is why the class doc stresses the `DataSources` collapse onto a
...
- **Why it's built this way** - [ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html) (localization) requires supported cultures to be fully translated; the derived allowlist plus the minimum-count floor turn that into a self-maintaining CI gate.
