...
**4,905** distinct type nodes maps to exactly one group with no silent drops. See the
...
| [`00-inventory.md`](00-inventory.md) | Phase 0, every in-scope type (4,905 distinct), mechanically extracted, with `file:line` |
...
| 17 | [ADC Conference, Domain Model & Module Contracts](group-17-conference-domain.md) | 112 | Event/Session/Speaker/Category/Question/Partner aggregates + domain events + invariants + Shared contracts (incl. `ConferencePermissions`, the current/next-event selector + live-validation contracts) |
...
| 20 | [ADC Conference, API, gRPC Contracts & Service Host](group-20-conference-api-grpc.md) | 46 | REST controllers, `.Contracts` gRPC, the extractable service host (incl. its Kestrel config), the gRPC adapters (incl. cross-service live-validation), localized error resources |
| 21 | [ADC Conference, UI](group-21-conference-ui.md) | 168 | Conference Blazor pages + UI services (incl. the Partner management pages), the single canonical ADC Home page + its view models, the AI-scoring poll recovery tracker, calendar/QR export UI, OfflineBanner, PresenterLayout on Common theme providers |
