...
**4,257** distinct type nodes maps to exactly one group with no silent drops. See the
...
| [`00-inventory.md`](00-inventory.md) | Phase 0, every in-scope type (4,257 distinct), mechanically extracted, with `file:line` |
...
| 4 | [Domain & Integration Events + Outbox Dual-Dispatch](group-04-events-outbox.md) | 37 | Event contracts, dispatcher, transactional outbox/inbox (incl. the async `OutboxFinalizer`), message buses, `OutboxSettings` |
...
| 16 | [Aspire Orchestration & Service Defaults](group-16-aspire-orchestration.md) | 60 | AppHost wiring, ServiceDefaults, warmup, telemetry, security helpers, the shared `HttpResilienceDefaults` Polly source of truth, the shared `HealthCheckTags` liveness/readiness vocabulary |
...
| 19 | [ADC Conference, Infrastructure & Persistence](group-19-conference-infrastructure.md) | 37 | Conference DbContext registration, EF configs, seeding, infra services |
