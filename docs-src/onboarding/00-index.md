...
cross-linked. Twenty-one dependency **cycles** exist (mutually-dependent types, e.g. an aggregate root and
...
[dependency manifest](00-dependency-manifest.md#cycles-scc-size--1-21). One of the twenty-one is an
extraction artifact rather than a real dependency: the two identically-named `SelfHttpWarmupTask`
classes (`MMCA.ADC.Engagement.Service/SelfHttpWarmupTask.cs:23` and
`MMCA.ADC.Identity.Service/SelfHttpWarmupTask.cs:23`) are distinct types in different services, and the
...
globally-unique-name fallback (358 edges), and 28 references are dropped as ambiguous. The functional grouping is
then applied mechanically (`Tools/invtool/classify.ps1` → `00-group-taxonomy.md`), so every one of the
**2,864** distinct type nodes maps to exactly one group with no silent drops. See the
...
| [`00-inventory.md`](00-inventory.md) | Phase 0, every in-scope type (2,864 distinct), mechanically extracted, with `file:line` |
| [`00-dependency-manifest.md`](00-dependency-manifest.md) | Phase 1a, per-type first-party deps + computed Level; the 21 cycles |
...
| 14 | [Module System, Composition & Configuration](group-14-module-system-composition.md) | 50 | `IModule` + Kahn-ordered loader, DI composition roots, data-source attributes, options binding |
| 15 | [Common UI Framework](group-15-common-ui-framework.md) | 85 | Reusable MudBlazor building blocks: data-grid list page base, theme, common pages/services, i18n culture bootstrap + day/dark `ThemeService`, user-preference readers/writers, pseudo-localization gate, OAuth UI settings + token storage (Web/WASM), hub-channel subscriptions ([ADR-039](https://ivanball.github.io/docs/adr/039-live-channel-push.html)) |
| 16 | [Aspire Orchestration & Service Defaults](group-16-aspire-orchestration.md) | 21 | AppHost wiring, ServiceDefaults, warmup, telemetry, security helpers, the shared `HttpResilienceDefaults` Polly source of truth, the shared `HealthCheckTags` liveness/readiness vocabulary |
| 17 | [ADC Conference, Domain Model & Module Contracts](group-17-conference-domain.md) | 87 | Event/Session/Speaker/Category/Question aggregates + domain events + invariants + Shared contracts (incl. `ConferencePermissions`, the current/next-event selector + live-validation contracts) |
| 18 | [ADC Conference, Application & Use Cases](group-18-conference-application.md) | 229 | Conference CQRS handlers, validators (incl. the per-field session validation-rule family), DTOs, specs, Sessionize import, decision-support analytics, batch bookmark-count query, event-filtering-by-role handlers, calendar (.ics) export slice ([ADR-042](https://ivanball.github.io/docs/adr/042-device-capability-abstraction.html)) |
...
| 21 | [ADC Conference, UI](group-21-conference-ui.md) | 88 | Conference Blazor pages + UI services, the single canonical ADC Home page + its view models, the AI-scoring poll recovery tracker, calendar/QR export UI, OfflineBanner, PresenterLayout on Common theme providers |
| 22 | [ADC Engagement Module (Session Bookmarks)](group-22-engagement-module.md) | 78 | The async session-bookmarking slice of the Engagement bounded context: bookmark aggregate, use cases, persistence, API/contracts/service, feedback UI, the durable live-channel publish queue ([ADR-039](https://ivanball.github.io/docs/adr/039-live-channel-push.html)), the cross-service user-engagement export slice (gRPC) |
