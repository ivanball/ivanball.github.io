...
cross-linked. Twenty-six dependency **cycles** exist (mutually-dependent types, e.g. an aggregate root and
...
[dependency manifest](00-dependency-manifest.md#cycles-scc-size--1-26). Three of the twenty-six are
...
globally-unique-name fallback (481 edges), and 28 references are dropped as ambiguous. The functional grouping is
then applied mechanically (`Tools/invtool/classify.ps1` → `00-group-taxonomy.md`), so every one of the
**3,264** distinct type nodes maps to exactly one group with no silent drops. See the
...
| [`00-inventory.md`](00-inventory.md) | Phase 0, every in-scope type (3,264 distinct), mechanically extracted, with `file:line` |
| [`00-dependency-manifest.md`](00-dependency-manifest.md) | Phase 1a, per-type first-party deps + computed Level; the 26 cycles |
...
| 3 | [Querying: Specifications, Filtering & the Entity Query Service](group-03-querying-specifications.md) | 30 | Specification pattern (incl. cross-source), dynamic filter/sort/page (incl. IN-list value parsing), the generic query pipeline |
| 4 | [Domain & Integration Events + Outbox Dual-Dispatch](group-04-events-outbox.md) | 30 | Event contracts, dispatcher, transactional outbox/inbox (incl. the async `OutboxFinalizer`), message buses |
| 5 | [CQRS: Commands, Queries & the Decorator Pipeline](group-05-cqrs-pipeline.md) | 29 | Handler abstraction + the logging/transaction/caching/feature-gate/idempotency decorators (incl. the cache-stampede lock) |
| 6 | [Validation](group-06-validation.md) | 17 | FluentValidation contracts + failure mapping that gate commands |
| 7 | [Persistence & EF Core](group-07-persistence-ef-core.md) | 113 | `SQLServerDbContext` over abstract `ApplicationDbContext`, interceptors (incl. the deferred-dispatch record), repositories, engine-aware entity config, data-source routing, conventions (incl. the soft-delete unique-index convention), factories, managed file storage + image processing ([ADR-045](https://ivanball.github.io/docs/adr/045-managed-file-storage-and-avatars.html)), native-push device registrar ([ADR-044](https://ivanball.github.io/docs/adr/044-native-push-delivery.html)) |
...
| 14 | [Module System, Composition & Configuration](group-14-module-system-composition.md) | 67 | `IModule` + Kahn-ordered loader, DI composition roots, data-source attributes, options binding |
...
| 16 | [Aspire Orchestration & Service Defaults](group-16-aspire-orchestration.md) | 22 | AppHost wiring, ServiceDefaults, warmup, telemetry, security helpers, the shared `HttpResilienceDefaults` Polly source of truth, the shared `HealthCheckTags` liveness/readiness vocabulary |
...
| 19 | [ADC Conference, Infrastructure & Persistence](group-19-conference-infrastructure.md) | 29 | Conference DbContext registration, EF configs, seeding, infra services |
