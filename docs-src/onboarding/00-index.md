...
**2,588** distinct type nodes maps to exactly one group with no silent drops. See the
...
| [`00-inventory.md`](00-inventory.md) | Phase 0, every in-scope type (2,588 distinct), mechanically extracted, with `file:line` |
...
| 3 | [Querying: Specifications, Filtering & the Entity Query Service](group-03-querying-specifications.md) | 27 | Specification pattern (incl. cross-source), dynamic filter/sort/page (incl. IN-list value parsing), the generic query pipeline |
...
| 27 | [Testing & Quality Infrastructure](group-27-testing-infrastructure.md) | 1244 | All test projects + reusable Testing/Testing.E2E/Testing.UI/**Testing.Architecture** bases (incl. the handler + decorator-pipeline test bases and the Gallery auth stubs) + architecture-fitness tests + Gallery + the BenchmarkDotNet perf-smoke suite |
