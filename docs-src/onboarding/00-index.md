...
cross-linked. Twenty dependency **cycles** exist (mutually-dependent types, e.g. an aggregate root and
...
[dependency manifest](00-dependency-manifest.md#cycles-scc-size--1-20). One of the twenty is an
...
globally-unique-name fallback (348 edges), and 28 references are dropped as ambiguous. The functional grouping is
then applied mechanically (`Tools/invtool/classify.ps1` → `00-group-taxonomy.md`), so every one of the
**2,737** distinct type nodes maps to exactly one group with no silent drops. See the
...
| [`00-inventory.md`](00-inventory.md) | Phase 0, every in-scope type (2,737 distinct), mechanically extracted, with `file:line` |
| [`00-dependency-manifest.md`](00-dependency-manifest.md) | Phase 1a, per-type first-party deps + computed Level; the 20 cycles |
...
| 3 | [Querying: Specifications, Filtering & the Entity Query Service](group-03-querying-specifications.md) | 29 | Specification pattern (incl. cross-source), dynamic filter/sort/page (incl. IN-list value parsing), the generic query pipeline |
| 4 | [Domain & Integration Events + Outbox Dual-Dispatch](group-04-events-outbox.md) | 29 | Event contracts, dispatcher, transactional outbox/inbox (incl. the async `OutboxFinalizer`), message buses |
...
| 14 | [Module System, Composition & Configuration](group-14-module-system-composition.md) | 38 | `IModule` + Kahn-ordered loader, DI composition roots, data-source attributes, options binding |
...
| 22 | [ADC Engagement Module (Session Bookmarks)](group-22-engagement-module.md) | 79 | The async session-bookmarking slice of the Engagement bounded context: bookmark aggregate, use cases, persistence, API/contracts/service, feedback UI, the durable live-channel publish queue ([ADR-039](https://ivanball.github.io/docs/adr/039-live-channel-push.html)), the cross-service user-engagement export slice (gRPC) |
