...
(`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Persistence/Repositories/EFReadRepository.cs:617`)
returns a `Result<KeysetCollectionResult<TEntity>>`, turning an unknown sort column
(`EFReadRepository.cs:624-633`) or a malformed cursor (`EFReadRepository.cs:641-649`) into a `Result`
failure rather than an exception, fetching `PageSize + 1` rows so the extra row acts as a next-page
probe instead of paying for a `COUNT` (`EFReadRepository.cs:656-662`), and encoding the last kept row
into the next cursor via [`KeysetQueryBuilder`](group-07-persistence-ef-core.md#keysetquerybuilder)
(`EFReadRepository.cs:667-670`). Per
