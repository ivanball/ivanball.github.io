...
[`IEntityQueryPipeline`](#ientityquerypipeline) (`MMCA.Common/Source/Core/MMCA.Common.Application/Services/Query/IEntityQueryPipeline.cs:10`) is the execution contract, implemented by the sealed [`EntityQueryPipeline`](#entityquerypipeline) (`.../Services/Query/EntityQueryPipeline.cs:13`), which talks to the database through the [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor) abstraction rather than referencing EF Core from the Application layer ([Rubric §3, Clean Architecture]). Its inputs are bundled into [`EntityQueryParameters<TEntity>`](#entityqueryparameterstentity) (`.../Services/Query/EntityQueryParameters.cs:11`), an immutable record carrying the specification `Criteria`, the dynamic `Filters`, sort column and direction, `Fields`, page number and size, the two include flags, and the DTO-to-entity property map (defaulting to an empty `FrozenDictionary`, `EntityQueryParameters.cs:44`).

`ExecuteAsync` (`EntityQueryPipeline.cs:25`) first adds every supported navigation as an `.Include()` (`EntityQueryPipeline.cs:41`), forcing `AsSplitQuery()` when a child collection is among them (`EntityQueryPipeline.cs:48`). The comment above that line records the hard-won reason, annotated `R24/§8`: paginating a single-query collection include truncates child rows because EF applies `Skip`/`Take` to the JOIN-expanded set, so list reads returned empty child collections while by-id reads worked (`EntityQueryPipeline.cs:44`). It then applies the specification criteria and the dynamic filters **before** materializing anything (`EntityQueryPipeline.cs:55`, `EntityQueryPipeline.cs:58`), so the data source does as much of the work as possible. From there it branches. **Path 1, server-side includes** (`EntityQueryPipeline.cs:121`): sort, count before paging, `Skip`/`Take`, field-selection `Select`, materialize. **Path 2, manual navigation** (`EntityQueryPipeline.cs:75`), taken when any requested navigation crosses a physical data source and cannot be joined: sort and page at the database first, materialize the page, then invoke the `navigationPopulator` callback to batch-load those navigations in a second query (`EntityQueryPipeline.cs:106`), the [`INavigationPopulator<in TEntity>`](group-11-navigation-populators.md#inavigationpopulatorin-tentity) escape hatch of ADR-002, and apply field selection in memory afterwards (`EntityQueryPipeline.cs:113`). Both paths share one [Rubric §12, Performance] safety ceiling: an unpaginated query is capped at `MaxUnboundedResultLimit`, a public `const int` of 1000 (`EntityQueryPipeline.cs:22`, applied at `EntityQueryPipeline.cs:100` and `EntityQueryPipeline.cs:144`), so a service caller who forgets paging can never trigger an unbounded full-table load.

Which navigations are eligible, and which path each takes, is decided by [`NavigationMetadataProvider`](#navigationmetadataprovider) (`.../Services/Query/NavigationMetadataProvider.cs:20`) behind [`INavigationMetadataProvider`](#inavigationmetadataprovider) (`.../Services/Query/INavigationMetadataProvider.cs:9`). It reflects over the entity's public properties looking for [`NavigationAttribute`](group-11-navigation-populators.md#navigationattribute) (`NavigationMetadataProvider.cs:74`), unwraps collection types to find the target entity (`NavigationMetadataProvider.cs:106`), and asks [`IDataSourceService`](group-07-persistence-ef-core.md#idatasourceservice) whether the two ends share a JOIN-capable source, sorting each into the supported or unsupported bucket of [`NavigationMetadata`](group-11-navigation-populators.md#navigationmetadata) (`NavigationMetadataProvider.cs:96`). Results are cached per (entity type, [`NavigationType`](group-11-navigation-populators.md#navigationtype)) in an **instance-level** dictionary, not a static one, precisely so that a process hosting more than one data-source configuration (integration tests, for example) cannot share classifications across hosts (`NavigationMetadataProvider.cs:28`).

## The query service, the public face

[`IEntityQueryService<TEntity, TEntityDTO, TIdentifierType>`](#ientityqueryservicetentity-tentitydto-tidentifiertype) (`MMCA.Common/Source/Core/MMCA.Common.Application/Interfaces/IEntityQueryService.cs:19`) and its concrete [`EntityQueryService<TEntity, TEntityDTO, TIdentifierType>`](#entityqueryservicetentity-tentitydto-tidentifiertype) (`.../Services/EntityQueryService.cs:31`) are what controllers and handlers inject. The service is constructed from [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), the metadata provider, the pipeline, an [`IEntityDTOMapper<TEntity, TEntityDTO, TIdentifierType>`](group-12-api-hosting-mapping.md#ientitydtomappertentity-tentitydto-tidentifiertype) (ADR-001), and an `INavigationPopulator<TEntity>` (`EntityQueryService.cs:32` to `EntityQueryService.cs:36`), and it resolves its [`IReadRepository<TEntity, TIdentifierType>`](group-07-persistence-ef-core.md#ireadrepositorytentity-tidentifiertype) from the unit of work through a `virtual` property (`EntityQueryService.cs:48`).
...
- **What it is**: a tiny internal helper that splits the comma-separated value list carried by the
  `IN` filter operator into a typed list, skipping any entry that fails to parse.
...
- **Concept introduced, set membership in a URL filter, and lenient parsing as a contract.** The
  ...
  [`IFilterStrategy`](#ifilterstrategy)), so an `IN` filter has to smuggle a *set* through a single
  string. This class is the one place that decodes that convention:
  ...
  and predictable): every strategy that supports `IN` decodes the list the same way, so the client
  never has to learn a per-type list syntax. `[Rubric §15, Best Practices & Code Quality]` (assesses
  defensive, exception-free handling of untrusted input): the doc comment (`:3-7`) states the rule
  explicitly, an unparseable entry is *skipped*, matching the single-value strategies which silently
  return the unfiltered query rather than throwing, so one malformed entry never fails the request.
- **Walkthrough**
...
- **Why it's built this way**: the `Func<string, T?>` shape lets each caller pass its own
  ...
  [`IntFilterStrategy`](#intfilterstrategy)`.ApplyIn`, `IntFilterStrategy.cs:42`). Returning a
  `List<T>` rather than an array matters downstream: LINQ Dynamic binds it as the receiver of a
  `Contains` call.
- **Where it's used**: the three strategies that support `IN`,
  [`GuidFilterStrategy`](#guidfilterstrategy) (`GuidFilterStrategy.cs:36`),
  [`IntFilterStrategy`](#intfilterstrategy) (`IntFilterStrategy.cs:42`), and
  [`StringFilterStrategy`](#stringfilterstrategy) (`StringFilterStrategy.cs:45`).
...
- **What it is**: the Strategy-pattern contract for dynamic query filtering. Each implementation
  handles one CLR property-type family (string, int, DateTime, bool, decimal, Guid) and turns an
  operator key plus a raw string value into a `.Where()` clause on an `IQueryable<T>`.
...
- **Walkthrough**
  ...
  - `IReadOnlySet<string>? SupportedOperators => null` (`:24`), a **default interface member**. A
    ...
    entirely. Returning `null` means "skip operator validation" (`:21`), the tolerant default for a
    third-party strategy; the six built-in strategies override it with a `FrozenSet` so
...
- **Where it's used**: implemented by the six built-in strategies below; the registry, the dispatch,
  and the up-front validation all live in [`QueryFilterService`](#queryfilterservice).
...
> The **six built-in filter strategies** that follow share one shape: each is an `internal sealed`
...
> the operator to build a LINQ Dynamic `.Where()` string. Three rules hold across all six and are not
> repeated in every section below: values are passed as the positional parameter `@0` rather than
> concatenated into the expression text, so they cross into EF Core as SQL parameters and not as
> interpolated SQL (`[Rubric §11, Security]`, which assesses injection resistance); a value that
> fails to parse returns the **unfiltered** query rather than throwing; and the `_ =>` default arm
> returns the original query for an unrecognized operator, exactly the no-op the
> [`IFilterStrategy`](#ifilterstrategy) contract promises. The complete operator matrix, verbatim from
> source:
>
...
> | `GuidFilterStrategy` | `MMCA.Common/Source/Core/MMCA.Common.Application/Services/Filtering/GuidFilterStrategy.cs:11` | `EQUALS`, `NOT EQUALS`, `IN` |
> | `IntFilterStrategy` | `MMCA.Common/Source/Core/MMCA.Common.Application/Services/Filtering/IntFilterStrategy.cs:11` | `EQUALS`, `NOT EQUALS`, `GREATER THAN`, `LESS THAN`, `GREATER THAN OR EQUAL`, `LESS THAN OR EQUAL`, `IN` |
...
### BoolFilterStrategy
> MMCA.Common.Application · `MMCA.Common.Application.Services.Filtering` · `MMCA.Common/Source/Core/MMCA.Common.Application/Services/Filtering/BoolFilterStrategy.cs:11` · Level 1 · class (internal, sealed)

- **What it is**: the [`IFilterStrategy`](#ifilterstrategy) for `bool` and `bool?` properties, and the
  smallest member of the family: one operator, one comparison.
...
- **Walkthrough**: `SupportedOperators` is the single-entry frozen set `{ "IS" }` (`:13-16`).
  `Apply<T>` (`:18-28`) parses first and bails out before the switch: `if (!bool.TryParse(value, out
  var boolValue)) return query;` (`:20-21`), so `?isActive=IS:maybe` yields an unfiltered result
  rather than a 500. The single arm emits `query.Where($"{property} == @0", boolValue)` (`:25`).
- **Why it's built this way**: `IS` (rather than `EQUALS`) reads naturally for a boolean in a URL
  filter, and a boolean has no ordering, so no comparison operators exist to support.
...
- **Caveats / not-in-source**: there is no `IS EMPTY` / `IS NOT EMPTY` arm, so a `bool?` property
  cannot be filtered for null through this strategy.
...
- **What it is**: the [`IFilterStrategy`](#ifilterstrategy) for `DateTime` and `DateTime?`, covering
  the six temporal comparisons plus the two null checks.
- **Depends on**: [`IFilterStrategy`](#ifilterstrategy); `System.Globalization`,
...
- **Walkthrough**: `SupportedOperators` (`:17-21`) is the eight-entry frozen set. `Apply<T>`
  (`:23-41`) is a single expression-bodied `switch`, and unlike the other value strategies it parses
  **inside** each arm via a `when` clause, for example
  `"IS AFTER" when DateTime.TryParse(value, FormatProvider, DateTimeStyles.None, out var dt) =>
  query.Where($"{property} > @0", dt)` (`:30-31`). A failed parse means the `when` guard is false and
  the arm does not match, so control falls to `_ => query` (`:40`), the same unfiltered result the
  other strategies produce with an early return. The two null operators need no value at all:
  `"IS EMPTY" => query.Where($"{property} == null")` and
  `"IS NOT EMPTY" => query.Where($"{property} != null")` (`:38-39`).
- **Why it's built this way**: parsing per arm keeps the value-taking and the value-free operators in
  one switch without a pre-parse that would reject `IS EMPTY` for having no parsable value.
- **Where it's used**: registered against `typeof(DateTime)` and `typeof(DateTime?)` in
  [`QueryFilterService`](#queryfilterservice) (`QueryFilterService.cs:34-35`), as two instances.
- **Caveats / not-in-source**: there is no `BETWEEN` operator and no range shorthand; a date range is
  expressed as two filters (`IS ON OR AFTER` plus `IS ON OR BEFORE`). Nothing here normalizes to UTC:
  the parsed value is used as given (`DateTimeStyles.None`).

---

### DecimalFilterStrategy
> MMCA.Common.Application · `MMCA.Common.Application.Services.Filtering` · `MMCA.Common/Source/Core/MMCA.Common.Application/Services/Filtering/DecimalFilterStrategy.cs:12` · Level 1 · class (internal, sealed)

- **What it is**: the [`IFilterStrategy`](#ifilterstrategy) for `decimal` and `decimal?`, supporting
  equality and the four ordering comparisons.
- **Depends on**: [`IFilterStrategy`](#ifilterstrategy); `System.Globalization`,
  `System.Collections.Frozen`, `System.Linq.Dynamic.Core`.
- **Walkthrough**: `SupportedOperators` (`:14-18`) lists the six operators. `Apply<T>` (`:20-35`)
  parses once up front with the invariant culture,
  `decimal.TryParse(value, CultureInfo.InvariantCulture, out var decimalValue)` (`:22`), returning the
  unfiltered query on failure (`:23`), then switches to one of six `@0` comparisons (`:27-32`). The
  invariant culture is the load-bearing detail: it fixes `.` as the decimal separator, so a price
  filter cannot silently change meaning on a host with a different locale.
...
- **Where it's used**: registered against `typeof(decimal)` and `typeof(decimal?)` in
  [`QueryFilterService`](#queryfilterservice) (`QueryFilterService.cs:36-37`).
- **Caveats / not-in-source**: no `IN` arm (unlike `Int` and `Guid`) and no null-check operators.

---

### GuidFilterStrategy
> MMCA.Common.Application · `MMCA.Common.Application.Services.Filtering` · `MMCA.Common/Source/Core/MMCA.Common.Application/Services/Filtering/GuidFilterStrategy.cs:11` · Level 1 · class (internal, sealed)

- **What it is**: the [`IFilterStrategy`](#ifilterstrategy) for `Guid` and `Guid?`, supporting
  equality plus set membership.
- **Depends on**: [`IFilterStrategy`](#ifilterstrategy),
  [`FilterValueParser`](#filtervalueparser); `System.Collections.Frozen`,
  `System.Linq.Dynamic.Core`.
- **Concept introduced, the `IN` set-membership operator.** `[Rubric §12, Performance &
  Scalability]` (assesses whether a client can express a batch read as one query): without `IN`, a UI
  that has to fetch fifty entities by id issues fifty round trips or a chain of `OR`s. `ApplyIn`
  (`:34-38`) parses the comma list through
  [`FilterValueParser.ParseList`](#filtervalueparser) with a `static` lambda over `Guid.TryParse`
  (`:36`), then emits `query.Where($"@0.Contains({property})", values)` (`:37`). Note the inversion:
  the parsed `List<Guid>` is the *receiver* `@0` and the entity property is the argument, which is how
  LINQ Dynamic expresses `list.Contains(e.Prop)`, the shape EF Core translates to SQL `IN (...)`. An
  empty parsed list short-circuits to the unfiltered query (`:37`), so `IN` with no usable value is a
  no-op rather than a query that matches nothing.
- **Walkthrough**: `SupportedOperators` (`:13-16`) is `{ EQUALS, NOT EQUALS, IN }`. `Apply<T>`
  (`:18-32`) routes `IN` out first (`:20-21`) *before* the single-value `Guid.TryParse` (`:23`),
  because a comma-separated list would never parse as one GUID; the remaining two arms are the
  standard `@0` comparisons (`:28-29`).
- **Where it's used**: registered against `typeof(Guid)` and `typeof(Guid?)` in
  [`QueryFilterService`](#queryfilterservice) (`QueryFilterService.cs:38-39`). It is the strategy that
  serves by-id filtering wherever an entity's identifier alias resolves to `Guid`.
- **Caveats / not-in-source**: no ordering operators (GUIDs are compared for equality only) and no
  null-check operators.

---

### IntFilterStrategy
> MMCA.Common.Application · `MMCA.Common.Application.Services.Filtering` · `MMCA.Common/Source/Core/MMCA.Common.Application/Services/Filtering/IntFilterStrategy.cs:11` · Level 1 · class (internal, sealed)

- **What it is**: the [`IFilterStrategy`](#ifilterstrategy) for `int` and `int?`, the widest numeric
  surface: equality, the four ordering comparisons, and `IN`.
- **Depends on**: [`IFilterStrategy`](#ifilterstrategy),
  [`FilterValueParser`](#filtervalueparser); `System.Collections.Frozen`,
  `System.Linq.Dynamic.Core`.
- **Walkthrough**: `SupportedOperators` (`:13-17`) holds the seven operators. `Apply<T>` (`:19-38`)
  opens with the `IN` route and an inline comment stating the rule, "IN takes a comma-separated set;
  every other operator takes a single value" (`:21-23`), then the single-value `int.TryParse` guard
  (`:25-26`) and the six comparison arms (`:30-35`). `ApplyIn` (`:40-44`) mirrors
  [`GuidFilterStrategy`](#guidfilterstrategy): parse through
  [`FilterValueParser.ParseList`](#filtervalueparser) (`:42`), short-circuit on an empty list, and
  emit `@0.Contains({property})` (`:43`).
- **Why it's built this way**: `int` is the default identifier alias across most modules, so this
  ...
  gets the `IN` batch operator that `Decimal` does not.
...
- **Caveats / not-in-source**: there is no `long` strategy in the built-in table; a `long` key would
  need `QueryFilterService.RegisterStrategy` at startup (`QueryFilterService.cs:55`).
...
- **Walkthrough**: `SupportedOperators` (`:14-18`) is the nine-entry frozen set, the largest in the
  family. `Apply<T>` (`:20-30`) handles the six value-taking text operators, then delegates the rest
  ...
  `@0.Contains({property})` receiver-inverted form as the other two `IN` strategies.
...
- **Where it's used**: registered against `typeof(string)` in
  ...
  the dedicated `StringStrategy` field (`QueryFilterService.cs:47`) used for the string and
  nested-path routes in both `ApplyFilters` (`:105`) and validation (`:168`, `:203`).
...
- **Depends on**: [`IFilterStrategy`](#ifilterstrategy) and the six built-in strategies
  ...
  [`IntFilterStrategy`](#intfilterstrategy), [`StringFilterStrategy`](#stringfilterstrategy)),
...
- **Concept introduced, the strategy registry plus a validate-before-execute boundary.** `[Rubric
  ...
  `ConcurrentDictionary` (`:26-40`) seeds eleven entries, one per supported CLR type including each
  nullable variant, and `RegisterStrategy(Type, IFilterStrategy)` (`:55-60`) lets a host add a type at
...
- **Walkthrough**
  - `PropertyCache` (`:24`): `ConcurrentDictionary<(Type EntityType, string PropertyName),
    ...
    *negative* result too (the value is nullable), so a repeatedly bogus filter name does not
    re-reflect.
  - `StringStrategy` (`:47`): a dedicated [`StringFilterStrategy`](#stringfilterstrategy) instance
    held apart from the table, used for both string properties and nested paths.
  - `ApplyFilters<TEntity>` (`:71-114`): for each `(property, (op, value))` it translates the DTO name
    through `dtoToEntityPropertyMap` (`:79-81`), derives the **root** property name from a dotted path
    (`:84-86`), resolves the `PropertyInfo` through the cache (falling back from the DTO name to the
    root name, `:88-93`), and **silently skips** an unresolvable property (`:95-96`). The operator is
    uppercased once here (`:98`), which is the caller-side half of the
    [`IFilterStrategy`](#ifilterstrategy) contract. A string property or any dotted path routes to
    `StringStrategy` (`:102-107`); otherwise the per-type strategy is looked up and applied
    (`:109-110`).
  - `ValidateFilters<TEntity>` (`:124-139`): returns success for a null/empty filter map (`:128-129`),
    otherwise runs `ValidateSingleFilter` per entry and collects **all** errors into one
    [`Result`](group-01-result-error-handling.md#result) (`:136-138`), so a client sees every problem
    at once rather than one per round trip.
  - `ValidateSingleFilter<TEntity>` (`:141-185`) emits three distinct error codes:
    `Filter.Property.NotFound` when reflection cannot resolve the property (`:155-159`),
    `Filter.Type.NotSupported` when no strategy is registered for its CLR type (`:176-180`), and,
    via `ValidateOperatorSupported` (`:206-222`), `Filter.Operator.NotSupported` when the operator is
    outside the strategy's set (`:216-220`). A nested path is validated against the string operator
    set (`:166-169`). A strategy whose `SupportedOperators` is `null` skips the operator check
    entirely (`:214`).
  - `ResolvePropertyInfo` (`:187-199`) and `ResolveStrategy` (`:201-204`) are the small shared helpers
...
- **Caveats / not-in-source**: this is a **static class**, not a DI-resolved service, so
  ...
  registered as separate instances rather than a shared one (`:30-39`), which costs a few extra
...
- **Depends on**: injected through a primary constructor (`:31-36`),
  ...
  and [`INavigationPopulator<TEntity>`](group-11-navigation-populators.md#inavigationpopulatorin-tentity);
...
- **Concept introduced, one generic read pipeline for every entity.** `[Rubric §1, SOLID]`
  ...
  2. **Build and execute** through `BuildQueryAsync` (`:366-405`): pick `Repository.Table` when
     ...
     cannot join are batch-loaded after materialization (ADR-002).
...
- **Why it's built this way**: centralizing read mechanics means every entity gets identical
  ...
  is the database-per-service escape hatch (ADR-002 and ADR-006): EF cannot join across physical
