...
- **Concept introduced, upcasting a retired event contract.** `[Rubric §6, CQRS & Event-Driven]`
  ...
  `MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.Extensibility.cs:81-88`), and
  this registry converts on the way in so that exactly one handler shape exists in the codebase, the
  newest one. Registrations compose: V1 to V2 plus V2 to V3 delivers a V1 message to the V3 handler
  (`DependencyInjection.Extensibility.cs:69`).
...
- **Why it's built this way**: [ADR-090](https://ivanball.github.io/docs/adr/090-event-upcaster-registration.html)
  ...
  (`DependencyInjection.Extensibility.cs:57-58`). Envelope preservation is deliberately the registry's
  job rather than the upcaster author's (`EventUpcasterRegistry.cs:21-28` remarks): an upcaster maps
  payload fields only, and
...
- **Where it's used**: registered unconditionally as a singleton by `AddApplication`
  ...
  `MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.cs:40`), and populated by each `AddEventUpcaster<TSource, TTarget, TUpcaster>()`
  call, which appends the upcaster through `TryAddEnumerable`
  (`DependencyInjection.Extensibility.cs:81-88`, the descriptor at `:86`). Both delivery paths consume
  it: the
...
- **Where it's used**: implemented by the abstract base
  ...
  `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/Specifications/PublicSessionStatusSpecification.cs:21`);
...
- **Where it's used**: base class for
  ...
  `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.API/Controllers/Sessions/SessionsController.cs:50-51`,
...
- **Where it's used**: two ADC Conference call sites, both resolving published `Event` ids and filtering
  ...
  `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/Specifications/PublicSessionStatusSpecification.cs:24`):
