...
[`QueryFieldService`](#queryfieldservice) (`MMCA.Common/Source/Core/MMCA.Common.Application/Services/QueryFieldService.cs:16`) owns the rest of read shaping. `ApplySorting` (`QueryFieldService.cs:135`) resolves a DTO sort name through the server-authored map, and otherwise accepts the column **only** when it names a real public property of the entity (`QueryFieldService.cs:144-148`), falling back to the optional default sort when it does not (`QueryFieldService.cs:157`). That guard is deliberate and documented in the summary (`QueryFieldService.cs:119-127`): a client-supplied string can never reach Dynamic LINQ to order by nested paths or expressions the DTO does not expose. Map entries, being server-authored, may be expressions, and validation therefore accepts a map hit unconditionally instead of reflecting over the mapped value (`QueryFieldService.cs:318`, rationale at `QueryFieldService.cs:266-280`): ADC sorts speakers by `FullName` through the entry `"(FirstName + \" \" + LastName)"` (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Speakers/SpeakerEntityQueryService.cs:28-31`).

`ApplyFieldSelection` (`QueryFieldService.cs:169`) builds a `MemberInit` `Select` expression so a `fields=name,bio` request pulls only those columns from the database ([Rubric §12, Performance & Scalability]), restricted to writable properties because the projection needs setters (`QueryFieldService.cs:222-231`, the `CanWrite` filter at `QueryFieldService.cs:227`). The compiled lambda is cached per (entity type, normalized field set) (`QueryFieldService.cs:177`, `QueryFieldService.cs:191`, the cache itself at `QueryFieldService.cs:220`), and a `null` is cached on purpose to record "this field set projects nothing writable" so the miss is not recomputed per request (`QueryFieldService.cs:206-211`). `ShapeData` and `ShapeCollectionData` (`QueryFieldService.cs:75`, `QueryFieldService.cs:96`) produce the wire shape: an `ExpandoObject` (or a list of them) holding only the requested fields under camelCase keys. To make that cheap on large result sets the service caches a per-type array of [`PropertyAccessor`](#propertyaccessor) (`QueryFieldService.cs:42`), a private `readonly record struct` bundling each property's name, its precomputed camelCase key, and a compiled `Func<object, object?>` getter built with `Expression.Lambda(...).Compile()` rather than `PropertyInfo.GetValue` (`QueryFieldService.cs:46`, built at `QueryFieldService.cs:48-65`); the field-filtered subset is cached again per field set (`QueryFieldService.cs:405`, `QueryFieldService.cs:411`), under an order- and case-insensitive key so `name,id` and `Id, Name` share one entry (`QueryFieldService.cs:442-443`).

Both field-set caches are bounded, and the reason is the same one that shapes the filter cache. Their key is half client-supplied, so an entity with N properties admits up to 2^N distinct subsets and a caller could grow either dictionary by permuting the list (`QueryFieldService.cs:18-38`). The cap is a `const int MaxCacheEntries = 512` per cache (`QueryFieldService.cs:39`), with deliberately no LRU: past the cap `ApplyFieldSelection` skips server-side projection rather than admitting another key (`QueryFieldService.cs:188-189`, and the response is unchanged because shaping still trims the payload), and `GetShapedAccessors` filters per request instead (`QueryFieldService.cs:429-430`), which is a scan over an already-compiled accessor array rather than an expression rebuild. Validation mirrors the filter side: `Validate<TEntity>` rejects unknown field names and (when shaping) read-only properties (`QueryFieldService.cs:257` and the map-aware overload at `QueryFieldService.cs:292`, shared body at `QueryFieldService.cs:302`), and `ValidateSortDirection` accepts only `asc` or `desc` (`QueryFieldService.cs:355`).
...
- **Where it's used**: constructed by the concrete
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Application/Services/EntityQueryService.cs:425-437`), then handed to
...
- **Where it's used**: base class for
  ...
  [`AndSpecification`](#andspecificationtentity-tidentifiertype) /
  [`OrSpecification`](#orspecificationtentity-tidentifiertype) /
  [`NotSpecification`](#notspecificationtentity-tidentifiertype), the ownership filter
  [`OwnedByUserSpecification`](#ownedbyuserspecificationtentity-tidentifiertype) (next sections) and
  every module-specific access-control specification; it is also the declared type of the optional
  `specification` argument on the four scoped read methods of
  [`IEntityQueryService<TEntity, TEntityDTO, TIdentifierType>`](#ientityqueryservicetentity-tentitydto-tidentifiertype)
  (both `GetAllAsync` overloads, `GetEntityByIdAsync`, `GetByIdAsync`, e.g.
...
- **Where it's used**: it is the one combinator with production call sites. ADC's
  ...
  `MMCA.Common/Tests/Core/MMCA.Common.Domain.Tests/Specifications/SpecificationTests.cs:53`
...
- **Where it's used**: returned by
  ...
  `GetPublicSpeakerFilterHandler` (`.../Speakers/UseCases/GetPublicSpeakerFilter/GetPublicSpeakerFilterHandler.cs:31`).
...
- **Where it's used**: "exclude this set" predicates, composed with the other combinators and passed as
  ...
  nested inside an `OrSpecification` at `SpecificationTests.cs:126-127`, and wrapping the ownership
  filter at `OwnedByUserSpecificationTests.cs:75`.
...
- **Why it's built this way**: ownership is already recorded by the framework's audit-field convention
  ...
  composes with them (`NotSpecification` inverts it, `AndSpecification` narrows it further) with no
  extra machinery.
- **Where it's used**: ADC Conference's two question-answer controllers, which apply it for every
  ...
  (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.API/Controllers/SessionQuestionAnswersController.cs:66-67`,
  passed as the `specification` argument on its list, paged and by-id reads at
  `SessionQuestionAnswersController.cs:80`, `:108` and `:144`) and
  [`EventQuestionAnswersController`](group-20-conference-api-grpc.md#eventquestionanswerscontroller)
  (`.../Controllers/EventQuestionAnswersController.cs:66-67`, same three call sites at `:80`, `:108`,
  `:144`). Both read the caller's id from
...
- **Where it's used**: two ADC Conference call sites, both resolving published `Event` ids and filtering
  ...
  [`PublicSessionStatusSpecification.StatusCriteria`](group-18-conference-application.md#publicsessionstatusspecification)
  ...
  (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/GetPublicSessionFilter/GetPublicSessionFilterHandler.cs:29-36`),
  ...
  (`.../Common/PublicConferenceVisibility.cs:62-69`), which uses the identical criteria to resolve the
