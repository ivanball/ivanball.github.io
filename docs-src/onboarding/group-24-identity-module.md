...
- **Concept introduced, the assembly-marker pattern.** `[Rubric §2, Design Patterns]` (assesses whether the patterns in use are idiomatic and solve a real problem): instead of hard-coding an assembly-name string, a scanner takes a `typeof(...)` from a type it knows lives in the target assembly, so renaming the assembly cannot silently break discovery. `[Rubric §16, Maintainability]`: every layer of every ADC module ships this same pair, so registration and discovery code reads identically in every project; Identity's other three layers carry it too (`MMCA.ADC.Identity.Domain/AssemblyReference.cs:5,11` and `MMCA.ADC.Identity.Infrastructure/AssemblyReference.cs:5,11`).
