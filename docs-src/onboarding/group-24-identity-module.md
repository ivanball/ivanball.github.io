...
[`User`](#user), and around it sit the credential lifecycle, the role vocabulary
([`RoleNames`](#rolenames)), the change-password / password-recovery / email-confirmation /
change-preferences / avatar use cases, the administration surface an operator works from (lock,
unlock, change the role an account holds, edit what a role may do), the two privacy use cases that
make ADC compliant (data-subject **export** and **erasure**), the persistence and EF configuration,
the REST controllers, the gRPC contract that lets a peer service ask Identity a question, the
integration events that keep the User-to-Speaker link consistent across the service split, and the
Blazor profile, user-administration and role-administration UI. The per-type sections follow; this
overview shows how the pieces fit and how a request flows through them.
...
[`UserRole`](#userrole) value type (`UserRole.cs:18`), the [`UserInvariants`](#userinvariants) rule
...
implementations; its [`DependencyInjection`](#dependencyinjection) registers six services explicitly,
among them the shared
[`SoftDeletedUserValidator<TUser>`](group-14-module-system-composition.md#softdeleteduservalidatortuser)
closed over `User`, the [`AuthenticationServiceSettings`](#authenticationservicesettings) options
holder, and [`UserAdministrationService`](#useradministrationservice) as the module's
`IUserAdministrationService<UserAdminDTO>`
(`MMCA.ADC.Identity.Application/DependencyInjection.cs:35-47`), contributes the two
export sections in the order they appear in the exported document (`:53-54`), and leaves handlers,
mappers, validators, and domain-event handlers to `ScanModuleApplicationServices<ClassReference>()`
(`:58`). **`MMCA.ADC.Identity.Infrastructure`** holds the
...
while the cross-context `LinkedSpeakerId` is a nullable `SpeakerIdentifierType` (`User.cs:78`,
...
`User.cs:64`, `:67`), the profile (`Email`, `FirstName`, `LastName`, each marked
[`[Pii]`](group-02-domain-building-blocks.md#piiattribute), `User.cs:52`, `:56`, `:60`), the
preference store (`PreferredCulture` / `PreferredTheme`, `:108`, `:111`,
[ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html) /
[ADR-028](https://ivanball.github.io/docs/adr/028-dark-theme-mode.html)), the avatar URL holder
(`:120`, also `[Pii]`, BR-116a,
[ADR-045](https://ivanball.github.io/docs/adr/045-managed-file-storage-and-avatars.html)), the optional
MAUI device-metadata bag (`:81-99`), the external-OAuth link (`LoginProvider` / `ProviderKey`, `:102`,
`:105`), and the 1:1 cross-context `LinkedSpeakerId` pointing at a Conference speaker (`:78`, BR-207 /
...
[`IErasableUser`](group-08-auth.md#ierasableuser) (`User.cs:34-35`), joined by `IEmailConfirmableUser` (`:35`) now that an account carries a confirmation flag (`IsEmailConfirmed`, `:48`); `IErasableUser` is declared on
*this* type rather than inherited, and the class comment explains why it has to be (`:19-25`):
`Delete()` **hides** the base soft-delete with `new` (`:443`), so only re-listing the interface here
...
It follows the standard framework shape: a private EF constructor (`User.cs:156`), a private state
constructor (`:166`), and static factory methods returning
[`Result<T>`](group-01-result-error-handling.md#result). `Create` (`:199`) validates every invariant
with `Result.Combine(...)` *before* constructing anything (`:208-215`), so an invalid user is
unrepresentable; `CreateExternal` (`:247`) builds an OAuth account with empty credential arrays
(`:247-275`) and, when the provider asserts a verified address, marks it confirmed on the spot (`:271`). A subtlety worth carrying forward: the factory deliberately does **not** raise a
registration event. The `Id` is database-generated (`[IdValueGenerated]` at `:33`, `Id` set to
`default` until EF assigns it), so the cross-module [`UserRegistered`](#userregistered) is raised by the
application layer only after the insert has executed and a real id exists (`AuthenticationService.cs:167` does exactly
that). The behavior methods each guard their own rule: `ChangePassword` re-validates and raises
[`UserPasswordChanged`](#userpasswordchanged) (`:360-375`); `UpdatePreferences` validates against the
supported-culture allowlist and the light/dark theme values (`:330-343`); `Delete()` calls the G02
soft-delete and raises [`UserDeleted`](#userdeleted) (`:443-450`), and it does *not* need to touch
outstanding sessions, because the refresh flow re-fetches the account through the soft-delete query
filter, so every session stops working the moment the delete commits (`:436-440`).
...
(`UserRole.cs:18`) is a value object over the shared [`RoleValue`](group-08-auth.md#rolevalue) base
that fixes the ADC role set to three members: `Organizer` (`:21`), `Attendee` (the registration
default, `:24`), and `ContentEditor` (`:31`), each wrapping the matching [`RoleNames`](#rolenames) constant so the stored value and the JWT claim spell the role identically, a strict capability subset that curates the session
catalog but cannot change event structure, rooms, feedback questions, run session selection, or read
the user list (`:26-31`). Its `IsOrganizer(string?)` helper (`:76`) does a case-insensitive compare,
...
`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/AuthenticationService.cs:14-45`.
[`AuthenticationService`](#authenticationservice) (`AuthenticationService.cs:46`) fills in only the
context-specific pieces: `CreateUser` supplies the `Attendee` default role (BR-45, `:140`),
`CreateAccessToken` attaches the `speaker_id` claim when the user is linked to a speaker (BR-209,
`:150` via the `SpeakerClaims` helper at `:371`), `OnUserRegisteredAsync` raises the
[`UserRegistered`](#userregistered) integration event (`:167`), and `ExternalLoginAsync` drives the
OAuth find-by-provider, else link-by-email, else create flow (`:193`, body at `:204`,
...
whole base workflow in `UnitOfWork.ExecuteInTransactionAsync` (`AuthenticationService.cs:86`), so
the user insert (first save) and the outbox row for `UserRegistered` (raised in
`OnUserRegisteredAsync` once the id exists, `:167`) commit together.
...
cross-service Conference consumer cannot resolve (`:24-45`). The external-login path does the same
through `ExternalLoginAsync` (`:200`, with the event raised only for a brand-new
...
`Auth.ExternalEmailNotVerified` when the answer is no (in the external-login body opened at `AuthenticationService.cs:204`); the check
...
login (same body). Its implementation
...
(`MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/Program.cs:207-208`) and does the same for Apple's
...
opposite of the obvious one: it passes `ignoreQueryFilters: true` (`AuthenticationService.cs:136-137`)
...
a legacy or placeholder row surface as a save-time surprise (`:128`).
...
ADC subclass adds only two overrides and two command factories. `RegisterAsync` (`:58`) captures the
client IP for registration rate limiting and the user-agent that names the new refresh session
(BR-213, `:62-66`) and carries the per-IP `auth-ip` fixed window; `LoginAsync`
re-declares the same window as a password-spray guard, because the per-email lockout alone cannot
throttle one source spraying one password across many addresses.
`CreateChangePasswordCommand` and `CreateChangePreferencesCommand` are the only wiring
...
[`ChangePasswordHandler`](#changepasswordhandler) (`ChangePasswordHandler.cs:24`,
...
(`MMCA.ADC.Identity.Shared/Authorization/IdentityPermissions.cs:13`) is handed to Organizer, and to no other role, through `IdentityPermissionGrants.Apply`
in `AddModuleIdentityAPI` (`MMCA.ADC.Identity.API/DependencyInjection.cs:49`, map at `IdentityPermissionGrants.cs:29`,
...
(`:109-110`) that makes the `ORDER BY` total: without it, rows sharing a sort key can repeat or vanish
...
(`ResetPasswordHandler.cs:24`) is a body-less subclass of
[`ResetPasswordHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#resetpasswordhandlerbasetuser-tcommand)
...
(`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:466-471`). The storage decision behind the token
...
## Email confirmation: proving the address is reachable

ADC asks a new account to prove its address, and deliberately does not gate sign-in on the answer:
`Authentication:EmailConfirmation:RequireConfirmedEmail` is false
(`MMCA.ADC.Identity.API/Controllers/EmailConfirmationController.cs:18-20`). The aggregate carries the
one bit, `IsEmailConfirmed` (`User.cs:48`), behind `IEmailConfirmableUser` (`:35`); it is flipped by
the idempotent `ConfirmEmail()` (`:283`), set directly when an OAuth provider already asserts a
verified address (`:271`, `:304`), and reset during erasure (`:500`). EF maps it as a required column
with a `false` default, so a row written by anything that does not set it stays valid and the
migration can backfill the accounts that existed before the flag did (`UserConfiguration.cs:109-113`).

Both halves are ADC specializations over framework bases.
[`SendEmailConfirmationCommand`](#sendemailconfirmationcommand)
(`.../UseCases/SendEmailConfirmation/SendEmailConfirmationCommand.cs:20`) carries one address and
serves two entry points: the endpoint dispatches it inline, and registration schedules it as an
`IInternalCommand` named `Identity.SendEmailConfirmation.v1` (`:19`), so a mail outage or a crash
right after the 201 defers the send instead of losing it, and its one-string payload round-trips
through the JSON the durable queue requires (`:12-17`, ADR-114).
[`SendEmailConfirmationHandler`](#sendemailconfirmationhandler) (`SendEmailConfirmationHandler.cs:27`)
inherits the framework `SendEmailConfirmationHandlerBase<TUser, TCommand>` and overrides exactly one
member, the untracked lookup by address (`:37-41`), the same one
[`ForgotPasswordHandler`](#forgotpasswordhandler) and
[`AuthenticationService`](#authenticationservice) already use; every outcome is a success so the
response cannot be used to enumerate registered addresses, and an SMTP failure is swallowed by the
base, leaving the token valid for another attempt (`:12-16`). The redemption half overrides nothing at
all: [`ConfirmEmailCommand`](#confirmemailcommand) (`ConfirmEmailCommand.cs:11`) is anonymous by
design because the link is followed from an email client with no session (`:6-9`), and
[`ConfirmEmailHandler`](#confirmemailhandler) (`ConfirmEmailHandler.cs:21`) lets the base validate and
consume the token, load the account the token names, and call the aggregate idempotent `ConfirmEmail`,
so a token that is wrong, expired or already spent is answered as invalid, which is also what an
attacker guessing tokens sees (`:12-17`).

[`EmailConfirmationController`](#emailconfirmationcontroller) (`EmailConfirmationController.cs:34`)
exposes the pair anonymously on the same `Auth` prefix as
[`PasswordResetController`](#passwordresetcontroller), and for the same reason:
[`AuthController`](#authcontroller) already occupies the single inheritance chain, and riding the
prefix means both endpoints reach the gateway through the existing `/Auth` route with no gateway
change; the framework ships the handler bases but no controller base for this pair, so the two actions
are written here (`:22-28`). The send action always answers 202, whether or not the address holds an
account and whether or not it is already confirmed, since any other answer would let a caller
enumerate registered addresses (`:38-42`), and it carries `[Idempotent]`, `[AllowAnonymous]` and the
`auth-ip` rate-limit window together (`:46-52`). The emailed link carries the address and the token in
the URI **fragment**, which browsers never send to a server, so the live token stays out of ingress
access logs, request telemetry and `Referer` headers (`:64-67`).

## Administration: accounts, roles, and the capability map

The organizer-facing administration surface is a framework surface with ADC vocabulary plugged into
it. [`UsersAdminController`](#usersadmincontroller) (`UsersAdminController.cs:25`) subclasses the
framework `UsersAdminControllerBase<TUserAdminDTO>` routed at the literal `Admin/Users` (`:23`), so
the fixed action templates publish `/Admin/Users/paged`, `/Admin/Users/{userId}`,
`/Admin/Users/{userId}/lock` and the rest (`:10-14`), every action gated on the `users:manage`
capability by the base rather than on a role name (`:16-19`). Its only own members are two overrides
that exist to declare idempotency intent for the architecture suite, both `[NonIdempotent]` with the
reason spelled out: locking is already idempotent in the domain, so a replayed response protects
nothing, and caching one would answer a later unlock-then-lock cycle from the cache and report a state
the account is not in (`:33`, `:42`). [`AdminRolesController`](#adminrolescontroller)
(`AdminRolesController.cs:36`) is the same shape at `Admin/Roles` (`:34`), gated on `roles:manage`
(`:16-22`), and it supplies no ADC service at all: the host `AddStoredPermissionGrants` call registers
a complete role-administration service over the framework own registry and grant store, and a set
edits stored rows only, so nothing reachable there can take away a capability the host compiled in
(`:23-29`).

The ADC half is [`UserAdministrationService`](#useradministrationservice)
(`MMCA.ADC.Identity.Application/Users/Administration/UserAdministrationService.cs:38`), the module
`IUserAdministrationService<UserAdminDTO>`: the paged roster (`:53`), one account (`:95`), lock and
unlock (`:111`), and the role an account holds (`:146`). Two decisions in it are worth reading.
**A lock revokes the live sessions**: a lock that only refused the next sign-in would leave the
account working for the remaining lifetime of every access token it already holds, and its refresh
sessions would keep minting new ones, so the same revocation the framework credential-rotation paths
run is replicated here over [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore) (`:20-26`).
**One role, not a set**: the framework contract takes a role list because an app may hold several, and
ADC [`User`](#user) stores exactly one, so a request carrying zero or two is a validation failure
rather than something to silently truncate (`:27-31`). Paging goes through
[`PagingMath`](group-03-querying-specifications.md#pagingmath) with a 100-row cap restated rather than
referenced, because the Application layer does not see the API package and the controller clamps too,
so drift here can only ever make this the tighter of the two (`:45-50`, `:60`).
[`UserAdminDTO`](#useradmindto) (`UserAdminDTO.cs:23`) is the row it projects, a DTO of its own rather
than more fields on [`UserListDTO`](#userlistdto) because the two are reached through different
permissions: `identity:users:read` serves the organizer roster, `users:manage` serves the
administration screens, device metadata stays out of both, and no credential material is ever
projected (`:8-13`). [`UserListDTO`](#userlistdto) does answer the framework `IUserAdminDTO` so the
shared list component can render it, mapping the lock state off `LockedOn` explicitly rather than
renaming the wire contract (`UserListDTO.cs:9-13`, `:39`, `:42`). On the aggregate the lock itself is
a nullable instant, `LockedOn` (`User.cs:127`), with `IsLocked` computed from it rather than stored so
the state and the instant it started cannot disagree (`:132-141`), moved by `Lock` and `Unlock`
(`:384`, `:389`), while the role is changed through the validating `ChangeRole` (`:402`).

The capability map is the third piece. [`IdentityPermissions`](#identitypermissions) declares
`identity:users:read` (`IdentityPermissions.cs:13`) and then deliberately *aliases* two framework
strings rather than coining ADC names, `UsersManage` (`:25`) and `RolesManage` (`:38`), because the
framework admin controller bases gate their actions on those exact capability strings (`:18-24`,
`:30-37`). [`IdentityPermissionGrants`](#identitypermissiongrants) grants all three to Organizer and
to no other role (`IdentityPermissionGrants.cs:29`), which is the strict-subset property ContentEditor
is defined by (`:19-23`); `RolesManage` is granted in compiled code and never from a stored row,
because it is the only protection on the screen that edits stored rows and a deleted row could
otherwise lock every operator out of the surface that would restore it
(`IdentityPermissions.cs:30-37`). The extracted Identity host applies more than its own map:
[`TokenPermissionGrants`](#tokenpermissiongrants)
(`MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/Authorization/TokenPermissionGrants.cs:34`) calls
every module map (`:53-59`, wired at `Program.cs:270`, with the stored half added at `:277`), because
`TokenService` emits one `permission` claim per permission the *minting* host registry grants the
account role, and a downstream service (or the Blazor sidebar) reads that claim instead of its own
registry: a token minted with only Identity grants would deny in the UI and in the CQRS pipeline what
the owning service allows at its endpoint (`:14-25`). Nothing is restated there, each map is called
where its module declares it, and only the contracts assemblies come along, which keeps another module
entity configurations out of this service database model (`:26-32`); grants accumulate and union, so
applying a map alongside the module own registration is additive and order-independent (`:38-42`).
[Rubric §11, Security] and [ADR-020](https://ivanball.github.io/docs/adr/020-permission-based-authorization.html)
are the lenses here: the endpoint decides on a capability, the role is only the key that map is keyed
by, and the map itself is editable at runtime without a deploy.
...
(`MMCA.ADC.Identity.Application/Users/UseCases/DeleteUser/DeleteUserHandler.cs:29`) keeps the class
...
caller's own id and role (`DeleteUserCommand.cs:19`). `HasDeletePrivilege` (`:39-40`) says the
...
[`UserDeleted`](#userdeleted) integration event on the aggregate (`:59`), so the outbox row is written
...
in-process domain event, so the fact and its announcement must not be able to come apart (`:53-58`).
...
name pinned by `[EventName("Identity.UserDeleted.v1")]` at `:25`). It then schedules the erasure of the avatar photo, which is personal data too: rather than queueing a
best-effort post-commit call, the hook enqueues a durable
[`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand) row (`:61-71`, BR-116a,
ADR-114). The hook runs inside the erasure transaction, so the instruction is enrolled and commits
with the anonymized aggregate or rolls back with it, and a scheduling failure fails the erasure
rather than committing a partial one the caller was told had succeeded (`:76-86`). That is the whole
reason for the change: the best-effort call it replaces lost the instruction whenever storage was
unreachable or the host died in the post-commit tail, which for an erasure request is a photo of a
real person surviving the deletion the account was promised
(`.../DeleteAvatarBlob/DeleteAvatarBlobInternalCommand.cs:8-14`). Note the ordering detail at
`:49-51`: the blob name is captured *before* `Anonymize` clears the URL. The erasure itself
lives in `User.Anonymize()` (`User.cs:465`), which irreversibly overwrites the personal fields with
...
PRIVACY.md §5 "delete within 30 days" promise (`User.cs:454-463`).
...
two, in document order (`MMCA.ADC.Identity.Application/DependencyInjection.cs:53-54`):
...
(`MMCA.ADC.Identity.Application/Users/UseCases/SetUserAvatar/SetUserAvatarHandler.cs:26`) handles a
...
(`:26`). It never trusts the client-declared content type: it sniffs magic bytes through the shared
...
[`IImageProcessor`](group-07-persistence-ef-core.md#iimageprocessor) (`:28`), uploads under a
randomized blob name through
[`IFileStorageService`](group-07-persistence-ef-core.md#ifilestorageservice) (`:29`), and parks the
*replaced* blob's name in the [`MutationContext`](group-05-cqrs-pipeline.md#mutationcontext) (`:98`)
so `OnMutatedAsync` (`:113`) can schedule its deletion once the new URL is persisted. That deletion is
not best effort: the handler enqueues a
[`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand) through the framework
internal-command scheduler (`:30`, `:127-133`), a durable row the processor retries until storage
answers (ADR-114). The random suffix
means a replacement never reuses the old URL, so stale caches self-resolve (`:12-14`). The result
travels back as a [`UserAvatarDTO`](#useravatardto) built in `BuildResult` (`:138`).
...
the siblings on the same resource, and the remove path schedules the same durable deletion
(`RemoveUserAvatarHandler.cs:20`, `:74-80`). All three avatar paths (replace, remove, erase the
account) therefore converge on
[`DeleteAvatarBlobInternalCommandHandler`](#deleteavatarblobinternalcommandhandler), which treats an
already-missing blob as the state the command asks for and completes the row, and returns any other
failure so the framework backoff retries it (`DeleteAvatarBlobInternalCommandHandler.cs:33-48`,
rationale at `:12-17`);
[`DeleteAvatarBlobInternalCommandValidator`](#deleteavatarblobinternalcommandvalidator) rejects an
empty name before the row is executed, which guards a scheduling bug rather than a caller
(`DeleteAvatarBlobInternalCommandValidator.cs:12-14`, `:5-9`).
...
members (`:116-118`), and pins three indexes that encode business rules as schema ([Rubric §8, Data
Architecture]): unique `Email` (`:123`), a filtered **unique** index on `LinkedSpeakerId` that enforces
the 1:1 User-to-Speaker relationship of BR-208 (`:125-127`), and a filtered unique composite on
`(LoginProvider, ProviderKey)` for external accounts (`:129-131`). None of them spells the soft-delete
...
appends `AND [IsDeleted] = 0` at model finalization, and writing it here would double it (`:120-122`,
...
`Program.cs:344` (the response enumerates attendee user ids, so internal-only ingress is not enough,
...
(`Program.cs:82`, rationale at `:70-80`,
...
at `Program.cs:170`, replays the organizer user-list read against its own Kestrel endpoint (`:33-36`)
...
(`MMCA.ADC.Identity.Shared/Users/IntegrationEvents/UserRegistered.cs:36`, a
[`BaseIntegrationEvent`](group-04-events-outbox.md#baseintegrationevent) named
`Identity.UserRegistered.v1` on the wire, `:35`) on the aggregate, and the outbox carries it to
...
queue. The host registers both as broker consumers (`Program.cs:303-304`, rationale at `:279-286`).
...
(`MMCA.ADC.Identity.UI/IdentityUIModule.cs:15`), an
[`IUIModule`](group-15-common-ui-framework.md#iuimodule) descriptor that contributes three
[`NavItem`](group-15-common-ui-framework.md#navitem)s as resource keys, "My Profile" in the user
section for every signed-in user, "Users" in the admin section for Organizers, and "Roles" in the
admin section (`:17-33`,
[ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html)). The third one is gated on
the *capability* rather than on a role name (`:26-32`), and the comment above it says why: role
administration is the one screen whose whole point is that the role-to-capability map is editable, so
an entry naming a role would state the thing the screen exists to change (`:22-25`). The sidebar reads
the `permission` claim the Identity host mints from its registry, which is why that host applies every
module's grants. The routes come from the [`IdentityRoutePaths`](#identityroutepaths) constants
`/users`, `/profile`, `/roles` and `/confirm-email` (`IdentityRoutePaths.cs:8-9`, `:12`, `:15`) plus
two builders, one per account (`:22-23`) and one per role, where the role *name* is the key because a
role has no surrogate id anywhere in the system (`:32-33`); the UI-layer
[`DependencyInjection`](#dependencyinjection) wires the module descriptor, the user service and the
anonymous confirmation client (`MMCA.ADC.Identity.UI/DependencyInjection.cs:39`, `:43`).
...
(`MMCA.ADC.Identity.UI/Pages/Users/UserList.razor.cs:24`) is the Organizer-only management screen, and
it is now a thin shell over the framework shared `UserAdminList<TUser>` component, which renders the
roster, the mobile cards and the lock / unlock / change-role actions against the `Admin/Users`
endpoints (`:12-13`); ADC supplies the assignable roles from the shared vocabulary (`:31`) and the
mapping from the component search-filter key onto its own query, which wins over the Email column
filter (`:38`, `:56`).

Three administration pages sit beside the profile page, all of them thin shells over framework
components. [`UserDetail`](#userdetail)
(`MMCA.ADC.Identity.UI/Pages/Users/UserDetail/UserDetail.razor.cs:22`) shows read-only account facts
plus the two administrative changes, lock or unlock and the single role the account holds, both going
through the `users:manage`-gated `Admin/Users` endpoints (`:12-15`); the assignable roles come from
[`RoleNames`](#rolenames) least to most privileged (`:27-28`), and the actions are hidden on the
signed-in operator own account, because locking or demoting yourself removes the capability needed to
undo it and the API has no notion of "the caller" (`:17-21`). [`RoleList`](#rolelist)
(`MMCA.ADC.Identity.UI/Pages/Roles/RoleList.razor.cs:27`) carries no members at all: the shared
`RoleAdminList` owns the load, the empty state and the inline failure, and the class exists only
because `IStringLocalizer<RoleList>` resolves by type full name, so the page needs a type of its own
for its `.resx` pair to attach to (`:12-18`). [`RoleEdit`](#roleedit)
(`MMCA.ADC.Identity.UI/Pages/Roles/RoleEdit.razor.cs:16`) adds one route parameter, the role name
(`:19-20`), over the shared `RoleAdminEdit`, which renders the host whole permission catalog grouped
by area, ticks and locks what compiled code already grants, and saves only the stored set (`:6-14`).
Both keep their page authorization as a *role* while the sidebar entry that links there states the
capability, and that split is deliberate: the route gate is what `IdentityRouteAuthorizationTests`
pins so an organizer-only surface cannot silently regress to a bare `[Authorize]`, while the endpoints
behind the page are gated on the capability by the controller base, which is the check that actually
decides (`RoleList.razor.cs:19-25`). The
...
(`MMCA.ADC/Tests/E2E/MMCA.ADC.E2E.Tests/Workflows/AccessibilityTests.cs:416`, with the rationale for
...
(`AuthenticationService.cs:86-88`) and hands off to the shared G08 engine. The request shape was
...
taken (`AuthenticationService.cs:136-137`), call `User.Create(...)` with the `Attendee` role
...
([ADR-003](https://ivanball.github.io/docs/adr/003-outbox-dual-dispatch.html)). One step follows the
commit: [`AuthController`](#authcontroller) schedules a
[`SendEmailConfirmationCommand`](#sendemailconfirmationcommand) as a durable internal-command row
rather than sending the confirmation inline, and a scheduling failure is logged rather than failing
the 201, because the account exists and the user can ask for a link from the confirmation page
(`AuthController.cs:73-93`). The refresh half of
...
(`AuthenticationService.cs:150-151`). No password left the domain in plaintext, no cross-database
...
refresh sessions) are the primary references; the email-confirmation and user / role administration
surface is recorded in ADR-116, and the durable internal-command queue behind the scheduled sends and
blob deletions in ADR-114.
...
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users` · `MMCA.ADC.Identity.Application/Users/AuthenticationServiceSettings.cs:18` · Level 1 · class (sealed)

- **What it is**: a small options-holder that bundles the two `IOptions<>` bundles [`AuthenticationService`](#authenticationservice) passes up to its base-class constructor, so adding a second base-class option did not grow the service's own primary-constructor parameter list.
- **Depends on**: `IOptions<RefreshSessionSettings>` (see [`RefreshSessionSettings`](group-08-auth.md#refreshsessionsettings)) and `IOptions<EmailConfirmationSettings>`(group-08-auth.md#emailconfirmationsettings) (Microsoft.Extensions.Options).
- **Concept introduced, an options facade to hold a dependency-count ceiling.** `[Rubric §1, SOLID]` and `[Rubric §15, Best Practices & Code Quality]`. [`AuthenticationService`](#authenticationservice) is checked by the architecture suite's constructor-dependency-count test (`MMCA.ADC/Tests/Architecture/MMCA.ADC.Architecture.Tests/Cqrs/ConstructorDependencyCountTests.cs`); ADR-116 added email-confirmation state to what [`AuthenticationServiceBase<TUser>`](group-08-auth.md#authenticationservicebasetuser) reads, which would have added a tenth bare constructor parameter. Wrapping the two option bundles behind one injected settings object (the registering comment says so, `MMCA.ADC.Identity.Application/DependencyInjection.cs:28-30`) keeps the service at that ceiling, at the cost of one extra indirection to read either value.
- **Walkthrough**: a two-parameter primary constructor (`:25-27`) exposing `RefreshSessions` (`:30`) and `EmailConfirmation` (`:33`), both `get`-only properties assigned straight from the constructor parameters. No logic beyond that.
- **Why it's built this way**: grouping the two options is purely a dependency-count concession, not a domain concept in its own right; it exists to keep [`AuthenticationService`](#authenticationservice)'s constructor under the architecture suite's ceiling once a second base-class option bundle was needed.
- **Where it's used**: registered `TryAddScoped<AuthenticationServiceSettings>()` by the Application-layer [`DependencyInjection`](#dependencyinjection-1) (`MMCA.ADC.Identity.Application/DependencyInjection.cs:31`); injected into [`AuthenticationService`](#authenticationservice)'s primary constructor (`AuthenticationService.cs:54`) and unpacked into the base constructor call as `settings.RefreshSessions` and `settings.EmailConfirmation` (`:60-62`).
...
- **Where it's used**: discovered and registered in Kahn-topological order by the [`ModuleLoader`](group-14-module-system-composition.md#moduleloader) at host startup; `RegisterDisabledStubs` runs in hosts where the Identity module is not enabled, for example `MMCA.ADC.Notification.Service`, whose composition root then calls `AddIdentityAttendeeClient()` over the top (`MMCA.ADC/Source/Services/MMCA.ADC.Notification.Service/Program.cs:225`, with the rationale at `:204`).

---

### DependencyInjection
> MMCA.ADC.Identity.API · `MMCA.ADC.Identity.API` · `MMCA.ADC.Identity.API/DependencyInjection.cs:17` · Level 4 · class (static)

- **What it is**: the **API-layer** composition root for the Identity module. It exposes `AddIdentityModule(...)`, the single call that registers every layer of the module, plus `AddModuleIdentityAPI()`, which applies the module's role-to-permission grants and wires the OAuth email-verification gate.
- **Depends on**: `IServiceCollection` and `TryAddScoped` (Microsoft.Extensions.DependencyInjection); [`ApplicationSettings`](group-14-module-system-composition.md#applicationsettings); the Application-layer `AddModuleIdentityApplication` (see [`DependencyInjection`](#dependencyinjection-1) for the Application layer) and the Infrastructure-layer `AddModuleIdentityInfrastructure`; `AddPermissions` from `MMCA.Common.API.Authorization` and `IdentityPermissionGrants.Apply` from the module's Shared project; [`IExternalLoginEmailVerifier`](#iexternalloginemailverifier) and [`HttpContextExternalLoginEmailVerifier`](#httpcontextexternalloginemailverifier).
- **Concept introduced, the layered DI fan-out via `extension(IServiceCollection)`.** `[Rubric §3, Clean Architecture]` (assesses inward-pointing dependencies and a single composition point per module): the API layer is the only layer that can see *all* the others, so it owns the aggregate registration. The method hangs off `IServiceCollection` through the C# `extension(IServiceCollection services)` block (`DependencyInjection.cs:19`), the workspace idiom for DI registration (see [primer §4](00-primer.md#4-c-build-and-code-style-conventions)). `[Rubric §15, Best Practices & Code Quality]`: the three-call body mirrors the layering, so registration order matches dependency order and there is exactly one place to look when wiring changes.
- **Walkthrough**
  ...
  - `AddModuleIdentityAPI()` (`:42-58`) does two things. First it calls `services.AddPermissions(IdentityPermissionGrants.Apply)`, handing the whole grant map to the framework's `AddPermissions` helper rather than building `.Grant(...)` calls inline; those grants back the module's `[HasPermission(...)]`-gated endpoints, including the organizer user list ([ADR-020](https://ivanball.github.io/docs/adr/020-permission-based-authorization.html)). Second it registers the OAuth auto-link gate: `AddHttpContextAccessor()` plus `TryAddScoped<IExternalLoginEmailVerifier, HttpContextExternalLoginEmailVerifier>()`. The inline comment explains the placement: the verified-email assertion lives in the external-login cookie principal, so the verifier is an API-edge concern.
  - Controllers are not registered here; ASP.NET Core's controller convention discovers them (doc comment).
- **Why it's built this way**: one entry point per module is what [`IdentityModule`](#identitymodule) calls, so module wiring stays discoverable. The grant map itself was pulled out into `IdentityPermissionGrants` in the module's Shared project, because the token-minting host applies the same map: a token must carry the permission claims of every module, not only of the module the minting host boots, so the map has to be reachable from outside the API project too. `TryAddScoped` (rather than `AddScoped`) leaves the door open for a host to pre-register a different verifier.
...
- **Where it's used**: registered as the `IAttendeeQueryService` implementation by the Application-layer [`DependencyInjection`](#dependencyinjection-1) (`MMCA.ADC.Identity.Application/DependencyInjection.cs:43`); consumed by the Notification module. In the extracted topology the registration is replaced by [`AttendeeQueryServiceGrpcAdapter`](#attendeequeryservicegrpcadapter), and this class becomes the code behind [`AttendeesGrpcService`](#attendeesgrpcservice) on the Identity side.
...
- **Concept introduced (3), the outbox-atomic registration event, and what its `EmailVerified` flag settles.** `[Rubric §6, CQRS & Event-Driven]`, `[Rubric §8, Data Architecture]`, `[Rubric §29, Resilience & Business Continuity]`. `User.Id` is a database-generated identity column, so at the moment the aggregate is created the id is still `0`. The outbox serializes an event's payload *at capture time*, so raising [`UserRegistered`](#userregistered) before the insert would persist `UserId = 0`, and the cross-service Conference consumer, which has no access to the Identity database to re-match by email, could never resolve it. The fix is visible in two places:
  - `RegisterAsync` (`:81-88`) re-implements the interface member with `new` and wraps the base implementation in `UnitOfWork.ExecuteInTransactionAsync(token => base.RegisterAsync(request, ipAddress, userAgent, token), cancellationToken)`. The interface is re-listed on the class declaration specifically so this override wins for callers holding an [`IAuthenticationService`](group-08-auth.md#iauthenticationservice), since the base method is not virtual.
  - `OnUserRegisteredAsync` (`:167-173`) runs *after* the base's first save, when the identity value exists: it calls `user.AddDomainEvent(new UserRegistered(user.Id, user.Email.Value, user.FirstName, user.LastName, user.Role, EmailVerified: false))`, saves a second time so the outbox row is captured, and returns the user. Local registration always passes `EmailVerified: false`, because the address was typed into the form and nothing proves the caller owns it: Conference logs any speaker whose email matches for an organizer to link by hand (BR-209) rather than auto-linking. Both saves sit inside the one transaction opened by `RegisterAsync`, so a crash before commit rolls back user and event together, and after commit the [`OutboxProcessor`](group-04-events-outbox.md#outboxprocessor) guarantees delivery.
- **Concept introduced (4), the three-way external-login resolution, now split into its own link helper.** `[Rubric §11, Security]`. `ExternalLoginAsync` (`:193-202`) is a transaction wrapper around `ExternalLoginCoreAsync` (`:205-305`), which resolves the caller into exactly one of three paths:
  1. **Known external identity** (`:214-217`), a tracked lookup by `LoginProvider` **and** `ProviderKey`. Found means log in, nothing else runs.
  2. **Email matches an existing local account** (`:243-258`). The raw claim is parsed through the value object first, `Email.Create(email)` (`:233`), failing closed with `Error.Validation("Auth.ExternalEmailInvalid", ...)` (`:236-240`) rather than taking `.Value` directly: `Result<T>.Value` is `null` on failure and does not throw, so an unparseable provider claim would otherwise turn the lookup into "find the users whose email IS NULL". When a match is found, the linking decision itself is now delegated to `TryLinkProviderToExistingAccountAsync(existingUser, loginProvider, providerKey)` (`:250-255`), a private helper (`:316-373`) that holds both refusals: a **one-link-per-account** check (the aggregate stores a single `(LoginProvider, ProviderKey)` pair, so linking a second provider would strand the original login, `IsExternalLogin` is `LoginProvider is not null`), and the **takeover guard**, `externalLoginEmailVerifier.IsCurrentExternalLoginEmailVerifiedAsync()`, refusing `Error.Unauthorized("Auth.ExternalEmailNotVerified", ...)` when the provider did not assert a verified email. Both refusal messages point at the forgot-password recovery path, so neither is a dead end.
  3. **Brand new user** (`:260-282`). Unlike the link path, account creation is **not gated** on the verified-email assertion: `emailVerified` is captured from the same `IsCurrentExternalLoginEmailVerifiedAsync()` call (`:265-266`) and passed straight into `User.CreateExternal(email, firstName, lastName, loginProvider, providerKey, emailVerified)` (`:272-273`) rather than used to refuse. Refusing creation would close sign-up entirely for a provider that asserts nothing (GitHub carries no `email_verified` claim); instead the flag rides along on two downstream decisions: ADR-116 uses it to decide whether the new account starts already confirmed (a verified assertion) or unconfirmed and sent a confirmation link like a local registration, and it is carried on the `UserRegistered` event (`:292-297`) so Conference only auto-links a speaker profile when the address was actually proven.

  All surviving paths then converge (`:285`): save; for a new user only, raise [`UserRegistered`](#userregistered) with the captured `emailVerified` flag and save again (`:292-297`), the same post-identity pattern as local registration; and finally hand off to the base's `IssueTokensAsync(user, cancellationToken: cancellationToken)` (`:304`), which opens a refresh session through the shared workflow (hash at rest, per-user cap, rotation chain) rather than stamping a plaintext token on the aggregate. The OAuth callback arrives through the framework's `OAuthControllerBase`, which does not surface the caller's IP or user-agent, so the session records neither; both fields are informational and nothing validates against them.
- **Why it's built this way**: the base class keeps every application on one audited auth workflow, so a fix to lockout or refresh-session reuse detection lands once in `MMCA.Common` rather than per app; the hooks keep ADC's divergences (role, claim, event, lock check) small and named. [`AuthenticationServiceSettings`](#authenticationservicesettings) replaced a bare `IOptions<RefreshSessionSettings>` constructor parameter once ADR-116 added a second required options bundle, so the service's constructor dependency count did not grow past the architecture suite's ceiling. The `UserRegistered` integration event, rather than an in-process domain event, is a deliberate divergence from MMCA.Store, because Conference runs in a separate process with its own database and can only learn about a registration asynchronously ([ADR-003](https://ivanball.github.io/docs/adr/003-outbox-dual-dispatch.html) outbox, [ADR-006](https://ivanball.github.io/docs/adr/006-database-per-service.html) database-per-service). The `EmailVerified` flag is what lets Conference tell a typed-in address from a provider-proven one without re-implementing the OAuth trust decision on its own side of the boundary. The eventual-consistency cost stays the same either way: the first token issued does not yet carry `speaker_id`, and the claim appears on the next refresh once Conference has published `SpeakerLinkedToUser` back.
...
### DependencyInjection
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application` · `MMCA.ADC.Identity.Application/DependencyInjection.cs:21` · Level 15 · class (static)

- **What it is**: the **Application-layer** registration for Identity. It explicitly binds six services that convention scanning cannot infer, contributes the module's two data-subject export sections, then runs Scrutor scanning to auto-register every handler, mapper, validator, and domain-event handler in the assembly.
- **Depends on**: `IServiceCollection`, `TryAddScoped`; [`ApplicationSettings`](group-14-module-system-composition.md#applicationsettings); [`IAuthenticationService`](group-08-auth.md#iauthenticationservice) with [`AuthenticationService`](#authenticationservice), [`AuthenticationValidators`](group-08-auth.md#authenticationvalidators), [`AuthenticationServiceSettings`](#authenticationservicesettings), [`ISoftDeletedUserValidator`](group-08-auth.md#isoftdeleteduservalidator) with the framework's generic [`SoftDeletedUserValidator<TUser>`](group-14-module-system-composition.md#softdeleteduservalidatortuser) closed over [`User`](#user), [`IAttendeeQueryService`](#iattendeequeryservice) with [`AttendeeQueryService`](#attendeequeryservice), and `IUserAdministrationService<UserAdminDTO>` with `UserAdministrationService`; the [`IUserDataExportSection`](group-14-module-system-composition.md#iuserdataexportsection) contributors [`EngagementUserDataExportSection`](#engagementuserdataexportsection) and [`NotificationUserDataExportSection`](#notificationuserdataexportsection); the `AddUserDataExportSection<TSection>` and `ScanModuleApplicationServices<TAssemblyMarker>` helpers from `MMCA.Common.Application`; [`ClassReference`](#classreference).
- **Concept introduced (1), explicit registration for the ambiguous, convention scanning for the rest.** `[Rubric §2, Design Patterns]` and `[Rubric §15, Best Practices & Code Quality]`. Handlers, mappers, and validators follow a one-interface-one-implementation convention, so Scrutor can find them: `services.ScanModuleApplicationServices<ClassReference>()` means adding a new use-case slice, [`GetUserAvatarHandler`](#getuseravatarhandler) for instance, needs **no DI edit at all**. The services that are not convention-discoverable, because their interfaces live in other assemblies, have more than one plausible implementation, or (for `IUserAdministrationService<UserAdminDTO>`) are neither a handler, a mapper, nor a validator so the scan would never find them, are registered by hand. `TryAddScoped` rather than `AddScoped` is the load-bearing detail: a host that has already registered an override keeps it, and the module does not clobber it. (The gRPC swap for `IAttendeeQueryService` goes further and uses `Replace`, precisely because it must win over whatever is already there: `MMCA.ADC/Source/Services/MMCA.ADC.Identity.Contracts/DependencyInjection.cs:47`.)
- **Concept introduced (2), closing a framework generic over the app's own entity.** `[Rubric §1, SOLID]` and `[Rubric §11, Security]`. `ISoftDeletedUserValidator` is a framework contract used by the shared [`SoftDeletedUserMiddleware`](group-12-api-hosting-mapping.md#softdeletedusermiddleware) to reject an access token that outlived its account ([ADR-047](https://ivanball.github.io/docs/adr/047-soft-deleted-user-session-revocation.html)). Identity satisfies it not with a hand-written class but by closing the framework's generic [`SoftDeletedUserValidator<TUser>`](group-14-module-system-composition.md#softdeleteduservalidatortuser) over its own [`User`](#user) type. The revocation check therefore has exactly one implementation across the whole workspace, and each app contributes only its entity type.
- **Concept introduced (3), the export section as an additive, ordered contribution.** `[Rubric §30, Compliance / Privacy / Data Governance]` and `[Rubric §7, Microservices Readiness]`. The data-subject export (PRIVACY.md §7) has to include data Identity does not own, so the framework models each contributor as an [`IUserDataExportSection`](group-14-module-system-composition.md#iuserdataexportsection) and the export handler fans out over the whole `IEnumerable<>`. Here Identity registers the two peers' sections in order, `EngagementUserDataExportSection` then `NotificationUserDataExportSection`, which records two consequences: **registration order is the section order in the exported document**, and the export handler itself needs no registration because the scan below picks it up as an ordinary `IQueryHandler`. On the framework side, `AddUserDataExportSection<TSection>` registers the concrete type with `TryAddScoped` and appends the interface mapping with `TryAddEnumerable`, so the same section registered twice is added once; the scoped lifetime is deliberate so a section may take repositories or gRPC clients.
- **Concept introduced (4), an options facade to hold a dependency-count ceiling under ADR-116.** `[Rubric §1, SOLID]`. `services.TryAddScoped<AuthenticationServiceSettings>()` is registered immediately after `AuthenticationValidators`, with a comment recording why: it bundles the two options bundles [`AuthenticationService`](#authenticationservice) hands to its base constructor behind one injected holder, so the service stays at the architecture suite's nine-dependency ceiling once ADR-116 added email confirmation to what the base reads. See [`AuthenticationServiceSettings`](#authenticationservicesettings) for the type itself.
- **Walkthrough**: `AddModuleIdentityApplication(ApplicationSettings)` lives inside an `extension(IServiceCollection services)` block (`:21`). Body order (`:29-50`): `_ = applicationSettings;` discards the parameter with a comment marking it reserved for future decorator configuration; then `IAuthenticationService` to [`AuthenticationService`](#authenticationservice), the [`AuthenticationValidators`](group-08-auth.md#authenticationvalidators) parameter object, [`AuthenticationServiceSettings`](#authenticationservicesettings) (Concept 4), `ISoftDeletedUserValidator` to `SoftDeletedUserValidator<User>`, `IAttendeeQueryService` to [`AttendeeQueryService`](#attendeequeryservice), and `IUserAdministrationService<UserAdminDTO>` to `UserAdministrationService`, the framework's opt-in Admin/Users endpoint backing (ADR-116); then the two export sections, the Scrutor scan, and `return services`.
- **Why it's built this way**: mixing explicit and convention registration keeps the common case zero-ceremony while retaining precise control over the handful of services that need a specific lifetime or an override point. Registering the cross-boundary implementations (`ISoftDeletedUserValidator`, `IAttendeeQueryService`) here is what closes the inversion those contracts set up: the framework and the Notification module declare the need, Identity satisfies it. `AuthenticationServiceSettings` is registered here rather than left for callers to construct, because it is itself just an options facade with no state to configure beyond what DI already resolves.
- **Where it's used**: called by the API-layer [`DependencyInjection`](#dependencyinjection-1)'s `AddIdentityModule` (`MMCA.ADC.Identity.API/DependencyInjection.cs:28`), which [`IdentityModule`](#identitymodule) calls in turn.
...
- **Where it's used**: the Infrastructure copy is what the Identity migrations host hands to
  ...
  `MMCA.ADC/Source/Hosting/MMCA.ADC.Migrations.SqlServer.Identity/DesignTimeSQLServerDbContextFactory.cs:55`,
...
- **Concept**: cross-reference the marker idiom taught under
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.ModuleScanning.cs:28-30`), a
  constraint a static class cannot satisfy. Its own doc names the expected argument outright, "a type
  in the module's Application assembly (typically `ClassReference`)"
  (`DependencyInjection.ModuleScanning.cs:22`). `[Rubric §33, Developer
...
- **Walkthrough**: `public class ClassReference { }` (line 11 of each file), no members. Its only
  ...
  `MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.ModuleScanning.cs:46`).
...
- **Where it's used**: neither of these two copies is passed as a type argument anywhere in ADC today.
  ...
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:58`,
...
- **Where it's used**: the Application copy is the type argument at `MMCA.ADC.Identity.Application/DependencyInjection.cs:58`, which resolves to `ScanModuleApplicationServices<TAssemblyMarker>` in `MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.ModuleScanning.cs:28-30`, a one-line generic wrapper that forwards `typeof(TAssemblyMarker).Assembly` to the `Assembly`-typed overload declared at `:46`. See [`DependencyInjection`](#dependencyinjection) for the Application layer.
...
- **Walkthrough**
  ...
  - **Count, then sort, then page** (`:39-59`): `queryableExecutor.CountAsync(baseQuery, cancellationToken)` issues a `SELECT COUNT` against the *filtered but unpaged* query (`:39`); `ApplySorting` adds the `ORDER BY` (`:42`); then `Skip(skip).Take(take)` plus the `Select` into [`UserListDTO`](#userlistdto), now including `LockedOn = u.LockedOn` (`:45-57`), become `OFFSET`/`FETCH` with an explicit column list, materialized by `queryableExecutor.ToListAsync` (`:59`).
  - **Wrap** (`:61-62`): `new PaginationMetadata(totalCount, take, Math.Max(query.PageNumber, 1))` reports the *clamped* page size and the floored page number, so the metadata describes the page that was actually served rather than the one that was asked for; the result is returned as `Result.Success(new PagedCollectionResult<UserListDTO>(paged, metadata))`.
  - `ApplyFilters` (`:79-97`): one `Where` per non-null filter. `Email` runs through `Email.Create` and compares whole value objects on success (`:81-86`); `FirstName` and `LastName` use `Contains` on plain string columns (`:89-92`); `Role` compares the `string` column to the query's `string?` directly (`:93-94`), which works because [`User`](#user) stores the role as a `string` (`MMCA.ADC.Identity.Domain/Users/User.cs:71`), so no value-object unwrapping is needed here.
  - `ApplySorting` (`:99-111`): the direction is descending when `SortDirection` equals `"desc"` case-insensitively **or** is null or whitespace (`:101-102`), which is how the documented `desc` default is realized. It delegates the key choice to `SelectSortKey` (`:104`), then appends the `Id` tie-break (`:106-110`).
  - `SelectSortKey` (`:113-136`): a private static helper, factored out of `ApplySorting` so the column-name switch reads as its own unit. Its doc (`:113-119`) states the intent: every sortable grid column is named here rather than left to the fallback, so a click orders by the column the reader clicked. A `switch` on `sortColumn?.ToUpperInvariant()` matches `"USERID"` or `"ID"` to `u => u.Id` (`:127`), documented by the comment above it (`:123-126`) as needed because the grid's first column binds to `UserListDTO.UserId` and sends that name, with `"ID"` accepted too since the entity property and the REST vocabulary both call it Id; without either mapping the column fell through to the `CreatedOn` default and a click on it silently reordered by creation time. `"EMAIL"` / `"FIRSTNAME"` / `"LASTNAME"` / `"ROLE"` / `"CREATEDON"` map to their own columns (`:130-134`), and anything else, including null, falls to `CreatedOn` (`:135`).
- **Why it's built this way**: routing count, sort, page and projection through [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor) keeps the work set-based and testable (the executor is the mockable boundary over EF's async extension methods), while the read repository keeps a pure read off the tracking path. The explicit column projection is a type-level guarantee, not a convention: adding a sensitive field to [`User`](#user) cannot leak it through this endpoint without someone editing the `Select`. Delegating the cap to the shared [`PagingMath`](group-03-querying-specifications.md#pagingmath) rather than open-coding `Math.Min` means the hostile inputs (page 0, negative size, a page number near `int.MaxValue`) are handled the same way in every module. Naming every sortable column explicitly in `SelectSortKey`, rather than only the columns the original grid shipped with, means adding a new sortable grid column is a switch-arm edit instead of a silent fallback to `CreatedOn`.
...
- **Where it's used**: invoked from the API layer's `AddIdentityModule(...)` at
  `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/DependencyInjection.cs:29`, between
...
- **What it is**: the canonical role-name string constants for the ADC Identity module: three roles,
  `Organizer`, `Attendee`, and `ContentEditor` (`RoleNames.cs:20-34`), referenced instead of magic
  strings.
- **Depends on**: nothing first-party.
- **Concept, centralized constants over magic strings.** `[Rubric §11, Security]` assesses
  authorization correctness and `[Rubric §15, Best Practices & Code Quality]` assesses duplication and
  single points of change. Roles are stored as plain strings and emitted as claims, so a typo in a
  literal does not fail to compile: it silently fails to authorize, or worse, silently authorizes
  nobody while looking correct. Referencing `RoleNames.Organizer` instead of `"Organizer"` moves that
  class of bug to compile time. `ContentEditor` is documented as a strict subset of the Organizer's
  capabilities (`RoleNames.cs:28-32`): it curates the session catalog without rights over event
  structure, rooms, questions, session selection, or user administration. That subset is precisely the
  case [ADR-020](https://ivanball.github.io/docs/adr/020-permission-based-authorization.html) uses to
...
- **Walkthrough**: three `public const string` fields. `Organizer = "Organizer"` (`RoleNames.cs:23`)
  is documented as managing conference master data (`:22`); `Attendee = "Attendee"` (`:26`) is the
  default for new registrations (`:25`); `ContentEditor = "ContentEditor"` (`:33`) curates the session
  catalog (sessions, speakers, categories, sponsors, activities) without full organizer rights, and its
  doc comment (`:28-32`) spells out exactly which rights it lacks.
- **Why it's built this way**: `const` rather than `static readonly` so the values can appear in
  attribute and option initializers that require compile-time constants.
- **Where it's used**: the per-app role type
  ...
  and the ownership-filter bypass role in ADC's Engagement module
  (`MMCA.ADC/Source/Modules/Engagement/MMCA.ADC.Engagement.API/DependencyInjection.cs:54`).

### IdentityPermissions
> MMCA.ADC.Identity.Shared · `MMCA.ADC.Identity.Shared.Authorization` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Shared/Authorization/IdentityPermissions.cs:10` · Level 1 · class (static)
...
- **What it is**: the capability-permission catalog for the Identity module: string constants that
  ...
  Three capabilities today: reading the user list, administering user accounts, and administering
  roles.
- **Depends on**: `AdministrationPermissions` (the framework's shared administration capability names,
  aliased here rather than re-coined). BCL `IReadOnlyList<string>` plus a collection expression.
...
- **Walkthrough**: `public const string UsersRead = "identity:users:read"` (`IdentityPermissions.cs:13`),
  documented as "list or read all user accounts (organizer user-management screens)" (`:12`).
  `public const string UsersManage = AdministrationPermissions.ManageUsers` (`:25`) administers user
  accounts (lock, unlock, change role); its doc and remarks (`:15-24`) note it aliases the framework's
  `AdministrationPermissions.ManageUsers` because `UsersAdminControllerBase` gates its actions on that
  exact capability string, and that ContentEditor holds none of these, which is the strict-subset
  property that role is defined by. `public const string RolesManage = AdministrationPermissions.ManageRoles`
  (`:38`) administers roles (list them, edit the stored permission grants attached to each); its doc
  and remarks (`:27-37`) state it is granted in **compiled** code and never from a stored row, because
  it is the only protection on the screen that edits stored rows, so a deleted row could otherwise lock
  every operator out of the surface that would restore it. `public static IReadOnlyList<string> All
  { get; }` (`:41`) is a collection expression holding all three (`:42-46`), so a role can be granted
...
- **Why it's built this way**: a `namespace:resource:action` string convention keeps permissions
  ...
  was granted the set picks it up. Aliasing `AdministrationPermissions.ManageUsers` /
  `.ManageRoles` rather than coining Identity-specific names keeps the controller gates and the
  compiled role grant agreeing on one string per capability.
- **Where it's used**: demanded by the user-list endpoint on [`UsersController`](#userscontroller)
  ...
  guarding the `[HttpGet]` action declared at `:138`), by `AdminRolesController` and by
  `UsersAdminController`; granted in [`IdentityPermissionGrants`](#identitypermissiongrants)`.Apply`
  through `permissions.Grant(RoleNames.Organizer, [.. IdentityPermissions.All])`.

### IdentityPermissionGrants
> MMCA.ADC.Identity.Shared · `MMCA.ADC.Identity.Shared.Authorization` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Shared/Authorization/IdentityPermissionGrants.cs:17` · Level 3 · class (static)

- **What it is**: the module's one role-to-permission grant: it hands every
  [`IdentityPermissions`](#identitypermissions) capability to `RoleNames.Organizer` and to no other
  role.
- **Depends on**: [`RoleNames`](#rolenames); [`IdentityPermissions`](#identitypermissions);
  `PermissionRegistryBuilder` (the framework's grant-accumulation type, from
  `MMCA.Common.Application`).
- **Concept**: cross-reference [`IdentityPermissions`](#identitypermissions) for the permission-based
  authorization model this grant populates. `[Rubric §11, Security]` assesses least privilege:
  factoring the grant into its own static class, called once from module registration, keeps "who can
  administer users and roles" answerable by reading one method rather than tracing every
  `[HasPermission]` attribute back to a role.
- **Walkthrough**: one method, `public static void Apply(PermissionRegistryBuilder permissions)`
  (`IdentityPermissionGrants.cs:25`), guarded by `ArgumentNullException.ThrowIfNull(permissions)`
  (`:27`). The doc comment (`:19-24`) states the policy in full: every Identity capability goes to
  Organizer, and content editors deliberately hold none, because their strict-subset property is
  defined by holding no user or role administration. The body is one call:
  `permissions.Grant(RoleNames.Organizer, [.. IdentityPermissions.All])` (`:29`).
- **Why it's built this way**: a dedicated `Apply` method, rather than inline calls at the module's
  `DependencyInjection`, gives the grant a single, independently testable entry point that both the
  in-process API host and the standalone Identity service can call.
- **Where it's used**: called from `AddModuleIdentityAPI()`
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/DependencyInjection.cs`) and from the
  Identity service's token permission grants
  (`MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/Authorization/TokenPermissionGrants.cs`), so the
  in-process host and the standalone service register the identical grant.
...
- **Where it's used**: **nowhere in ADC today.** A repository-wide search for `IdentitySettings`
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/LoginProtectionService.cs:100`). The test
...
- **Caveats / not-in-source**: unlike its sibling, this command has **no** FluentValidation validator,
  ...
  [`CommandValidatorCoverageTests`](group-28-testing-infrastructure.md#commandvalidatorcoveragetests)
...
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.RemoveUserAvatar` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/RemoveUserAvatar/RemoveUserAvatarHandler.cs:18` · Level 16 · class (sealed, partial)

- **What it is**: the command handler that removes the signed-in user's avatar (BR-116a). It clears
  the stored URL on the [`User`](#user) aggregate first (the user-visible state), then schedules the
  blob's deletion on the durable internal-command queue (ADR-114). It is idempotent: when no avatar is
  set it succeeds without writing, without logging and without touching storage.
- **Depends on**:
  ...
  `IInternalCommandScheduler` (the framework's durable internal-command queue);
  `DeleteAvatarBlobInternalCommand` (the scheduled command this handler enqueues, handled by
  `DeleteAvatarBlobInternalCommandHandler`);
  [`SetUserAvatarHandler`](#setuseravatarhandler) (it calls that class's `internal static`
  `TryGetBlobName`, imported at `RemoveUserAvatarHandler.cs:3`, which is why it sits one level higher
...
- **Concept reinforced, the idempotent delete expressed as a workflow short-circuit.** The mutation
  ...
  at `MMCA.Common/Source/Core/MMCA.Common.Application/UseCases/Crud/MutateEntityHandlerBase.cs:302-305`).
  ...
  removal is recorded through a source-generated `[LoggerMessage]` method (`:84-85`), so the class is
  `partial`, and the skip path deliberately logs nothing, so a repeated delete does not inflate the
  "avatar removed" signal.
- **Concept introduced, the blob delete is now a durable internal command, not a best-effort inline
  call.** Previously this handler called `IFileStorageService.DeleteAsync` directly after the commit;
  it now schedules a `DeleteAvatarBlobInternalCommand` through `IInternalCommandScheduler` instead
  (`:74-76`). `[Rubric §6, CQRS & Event-Driven]` and `[Rubric §13, Observability and Operability]`:
  ADR-114's internal-command table gives the delete retry semantics a bare `try/await` cannot, a
  transient storage outage defers the cleanup instead of losing it silently. The scheduling call can
  still fail (the row itself might not persist), and that failure is not escalated to the caller: it
  is logged at Warning through `LogAvatarBlobScheduleFailed` (`:78-81`, message declared at `:87-88`),
  whose text states the consequence in the log itself: the blob "is orphaned in storage and must be
  removed by hand."
- **Walkthrough**
  - Primary-constructor injection of [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), an
    `IInternalCommandScheduler` and an `ILogger<RemoveUserAvatarHandler>` (`RemoveUserAvatarHandler.cs:18-21`),
    with the unit of work forwarded to
    `MutateEntityHandlerBase<RemoveUserAvatarCommand, User, UserIdentifierType>` (`:22`). Note the base
    without a payload type parameter: this command answers with a bare
    [`Result`](group-01-result-error-handling.md#result). One constant,
    `private const string BlobNameKey = "Avatar.BlobName"` (`:25`), the context key the orphaned
    blob's name travels under.
  - `EntityId` (`:28`) returns `command.UserId`, so the base loads that aggregate and answers
    `Error.NotFound` itself when the account does not exist.
  - **The mutation** (`:31-50`): it derives the blob name with
    `SetUserAvatarHandler.TryGetBlobName(entity.AvatarUrl)` (`:37`). When that is `null` (no avatar
    stored, or a URL with no usable final segment) it calls `context.SkipSave()` and returns
    `Result.Success()` (`:42-43`), with the comment at `:40-41` naming the intent: "an
    already-satisfied command, not a refused one. No save, no log, no storage call." Otherwise it
    stashes the name under `BlobNameKey` (`:46`) and calls `entity.SetAvatarUrl(null)` (`:47`, domain
    setter at `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Domain/Users/User.cs:352`).
  - `LogMutated` (`:53-54`) fires the source-generated `LogAvatarRemoved` at Information level with
    the message "User {UserId} avatar removed" (`:84-85`).
  - `OnMutatedAsync` (`:62-82`), post-commit: reads `context.GetOrDefault<string>(BlobNameKey)`
    (`:68`) and returns immediately when absent (`:69-72`). Otherwise it schedules
    `new DeleteAvatarBlobInternalCommand(blobName)` through
    `internalCommandScheduler.ScheduleAsync` (`:74-76`), and logs
    `LogAvatarBlobScheduleFailed` when scheduling itself fails (`:78-81`). The doc above the method
    (`:56-60`) states the ordering rationale: the row is already committed without the URL, and the
    blob delete becomes an internal-command row the framework retries, so a storage outage defers it
    instead of leaving an orphan nothing knows about.
- **Why it's built this way**: clearing the URL before scheduling the blob delete makes the persisted
  "no avatar" state authoritative, so a later storage failure leaves an orphaned file rather than a
  dangling reference to a blob that no longer exists. That ordering is the same
  [ADR-096](https://ivanball.github.io/docs/adr/096-best-effort-side-effects.html) discipline as
  ...
  which blob a stored URL names. Routing the delete itself through the internal-command queue (ADR-114)
  rather than an inline `await fileStorage.DeleteAsync` trades an immediate best-effort attempt for a
  durable, retried one.
- **Where it's used**: dispatched by [`UsersController`](#userscontroller)`.RemoveAvatarAsync`
  ...
  `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.Application.Tests/Users/UseCases/RemoveUserAvatar/RemoveUserAvatarHandlerTests.cs`.
- **Caveats / not-in-source**: the internal-command row's own retry and eventual-failure handling live
  in the internal-command processing pipeline, not in this handler; whether a command that keeps
  failing is ever surfaced anywhere beyond the Warning log is `Not determinable from source` within
  this unit.
...
- **Concept introduced, the permission-scoped sibling DTO.** `[Rubric §8, Data Architecture]` (assesses whether reads project only the columns a screen needs) and `[Rubric §11, Security]` (assesses that secrets never reach a serialization boundary). The type's `remarks` (`UserAdminDTO.cs:8-13`) states directly why this is not more fields bolted onto [`UserListDTO`](#userlistdto): the two DTOs are reached through different permissions, `identity:users:read` serves the organizer's roster, `users:manage` serves the administration screens. Device metadata stays out of both (attendee device privacy), and no credential material is ever projected here either, the same restraint [`UserDataExportSubjectDTO`](#userdataexportsubjectdto) documents for the export path.
...
- **Depends on**: the `UserIdentifierType` alias; [`IUserAdminDTO`](group-08-auth.md#iuseradmindto) (`MMCA.Common.Shared.Auth.Administration`, imported at `UserListDTO.cs:1`); BCL (`DateTime`).
- **Concept introduced, the list projection DTO as a privacy boundary, and now also the framework's shared roster row.** `[Rubric §8, Data Architecture]` and `[Rubric §30, Compliance, Privacy and Data Governance]` for the projection; `[Rubric §7, Microservices Readiness]` and `[Rubric §2, Design Patterns]` for the interface. The type's doc comment is explicit that device-specific fields are excluded to protect attendee device privacy: the [`User`](#user) aggregate carries `DeviceId`, `DeviceModel`, `DeviceManufacturer` and friends, but an organizer browsing the user grid has no business seeing them. Its `remarks` (ADR-116) add a second fact on top: this is also the row the framework's shared user-administration list renders, so the type answers [`IUserAdminDTO`](group-08-auth.md#iuseradmindto) by reusing `UserId`, `Email`, and `Role` as-is, without renaming the wire contract, and maps the lock state explicitly off `LockedOn` (`UserListDTO.cs:14`). Because the projection is still built inside the query, a `.Select(u => new UserListDTO { ... })` translated to SQL after `Skip`/`Take` (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/GetUsers/GetUsersHandler.cs:45-57`), the excluded columns are never even read from the database, so this is a privacy boundary *and* a performance win at once (`[Rubric §12, Performance and Scalability]`).
- **Walkthrough**: seven members (`UserListDTO.cs:17-42`). `UserId` (`:17`), `Email` (`:20`), `FirstName` (`:23`), `LastName` (`:26`), and `Role` (`:29`) are `required init`; `CreatedOn` is a plain `init` `DateTime` (`:32`). `Role` is a `string`, not the [`UserRole`](#userrole) value object: the wire format stays primitive, and the closed-set type is a domain concern. The nullable `LockedOn` (`:39`, documented `:34-38`) is new: when an administrator locked the account, or `null` while it is usable, carried on the roster so an operator can see which accounts are locked without opening each one; the lock itself is changed through the `Admin/Users` endpoints. `bool IUserAdminDTO.IsLocked => LockedOn is not null;` (`:42`) is an *explicit* interface implementation, so `IsLocked` is reachable only through an `IUserAdminDTO`-typed reference, leaving the record's own public surface unchanged for every existing consumer that names `UserListDTO` directly.
- **Why it's built this way**: keeping the list DTO separate from [`UserDTO`](#userdto) lets the grid evolve (sortable columns, `CreatedOn`, now `LockedOn`) without touching the general-purpose account DTO, and it keeps the list endpoint's payload small enough to page cheaply. Counting, sorting, paging and projecting all happen at the database level in that order (`GetUsersHandler.cs:39`, `:42`, `:45-56`), so the DTO is the only shape that ever leaves SQL Server. Implementing [`IUserAdminDTO`](group-08-auth.md#iuseradmindto) explicitly, rather than adding a second public lock-state property, lets `UserListDTO` also satisfy the framework's shared roster contract (ADR-116) without growing the type's own public surface: the interface exists so a framework-owned consumer can render either this DTO or [`UserAdminDTO`](#useradmindto) (which the app-side `Admin/Users` screens use instead) through the same shape.
- **Where it's used**: produced by [`GetUsersHandler`](#getusershandler) inside a [`PagedCollectionResult<T>`](group-01-result-error-handling.md#pagedcollectionresultt) (`GetUsersHandler.cs:62`), returned by [`UsersController`](#userscontroller)'s list endpoint (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/Controllers/UsersController.cs:138-141`, `:158`), and consumed client-side through [`IUserUIService`](#iuseruiservice) / [`UserService`](#userservice) (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Services/UserService.cs:27`, `:64`, `:71-72`) as the `MudDataGrid` row type on the [`UserList`](#userlist) page, which inherits [`DataGridListPageBase<TDto>`](group-15-common-ui-framework.md#datagridlistpagebasetdto) closed over it (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Pages/Users/UserList.razor.cs:17`). The grid's filter keys are read back off the DTO with `nameof(UserListDTO.Email)` and friends (`UserList.razor.cs:53-56`, `:63`), so a renamed property breaks at compile time rather than at runtime. Also referenced from [`UserAdminDTO`](#useradmindto)'s own doc comment (`UserAdminDTO.cs:5`) and consumed from [`SpeakerDetail`](group-21-conference-ui.md#speakerdetail)/`SpeakerUserSearch` in the Conference module (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Speakers/SpeakerDetail.razor.cs`, `SpeakerUserSearch.cs`).

### ConfirmationState
> MMCA.ADC.Identity.UI · `MMCA.ADC.Identity.UI.Pages.Users.ConfirmEmail` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Pages/Users/ConfirmEmail/ConfirmEmail.razor.cs:44` · Level 0 · enum (private)

- **What it is**: the four states the [`ConfirmEmail`](#confirmemail) page cycles through while redeeming an email-confirmation link: `Working`, `Confirmed`, `Failed`, `Resent`.
- **Depends on**: nothing; a private nested enum with no other type references.
- **Concept**: a private nested enum used purely as component render-state, the same shape used throughout the UI layer to drive conditional markup without a separate view-model class. See [`ConfirmEmail`](#confirmemail) for the full state machine this enum drives.
- **Walkthrough**: `Working` is the initial value and covers both the automatic-redeem attempt and any resend in flight (`ConfirmEmail.razor.cs:47`); `Confirmed` marks a successful redeem (`:50`); `Failed` covers a missing, wrong or expired token, and is also the state chosen up front when neither an email nor a token is available to redeem (`:53`); `Resent` marks a completed resend request (`:56`).
- **Why it's built this way**: a private enum keeps the state vocabulary scoped to the one component that interprets it, rather than promoting it to a shared UI-layer type nothing else needs.
...
- **Concept reinforced, route constants as the module's public navigation surface.** `[Rubric §25, Navigation and Information Architecture]` (assesses whether routes are declared in one place so menu entries, redirects, and tests cannot drift from the pages themselves) and `[Rubric §15, Best Practices & Code Quality]`. The nav items in [`IdentityUIModule`](#identityuimodule) reference `IdentityRoutePaths.Profile`, `.Users` and `.Roles` (`IdentityUIModule.cs:17-27`) rather than repeating the strings, so renaming a route is a one-line change here.
...
- **Caveats / not-in-source**: the `@page` directives on [`UserList`](#userlist), [`Profile`](#profile), [`RoleList`](#rolelist) and [`RoleEdit`](#roleedit) still spell their route literally, because a Razor `@page` directive requires a compile-time `const`, and these are `static readonly` fields, not `const`. So this type is the single source of truth for *navigation* and for building a link to a specific detail, not for the page routing attribute itself.
- **Where it's used**: [`IdentityUIModule`](#identityuimodule)'s `NavItems`, the Roles tests (`MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.UI.Tests/Pages/Roles/RoleListTests.cs`, `RoleEditTests.cs`), and [`UserDetail`](#userdetail)'s own page (`UserDetail.razor.cs`).
...
- **Concept**: a minimal route-bound component parameter, the same shape used across the UI layer wherever a page's identity comes entirely from its URL segment; see [`IdentityRoutePaths`](#identityroutepaths)`.RoleEdit(role)`, the builder that constructs the link this page receives `Role` from.
...
- **Where it's used**: reached from [`IdentityRoutePaths`](#identityroutepaths)`.RoleEdit(role)`, which [`RoleList`](#rolelist) links to per row. Exercised by `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.UI.Tests/Pages/Roles/RoleEditTests.cs` and cross-checked for authorization by `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.UI.Tests/IdentityRouteAuthorizationTests.cs`.
...
- **Concept**: a bare partial declaration is the minimum a routable Razor component needs when the markup file owns all state and logic; contrast with [`RoleEdit`](#roleedit), whose code-behind at least carries a route parameter.
...
- **Where it's used**: reached via [`IdentityRoutePaths`](#identityroutepaths)`.Roles`, linked from [`IdentityUIModule`](#identityuimodule)'s "Roles" nav item. Exercised by `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.UI.Tests/Pages/Roles/RoleListTests.cs` and `IdentityRouteAuthorizationTests.cs`.
...
- **Depends on**: `ConfirmEmailRequest` (the shared request DTO), `ICommandWithRequest<ConfirmEmailRequest>` (the marker that routes it through the shared request-validator convention, the same mechanism [`ForgotPasswordCommand`](#forgotpasswordcommand) uses).
- **Concept**: the same anonymous-command shape [`ForgotPasswordCommand`](#forgotpasswordcommand) teaches: the caller has no session yet (the link is followed from an email client), so the command carries only the request payload and no authenticated user id.
...
- **Why it's built this way**: routing through `ICommandWithRequest<TRequest>` gets the command an auto-registered `IValidator<ConfirmEmailCommand>` for free via [`CommandRequestValidator<TCommand, TRequest>`](group-06-validation.md#commandrequestvalidatortcommand-trequest), the same convention [`ForgotPasswordCommand`](#forgotpasswordcommand) relies on, so ADC supplies no bespoke validator for this command.
- **Where it's used**: constructed by [`EmailConfirmationController`](#emailconfirmationcontroller) and handled by [`ConfirmEmailHandler`](#confirmemailhandler). Covered by `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.API.Tests/Controllers/EmailConfirmationControllerTests.cs` and `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.Application.Tests/Users/UseCases/ConfirmEmail/ConfirmEmailHandlerTests.cs`, and pinned by `CommandValidatorCoverageTests`.
...
- **Walkthrough**: a two-line positional record with a single member (`ForgotPasswordCommand.cs:12-13`).
  - `Request` is the whole payload, so the marker interface is the load-bearing part of the declaration. Because the record implements `ICommandWithRequest<ForgotPasswordRequest>`, the module scan auto-registers `IValidator<ForgotPasswordCommand>` as a [`CommandRequestValidator<TCommand, TRequest>`](group-06-validation.md#commandrequestvalidatortcommand-trequest) that delegates to whatever `IValidator<ForgotPasswordRequest>` is registered (`MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.ModuleScanning.cs:121-134`).
  - That request validator is [`ForgotPasswordRequestValidator`](group-08-auth.md#forgotpasswordrequestvalidator), and it lives in the framework rather than in ADC (`MMCA.Common/Source/Core/MMCA.Common.Application/Auth/Validation/ForgotPasswordRequestValidator.cs:11`), registered by `AddValidatorsFromAssemblyContaining<ClassReference>()` because a module scan only sees its own assembly (`MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.cs:44-48`). It checks the shape of the address and nothing else, and its doc comment states the reason (`ForgotPasswordRequestValidator.cs:9`): a 400 that depended on whether the address had an account would be exactly the oracle the always-accepted response closes.
...
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.SetUserAvatar` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/SetUserAvatar/SetUserAvatarCommand.cs:12` · Level 1 · record (sealed)
...
- **What it is**: the CQRS command that sets (uploads or replaces) the signed-in user's avatar photo
  ...
  It carries the owning `UserId` plus the raw uploaded image bytes, and declares its own execution
  timeout.
- **Depends on**: `UserIdentifierType` (the Identity identifier alias),
  [`IHasTimeout`](group-05-cqrs-pipeline.md#ihastimeout) (the timeout-declaring marker interface), and
  BCL `System.ReadOnlyMemory<byte>` for the payload and `System.TimeSpan` for the budget.
- **Concept**: the upload-security pipeline this command feeds is taught in full at
  ...
  (`SetUserAvatarCommand.cs:5-9`) is explicit that the handler validates the true format from those
  ...
  format is sniffed instead of believed. `[Rubric §12, Performance and Scalability]`: implementing
  `IHasTimeout` gives the pipeline an explicit per-command execution budget rather than a global
  default.
- **Walkthrough**: a positional record
  `SetUserAvatarCommand(UserIdentifierType UserId, ReadOnlyMemory<byte> Content) : IHasTimeout`
  (`SetUserAvatarCommand.cs:12`). `UserId` is controller-stamped from the authenticated principal and
  never client-supplied; `Content` is the uploaded image. The body adds one member,
  `public TimeSpan Timeout => TimeSpan.FromSeconds(60)` (`:19-20`), whose doc comment (`:13-18`) states
  the budget covers the re-encode plus the blob PUT: the source bytes are capped by the upload limit
  and the stored image is a fixed 256x256 JPEG, so sixty seconds exists to bound a storage call that
  never returns rather than to pace legitimate work.
- **Why it's built this way**: modeling the upload as an immutable command with a read-only memory
  ...
  the bytes it can sniff and re-encode without owning a disposable stream. Declaring `IHasTimeout`
  directly on the command keeps the budget next to the payload it bounds, rather than in a
  configuration file the reader has to cross-reference.
...
- **Concept, the pipeline-stage validator as a *defense-in-depth copy* rather than a single gate.**
  ...
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:58`) and run
...
- **Where it's used**: never referenced by name. It is registered by the convention scan at
  `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:58` and
...
- **Depends on**: [`IUIModule`](group-15-common-ui-framework.md#iuimodule) (the contract, `IdentityUIModule.cs:5`, `:13`), [`NavItem`](group-15-common-ui-framework.md#navitem) and [`NavSection`](group-15-common-ui-framework.md#navsection) from `MMCA.Common.UI.Common` (`:3`), [`IdentityRoutePaths`](#identityroutepaths), [`RoleNames`](group-08-auth.md#rolenames) and [`AdministrationPermissions`](group-08-auth.md#administrationpermissions) from `MMCA.Common.Shared.Auth` (`:2`); externals: MudBlazor `Icons.Material.Filled` (`:5`), BCL `System.Reflection.Assembly` (`:1`).
...
- **Walkthrough**: two members.
  ...
  - Every entry passes `typeof(IdentityUIModule)` as the `TitleResource` argument, which per the [`NavItem`](group-15-common-ui-framework.md#navitem) contract turns `"Nav.MyProfile"`, `"Nav.Users"` and `"Nav.Roles"` into *resource keys* resolved against this type's resources at render time, per circuit, so the menu follows the active culture; a key the resource type does not declare renders as the raw string, which keeps a not-yet-translated entry legible instead of blank (`NavItem.cs:13-18`, [ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html)).
...
- **Where it's used**: passed as the type argument to the framework's `AddUIModule<TModule>()` inside [`DependencyInjection`](#dependencyinjection-3)`.AddIdentityUI` (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/DependencyInjection.cs:24`). That one call does both jobs: it is the Scrutor scan root (`FromAssemblyOf<TModule>`) and the descriptor registration (`AddSingleton<IUIModule, TModule>`), at `MMCA.Common/Source/Presentation/MMCA.Common.UI/DependencyInjection.cs:272-278`.

### ConfirmEmail
> MMCA.ADC.Identity.UI · `MMCA.ADC.Identity.UI.Pages.Users.ConfirmEmail` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Pages/Users/ConfirmEmail/ConfirmEmail.razor.cs:23` · Level 4 · class (sealed, partial)

- **What it is**: the anonymous landing page for the emailed confirmation link (ADR-116). It reads the address and token from the query string or from a URL fragment, attempts to redeem them automatically on first render, and falls back to manual entry plus a resend option when redemption fails.
- **Depends on**: [`IEmailConfirmationUIService`](#iemailconfirmationuiservice) (injected, `ConfirmEmail.razor.cs:39`), `CapabilitiesJsModule` (the optional JS interop module used to read the URL fragment, resolved at `:105`), [`ConfirmationState`](#confirmationstate) (the private nested render-state enum, `:44-57`); externals: `Microsoft.AspNetCore.Components` (`[SupplyParameterFromQuery]`, `[Inject]`, `ComponentBase`), BCL `System.Uri`, `System.StringComparison`, `System.Threading.CancellationToken`.
- **Concept introduced, redeeming a token from two possible transports on one page.** `[Rubric §24, Forms, Validation and UX Safety]` (assesses graceful degradation when required input is missing or malformed) and `[Rubric §26, Front-End Security]` (assesses how sensitive tokens travel through the URL). The component accepts the email and token two ways: as ordinary query parameters (`EmailFromQuery`, `TokenFromQuery`, `:31-37`), or as a URL fragment read client-side through `CapabilitiesJsModule` (`TakeFragmentAsync`, `:101-111`). The fragment path exists because a URL fragment is never sent to the server in the request line, so a host that wants the token to never touch server logs can mint links that carry it there instead; the query-parameter path stays as a fallback for a host, or a link, that doesn't. `ReadFragment` (`:113-140`) only fills a field that is still empty (`:131`, `:135`), so a query value already present is never overwritten by the fragment.
- **Walkthrough**: five private fields track render state (`_state`, `_email`, `_token`, `_isBusy`, `_attempted`, `:25-29`), and two are query-bindable via `[SupplyParameterFromQuery]` (`EmailFromQuery`, `:31-33`; `TokenFromQuery`, `:35-37`).
  - `OnParametersSet()` (`:59-75`) only fills `_email`/`_token` from the query parameters when the field is still empty, so a value the visitor corrected by hand survives a re-render.
  - `OnAfterRenderAsync(firstRender)` (`:77-99`) runs the redeem attempt exactly once (`_attempted`, `:80-83`): it reads the fragment (`:85`), and if neither an email nor a token is available after that, sets `_state = ConfirmationState.Failed` and returns without ever calling the confirmation service (`:89-96`), so a bare visit to the page (no link data at all) goes straight to manual entry instead of posting a request that can only fail.
  - `TakeFragmentAsync()` (`:101-111`) resolves `CapabilitiesJsModule` from `Services` and returns `null` when it is absent (`:105`, `:107-108`); the comment above it (`:103-104`) notes this happens during SSR prerender and in a host without the capabilities module, and its absence is treated as normal, not an error.
  - `ConfirmAsync()` (`:142-164`) posts through `ConfirmationService.ConfirmAsync(_email, _token)` and sets `_state` to `Confirmed` or `Failed` from the result, wrapped in `_isBusy`/`StateHasChanged` bracketing so the UI can show a spinner.
  - `ResendAsync()` (`:166-189`) calls `ConfirmationService.ResendAsync(_email)` and sets `_state = ConfirmationState.Resent`. The comment above it (`:178-180`) restates the same anti-enumeration rule the [`ForgotPasswordCommand`](#forgotpasswordcommand) workflow uses: the endpoint answers 202 whether or not the address holds an account, so the page reports the same thing either way, because telling the visitor which addresses are registered is exactly what the endpoint refuses to do.
- **Why it's built this way**: separating the automatic redeem attempt from the manual/resend UI means a stale, already-used or malformed link degrades to a working form instead of a dead end, and the fragment-vs-query split lets a deployment choose whether the token transits the server's own request logs.
- **Where it's used**: reached at [`IdentityRoutePaths`](#identityroutepaths)`.ConfirmEmail`. Backed by [`IEmailConfirmationUIService`](#iemailconfirmationuiservice) / [`EmailConfirmationService`](#emailconfirmationservice), registered by [`DependencyInjection`](#dependencyinjection-3)`.AddIdentityUI`. Exercised by `MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.UI.Tests/Pages/Users/ConfirmEmail/ConfirmEmailTests.cs`.
- **Caveats / not-in-source**: the paired `.razor` markup (not read here) is what actually renders the four states; this code-behind only manages the state transitions. Which query names ("email", "token") a real confirmation email uses is set by whatever composes the link server-side, not by this component.
...
- **What it is**: the one-call registration entry point for the Identity UI layer. `AddIdentityUI()` wires the module's entity services, its user-administration and role-administration clients (ADR-116), its email-confirmation client (ADR-116), its bespoke user service, and its [`IdentityUIModule`](#identityuimodule) descriptor into any Blazor host.
- **Depends on**: [`IdentityUIModule`](#identityuimodule), [`IUserUIService`](#iuseruiservice) / [`UserService`](#userservice) from `MMCA.ADC.Identity.UI.Services`, [`IEmailConfirmationUIService`](#iemailconfirmationuiservice) / [`EmailConfirmationService`](#emailconfirmationservice), [`UserAdminDTO`](#useradmindto) (the DTO the user-administration client closes over), and the framework's `AddUIModule<TModule>()`, `AddUserAdministrationUI<TUserAdminDTO>()` and `AddRoleAdministrationUI()` extensions from `MMCA.Common.UI`; externals: `Microsoft.Extensions.DependencyInjection.IServiceCollection` (`:1`).
...
- **Walkthrough**: `AddIdentityUI()` (`DependencyInjection.cs:20-46`) does four things and returns `services` for chaining (`:45`).
  - `services.AddUIModule<IdentityUIModule>()` (`:24`), the shared two-step prologue every module's `Add{Module}UI()` opens with, with the intent restated in the comment above it (`:22-23`). Inside `MMCA.Common.UI` that single call runs the Scrutor scan over the descriptor's assembly, registering every [`IEntityService<TEntityDTO, TIdentifierType>`](group-15-common-ui-framework.md#ientityservicetentitydto-tidentifiertype) implementation `AsImplementedInterfaces` with a scoped lifetime, then registers the descriptor with `AddSingleton<IUIModule, TModule>` (`MMCA.Common/Source/Presentation/MMCA.Common.UI/DependencyInjection.cs:272-278`). Convention over configuration: a new standard CRUD-shaped UI service needs no registration edit here.
  - `services.AddUserAdministrationUI<UserAdminDTO>()` (`:31`), the framework's client for the shared Admin/Users endpoints, gated server-side on `users:manage`. The comment above it (`:26-30`) is explicit that this is closed over ADC's own [`UserAdminDTO`](#useradmindto), which the user detail page reads and the shared user-administration list acts through, and that it is a *different resource under a different permission* from the "Users" roster nav entry, not more methods on that roster's client.
  ...
  - An explicit `services.AddScoped<IUserUIService, UserService>()` (`:39`). The comment above it (`:38`) explains why this one is hand-written: users are a custom contract, not an `IEntityService`, so the scan cannot pick it up (the same asymmetry [`IUserUIService`](#iuseruiservice) documents, because the Users API returns [`UserListDTO`](#userlistdto) rather than an `IBaseDTO`-shaped entity DTO and exposes only list plus delete).
  - `services.AddScoped<IEmailConfirmationUIService, EmailConfirmationService>()` (`:43`), registered for email confirmation (ADR-116). The comment above it (`:41-42`) states it is anonymous by contract: the link is followed from an email client with no session, so this client attaches no bearer token.
- **Why it's built this way**: one host-facing method per module keeps the three hosts symmetric: they each call `AddIdentityUI()` and nothing else. The mix of shared scan helpers, generic and non-generic administration clients, and explicit registrations for the two bespoke contracts is deliberate, and the framework doc for `AddUIModule<TModule>()` (`MMCA.Common/Source/Presentation/MMCA.Common.UI/DependencyInjection.cs:257-263`) records the ordering rationale: module-specific services are registered by the caller *after* the prologue, so a module whose service must win over a shared default still controls its own registration order.
- **Where it's used**: called by all three UI hosts, `MMCA.ADC/Source/Hosts/UI/MMCA.ADC.UI.Web/Program.cs:136` (Blazor Server), `MMCA.ADC/Source/Hosts/UI/MMCA.ADC.UI.Web.Client/Program.cs:62` (the WebAssembly client), and `MMCA.ADC/Source/Hosts/UI/MMCA.ADC.UI/MauiProgram.cs:128` (MAUI), which is what lets the same Razor Class Library render on web and mobile.
...
- **What it is**: ADC's handler for [`ConfirmEmailCommand`](#confirmemailcommand) (ADR-116). Like [`ForgotPasswordHandler`](#forgotpasswordhandler), it is a thin primary-constructor subclass with no body of its own: every step of the redeem workflow lives in the shared base.
- **Depends on**: `ConfirmEmailHandlerBase<TUser, TCommand>` (the framework base class, `MMCA.Common.Application`), [`User`](#user), [`ConfirmEmailCommand`](#confirmemailcommand), [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), `IEmailConfirmationTokenService`; externals: `Microsoft.Extensions.Logging.ILogger<T>`.
- **Concept**: the same template-method split [`ForgotPasswordHandler`](#forgotpasswordhandler) teaches: the framework owns the entire redeem sequence (token lookup, expiry check, marking the user confirmed) and ADC supplies nothing beyond the type arguments and constructor forwarding, because confirming an email needs no app-specific lookup step the way starting a reset does.
...
- **What it is**: the command handler that uploads or replaces the signed-in user's avatar (BR-116a,
  ...
  fresh random blob name, persists the new URL on the [`User`](#user) aggregate, then schedules the
  previous blob for deletion through the durable internal-command queue
  ([ADR-114](https://ivanball.github.io/docs/adr/114-internal-commands-durable-job-queue.html)). It
  returns a [`UserAvatarDTO`](#useravatardto) with the new URL.
- **Depends on**:
  [`MutateEntityPayloadHandlerBase<TCommand, TEntity, TIdentifierType, TResultPayload>`](group-05-cqrs-pipeline.md#mutateentitypayloadhandlerbasetcommand-tentity-tidentifiertype-tresultpayload)
  ...
  [`IInternalCommandScheduler`](group-14-module-system-composition.md#iinternalcommandscheduler);
  [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand);
...
- **Concept introduced, safe handling of a user-supplied binary upload
  ...
  called at `SetUserAvatarHandler.cs:54`, implemented at
  ...
  `[Pii]`-tagged property
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Domain/Users/User.cs:118-120`), so it
...
- **Concept introduced (second), the templated mutation workflow with a side-data context.** This
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Application/UseCases/Crud/MutateEntityHandlerBase.cs:271-322`):
  ...
  success path logs through a compile-time `[LoggerMessage]` source-generated method (`:153-154`),
...
- **Walkthrough**
  - Primary-constructor injection of [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork),
    [`IImageProcessor`](group-07-persistence-ef-core.md#iimageprocessor),
    [`IFileStorageService`](group-07-persistence-ef-core.md#ifilestorageservice),
    [`IInternalCommandScheduler`](group-14-module-system-composition.md#iinternalcommandscheduler) and
    an `ILogger<SetUserAvatarHandler>` (`SetUserAvatarHandler.cs:26-31`), with the unit of work
    forwarded to the base `MutateEntityPayloadHandlerBase<SetUserAvatarCommand, User,
    UserIdentifierType, UserAvatarDTO>` (`:32`). Two constants: `internal const int AvatarSize = 256`,
    the canonical edge length (`:35`), and `private const string PreviousBlobNameKey =
    "Avatar.PreviousBlobName"`, the context key the replaced blob's name travels under (`:38`).
  - `EntityId` (`:41`) tells the base which aggregate to load: `command.UserId`.
  - **Format gate, before the aggregate is loaded** (`:48-63`): `HandleAsync` is overridden purely to
    run `ImageContentSniffer.IsAllowedImage(command.Content.Span)` (`:54`) and return
    `Error.Validation` with code `"Avatar.UnsupportedFormat"` and the source stamped as the handler
    name (`:56-59`) *before* delegating to `base.HandleAsync` (`:62`). The doc (`:43-46`) gives the
  ...
  - **The mutation** (`:66-102`), which runs with the aggregate already loaded and tracked. The bytes
    are wrapped in a non-writable `MemoryStream` (`:72`) and handed to
    `imageProcessor.NormalizeToSquareJpegAsync(content, AvatarSize, ...)` inside an `await using`
    block (`:74-77`); a failed `Result<byte[]>` re-returns its own errors (`:79-82`), so the failure
    detail from the image layer reaches the client unchanged and, critically, **the save never
    happens** (the base short-circuits at `MutateEntityHandlerBase.cs:299-300`).
  - **Upload** (`:84-96`): a fresh eight-hex-character suffix comes from
    `Guid.NewGuid().ToString("N")[..8]` (`:84`), and the blob name is built culture-invariantly with
    `string.Create(CultureInfo.InvariantCulture, $"{command.UserId}-{suffix}.jpg")` (`:85`). The JPEG
    is streamed to `fileStorage.UploadAsync(blobName, jpeg, "image/jpeg", ...)` (`:90`); a failed
    upload short-circuits the same way (`:93-96`).
  - **Hand the orphan forward, then mutate** (`:98-99`): `context.Set(PreviousBlobNameKey,
    TryGetBlobName(entity.AvatarUrl))` captures the blob this write is about to orphan **before** the
    property is overwritten (`:98`), then `entity.SetAvatarUrl(uploaded.Value!.AbsoluteUri)`
    (`:99`, domain setter at
    `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Domain/Users/User.cs:352`). The base commits.
  - `LogMutated` (`:105-106`) fires the source-generated `LogAvatarSet` at Information level with the
    message "User {UserId} avatar updated" (`:153-154`).
  - `OnMutatedAsync` (`:113-135`), the post-commit hook: it reads
    `context.GetOrDefault<string>(PreviousBlobNameKey)` (`:119`) and returns immediately when it is
    null (`:120-123`). Otherwise it schedules a
    [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand) through
    `internalCommandScheduler.ScheduleAsync(...)` (`:127-129`) instead of deleting the blob inline; a
    scheduling failure is logged via `LogAvatarBlobScheduleFailed` at Warning (`:131-134`) rather than
    surfaced to the caller, since the mutation itself already committed. The comment above the call
    (`:125-126`) explains why: no transaction is active this late, so the row is already written and
    signalled immediately, and the internal-command processor picks the delete up on its next tick
    rather than waiting for the poll interval.
...
- **Why it's built this way**: the random blob-name suffix means a replacement never reuses the old
  URL, so browser and CDN caches self-resolve without an explicit purge (class doc, `:1-6`). Doing
  the blob work *inside* the mutation and deferring the cleanup to a durable queue is the
  [ADR-096](https://ivanball.github.io/docs/adr/096-best-effort-side-effects.html) shape combined with
  the [ADR-114](https://ivanball.github.io/docs/adr/114-internal-commands-durable-job-queue.html)
  durable-job pattern, and the class remarks name the change outright: an upload that fails still stops
  the write before the row is touched, but the blob the write orphans is now handed to the
  internal-command queue rather than deleted inline, so a storage outage or a crash in the post-commit
  tail defers the delete instead of leaking an image with nothing left that knows about it. Sniffing
  plus mandatory re-encoding is the
...
- **Caveats / not-in-source**: the concrete resize and crop strategy behind
  ...
  says the size limit stays app-side. The actual blob deletion no longer runs inside this handler at
  all: `OnMutatedAsync` only schedules
  [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand); its processor and retry
  behavior live in the internal-command infrastructure, not here. A failed `ScheduleAsync` call is
  logged but its `Result` is otherwise not further inspected.
...
- **Where it's used**: raised by [`User.Delete`](#user) (`User.cs:443-451`) and asserted by the domain tests (`MMCA.ADC/Tests/Modules/Identity/MMCA.ADC.Identity.Domain.Tests/Users/UserInvariantsAndRoleTests.cs:271-281`).
...
- **Caveats / not-in-source**: like the domain [`UserDeleted`](#userdeleted), no handler subscribes to it in ADC source today. Session revocation on a password change is not driven from this event, and the aggregate itself no longer carries a refresh token to revoke: sessions live in their own table (`User.cs:438-440`).
...
> MMCA.ADC.Identity.UI · `MMCA.ADC.Identity.UI.Services` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Services/IEmailConfirmationUIService.cs:15` · Level 3 · interface

- **What it is**: the client-side contract for the email-confirmation flow: redeeming a confirmation link's token, and asking for a fresh one when the first has expired or was never received.
- **Depends on**: [`Result`](group-01-result-error-handling.md#result); externals: BCL (`Task`, `CancellationToken`, `string`).
- **Concept reinforced, the bespoke UI service contract (introduced at [`IUserUIService`](#iuseruiservice)).** `[Rubric §18, UI Architecture]` and `[Rubric §26, Front-End Security]` (assesses that a resend cannot be used to enumerate registered addresses). Two members, both returning `Result` so a page branches on the outcome instead of catching. `ResendAsync`'s own doc comment states the anti-enumeration rule directly (`IEmailConfirmationUIService.cs:33-36`): the endpoint always answers success whether or not the address holds an account, so the client-side contract cannot be used to probe which addresses are registered.
- **Walkthrough**: two methods.
  - `ConfirmAsync(string email, string token, CancellationToken cancellationToken = default)` (`IEmailConfirmationUIService.cs:31`) redeems a single-use token for an address.
  - `ResendAsync(string email, CancellationToken cancellationToken = default)` (`IEmailConfirmationUIService.cs:39`) requests a fresh link, always reported as success at this layer regardless of whether the address is known.
- **Why it's built this way**: a narrow interface here, rather than the generic [`IEntityService<,>`](group-15-common-ui-framework.md#ientityservicetentitydto-tidentifiertype), matches the same reasoning [`IUserUIService`](#iuseruiservice) documents: confirming an email is not a CRUD operation on a resource with an identity, so it gets its own contract instead of being bent to fit one (`[Rubric §14, Testability]`).
- **Where it's used**: implemented by [`EmailConfirmationService`](#emailconfirmationservice); injected into the `ConfirmEmail` page (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Pages/Users/ConfirmEmail/ConfirmEmail.razor.cs`); registered in `AddIdentityUI` (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/DependencyInjection.cs`).
...
- **Caveats / not-in-source**: the type's own doc comment says the event is "Published by `AuthenticationService` AFTER the unit-of-work commit" (`UserRegistered.cs:7-9`), and the `User.Create` remarks repeat the phrasing (`User.cs:186-190`); neither still describes the mechanism. Current source raises it via `AddDomainEvent` on the aggregate after the first save, inside the transaction, so the outbox row commits with the insert (`AuthenticationService.cs:134-135`). The code is ground truth here; both doc comments predate the atomicity fix described at `AuthenticationService.cs:28-38`. Whether `UserRegisteredHandler` currently reads `EmailVerified` to gate the auto-link, or the flag is carried on the wire ahead of that consumer-side change, is not determinable from this unit's source.

### EmailConfirmationService
> MMCA.ADC.Identity.UI · `MMCA.ADC.Identity.UI.Services` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/Services/EmailConfirmationService.cs:21` · Level 4 · class (sealed)

- **What it is**: the HTTP implementation of [`IEmailConfirmationUIService`](#iemailconfirmationuiservice). It posts to the confirm and resend endpoints and turns every response into a [`Result`](group-01-result-error-handling.md#result).
- **Depends on**: [`IEmailConfirmationUIService`](#iemailconfirmationuiservice), [`HttpResultExecutor`](group-15-common-ui-framework.md#httpresultexecutor), [`ProblemDetailsResultReader`](group-08-auth.md#problemdetailsresultreader); externals: `IHttpClientFactory`.
- **Concept reinforced, the same sandwich as [`UserService`](#userservice), at its smallest.** `[Rubric §18, UI Architecture]`, `[Rubric §29, Resilience & Business Continuity]`. Both public methods delegate to one private `PostAsync<TRequest>` helper (`EmailConfirmationService.cs:38-56`), which wraps [`HttpResultExecutor.ExecuteAsync`](group-15-common-ui-framework.md#httpresultexecutor) around a `PostAsJsonAsync` call and reads the body back through [`ProblemDetailsResultReader.ReadAsync`](group-08-auth.md#problemdetailsresultreader). Unlike [`UserService`](#userservice), this class does not use `AuthenticatedServiceBase`: it resolves a plain `IHttpClientFactory` client by name (`ApiClientName`, `EmailConfirmationService.cs:23`), because both endpoints it calls are anonymous, there is no bearer token to attach.
- **Walkthrough**: two public methods and one private helper.
  - `ConfirmAsync(string email, string token, CancellationToken cancellationToken = default)` (`EmailConfirmationService.cs:26-31`) posts `ConfirmEmailRequest(email, token)` to `Auth/confirm-email`.
  - `ResendAsync(string email, CancellationToken cancellationToken = default)` (`EmailConfirmationService.cs:34-36`) posts `SendEmailConfirmationRequest(email)` to `Auth/send-email-confirmation`.
  - `PostAsync<TRequest>(string relativeUrl, TRequest body, CancellationToken cancellationToken)` (`EmailConfirmationService.cs:38-56`) is the shared sandwich; the comment above the POST call states why there is no retry (`EmailConfirmationService.cs:47-48`): both endpoints spend a single-use token or a per-address request budget, so a retried call is a second request rather than a replayed response, the same non-idempotent reasoning [`UserService.UploadMyAvatarAsync`](#userservice) applies to its own no-retry call.
- **Why it's built this way**: keeping this class free of `AuthenticatedServiceBase` is a correctness choice, not an oversight: attaching a bearer token to an anonymous endpoint would do nothing useful, and the class stays honest about which calls need authentication and which do not.
- **Where it's used**: registered as `IEmailConfirmationUIService` in `AddIdentityUI` (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.UI/DependencyInjection.cs`) and consumed by the `ConfirmEmail` page through the interface.
...
- **Where it's used**: auto-registered as `IValidator<RegisterRequest>` by `ScanModuleApplicationServices<ClassReference>()` (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:58`) and injected into [`AuthenticationValidators`](group-08-auth.md#authenticationvalidators), which [`AuthenticationService`](#authenticationservice) receives through its base and runs at the top of the registration workflow. The endpoint is `POST auth/register` on [`AuthController`](#authcontroller).
...
- **What it is**: a one-field internal command carrying the blob name to delete from storage: `DeleteAvatarBlobInternalCommand(string BlobName)` (`DeleteAvatarBlobInternalCommand.cs:17`).
- **Depends on**: [`IInternalCommand`](group-14-module-system-composition.md#iinternalcommand); externals: none.
- **Concept reinforced, the fire-and-forget internal command.** `[Rubric §6, CQRS & Event-Driven]`: marking this `IInternalCommand` rather than a public `ICommand` signals it is dispatched from inside the module's own handlers, not from an API controller. [`DeleteAvatarBlobInternalCommandHandler`](#deleteavatarblobinternalcommandhandler) is the sole handler.
- **Walkthrough**: a `sealed record` with one positional parameter, `BlobName`. No validation on the type itself; that lives in [`DeleteAvatarBlobInternalCommandValidator`](#deleteavatarblobinternalcommandvalidator).
- **Why it's built this way**: three call sites ([`DeleteUserHandler`](#deleteuserhandler), [`RemoveUserAvatarHandler`](#removeuseravatarhandler), [`SetUserAvatarHandler`](#setuseravatarhandler)) each need to delete a blob after a domain change succeeds; carrying only the blob name keeps the command decoupled from which use case triggered it.
- **Where it's used**: dispatched by [`DeleteUserHandler`](#deleteuserhandler), [`RemoveUserAvatarHandler`](#removeuseravatarhandler), and [`SetUserAvatarHandler`](#setuseravatarhandler); handled by [`DeleteAvatarBlobInternalCommandHandler`](#deleteavatarblobinternalcommandhandler).
...
- **What it is**: the command that kicks off the email-confirmation send, wrapping a [`SendEmailConfirmationRequest`](group-08-auth.md#sendemailconfirmationrequest) (`SendEmailConfirmationCommand.cs:20-21`).
- **Depends on**: [`ICommandWithRequest<TRequest>`](group-05-cqrs-pipeline.md#icommandwithrequestout-trequest), [`IInternalCommand`](group-14-module-system-composition.md#iinternalcommand), [`SendEmailConfirmationRequest`](group-08-auth.md#sendemailconfirmationrequest); externals: none.
- **Concept reinforced, the dual-marker command.** Implementing both `ICommandWithRequest<TRequest>` and `IInternalCommand` in one declaration lets the same command type be dispatched from a public controller action (carrying a wire request) and re-dispatched internally (e.g. after registration) without a second type.
- **Walkthrough**: a `sealed record` wrapping one `SendEmailConfirmationRequest Request` property.
- **Where it's used**: raised by [`EmailConfirmationController`](#emailconfirmationcontroller) and [`AuthController`](#authcontroller); handled by [`SendEmailConfirmationHandler`](#sendemailconfirmationhandler).
...
- **What it is**: the composition point that applies every module's permission-grant map to one host's shared [`PermissionRegistryBuilder`](group-08-auth.md#permissionregistrybuilder), used by the Identity Service host that issues tokens for all four modules.
- **Depends on**: [`PermissionRegistryBuilder`](group-08-auth.md#permissionregistrybuilder), [`IdentityPermissionGrants`](#identitypermissiongrants), [`ConferencePermissionGrants`](group-17-conference-domain.md#conferencepermissiongrants), [`EngagementPermissionGrants`](group-22-engagement-module.md#engagementpermissiongrants), [`NotificationPermissionGrants`](group-10-notifications.md#notificationpermissiongrants); externals: `Microsoft.Extensions.DependencyInjection.IServiceCollection`.
...
- **Walkthrough**
  ...
  - `IsValid(string role)` (`UserRole.cs:66`) is the boolean form used by [`UserInvariants.EnsureRoleIsValid`](#userinvariants), with the same null-to-empty coalesce.
...
- **Where it's used**: by [`UserInvariants.EnsureRoleIsValid`](#userinvariants) (`UserInvariants.cs:65`); by [`User`](#user)'s EF constructor default and by `CreateExternal`, both writing `UserRole.Attendee.Value` (`User.cs:163`, `:218`); by [`AuthenticationService`](#authenticationservice) for the BR-45 attendee default on registration (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/AuthenticationService.cs:147`); by the seeder (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Infrastructure/Persistence/DbContexts/Seeding/IdentityModuleDbSeeder.cs:36-38`); and by the two ownership-checked handlers whose privilege override is a one-liner, [`DeleteUserHandler`](#deleteuserhandler) (`.../UseCases/DeleteUser/DeleteUserHandler.cs:42-43`) and [`ExportUserDataHandler`](#exportuserdatahandler) (`.../UseCases/ExportUserData/ExportUserDataHandler.cs:38`).
...
### DeleteAvatarBlobInternalCommandHandler
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.DeleteAvatarBlob` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/DeleteAvatarBlob/DeleteAvatarBlobInternalCommandHandler.cs:21` · Level 5 · class (sealed, partial)

- **What it is**: the handler for [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand): deletes an avatar blob from storage and tolerates the blob already being gone.
- **Depends on**: [`IFileStorageService`](group-07-persistence-ef-core.md#ifilestorageservice), [`ICommandHandler<TCommand, TResult>`](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult), [`ErrorType`](group-01-result-error-handling.md#errortype), [`Result`](group-01-result-error-handling.md#result); externals: `Microsoft.Extensions.Logging` (`[LoggerMessage]` source generation).
- **Concept reinforced, idempotent cleanup on a not-found.** `[Rubric §29, Resilience & Business Continuity]`: `HandleAsync` (`DeleteAvatarBlobInternalCommandHandler.cs:119-141`) treats `ErrorType.NotFound` from `fileStorage.DeleteAsync` as success (`:133-137`), so a retried delete (at-least-once dispatch, or a blob already removed by a prior attempt) does not surface as a failure.
- **Walkthrough**: `HandleAsync` awaits `fileStorage.DeleteAsync(command.BlobName, cancellationToken)` (`:125`). On success it logs `LogAvatarBlobDeleted` and returns (`:127-131`). On a not-found error it logs `LogAvatarBlobAlreadyGone` and returns `Result.Success()` (`:133-137`). Any other failure logs `LogAvatarBlobDeleteFailed` with the first error's message and returns the failed result unchanged (`:139-140`), so the row is retried. The three `[LoggerMessage]` partial methods (`:143-150`) are the source-generated logging pattern used across the codebase.
- **Where it's used**: registered as the handler for [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand) through the module's DI registration.
- **Caveats / not-in-source**: the class is `partial` to host the source-generated `[LoggerMessage]` methods; the generated bodies are not in this file.

### DeleteAvatarBlobInternalCommandValidator
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.DeleteAvatarBlob` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/DeleteAvatarBlob/DeleteAvatarBlobInternalCommandValidator.cs:10` · Level 5 · class (sealed)

- **What it is**: the FluentValidation validator for [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand): requires `BlobName` to be non-empty (`DeleteAvatarBlobInternalCommandValidator.cs:10-15`).
- **Depends on**: `FluentValidation.AbstractValidator<T>` (external), [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand).
- **Concept reinforced, the one-rule request validator.** A minimal sibling of the validators taught in depth elsewhere in this group ([`RegisterRequestValidator`](#registerrequestvalidator), [`SetUserAvatarCommandValidator`](#setuseravatarcommandvalidator)): one `RuleFor` in the constructor, `NotEmpty()`.
- **Where it's used**: wired into the CQRS pipeline's decorator chain for [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand); its presence is asserted by the architecture fitness test `CommandValidatorCoverageTests`, which checks every command has a validator.
...
- **Where it's used**: by [`User.Create`](#user) (`User.cs:210-214`), [`User.CreateExternal`](#user) (`User.cs:258-259`), [`User.UpdatePreferences`](#user) (`User.cs:333-334`), and [`User.ChangePassword`](#user) (`User.cs:363-364`); the length constants are reused by [`RegisterRequestValidator`](#registerrequestvalidator) and [`UserConfiguration`](#userconfiguration).
- **Caveats / not-in-source**: `EnsureEmailIsValid` has no callers in current ADC source. The aggregate validates its address through the [`Email`](group-02-domain-building-blocks.md#email) value object's own `Create` instead (`User.cs:207`, `:207`, `:368`), so the email invariant here is a second, currently unused expression of the same rule.
...
- **Concept introduced, the interface list as a workflow contract.** `[Rubric §4, DDD]` assesses whether the aggregate is the consistency boundary and enforces its own invariants, and `[Rubric §1, SOLID]` assesses interface segregation: five narrow capability interfaces instead of one fat base (`User.cs:34-35`). The class remarks (`User.cs:18-32`) explain something genuinely easy to get wrong. The shared framework workflows are generic over capability interfaces: [`ChangePasswordHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#changepasswordhandlerbasetuser-tcommand) constrains on `IPasswordChangeableUser` (`MMCA.Common/Source/Core/MMCA.Common.Application/Users/UseCases/ChangePassword/ChangePasswordHandlerBase.cs:42`), the erasure workflow constrains on `IErasableUser`, and [`SendEmailConfirmationHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#sendemailconfirmationhandlerbasetuser-tcommand) constrains on `IEmailConfirmableUser` (added for [ADR-116](https://ivanball.github.io/docs/adr/116-identity-completions-opt-in.html)). Listing `IErasableUser` **on this type directly** is load-bearing because `Delete` here *hides* the base soft-delete with `new` (`User.cs:443`); only re-declaring the interface on `User` re-maps the interface slot onto this type's own member, which is what keeps the [`UserDeleted`](#userdeleted) domain event inside the shared erasure path. Remove the interface from the declaration list and the code still compiles while quietly calling the base method instead.
  ...
  - `[IdValueGenerated]` (`User.cs:33`) tells the persistence layer the id is database-generated. That single attribute is the root cause of the [`UserRegistered`](#userregistered) two-save dance: the id does not exist until after the insert, which the `Create` doc states outright (`User.cs:185-191`).
- **Walkthrough**
  - **State** (`User.cs:50-133`). `IsEmailConfirmed` (`User.cs:47`) records whether the address has been proven reachable, either by redeeming a confirmation link or by an external provider asserting it (ADR-116). `Email` is an [`Email`](group-02-domain-building-blocks.md#email) value object, not a string, and is marked `[Pii]` along with `FirstName`, `LastName`, and `AvatarUrl`; the `[Pii]` marks are what let the privacy tooling find personal columns ([ADR-005](https://ivanball.github.io/docs/adr/005-soft-delete-vs-erasure.html)). `PasswordHash` and `PasswordSalt` are `byte[]` mapped to `varbinary(max)`, inside a scoped `CA1819` suppression that names EF mapping as the reason. `Role` is a `string` for EF and JWT round-tripping while [`UserRole`](#userrole) owns the rules. `LinkedSpeakerId` is a nullable `SpeakerIdentifierType` scalar, the 1:1 bidirectional counterpart of `Speaker.LinkedUserId` (BR-208). `LockedOn` (`User.cs:491`) records when an administrator locked the account, or `null` while usable; a locked account keeps every row it owns (bookmarks, feedback, points) and can be unlocked, which separates it from the irreversible `Anonymize`. Every setter is `private`: state changes only through the methods below.
  ...
  - **`Create`** (`User.cs:563-591`): parses the email, then `Result.Combine`s the email result with five [`UserInvariants`](#userinvariants) checks, returns `Result.Failure<User>(result.Errors)` on any failure, and otherwise constructs with `Id = default`.
...
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.SendEmailConfirmation` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/SendEmailConfirmation/SendEmailConfirmationHandler.cs:27` · Level 14 · class (sealed)

- **What it is**: ADC's concrete handler for [`SendEmailConfirmationCommand`](#sendemailconfirmationcommand), supplying only the one piece the shared framework base cannot know: how to look up a [`User`](#user) by email.
- **Depends on**: [`SendEmailConfirmationHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#sendemailconfirmationhandlerbasetuser-tcommand), [`User`](#user), [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IEmailConfirmationTokenService`](group-08-auth.md#iemailconfirmationtokenservice), [`IEmailSender`](group-10-notifications.md#iemailsender), [`EmailConfirmationSettings`](group-08-auth.md#emailconfirmationsettings), [`SendEmailConfirmationCommand`](#sendemailconfirmationcommand); externals: `Microsoft.Extensions.Options`, `Microsoft.Extensions.Logging`.
- **Concept reinforced, the template-method handler.** `[Rubric §2, Design Patterns]`: the framework base class owns the whole send workflow (token issuance, email dispatch, logging) and declares one abstract extension point, `FindUntrackedByEmailAsync`; ADC's job is a two-line override, not a reimplementation. This is the same shape as [`ChangePasswordHandler`](#changepasswordhandler) over `ChangePasswordHandlerBase<TUser, TCommand>`.
- **Walkthrough**: the primary constructor forwards `unitOfWork`, `tokenService`, `emailSender`, `settings`, `logger` straight to the base (`SendEmailConfirmationHandler.cs:25-26`). `FindUntrackedByEmailAsync` (`:29-33`) queries `UnitOfWork.GetReadRepository<User, UserIdentifierType>()` with `asTracking: false` for the first `User` whose `Email` matches, returning `null` when there is none.
- **Why it's built this way**: reading untracked (`asTracking: false`) is deliberate: the confirmation-send path only needs to read the user to email a token, never to mutate the entity, so skipping EF's change tracker avoids paying its cost for a query that will not be saved.
- **Where it's used**: registered as the handler for [`SendEmailConfirmationCommand`](#sendemailconfirmationcommand); invoked by [`EmailConfirmationController`](#emailconfirmationcontroller) and [`AuthController`](#authcontroller).
...
> MMCA.ADC.Identity.API · `MMCA.ADC.Identity.API.Controllers` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/Controllers/EmailConfirmationController.cs:34` · Level 6 · class (sealed)

- **What it is**: the two-action email-confirmation surface: `POST send-email-confirmation`, which schedules a confirmation link, and `POST confirm-email`, which redeems it.
- **Depends on**: [`ApiControllerBase`](group-12-api-hosting-mapping.md#apicontrollerbase), [`ICommandHandler<in TCommand, TResult>`](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult) closed over [`SendEmailConfirmationCommand`](#sendemailconfirmationcommand) and [`ConfirmEmailCommand`](#confirmemailcommand), [`IdempotentAttribute`](group-12-api-hosting-mapping.md#idempotentattribute); externals: ASP.NET Core rate limiting.
- **Concept reinforced, the enumeration-safe anonymous endpoint.** `[Rubric §11, Security]`, `[Rubric §30, Compliance, Privacy & Data Governance]`. `SendEmailConfirmationAsync` (`EmailConfirmationController.cs:44-53`) always answers `202 Accepted`, whether or not the address holds an account and whether or not it is already confirmed (`:30-32`): any other answer would let a caller enumerate registered addresses, the same posture [`PasswordResetController`](#passwordresetcontroller) takes on `forgot-password`. Both actions carry `[AllowAnonymous]`, `[Idempotent]`, and `[EnableRateLimiting(WebApplicationBuilderExtensions.RateLimitPolicyAuthIp)]` (`:38-40`, `:65-67`), the same `"auth-ip"` fixed window that guards login, register, and password reset ([ADR-019](https://ivanball.github.io/docs/adr/019-rate-limiting.html)).
- **Walkthrough**
  - `SendEmailConfirmationAsync` (`:44-53`): dispatches [`SendEmailConfirmationCommand`](#sendemailconfirmationcommand) and returns `Accepted()` on success.
  - `ConfirmEmailAsync` (`:71-80`): dispatches [`ConfirmEmailCommand`](#confirmemailcommand) and returns `NoContent()`. The doc comment states the transport reason the link works the way it does (`:56-58`): the address and the token travel in the URI fragment, so the confirmation page reads them client-side and posts them here, and neither ever reaches a server log or a `Referer` header.
...
- **Where it's used**: registration schedules a `SendEmailConfirmationCommand` from [`AuthController`](#authcontroller) after a successful `POST register` (ADR-116); the confirmation page then calls `confirm-email` with the token from the emailed link. The architecture suite's anonymous-endpoint test enumerates both actions explicitly.
...
- **Depends on**: [`IUserAdminUIService<TUserDto>`](group-15-common-ui-framework.md#iuseradminuiservicetuserdto) (closed over `UserAdminDTO`), [`IAppDialogService`](group-15-common-ui-framework.md#iappdialogservice), [`IToastService`](group-15-common-ui-framework.md#itoastservice), [`RoleNames`](#rolenames); externals: Blazor's `AuthenticationStateProvider`, `BreadcrumbItem`.
...
- **Walkthrough**
  ...
  - `OnParametersSetAsync` (`:154`): calls `LoadAsync` on every parameter set rather than only on first render, so navigating from one account to another inside the same page instance reloads the account named in the URL, the same pattern [`UserList`](#userlist)'s sibling pages rely on.
...
- **Where it's used**: routed as the organizer user-management page in the ADC web and MAUI UI; the server side it calls for the roster is [`UsersController`](#userscontroller), whose list endpoint is gated on the `UsersRead` capability in [`IdentityPermissions`](#identitypermissions) (`UsersController.cs:139`); the lock/unlock/role actions it delegates to the base call [`UsersAdminController`](#usersadmincontroller). [`RoleList`](#rolelist) reuses `UserList.razor.cs` as a reference for the same list-shell pattern applied to roles.
...
- **What it is**: ADC's `Admin/Users` endpoint surface (list, get, lock, unlock, set role). It declares no new behavior beyond two attribute-only overrides: every action is inherited from [`UsersAdminControllerBase<TUserDto>`](group-12-api-hosting-mapping.md#usersadmincontrollerbasetuserdto).
- **Depends on**: [`UsersAdminControllerBase<TUserDto>`](group-12-api-hosting-mapping.md#usersadmincontrollerbasetuserdto) (base, closed over `UserAdminDTO`), [`IUserAdministrationService<TUserDto>`](group-08-auth.md#iuseradministrationservicetuserdto), [`NonIdempotentAttribute`](group-12-api-hosting-mapping.md#nonidempotentattribute).
- **Concept reinforced, the sibling controller pattern applied to administration.** `[Rubric §15, Best Practices & Code Quality]`. Like [`OAuthController`](#oauthcontroller) and [`UsersDataExportController`](#usersdataexportcontroller), the class exists to bind a shared base to an app-specific service, here [`UserAdministrationService`](#useradministrationservice) through [`IUserAdministrationService<TUserDto>`](group-08-auth.md#iuseradministrationservicetuserdto). Both overrides carry `[NonIdempotent(...)]` (`UsersAdminController.cs:32`, `:41`), and each message explains why replaying a cached response would be wrong rather than merely unnecessary: locking and unlocking are already idempotent in the domain (a second lock keeps the original instant, `:32`), so caching a response risks answering a later unlock-then-lock cycle from the cache and reporting a state the account is no longer in.
- **Walkthrough**: two overrides, `LockAsync` (`:23-24`) and `UnlockAsync` (`:32-33`), each a one-line `base.___Async(...)` call whose only reason to exist is the attribute the architecture suite requires on every POST action.
...
- **Where it's used**: mounted by the Identity service host, routed through the Gateway under `Admin/Users`; driven by [`UserDetail`](#userdetail) and [`UserList`](#userlist) through the framework's `IUserAdminUIService<TUserDto>` client. [`AdminRolesController`](#adminrolescontroller) is the equivalent surface for roles.
...
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.Administration` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/Administration/UserAdministrationService.cs:38` · Level 8 · class (sealed, partial)

- **What it is**: ADC's implementation of the framework's `IUserAdministrationService<UserAdminDTO>`, the service `UsersAdminController` calls: list/search the roster, get one account, lock/unlock, and change role.
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor), [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore), `TimeProvider`, [`User`](#user), [`UserRole`](#userrole), [`PagingMath`](group-03-querying-specifications.md#pagingmath), [`PaginationMetadata`](group-01-result-error-handling.md#paginationmetadata) / [`PagedCollectionResult<T>`](group-01-result-error-handling.md#pagedcollectionresultt), and `[LoggerMessage]` logging.
- **Concept introduced, revoking live sessions on a privilege-relevant write.** `[Rubric §11, Security]`, `[Rubric §29, Resilience & Business Continuity]`. Both `SetLockedAsync` (`:627-659`) and `SetRolesAsync` (`:662-712`) call `RevokeSessionsAsync` (`:762-779`) after a successful lock or a genuine role change, but only then: locking a session that is already locked, or reassigning a role to the value it already holds, returns early and revokes nothing (`:696-701` for the role case). The comment at `:705-706` states why revocation matters for role: the role travels in the access token, so leaving live sessions alone would keep the old capability set working until every issued token expired on its own. Locking revokes unconditionally because the account must stop working immediately, not merely stop being able to mint new tokens.
- **Walkthrough**
  - `MaxPageSize = 100` (`:566`): restated rather than referenced from the controller's own constant, because the Application layer does not depend on the API package; the comment notes a drift here can only make this the tighter of the two caps.
  - `ListAsync` (`:569-608`): clamps paging through [`PagingMath.Clamp`](group-03-querying-specifications.md#pagingmath) (`:576`), narrows with `ApplyFilters`, counts, then orders `.OrderBy(u => u.Email).ThenBy(u => u.Id)` before `Skip`/`Take` (`:588-590`). The `Id` tie-break is called out in the surrounding comment (`:586-587`): without it, rows sharing an email sort key can repeat or vanish across pages, because `OFFSET`/`FETCH` has no stable row order to page over otherwise. The projection to `UserAdminDTO` happens in the query (`:593-602`).
  - `ApplyFilters` (`:739-760`): the search term matches the whole email address by parsing it through [`Email.Create`](group-02-domain-building-blocks.md#email) first (`:744-748`), because `User.Email` is a converter-backed value object the provider can only compare against another converted `Email`; an unparseable term simply matches no address, the same answer the database gives for an address nobody holds. The two name predicates use `Contains` and stay independent of whether the email half parsed.
  - `SetRolesAsync` (`:662-712`): rejects anything but exactly one role with a `User.Role.ExactlyOne` validation error (`:667-673`, ADC accounts hold exactly one role), parses it through [`UserRole.FromString`](#userrole), then defers to `User.ChangeRole` for the domain rule.
  - `GetAsync` / `ToDto` (`:611-624`, `:714-723`): a not-found `Result` when the id misses, otherwise the same DTO shape `ListAsync` projects, assembled here from a tracked-or-not-tracked load instead of a query projection.
  - `RevokeSessionsAsync` (`:762-779`): loads the account's unrevoked sessions, revokes each with `RefreshSession.ReasonSignedOut`, and saves once; an empty session set returns immediately without a write.
- **Why it's built this way**: the two `[LoggerMessage]` partials (`:781-785`) log every lock/unlock and every role change at Information with the acting context, so an audit trail of administrative account changes exists without a bespoke audit table.
- **Where it's used**: registered as `IUserAdministrationService<UserAdminDTO>` in the Application layer's `DependencyInjection` and resolved by [`UsersAdminController`](#usersadmincontroller); its DTO shape is the one [`UserDetail`](#userdetail) and [`UserList`](#userlist) render.
...
> MMCA.ADC.Identity.API · `MMCA.ADC.Identity.API.Controllers` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/Controllers/AdminRolesController.cs:36` · Level 11 · class (sealed)

- **What it is**: ADC's role-administration endpoint surface. Like [`OAuthController`](#oauthcontroller), it is a body-less class: every action comes from [`RolesAdminControllerBase`](group-12-api-hosting-mapping.md#rolesadmincontrollerbase), and this file exists only to supply the concrete dependencies.
- **Depends on**: [`RolesAdminControllerBase`](group-12-api-hosting-mapping.md#rolesadmincontrollerbase) (base), [`IRoleAdministrationService`](group-08-auth.md#iroleadministrationservice), [`ICurrentUserService`](group-08-auth.md#icurrentuserservice).
- **Concept reinforced, the derived-controller-as-configuration pattern.** `[Rubric §15, Best Practices & Code Quality]`. The whole class is a primary constructor forwarding two dependencies to the base, terminated with a semicolon (`AdminRolesController.cs:36-37`): no route, no version attribute, no action body, the same shape [`OAuthController`](#oauthcontroller) and [`UsersDataExportController`](#usersdataexportcontroller) use for a workflow that is app-agnostic once the app's own service is supplied.
- **Walkthrough**: one line, `public sealed class AdminRolesController(IRoleAdministrationService administration, ICurrentUserService currentUser) : RolesAdminControllerBase(administration, currentUser);` (`:36-37`).
- **Why it's built this way**: role administration (list roles, view members, grant/revoke) is identical in every app that shares the framework's role vocabulary, so the base owns the whole surface and each app contributes only the service that reads and writes its own `User` rows.
- **Where it's used**: mounted by the Identity service host's `Program.cs` and routed through the Gateway; [`UsersAdminController`](#usersadmincontroller) is the equivalent surface for individual accounts rather than roles.
...
### AuthController
> MMCA.ADC.Identity.API · `MMCA.ADC.Identity.API.Controllers` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/Controllers/AuthController.cs:33` · Level 17 · class (sealed, partial)
...
- **Why it's built this way**: `[Route("[controller]")]` makes the prefix `/auth`, which is what the Gateway's route map fronts, and `[ApiVersion("1.0")]` keeps it on the header-based versioning scheme ([ADR-046](https://ivanball.github.io/docs/adr/046-http-api-versioning.html)). The class is now `partial` (`:33`) because the `[LoggerMessage]` source generator needs a partial method declaration for `LogConfirmationScheduleFailed` (`:965-966`). Both anonymous actions are marked `[AllowAnonymous]` with the fully qualified attribute name because the file's `using` set does not import the authorization namespace.
- **Where it's used**: the entry point for every authenticated ADC client. The Blazor and MAUI clients call login/register/refresh through the Gateway; the [`Profile`](#profile) page uses the inherited `PUT auth/password` and the preferences pair; [`ChangePasswordRequestValidator`](#changepasswordrequestvalidator) guards the password action through the Validating decorator; a successful register schedules the confirmation link handled by [`EmailConfirmationController`](#emailconfirmationcontroller). Its anonymous sibling on the same `/Auth` prefix is [`PasswordResetController`](#passwordresetcontroller).
...
- **Where it's used**: constructed by [`AuthController`](#authcontroller)'s `CreateChangePasswordCommand` override (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/Controllers/AuthController.cs:118-120`) and handled by [`ChangePasswordHandler`](#changepasswordhandler).
...
- **Where it's used**: constructed by [`AuthController`](#authcontroller)'s `CreateChangePreferencesCommand` override (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.API/Controllers/AuthController.cs:123-125`) for the profile page and the app-bar culture and theme switchers; handled by [`ChangePreferencesHandler`](#changepreferenceshandler).

### DeleteUserCommand
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.DeleteUser` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/DeleteUser/DeleteUserCommand.cs:19` · Level 8 · record (sealed)
...
- **Depends on**: the `UserIdentifierType` alias; [`User`](#user) (for `typeof(User).FullName`); [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating); [`ITransactional`](group-05-cqrs-pipeline.md#itransactional); [`IUserOwnedRequest`](group-14-module-system-composition.md#iuserownedrequest).
...
- **Concept introduced, marking erasure as one atomic write.** `[Rubric §29, Resilience & Business Continuity]` and `[Rubric §30, Compliance, Privacy & Data Governance]`. The record now also implements `ITransactional` (`DeleteUserCommand.cs:46-51`), and its `<remarks>` explains why: the soft-delete flag, the anonymized personal data, the `UserDeleted` outbox row, and the internal-command row that schedules the avatar blob delete ([ADR-114](https://ivanball.github.io/docs/adr/114-internal-commands-durable-job-queue.html)) all need to commit together or not at all. [`DeleteUserHandler`](#deleteuserhandler)'s tail enrolls that internal-command row before the save, and an enrolled row is exactly what a transaction makes atomic with the change it belongs to; without `ITransactional` the schedule could flush ahead of a half-finished erasure.
...
- **Walkthrough**
  ...
  - `IsEmailConfirmed` (`:108-113`): required, with `HasDefaultValue(false)` ([ADR-116](https://ivanball.github.io/docs/adr/116-identity-completions-opt-in.html)). The comment explains the default: it keeps the column non-nullable for rows written by anything that does not set the flag, and the migration backfills the accounts that existed before it did.
...
- **Depends on**: [`ChangePasswordHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#changepasswordhandlerbasetuser-tcommand) (base), [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IPasswordHasher`](group-08-auth.md#ipasswordhasher), `ILogger<T>`, [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore), `TimeProvider`, [`User`](#user), and [`ChangePasswordCommand`](#changepasswordcommand).
...
- **Concept introduced, revoking live sessions on a credential change (SEC-Common-02).** `[Rubric §11, Security]`. The constructor now also takes [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore) and a `TimeProvider`, forwarded straight to the base (`ChangePasswordHandler.cs:24-35`), so a successful change revokes every live refresh session on the account, not only the credential the caller just replaced. Without them a stolen refresh chain would outlive the very password change meant to remediate it, which is the one recovery step a compromised account has. [`ResetPasswordHandler`](#resetpasswordhandler) carries the identical addition for the same reason.
- **Walkthrough**: a primary constructor taking [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IPasswordHasher`](group-08-auth.md#ipasswordhasher), `ILogger<ChangePasswordHandler>`, [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore) and `TimeProvider`, forwarding all five to the base (`ChangePasswordHandler.cs:24-35`), with an empty body (`:36-37`). The inherited workflow (`ChangePasswordHandlerBase.cs:56-101`) is: null-guard the command (`:46`); load the user through the mutating repository and return `Error.NotFound` tagged with `HandlerName` when absent (`:48-53`); verify the supplied current password with `passwordHasher.VerifyPassword(command.Request.CurrentPassword, user.PasswordHash, user.PasswordSalt)` and return `Error.Unauthorized("Auth.InvalidCurrentPassword", ...)` on a mismatch (`:55-59`); hash the new password into a fresh hash and salt pair (`:61`); call `user.ChangePassword(newHash, newSalt)` and, only when that succeeds, save and log (`:62-67`); return the aggregate's [`Result`](group-01-result-error-handling.md#result) unchanged (`:69`).
- **Why it's built this way**: `[Rubric §11, Security]`. Note the division of responsibility the base encodes. Proving knowledge of the current password is a cryptographic operation, so it happens where the stored hash and salt are in hand, not in a request validator (a validator can only check that a value is present). The domain then applies its own invariants on the new credential material, and nothing is persisted unless both gates pass. The generic constraint `where TUser : AuditableAggregateRootEntity<UserIdentifierType>, IPasswordChangeableUser` (`ChangePasswordHandlerBase.cs:42`) is what lets the base call `ChangePassword` on an application's aggregate without knowing the concrete type, and the hashing scheme itself is [ADR-032](https://ivanball.github.io/docs/adr/032-password-hashing.html).
- **Where it's used**: resolved as `ICommandHandler<ChangePasswordCommand, Result>` and injected into [`AuthController`](#authcontroller) (`AuthController.cs:36`), which exposes it through the base's `PUT /auth/password` action; the [`Profile`](#profile) page is the client.
...
- **Where it's used**: registered by [`DependencyInjection`](#dependencyinjection-4)'s `AddIdentityAttendeeClient` in this same project, which the Notification service host calls inside its application-pipeline registration (`MMCA.ADC/Source/Services/MMCA.ADC.Notification.Service/Program.cs:225`). Its server-side counterpart is [`AttendeesGrpcService`](#attendeesgrpcservice).
...
- **Why it's built this way**: `[Rubric §11, Security]`. The endpoint is mapped with `app.MapGrpcService<AttendeesGrpcService>().RequireAuthorization()` (`MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/Program.cs:373`), and the comment above the mapping (`:327-334`) is worth reading in full: the response enumerates attendee user ids, so internal-only ingress is not considered sufficient, and the authorization holds today only because the single caller chain is HTTP-triggered (Notification's [`AttendeeNotificationRecipientProvider`](group-10-notifications.md#attendeenotificationrecipientprovider) under an organizer-authorized controller), which means an inbound bearer always exists for [`JwtForwardingClientInterceptor`](group-13-grpc-contracts.md#jwtforwardingclientinterceptor) to forward. The comment states the standing condition explicitly: a scheduler or broker-consumer path would run with no `HttpContext`, forward no bearer, and would need its own credential before it ships.
- **Where it's used**: registered through `AddGrpcServiceDefaults()` (`Program.cs:330`, whose comment at `:298-303` names this class as the endpoint it publishes) and mapped at `Program.cs:373`; the only client is [`AttendeeQueryServiceGrpcAdapter`](#attendeequeryservicegrpcadapter). The service serves it over Kestrel's HTTP/2 cleartext channel, the transport profile of [ADR-012](https://ivanball.github.io/docs/adr/012-grpc-host-transport.html). Reflection is mapped only in Development (`:337-340`).
...
- **Where it's used**: never referenced by name. It is discovered by the module-registration scan (`DependencyInjection.ModuleScanning.cs:117`) and injected into [`ValidatingCommandDecorator<TCommand, TResult>`](group-05-cqrs-pipeline.md#validatingcommanddecoratortcommand-tresult) as one element of `IEnumerable<IValidator<ChangePreferencesCommand>>` around [`ChangePreferencesHandler`](#changepreferenceshandler).
...
### DeleteUserHandler
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.DeleteUser` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/DeleteUser/DeleteUserHandler.cs:29` · Level 9 · class (sealed)
...
- **Depends on**: [`DeleteUserHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#deleteuserhandlerbasetuser-tcommand) (base), [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IInternalCommandScheduler`](group-14-module-system-composition.md#iinternalcommandscheduler), [`ICacheService`](group-09-caching.md#icacheservice) (taken only to forward to the base, `:32`, `:34`), `TimeProvider`, [`UserRole`](#userrole), [`UserDeleted`](#userdeleted) (the Identity.Shared integration event), [`SetUserAvatarHandler`](#setuseravatarhandler) (for its `TryGetBlobName` helper), [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand), and [`Result`](group-01-result-error-handling.md#result).
...
- **Where it's used**: resolved as `ICommandHandler<DeleteUserCommand, Result>` and invoked by [`UsersController`](#userscontroller)'s `DeleteAsync` (`UsersController.cs:166-181`), which returns 204 on success; the callers are the [`Profile`](#profile) page's self-service deletion and the organizer [`UserList`](#userlist). The [`UserDeleted`](#userdeleted) integration event it raises is consumed downstream by Engagement's [`UserDeletedPointsHandler`](group-22-engagement-module.md#userdeletedpointshandler). The scheduled [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand) is drained later by [`DeleteAvatarBlobInternalCommandHandler`](#deleteavatarblobinternalcommandhandler).
...
### ResetPasswordHandler
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.ResetPassword` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/ResetPassword/ResetPasswordHandler.cs:24` · Level 9 · class (sealed)
...
- **Depends on**: [`ResetPasswordHandlerBase<TUser, TCommand>`](group-14-module-system-composition.md#resetpasswordhandlerbasetuser-tcommand) (base), [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IPasswordHasher`](group-08-auth.md#ipasswordhasher), [`IPasswordResetTokenService`](group-08-auth.md#ipasswordresettokenservice), [`ILoginProtectionService`](group-08-auth.md#iloginprotectionservice), `ILogger<T>`, [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore), `TimeProvider`, [`User`](#user), and [`ResetPasswordCommand`](#resetpasswordcommand).
- **Concept introduced, revoking live sessions on a completed reset (SEC-Common-02).** `[Rubric §11, Security]`. The constructor now also takes [`IRefreshSessionStore`](group-08-auth.md#irefreshsessionstore) and a `TimeProvider`, forwarded straight to the base alongside the other five dependencies (`ResetPasswordHandler.cs:24-39`), so a completed reset revokes every live session on the account. A reset is the remediation for an account someone else is signed into, and without this addition it evicted nobody. [`ChangePasswordHandler`](#changepasswordhandler) carries the identical addition for the same reason.
...
- **Caveats / not-in-source**: `User.ChangePassword` raises the in-process `UserPasswordChanged` domain event (`User.cs:374`), so the successful save writes an outbox row as well as the new credential; what subscribes to it is outside this vertical. The lockout clear at `ResetPasswordHandlerBase.cs:110` runs after the commit and is awaited without a try/catch, so a failure there surfaces to the caller even though the password has already changed.
