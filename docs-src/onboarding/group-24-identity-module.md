...
implementations; its [`DependencyInjection`](#dependencyinjection) registers five services explicitly,
among them the shared
[`SoftDeletedUserValidator<TUser>`](group-14-module-system-composition.md#softdeleteduservalidatortuser)
closed over `User` and [`UserAdministrationService`](#useradministrationservice) as the module's
`IUserAdministrationService<UserAdminDTO>`
(`MMCA.ADC.Identity.Application/DependencyInjection.cs:35-42`), contributes the two
export sections in the order they appear in the exported document (`:48-49`), and leaves handlers,
mappers, validators, and domain-event handlers to `ScanModuleApplicationServices<ClassReference>()`
(`:53`). **`MMCA.ADC.Identity.Infrastructure`** holds the
...
[`IdentityPermissions`](#identitypermissions) / [`IdentityPermissionGrants`](#identitypermissiongrants)
capability constants and grant map
(`MMCA.ADC.Identity.Shared/Authorization/IdentityPermissions.cs:10`,
`IdentityPermissionGrants.cs:17`). **`MMCA.ADC.Identity.UI`** sits in the same module folder and holds the
...
application layer only after the insert has executed and a real id exists (`AuthenticationService.cs:170` does exactly
...
`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/AuthenticationService.cs:16-47`.
[`AuthenticationService`](#authenticationservice) (`AuthenticationService.cs:48`) fills in only the
context-specific pieces: `CreateUser` supplies the `Attendee` default role (BR-45, `:143-150`),
`CreateAccessToken` attaches the `speaker_id` claim when the user is linked to a speaker (BR-209,
`:153-154` via the `SpeakerClaims` helper at `:374`), `OnUserRegisteredAsync` raises the
[`UserRegistered`](#userregistered) integration event (`:170`), `ValidateLoginCandidateAsync` and
`ValidateRefreshCandidateAsync` refuse an account an administrator has locked, after the password
check so the distinct message cannot be read by anyone sweeping addresses (`:100-119`), and
`ExternalLoginAsync` drives the OAuth find-by-provider, else link-by-email, else create flow (`:196`,
body at `:208`,
...
refresh rather than from an unstated default (`:56-57`, rationale at `:42-46`); a third,
`IOptions<`[`EmailConfirmationSettings`](group-08-auth.md#emailconfirmationsettings)`>`, is handed to
the base as well (`:58`, `:69`).

Three details in that class are worth reading closely. First, **atomicity**: `RegisterAsync` wraps the
whole base workflow in `UnitOfWork.ExecuteInTransactionAsync` (`AuthenticationService.cs:89`), so
the user insert (first save) and the outbox row for `UserRegistered` (raised in
`OnUserRegisteredAsync` once the id exists, `:170-174`) commit together.
...
cross-service Conference consumer cannot resolve (`:29-41`). The external-login path does the same
through `ExternalLoginAsync` (`:196`, transaction at `:203`, with the event raised only for a
brand-new account, `:295-300`). Second, **the link-by-email gate**: linking an external identity to an
existing local account on nothing but an email match would be an account-takeover path through any
provider that hands out unverified emails, so `TryLinkProviderToExistingAccountAsync`
(`AuthenticationService.cs:319`) consults
...
`Auth.ExternalEmailNotVerified` when the answer is no (`AuthenticationService.cs:341-350`); the check
...
login (`:324-334`), and the check after it refuses an account that already signs in with a password
(`Auth.ExternalLinkRequiresLocalSignIn`, `:352-365`), because the verifier proves only the provider
side of the address and nothing proves the local row's address was ever confirmed, so only an account
with no local credentials is claimable by an email match. Its implementation
...
opposite of the obvious one: it passes `ignoreQueryFilters: true` (`AuthenticationService.cs:139-140`)
...
a legacy or placeholder row surface as a save-time surprise (`:130-138`).
...
in `AddModuleIdentityAPI` (`MMCA.ADC.Identity.API/DependencyInjection.cs:49`, map at `IdentityPermissionGrants.cs:36-38`,
...
strings rather than coining ADC names, `UsersManage` (`:26`) and `RolesManage` (`:41`), because the
framework admin controller bases gate their actions on those exact capability strings (`:19-21`,
`:32-34`). Both aliases stay out of the module's `All` list, which holds only the capabilities ADC
itself defines (`:49-52`, rationale at `:43-48`), so
[`IdentityPermissionGrants`](#identitypermissiongrants) grants `All` plus the two framework
capabilities by name to Organizer and to no other role
(`IdentityPermissionGrants.cs:32-38`, rationale at `:25-30`), which is the strict-subset property
ContentEditor is defined by (`:19-24`); `RolesManage` is granted in compiled code and never from a
stored row, because it is the only protection on the screen that edits stored rows and a deleted row
could otherwise lock every operator out of the surface that would restore it
(`IdentityPermissions.cs:36-39`). The extracted Identity host applies more than its own map:
...
two, in document order (`MMCA.ADC.Identity.Application/DependencyInjection.cs:48-49`):
...
[`DeleteAvatarBlobInternalCommandHandler`](#deleteavatarblobinternalcommandhandler), a subclass of the
framework
...
(`DeleteAvatarBlobInternalCommandHandler.cs:17-20`) whose only own member names the blob kind for the
log lines (`:23`): the base owns the idempotent delete, treating an already-missing blob as the state
the command asks for and completing the row, and returning any other failure so the framework backoff
retries it (`:10-14`). The command record itself implements the framework
[`IDeleteBlobInternalCommand`](group-14-module-system-composition.md#ideleteblobinternalcommand) under
the wire name `Identity.DeleteAvatarBlob.v1`
(`DeleteAvatarBlobInternalCommand.cs:16-17`);
...
[`DependencyInjection`](#dependencyinjection) wires the module descriptor, the framework user- and
role-administration clients, and the user service (`MMCA.ADC.Identity.UI/DependencyInjection.cs:24`,
`:31`, `:36`, `:39`). The anonymous `/confirm-email` landing page and its client are no longer
Identity types: they are the framework
[`ConfirmEmail`](group-15-common-ui-framework.md#confirmemail) page and
[`IEmailConfirmationUIService`](group-15-common-ui-framework.md#iemailconfirmationuiservice), the
latter registered by the Common UI layer
(`MMCA.Common/Source/Presentation/MMCA.Common.UI/DependencyInjection.cs:128`).
...
(`AuthenticationService.cs:89-91`) and hands off to the shared G08 engine. The request shape was
...
taken (`AuthenticationService.cs:139-140`), call `User.Create(...)` with the `Attendee` role
(`:143-150`), hash the password through the shared
...
[`UserRegistered`](#userregistered) and save again (`:170-176`); both saves sit inside the one
...
(`AuthenticationService.cs:153-154`). No password left the domain in plaintext, no cross-database
...
- **Where it's used**: registered as the `IAttendeeQueryService` implementation by the Application-layer [`DependencyInjection`](#dependencyinjection-1) (`MMCA.ADC.Identity.Application/DependencyInjection.cs:38`); consumed by the Notification module. In the extracted topology the registration is replaced by [`AttendeeQueryServiceGrpcAdapter`](#attendeequeryservicegrpcadapter), and this class becomes the code behind [`AttendeesGrpcService`](#attendeesgrpcservice) on the Identity side.
...
- **Where it's used**: registered as the `IAuthenticationService` implementation by the Application-layer [`DependencyInjection`](#dependencyinjection-1) (`MMCA.ADC.Identity.Application/DependencyInjection.cs:35`); driven by [`AuthController`](#authcontroller) for local credentials and [`OAuthController`](#oauthcontroller) for the social paths.
...
- **Walkthrough**: `AddModuleIdentityApplication(ApplicationSettings)` lives inside an `extension(IServiceCollection services)` block (`:21`). Body order (`:33-55`): `_ = applicationSettings;` discards the parameter with a comment marking it reserved for future decorator configuration; then `IAuthenticationService` to [`AuthenticationService`](#authenticationservice) (`:35`), the [`AuthenticationValidators`](group-08-auth.md#authenticationvalidators) parameter object (`:36`), `ISoftDeletedUserValidator` to `SoftDeletedUserValidator<User>` (`:37`), `IAttendeeQueryService` to [`AttendeeQueryService`](#attendeequeryservice) (`:38`), and `IUserAdministrationService<UserAdminDTO>` to `UserAdministrationService` (`:42`), the framework's opt-in Admin/Users endpoint backing (ADR-116); then the two export sections (`:48-49`), the Scrutor scan (`:53`), and `return services` (`:55`). The `AuthenticationServiceSettings` facade that used to sit between `AuthenticationValidators` and `ISoftDeletedUserValidator` was deleted on 2026-09-23: [`AuthenticationService`](#authenticationservice) now takes its two `IOptions<>` bundles directly and DI resolves them without a module-side registration, since `IOptions<T>` is satisfied by the options infrastructure itself.
- **Why it's built this way**: mixing explicit and convention registration keeps the common case zero-ceremony while retaining precise control over the handful of services that need a specific lifetime or an override point. Registering the cross-boundary implementations (`ISoftDeletedUserValidator`, `IAttendeeQueryService`) here is what closes the inversion those contracts set up: the framework and the Notification module declare the need, Identity satisfies it. Removing the `AuthenticationServiceSettings` registration line here is the DI-side half of the same 2026-09-23 change: the facade added an indirection with no state of its own, and deleting it left the two options resolving through the ordinary `IOptions<T>` pipeline instead of a module-registered holder.
...
- **Caveats / not-in-source**: whether `applicationSettings` will ever be consumed is `Not determinable from source`; today it is only discarded (`:33`), taken solely to keep the signature uniform across modules.
...
- **Where it's used**: neither of these two copies is passed as a type argument anywhere in ADC today.
  ...
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:53`,
  ...
  `MMCA.ADC/Tests/Architecture/MMCA.ADC.Architecture.Tests/Cqrs/DecoratorPipelineOrderTests.cs:51` and
...
- **Where it's used**: the Application copy is the type argument at `MMCA.ADC.Identity.Application/DependencyInjection.cs:53`, which resolves to `ScanModuleApplicationServices<TAssemblyMarker>` in `MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.ModuleScanning.cs:28-30`, a one-line generic wrapper that forwards `typeof(TAssemblyMarker).Assembly` to the `Assembly`-typed overload declared at `:46`. See [`DependencyInjection`](#dependencyinjection) for the Application layer.
...
- **Where it's used**: the per-app role type
  ...
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Shared/Authorization/IdentityPermissionGrants.cs:39`)),
...
- **What it is**: the capability-permission catalog for the Identity module: string constants that
  ...
  It holds one ADC-defined capability, reading the user list; the two administration capabilities
  (users, roles) are the framework's own constants, aliased here and granted separately.
...
- **Walkthrough**: `public const string UsersRead = "identity:users:read"` (`IdentityPermissions.cs:13`),
  ...
  `public const string UsersManage = AdministrationPermissions.ManageUsers` (`:26`) administers user
  accounts (lock, unlock, change role); its doc and remarks (`:15-25`) note it aliases the framework's
  `AdministrationPermissions.ManageUsers` because `UsersAdminControllerBase` gates its actions on that
  exact capability string, that it stays OUT of `All` because `All` is the list of capabilities ADC
  itself defines, that [`IdentityPermissionGrants`](#identitypermissiongrants) grants it to Organizer
  by name instead, and that ContentEditor holds none of these, which is the strict-subset property
  that role is defined by. `public const string RolesManage = AdministrationPermissions.ManageRoles`
  (`:42`) administers roles (list them, edit the stored permission grants attached to each); its doc
  and remarks (`:28-41`) add the same "stays out of `All`, granted by name" note and state it is
  granted in **compiled** code and never from a stored row, because it is the only protection on the
  screen that edits stored rows, so a deleted row could otherwise lock every operator out of the
  surface that would restore it. `public static IReadOnlyList<string> All { get; }` (`:50`) is a
  collection expression holding only `UsersRead` (`:51-52`), documented (`:44-49`) as every Identity
  capability ADC itself defines, with `UsersManage` and `RolesManage` deliberately excluded because
  they are the framework's constants and are granted by name in
  [`IdentityPermissionGrants`](#identitypermissiongrants) instead.
- **Why it's built this way**: a `namespace:resource:action` string convention keeps permissions
  self-describing and greppable. Splitting `All` (ADC's own capabilities) from the two aliased
  framework constants keeps the catalog honest about provenance: a reader scanning `All` sees only
  what this module coined, while the grant that actually assigns roles is the single place that
  decides which framework capabilities Organizer also needs. Aliasing
  `AdministrationPermissions.ManageUsers` / `.ManageRoles` rather than coining Identity-specific names
  keeps the controller gates and the compiled role grant agreeing on one string per capability.
- **Where it's used**: demanded by the user-list endpoint on [`UsersController`](#userscontroller)
  ...
  IdentityPermissions.UsersManage, IdentityPermissions.RolesManage])`.
...
- **What it is**: the module's one role-to-permission grant: it hands every
  [`IdentityPermissions`](#identitypermissions) capability, plus the two framework administration
  capabilities the shipped controller bases gate on, to `RoleNames.Organizer` and to no other role.
...
- **Concept**: cross-reference [`IdentityPermissions`](#identitypermissions) for the permission-based
  authorization model this grant populates and for why `UsersManage`/`RolesManage` are named here
  rather than folded into `IdentityPermissions.All`. `[Rubric §11, Security]` assesses least
  privilege: factoring the grant into its own static class, called once from module registration,
  keeps "who can administer users and roles" answerable by reading one method rather than tracing
  every `[HasPermission]` attribute back to a role.
- **Walkthrough**: one method, `public static void Apply(PermissionRegistryBuilder permissions)`
  (`IdentityPermissionGrants.cs:35`), guarded by `ArgumentNullException.ThrowIfNull(permissions)`
  (`:37`). The doc comment (`:19-24`) states the policy in full: every Identity capability goes to
  Organizer, and content editors deliberately hold none, because their strict-subset property is
  defined by holding no user or role administration; the remarks (`:26-31`) add that `UsersManage`
  and `RolesManage` are the framework's own constants (`AdministrationPermissions`), not ADC's, so
  `All` stays the list ADC itself defines and this method grants the two framework capabilities by
  name instead, "same shape as MMCA.Store". The body is one call:
  `permissions.Grant(RoleNames.Organizer, [.. IdentityPermissions.All, IdentityPermissions.UsersManage,
  IdentityPermissions.RolesManage])` (`:39-41`).
...
- **Depends on**: [`IUIModule`](group-15-common-ui-framework.md#iuimodule) (the contract, `IdentityUIModule.cs:5`, `:13`), [`NavItem`](group-15-common-ui-framework.md#navitem) and [`NavSection`](group-15-common-ui-framework.md#navsection) from `MMCA.Common.UI.Common` (`:3`), [`IdentityRoutePaths`](#identityroutepaths), [`RoleNames`](#rolenames) and [`AdministrationPermissions`](group-08-auth.md#administrationpermissions) from `MMCA.Common.Shared.Auth` (`:2`); externals: MudBlazor `Icons.Material.Filled` (`:5`), BCL `System.Reflection.Assembly` (`:1`).
...
- **What it is**: the one-call registration entry point for the Identity UI layer. `AddIdentityUI()` wires the module's entity services, its user-administration and role-administration clients (ADR-116), its bespoke user service, and its [`IdentityUIModule`](#identityuimodule) descriptor into any Blazor host.
- **Depends on**: [`IdentityUIModule`](#identityuimodule), [`IUserUIService`](#iuseruiservice) / [`UserService`](#userservice) from `MMCA.ADC.Identity.UI.Services`, [`UserAdminDTO`](#useradmindto) (the DTO the user-administration client closes over), and the framework's `AddUIModule<TModule>()`, `AddUserAdministrationUI<TUserAdminDTO>()` and `AddRoleAdministrationUI()` extensions from `MMCA.Common.UI`; externals: `Microsoft.Extensions.DependencyInjection.IServiceCollection` (`:1`).
...
- **Depends on**: [`ChangePasswordRequest`](group-08-auth.md#changepasswordrequest) (the shared request DTO it validates), [`StrongPasswordRules<T>`](group-06-validation.md#requiredstringrulest-optionalstringrulest-emailrulest-positiveintrulest-positivedecimalrulest-nonnegativeintrulest-requiredidrulest-tid-optionalpositiveidrulest-tid-passwordrulest-strongpasswordrulest); externals: FluentValidation (`AbstractValidator<T>`, `RuleFor`, `Include`).
...
- **Concept, the pipeline-stage validator as a *defense-in-depth copy* rather than a single gate.**
  ...
  (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:53`) and run
...
- **Where it's used**: never referenced by name. It is registered by the convention scan at
  `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:53` and
...
- **Depends on**: [`RegisterRequest`](group-08-auth.md#registerrequest), [`EmailRules<T>`](group-06-validation.md#requiredstringrulest-optionalstringrulest-emailrulest-positiveintrulest-positivedecimalrulest-nonnegativeintrulest-requiredidrulest-tid-optionalpositiveidrulest-tid-passwordrulest-strongpasswordrulest), [`StrongPasswordRules<T>`](group-06-validation.md#requiredstringrulest-optionalstringrulest-emailrulest-positiveintrulest-positivedecimalrulest-nonnegativeintrulest-requiredidrulest-tid-optionalpositiveidrulest-tid-passwordrulest-strongpasswordrulest), [`RequiredStringRules<T>`](group-06-validation.md#requiredstringrulest-optionalstringrulest-emailrulest-positiveintrulest-positivedecimalrulest-nonnegativeintrulest-requiredidrulest-tid-optionalpositiveidrulest-tid-passwordrulest-strongpasswordrulest), [`AddressValidator`](group-06-validation.md#addressvalidator), [`UserInvariants`](#userinvariants); externals: FluentValidation.
...
- **Where it's used**: auto-registered as `IValidator<RegisterRequest>` by `ScanModuleApplicationServices<ClassReference>()` (`MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/DependencyInjection.cs:53`) and injected into [`AuthenticationValidators`](group-08-auth.md#authenticationvalidators), which [`AuthenticationService`](#authenticationservice) receives through its base and runs at the top of the registration workflow. The endpoint is `POST auth/register` on [`AuthController`](#authcontroller).
...
- **Depends on**: [`RoleValue`](group-08-auth.md#rolevalue) (the shared base), [`RoleNames`](#rolenames) (the string constants), [`Result`](group-01-result-error-handling.md#result) and [`Error`](group-01-result-error-handling.md#error); externals: `System.Collections.Frozen.FrozenDictionary`.
...
- **Depends on**: [`IDeleteBlobInternalCommand`](group-14-module-system-composition.md#ideleteblobinternalcommand); externals: none.
- **Concept reinforced, the fire-and-forget internal command.** `[Rubric §6, CQRS & Event-Driven]`: marking this `IDeleteBlobInternalCommand` (itself an `IInternalCommand`) rather than a public `ICommand` signals it is dispatched from inside the module's own handlers, not from an API controller. [`DeleteAvatarBlobInternalCommandHandler`](#deleteavatarblobinternalcommandhandler) is the sole handler, now the module's sealed subclass of the shared [`DeleteBlobInternalCommandHandlerBase<TCommand>`](group-14-module-system-composition.md#deleteblobinternalcommandhandlerbasetcommand).
...
- **Why it's built this way**: three call sites ([`DeleteUserHandler`](#deleteuserhandler), [`RemoveUserAvatarHandler`](#removeuseravatarhandler), [`SetUserAvatarHandler`](#setuseravatarhandler)) each need to delete a blob after a domain change succeeds; carrying only the blob name keeps the command decoupled from which use case triggered it. Implementing `IDeleteBlobInternalCommand` rather than `IInternalCommand` directly is what lets [`DeleteBlobInternalCommandHandlerBase<TCommand>`](group-14-module-system-composition.md#deleteblobinternalcommandhandlerbasetcommand) constrain its generic parameter and read `command.BlobName` without a per-command adapter.
...
### DeleteAvatarBlobInternalCommandHandler
> MMCA.ADC.Identity.Application · `MMCA.ADC.Identity.Application.Users.UseCases.DeleteAvatarBlob` · `MMCA.ADC/Source/Modules/Identity/MMCA.ADC.Identity.Application/Users/UseCases/DeleteAvatarBlob/DeleteAvatarBlobInternalCommandHandler.cs:17` · Level 6 · class (sealed)

- **What it is**: the handler for [`DeleteAvatarBlobInternalCommand`](#deleteavatarblobinternalcommand): a thin sealed subclass of the shared [`DeleteBlobInternalCommandHandlerBase<TCommand>`](group-14-module-system-composition.md#deleteblobinternalcommandhandlerbasetcommand) that only names the blob kind for its log lines (`DeleteAvatarBlobInternalCommandHandler.cs:17-24`).
- **Depends on**: [`DeleteBlobInternalCommandHandlerBase<TCommand>`](group-14-module-system-composition.md#deleteblobinternalcommandhandlerbasetcommand), [`IFileStorageService`](group-07-persistence-ef-core.md#ifilestorageservice); externals: `Microsoft.Extensions.Logging`.
- **Concept reinforced, idempotent cleanup on a not-found, extracted to a shared base.** `[Rubric §29, Resilience & Business Continuity]`: the not-found-is-success behavior now lives once in `DeleteBlobInternalCommandHandlerBase<TCommand>.HandleAsync`, which treats `ErrorType.NotFound` from `fileStorage.DeleteAsync` as success rather than a failure, so a retried delete (at-least-once dispatch, or a blob already removed by a prior attempt) does not surface as one. This module's handler no longer implements that logic itself; it only supplies the label.
- **Walkthrough**: the whole class body is a primary constructor forwarding `fileStorage` and `logger` to the base and one property override, `BlobKind => "Avatar blob"` (`:17-24`), which the base's `[LoggerMessage]` lines interpolate as `{BlobKind} {BlobName} deleted` / `was already gone` / `could not be deleted`.
- **Why it's built this way**: [`DeleteSessionAssetBlobInternalCommandHandler`](group-18-conference-application.md#deletesessionassetblobinternalcommandhandler) in the Conference module needed the identical delete-and-tolerate-not-found flow, so the shared logic (including the three `[LoggerMessage]` methods) moved to `MMCA.Common.Application.InternalCommands.DeleteBlobInternalCommandHandlerBase<TCommand>`; each module now contributes only its `BlobKind` label instead of duplicating the handler.
