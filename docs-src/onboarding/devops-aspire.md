...
>   `WithJwksDiscovery`, `WithE2eRsaKeys`, `WithE2eRegistrationThrottleLift`,
>   `WithSQLServerDataSource` / `WithCosmosDataSource` / `WithSqliteDataSource`, there is no
>   AppHost-local extensions file)
...
point (Program.cs:328) to launch the dashboard and all declared resources. The SDK also picks up
...
`AddMessageBroker()` lives in `MMCA.Common.Aspire.Hosting` (Common.Aspire.Hosting/Extensions.cs:47-52)
and wraps `builder.AddRabbitMQ(name).WithManagementPlugin()` with the resource name defaulting to
`"rabbitmq"` (`DefaultBrokerResourceName`, Extensions.cs:28); the management plugin exposes the admin
...
`MMCA.Common.Aspire.Hosting`'s `Extensions` class (Common.Aspire.Hosting/Extensions.cs:213-225); the
...
static method (Extensions.cs:194), which is why the receiver does not appear in the signature:
...
(Common.Aspire.Hosting/Extensions.cs:222) ensures the service process does not start until SQL Server
is healthy.
...
- `WithCosmosDataSource(service, database, logicalName)` (Extensions.cs:245-258) takes an
  ...
  SQL Server source).
- `WithSqliteDataSource(service, logicalName, filePath)` (Extensions.cs:275-287) takes a file path
...
Services are declared in this order: Notification, Engagement, Conference, Identity (Program.cs:88-193).
Declaration order matters because the gateway and UI references are added after all four are declared
(Program.cs:252-291). Each service follows the same pattern:
...
.WithEnvironment("MessageBus__Provider", "RabbitMq")` (Common.Aspire.Hosting/Extensions.cs:66-76),
...
**Conference service** has one extra line (Program.cs:162):
...
suite. The comment (Program.cs:159-161) explicitly restricts this to local dev and E2E CI; production
leaves this unset.

**Identity service** carries two extra lines. `Seeding__IncludeSampleUsers=true` (Program.cs:187) seeds
the well-known organizer/attendee accounts the Playwright suite logs in with; the comment
(Program.cs:184-186) restricts it to local dev and E2E CI so production creates no weak-credential
accounts. `WithE2eRsaKeys()` (Program.cs:193) is a framework extension, not AppHost-local code: when
...
(Common.Aspire.Hosting/Extensions.cs:137-152). Without either variable the method returns the resource
untouched (Extensions.cs:143-146), so locally and in production user-secrets / Azure Key Vault are used
instead.

### gRPC cross-service references

Six directed edges express the gRPC topology (Program.cs:213-237):
...
(Program.cs:219-226, `Notification.Contracts/DependencyInjection.cs:42`).

The deliberate asymmetry in the Conference↔Engagement pair is explained in the inline comment
(Program.cs:203-210): bidirectional `WaitFor` would produce a circular wait and deadlock. Instead,
...
best-effort ordering hint since Conference is the heavier producer (Program.cs:215). The three later
edges omit `WaitFor` for the same reason plus one of their own: Engagement's live-channel publish is
fire-and-forget-with-logging (Program.cs:219-226), and Engagement and Notification already wait on
Identity, so a reciprocal wait for the export aggregation would deadlock startup (Program.cs:227-237).
...
(e.g. Conference/Program.cs:358, Identity/Program.cs:311). On the client side
...
Program.cs:252-262. The gateway has no `WithSQLServerDataSource`, no `WithReference(redis)`, and no
...
endpoint is pinned to port 6001 (Program.cs:262) because the MAUI native client has this URL baked into
...
a Kestrel `Http2`-only endpoint rejects. The solution (Program.cs:271-273 and
Common.Aspire.Hosting/Extensions.cs:93-121) is `WithJwksDiscovery`, which sets
...
Program.cs:279-291. The UI waits for the gateway (and transitively the four services), ensuring it
...
line (Program.cs:326) passes the UI's HTTPS endpoint back to Identity as `OAuth__UIBaseUrl` so the
...
(Program.cs:293-322), all absent in production. `E2E_FORCE_WASM` sets `E2E__ForceWebAssembly` on the UI
...
`e2e.yml` deliberately does not set it (Program.cs:293-309). `E2E_FORCE_SERVER` sets `E2E__ForceServer`
to pin the UI to InteractiveServer, and loses to `E2E_FORCE_WASM` when both are set
(Program.cs:311-320). The third switch is not an `if` block in the AppHost at all: it is the single
call `identityService.WithE2eRegistrationThrottleLift(alsoLiftWhen: forceWasm)` (Program.cs:322). The
trigger (`E2E_LIFT_REGISTRATION_THROTTLE`), the target setting
(`LoginProtection__MaxRegistrationsPerIpPerHour`) and the lifted value all live inside the framework
extension, so the AppHost passes only its own extra condition: forcing WASM implies the same
registration volume, hence `alsoLiftWhen: forceWasm`. The literal is the framework constant
`E2eRegistrationsPerIpPerHour = 1000` (Common.Aspire.Hosting/Extensions.cs:35), lifted from the
production default of 10 (`MMCA.Common.Infrastructure/Auth/LoginProtectionSettings.cs:37`), because the
suite registers far more than ten accounts from one localhost IP.
...
(Program.cs:101 / 257), Engagement (98 / 327), Conference (115 / 372), Identity (111 / 321), the Gateway
(31 / 52) and the UI (30 / 119). This means the OpenTelemetry wiring, health checks, service discovery,
...
### Three things `AddServiceDefaults` deliberately does not do

Not every framework host extension is bundled into the defaults. Three are opt-in per host, called from
each service's own `Program.cs` rather than from the AppHost or from `AddServiceDefaults`, and each one
is **gated on a configuration key that is absent locally**, so it no-ops on a developer machine and in
the test tiers while doing real work in Azure. That gating is what keeps local-to-cloud parity honest:
the same call sites run in both places, and only the configuration differs.

- **`AddCommonKeyVaultConfiguration()`** (`MMCA.Common.Aspire/Configuration/KeyVaultConfigurationExtensions.cs:78-112`)
  layers an Azure Key Vault over the host configuration, so a secret in the vault reads back through
  `IConfiguration` like any other setting and overrides the sources added before it. The gate is
  `KeyVault:Uri`: absent or whitespace and the method returns the builder untouched
  (KeyVaultConfigurationExtensions.cs:80-88), which is why local dev, CI and the integration tier take
  no Azure dependency at startup. Two details worth knowing. It is called **early**, before anything
  binds settings, because `ConfigurationManager` builds and loads each source as it is added
  (`MMCA.ADC/Source/Services/MMCA.ADC.Conference.Service/Program.cs:124`, with the comment at 117-123
  saying exactly that); and a secret name cannot contain a colon, so the provider maps `--` onto the
  configuration separator (`Jwt--SigningKey` arrives as `Jwt:SigningKey`). Authentication is
  `DefaultAzureCredential`, so a deployed host uses its managed identity. All four ADC services call it,
  as does the UI (`MMCA.ADC.UI.Web/Program.cs:38`).
- **`AddCommonDataProtection()`** (`MMCA.Common.Aspire/DataProtection/DataProtectionExtensions.cs:52-88`)
  ...
  pattern. The gate is `DataProtection:BlobStorageUri` (DataProtectionExtensions.cs:54-62). A second,
  deliberately **separate** gate, `DataProtection:KeyVaultKeyUri`, encrypts that key ring at rest
  (DataProtectionExtensions.cs:81-85): the comment above it explains why the two are not folded together
  (blob persistence is what fixes cross-replica decryption and has to work on its own, because the Key
  Vault Crypto User role is granted out of band and can lag a deployment, so coupling them would turn a
  missing role assignment into a total auth outage). The two ADC hosts that do cookie cryptography call
  it: Identity (`MMCA.ADC.Identity.Service/Program.cs:126`, comment 120-125 citing OAuth cookies at
  `maxReplicas` 2 with no session affinity) and the UI (`MMCA.ADC.UI.Web/Program.cs:40`).
- **`AddScheduledJobs(builder.Configuration)`** (`MMCA.Common.Infrastructure/DependencyInjection.cs:304-318`)
  ...
  up with two runners racing for the same job rows, DependencyInjection.cs:311-315). Registering it is
  not the same as turning it on: the runner's `ExecuteAsync` logs once and returns when `Scheduler:Enabled`
  is false (`MMCA.Common.Infrastructure/Scheduling/ScheduledJobRunner.cs:77-82`). The runner owns no jobs
  of its own; jobs are `IScheduledJob` implementations
  (`MMCA.Common.Application/Interfaces/IScheduledJob.cs:36`) contributed separately. The framework ships
  one, `AuditTrailCleanupJob` (`MMCA.Common.Infrastructure/Persistence/AuditTrail/AuditTrailCleanupJob.cs:48`),
...
MMCA.Common meters `AddServiceDefaults` already registers (Conference.Service/Program.cs:126-134). It
...
is invisible until an organizer notices missing scores. Like the framework meters, the name is a literal
so the host's startup wiring does not have to reference an Infrastructure type.
...
tagged `"live"` (Extensions.cs:42, 199-202), `AddWarmupReadiness()` (Extensions.cs:43), and
...
Common.Aspire/Extensions.cs:121-191. Configures OTel logging (`IncludeFormattedMessage` + `IncludeScopes`),
...
1. **MMCA.Common meters**, four of them (Common.Aspire/Extensions.cs:160-163): `"MMCA.Common.Outbox"`
   ...
   and degraded counters) and `"MMCA.Common.Scheduler"` (the recurring runner's run outcomes, duration
   and schedule lag, inert in a host that never sets `Scheduler:Enabled`). These are registered
...
2. **`OutboxPollFilterProcessor`** is added to the tracing pipeline
   (Common.Aspire/Extensions.cs:172-177) before the exporters, so its `OnEnd` runs first.
...
3. **Three cost knobs, all off by default** ([Rubric §31, Cost and FinOps]).
   ...
   `TraceIdRatioBasedSampler` for head-based trace sampling (Extensions.cs:184-185). A value outside the
   open interval (0,1), or one that fails to parse, falls back to "sample everything" rather than
   silently blinding the host (Extensions.cs:366-374, 389-390). ADC's production Bicep sets
   `Telemetry__TracesSampleRatio` (`MMCA.ADC/infra/main.bicep:210`); an unset host behaves exactly as
   before the knobs existed.

### `MapDefaultEndpoints`

Common.Aspire/Extensions.cs:329-357. Maps three endpoints:
...
- `/health/ready`, readiness: everything except `"live"`-only **and `"optional"`** checks
  (Extensions.cs:351-354). This includes the warmup check (tagged `"ready"`) and any untagged dependency
  ...
  becomes a total outage (Extensions.cs:345-350).

The checks that carry that `"optional"` tag come from `AddInfrastructureHealthChecks(bool requireSqlServer)`
(Common.Aspire/Extensions.cs:230-264), which a host calls separately from `AddServiceDefaults`. It
...
(Extensions.cs:234-238).

### Dual telemetry export

`AddOpenTelemetryExporters` (Common.Aspire/Extensions.cs:279-296) activates two exporters, each
conditional:
...
processors to skip the span. It is registered before the exporters (Common.Aspire/Extensions.cs:172-177)
...
security headers (Gateway/Program.cs:37, 42). It is deliberately looser than `MMCA.Common.API`'s
...
the full `Aspire.Hosting` package (Common.Aspire.Hosting/Extensions.cs:13-17 class comment). It exports
eight extension methods used in the AppHosts that orchestrate extracted microservices
(`AddMessageBroker`, `WithBroker`, `WithJwksDiscovery`, `WithE2eRsaKeys`, `WithE2eRegistrationThrottleLift`, and the `WithSQLServerDataSource` / `WithCosmosDataSource` / `WithSqliteDataSource` helpers covered earlier).
It exposes no gRPC API at all: gRPC peers are wired with stock Aspire `WithReference`, as shown above.

### `AddMessageBroker`

Common.Aspire.Hosting/Extensions.cs:47-52. Wraps `builder.AddRabbitMQ(name).WithManagementPlugin()`.
...
Common.Aspire.Hosting/Extensions.cs:66-76. Chains `WithReference(broker).WaitFor(broker)
...
Common.Aspire.Hosting/Extensions.cs:93-121. Injects `Authentication__JwtBearer__Authority` pointing to
...
(Common.Aspire.Hosting/Extensions.cs:105-115): Identity runs `Http2`-only, so the default HTTP/1.1
...
(Extensions.cs:100-102) to add service discovery for any direct identity calls the consuming service
...
Common.Aspire.Hosting/Extensions.cs:137-152. Reads `E2E_JWT_PRIVATE_KEY_PEM` /
...
formats were found" and the readiness gate times out (Extensions.cs:132-134). Both ADC
(AppHost/Program.cs:193) and Store (`MMCA.Store/Source/Hosting/MMCA.Store.AppHost/Program.cs:144`) call
it, which is why it lives in the framework rather than in either AppHost.

### `WithE2eRegistrationThrottleLift`

Common.Aspire.Hosting/Extensions.cs:176-191. The eighth method, and the newest: it lifts Identity's
per-IP registration throttle for an E2E run. It sets
`LoginProtection__MaxRegistrationsPerIpPerHour` to the framework constant
`E2eRegistrationsPerIpPerHour` (1000, Extensions.cs:35) when either trigger fires, and otherwise
returns the resource untouched (Extensions.cs:186-191). The two triggers are OR-ed
(Extensions.cs:180-184):

- the `E2E_LIFT_REGISTRATION_THROTTLE` environment variable being `"true"` (case-insensitive), which
  the E2E workflow sets and which is absent locally and in production, so this is a no-op there;
- the optional `alsoLiftWhen` parameter (default `false`), an extra condition evaluated at the call
  site for an AppHost whose own E2E switch implies the same registration volume.

The shape is the interesting part. The trigger, the target setting name and the lifted value all live
in the framework; the AppHost passes only the condition that is its own. ADC calls
`identityService.WithE2eRegistrationThrottleLift(alsoLiftWhen: forceWasm)` (AppHost/Program.cs:322)
because its forced-WASM switch implies the same volume; Store calls the parameterless
`identityService.WithE2eRegistrationThrottleLift()`
(`MMCA.Store/Source/Hosting/MMCA.Store.AppHost/Program.cs:253`), leaving the environment variable as
the only trigger. Both AppHosts previously carried their own inline `if` block and their own copy of
the literal 1000; neither does now.
...
There are **three** forwarder configurations, not two, and they differ on two axes: HTTP version and
activity timeout (the header comment enumerates them, Program.cs:9-21).
...
to HTTP/1.1 and the `Http2`-only backend rejects it. `ForwardHttp2` stays configurable (default `true`,
Program.cs:73) so a deployment without HTTP/2 ingress can roll back without a code change; with it false,
`http2Config` collapses to a config carrying nothing but the timeout (Program.cs:103).

The two Notification routes do **not** share a config, and neither of them is YARP's bare default.
`notificationRestConfig` (Program.cs:108) leaves `Version` / `VersionPolicy` unset, so YARP's
HTTP/1.1-capable defaults apply, because SignalR's WebSocket transport begins with an HTTP/1.1 Upgrade
handshake and Notification therefore stays `Http1AndHttp2` (Program.cs:153-159). `hubConfig`
(Program.cs:110) keeps those same version defaults and diverges on exactly one value: the activity
timeout.

`ActivityTimeout` bounds how long the forwarder waits on a backend before it aborts the proxied
activity, which is what stops a wedged upstream from holding a gateway connection open indefinitely.
YARP's own default (100 seconds) is tied to nothing in this system, so both REST configs take
`restActivityTimeout` instead (`http2Config` at Program.cs:95, `notificationRestConfig` at
Program.cs:108). Its value is the config knob `Gateway:ForwarderActivityTimeoutSeconds`, defaulting to
the shared backend budget `HttpResilienceDefaults.TotalRequestTimeout` (90 s,
`Core/MMCA.Common.Shared/Resilience/HttpResilienceDefaults.cs:19`) plus a 10 second margin
(Program.cs:81-83). The margin is the whole point: the forwarder must **outlive** the backend's own
total request budget, or a call that exhausts the backend surfaces a gateway abort that hides the
backend's real error. The SignalR hub route is the one deliberate exemption: a hub connection is
long-lived by design (a WebSocket or long-poll stays open for the session), so applying the REST budget
to it would tear down healthy hubs. `hubConfig` reads `Gateway:HubActivityTimeoutSeconds`, one hour by
default (Program.cs:88-90). Both knobs are per-deployment overrides in the same config-read shape as
`ForwardHttp2`, so an operator can retune either without a code change.
...
(Common.Aspire.Hosting/Extensions.cs:100-102).
  ...
  └─ Database resources health (adc-identity, adc-conference, adc-engagement, adc-notification)
       └─ Identity Service  (WaitFor: identityDb, redis, mailDev, rabbit)
            ...
            └─ Notification Service (WaitFor: notificationDb, redis, mailDev, rabbit,
                 identityService twice over: explicit at Program.cs:213 and again via JWKS)
...
half of the bidirectional gRPC pair, Program.cs:218), Engagement→Notification (fire-and-forget live
channel, Program.cs:226), and Identity→Engagement / Identity→Notification (the export aggregation,
Program.cs:236-237). Each would close a cycle against a wait that already exists in the other
