...
| 042 | Device capability abstraction (MAUI Blazor Hybrid): per-capability contracts + TryAdd null/browser fallbacks + MAUI-native overrides, `IDeepLinkDispatcher`; `MMCA.Common.UI.Maui` is the one MAUI-TFM package, built outside the main solution | [g26](group-26-device-capability-layer.md) |
...
| 050 | ~~JWT + single rotating refresh token: a short-lived stateless access token plus one server-stored opaque refresh token per user that rotates on every use (mismatch/expiry revokes + 401); the sliding expiry re-stamps on rotation, and single-token-per-user signs other devices out~~ **superseded by [ADR-097](https://ivanball.github.io/docs/adr/097-multi-device-refresh-sessions.html)** (rotation, reuse detection and sliding expiry survive over per-device rows hashed at rest) | [g08](group-08-auth.md) |
...
| 052 | ~~Background job execution: work outliving a request runs as a bounded `Channel<T>` plus a `SingleReader` hosted drain, never an untracked `Task` from a controller, so the host can cancel and await it on shutdown; full mode encodes what the work is worth (`DropOldest` vs `Wait`)~~ **superseded by [ADR-121](https://ivanball.github.io/docs/adr/121-ephemeral-in-process-work-queue.html)** | [g23](group-23-engagement-live-layer.md) |
...
| 113 | PostgreSQL as a fourth first-class engine: `DataSource.PostgreSQL` is appended to the enum, a sealed `PostgreSQLDbContext` sits over the abstract `ApplicationDbContext` per ADR-006, and `EntityTypeConfigurationPostgreSQL` mirrors the SQL Server footprint one for one, differing only where the server forces a difference | [g07](group-07-persistence-ef-core.md) |
| 114 | Internal commands, a durable job queue on the outbox's machinery: deferred work is an ordinary CQRS command marked `IInternalCommand`, scheduled into a per-source `InternalCommands` table by `IInternalCommandScheduler` and executed later by `InternalCommandProcessor` through the normal decorator pipeline, with dead-lettering and an administration surface | [g14](group-14-module-system-composition.md)/[g07](group-07-persistence-ef-core.md)/[g18](group-18-conference-application.md) |
| 115 | Strongly typed identifiers opt in, aliases stay the default: `IStronglyTypedId<TSelf, TValue>` plus a two-line record declaration, with the EF value converter and comparer, model configuration, filter strategy and OpenAPI schema transformer shipped complete; no existing entity, consumer or test migrates and nothing pushes a consumer toward a wrapper | [g08](group-08-auth.md)/[g07](group-07-persistence-ef-core.md)/[g03](group-03-querying-specifications.md)/[g12](group-12-api-hosting-mapping.md) |
| 116 | Identity completions as opt-in bases, not features: two-factor (`ITwoFactorService` plus a challenge step), email confirmation, operator-editable stored permission grants (`IPermissionGrantStore`, `IPermissionCatalog`) and the user/role administration API (`UsersAdminControllerBase`, `RolesAdminControllerBase`) ship as contracts and abstract bases behind their own `Add*` calls; a consumer that adopts none observes no change | [g08](group-08-auth.md)/[g14](group-14-module-system-composition.md)/[g12](group-12-api-hosting-mapping.md)/[g15](group-15-common-ui-framework.md) |
| 117 | AppHost integration testing as a shipped package: `MMCA.Common.Testing.Aspire` gives a consumer's smoke tier `AppHostFixtureBase<TAppHost>` (one collection fixture, per-resource readiness waits) and a named skip when a precondition is missing, so the composition itself is asserted rather than only type-checked | [g28](group-28-testing-infrastructure.md)/[g16](group-16-aspire-orchestration.md)/[devops-testing](devops-testing.md) |
| 118 | Message transport library policy: MassTransit stays pinned at v8 behind `IMessageBus` (`DependencyVersionTestsBase` gates the major), the outbox and inbox stay custom precisely because that keeps a transport swap cheap, and the exit triggers and replacement order are recorded so the pin is a dated decision | [g04](group-04-events-outbox.md)/[g28](group-28-testing-infrastructure.md) |
| 119 | Delete behavior is an explicit business rule: `RestrictDeleteByDefaultConvention` sets Restrict on every relationship nobody configured when the model is finalized, a cascade is an opt-in with a stated reason, and the finished model records which of the two each foreign key is | [g07](group-07-persistence-ef-core.md) |
| 120 | Governed language-model boundary: `MMCA.Common.AI` owns the LLM call as one optional package, the provider half is the official SDK's `IChatClient` adapter, governance is a pipeline of delegating clients (`BoundedChatClient` bounds, `UsageRecordingChatClient` meters), prompts are versioned `PromptContract`s the framework hashes, and tools are off until a host turns them on | [g27](group-27-common-ai-integration.md) |
| 121 | Ephemeral in-process work queue (supersedes 052): best-effort work owned by one process runs as a bounded 1024-item `Channel<T>` singleton (`DropOldest`, the one full mode) plus a `SingleReader` hosted drain the host cancels and awaits, `TryWrite` always succeeds, nothing starts an untracked `Task` from a request, and nothing that must run lands here | [g23](group-23-engagement-live-layer.md) |
| 122 | Dev-only relaxations are environment-gated and fail closed: `SensitiveDataLoggingGate`, `SmtpTransportSecurity` and the AI telemetry gate apply a relaxation only when the environment is positively identified as Development; an absent or unparseable environment yields the production posture | [g07](group-07-persistence-ef-core.md)/[g14](group-14-module-system-composition.md)/[g27](group-27-common-ai-integration.md) |
| 123 | Speaker-published session materials: `SessionAsset` is its own Conference aggregate (a server-minted GUID id, plain `EventId` / `SessionId` keys, not a child of the Sessionize-synced `Session`), written by the session's own speakers or an organizer, stored public-read under an unguessable blob name, and admitted only by the framework's byte-level `DocumentContentSniffer` gate | [g17](group-17-conference-domain.md)/[g18](group-18-conference-application.md)/[g21](group-21-conference-ui.md) |
| 124 | The server-rendered Blazor UI host is its own hardened edge, because it is a separate origin the Gateway limiter never sees: `BoundedCircuitHandler` (a singleton so one count spans the replica) refuses circuits past `MaxActiveCircuits`, and `UiRateLimitingSettings` puts a fixed-window limiter on the page and negotiate endpoints, both bound from host-owned configuration sections | [g15](group-15-common-ui-framework.md)/[g25](group-25-adc-host-composition.md) |
| 125 | Parameterized SQL only: hand-written SQL has exactly one door, `IRawSqlQueryExecutor`, whose two entry points accept `FormattableString` and nothing else (a concatenated statement does not compile), `EFRawSqlQueryExecutor` names the relational-only limit, and `RawSqlConventionTestsBase` bans the four `*Raw` EF members in module code as a fitness gate | [g07](group-07-persistence-ef-core.md)/[g28](group-28-testing-infrastructure.md) |
| 126 | Event sourcing is not adopted: aggregates stay current-state EF rows that raise domain events for notification, carry audit fields and soft-delete, so no repository gains an event store, per-aggregate stream, projection layer or rehydration path | [g02](group-02-domain-building-blocks.md)/[g04](group-04-events-outbox.md) |
| 127 | The actor model is not adopted: per-entity state stays in the owning module's database behind optimistic concurrency, read pressure stays on the caching tiers, and live fan-out stays on the notification hub, with no actor runtime as a hosting layer | [g07](group-07-persistence-ef-core.md)/[g09](group-09-caching.md)/[g10](group-10-notifications.md) |
...
2. **Runtime (test)**, `Tests/Architecture/MMCA.Common.Architecture.Tests` (NetArchTest) asserts the
   ...
   **`MMCA.Common.Testing.Architecture`** package (one of the lockstep-released packages, see `FACTS.md`): a reusable rule library +
