...
| 098 | Aspire for orchestration, not for testing or production dashboards: each AppHost composes the local stack and every host calls `AddServiceDefaults`/`MapDefaultEndpoints`, but integration testing stays `WebApplicationFactory` + Testcontainers (`SqlServerIntegrationTestFixtureBase<TEntryPoint>` per service, `CrossServiceFixtureBase` for the three-host tier) and `DistributedApplicationTestingBuilder` stays out; production observability is Application Insights, not the Aspire dashboard | [devops-testing](devops-testing.md)/[g27](group-27-testing-infrastructure.md)/[devops-aspire](devops-aspire.md) |
