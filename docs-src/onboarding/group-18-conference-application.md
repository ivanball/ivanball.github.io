...
- **Why it's built this way**: the aggregate exposes `Set*` methods so the populator assigns child collections through the domain's own guarded mutators rather than back-door setters, keeping hydration consistent with the aggregate's rules (ADR-002).
