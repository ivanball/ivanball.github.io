...
the outbox (ADR-003), so the application code never references MassTransit. `[Rubric §6, CQRS &
...
- **Caveats / not-in-source**: the "ADR-042 Wave 8" attribution comes from the source doc comment; this walkthrough describes only what the method bodies do.
...
- **Why it's built this way** - `required init` gives an immutable, fully-constructed payload; the per-field XML docs double as OpenAPI descriptions. The URLs stay `string` deliberately so the import's raw values round-trip without a `Uri` parse. Manual DTO shaping follows ADR-001.
...
- **Why it's built this way** delegating to child mappers keeps each leaf projection defined once and reused, so a change to `RoomDTO` mapping propagates through every parent that includes rooms (ADR-001 for the manual-mapping stance).
...
- **Why it's built this way** - keeping mapping in a dedicated class satisfies the manual-mapping convention (ADR-001, no reflection-based mapper for entity construction) and isolates the one place that knows the `Event.Create` parameter order.
...
- **Why it's built this way**: the two modules own separate databases (ADR-006), so there is no foreign key between `Speaker` and `User`. The link is kept consistent through an integration event, and the `ITransactional` marker is what makes that event durable (see the handlers below).
...
- **What it is**: the immutable command that carries the intent "detach one category-item association from a session." A two-value payload: the owning session and the join entity to remove (`RemoveSessionCategoryItemCommand.cs:9-11`).
- **Depends on**: [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating) (the marker it implements), the `SessionIdentifierType` and `SessionCategoryItemIdentifierType` module aliases, and the [`Session`](group-17-conference-domain.md#session) domain type (referenced only to derive a cache key, not a runtime dependency).
- **Concept introduced: a write command as a declarative cache-invalidation trigger.** [Rubric §6, CQRS & Event-Driven] assesses whether writes and reads are cleanly separated and whether write-side effects (like cache eviction) are declared rather than hand-wired. This command states only its shape; by implementing [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating) it tells the caching decorator which region to purge on success. The `CachePrefix` property (`RemoveSessionCategoryItemCommand.cs:14`) computes `"{typeof(Session).FullName}:"`, so the whole `Session` cache namespace is evicted whenever a category link is removed. [Rubric §12, Performance & Scalability]: Conference read endpoints are output-cached, so a mutation must actively evict, otherwise a stale session projection would survive the cache TTL.
- **Walkthrough**: a `sealed record` with a positional constructor (`SessionId`, `SessionCategoryItemId`) and a single computed `CachePrefix` expression member (`.cs:14`). No behavior, no validation here; the ownership graph load and the mutation live in [`RemoveSessionCategoryItemHandler`](#removesessioncategoryitemhandler).
...
- **Where it's used**: dispatched by the `SessionsController` remove-category-item endpoint; handled by [`RemoveSessionCategoryItemHandler`](#removesessioncategoryitemhandler).
...
- **What it is**: the immutable command to remove one existing answer from a session's questions. A two-value payload: the owning session and the answer to remove (`RemoveSessionQuestionAnswerCommand.cs:9-11`).
- **Depends on**: [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating), the `SessionIdentifierType` and `SessionQuestionAnswerIdentifierType` module aliases, and [`Session`](group-17-conference-domain.md#session) (for the cache key only).
- **Concept reinforced: cache-invalidating command.** Structurally identical to [`RemoveSessionCategoryItemCommand`](#removesessioncategoryitemcommand): a `sealed record` whose `CachePrefix` evicts the `Session` namespace (`.cs:14`). The one behavioral difference lives downstream, not here: its handler enforces answer ownership (see [`RemoveSessionQuestionAnswerHandler`](#removesessionquestionanswerhandler)), because an answer has an author while a category link does not. [Rubric §6, CQRS & Event-Driven] applies as above.
- **Walkthrough**: positional constructor (`SessionId`, `SessionQuestionAnswerId`) plus the computed `CachePrefix` member (`.cs:14`). No validation or authorization at the command level.
- **Why it's built this way**: same rationale as the sibling remove commands: an immutable, self-describing data contract that declares its cache effect and defers behavior to its handler.
- **Where it's used**: dispatched by the `SessionsController` remove-answer endpoint; handled by [`RemoveSessionQuestionAnswerHandler`](#removesessionquestionanswerhandler).
...
- **What it is**: the immutable command to detach one speaker association from a session. A two-value payload: the owning session and the session-speaker join entity to remove (`RemoveSessionSpeakerCommand.cs:9-11`).
- **Depends on**: [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating), the `SessionIdentifierType` and `SessionSpeakerIdentifierType` module aliases, and [`Session`](group-17-conference-domain.md#session) (for the cache key only).
- **Concept reinforced: cache-invalidating command.** Same shape as [`RemoveSessionCategoryItemCommand`](#removesessioncategoryitemcommand); its `CachePrefix` evicts the `Session` namespace (`.cs:14`). The interesting wrinkle for this pair is in the handler, which tolerates a missing `SessionId` (the generic UI delete sends only the join-entity id): see [`RemoveSessionSpeakerHandler`](#removesessionspeakerhandler).
- **Walkthrough**: positional constructor (`SessionId`, `SessionSpeakerId`) plus the computed `CachePrefix` member (`.cs:14`).
- **Why it's built this way**: identical to the other remove commands: a pure, cache-aware data contract routed through the decorator pipeline.
- **Where it's used**: dispatched by the `SessionsController` remove-speaker endpoint; handled by [`RemoveSessionSpeakerHandler`](#removesessionspeakerhandler).
...
- **What it is**: the request DTO carrying every field needed to create a [`Session`](group-17-conference-domain.md#session): title, optional schedule and status, the plenum/service/informed/confirmed flags, live/recording/accessibility/resource strings, an optional duration, and the required parent `EventId` plus optional `RoomId` (`SessionCreateRequest.cs:10-65`).
- **Depends on**: [`ICreateRequest`](group-05-cqrs-pipeline.md#icreaterequest) (the generic-mapper-pipeline marker), [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating), the `SessionIdentifierType`/`EventIdentifierType`/`RoomIdentifierType` aliases, and [`Session`](group-17-conference-domain.md#session) (cache key only).
- **Concept introduced: the create-request DTO that doubles as its own command.** [Rubric §9, API & Contract Design] assesses whether the public write contract is explicit and decoupled from the domain model. Unlike the two-value remove commands, this is a `record class` with `required`/`init` members (`Title` at `.cs:19`, `EventId` at `.cs:61`), so the compiler enforces that the two mandatory fields are supplied while the rest stay optional. Implementing [`ICreateRequest`](group-05-cqrs-pipeline.md#icreaterequest) lets the generic request-mapper pipeline dispatch it, and it is also the command type handled directly by [`CreateSessionHandler`](#createsessionhandler) (there is no separate command wrapper). [Rubric §6, CQRS & Event-Driven]: it too implements [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating), evicting the `Session` cache on a successful create (`.cs:13`).
- **Walkthrough**: the computed `CachePrefix` (`.cs:13`); an `init` `Id` that defaults so the handler can auto-assign it (`.cs:16`); two `required` members, `Title` (`.cs:19`) and `EventId` (`.cs:61`); and the remaining optional scalars and booleans (`.cs:22-64`). Pure data, no behavior.
- **Why it's built this way**: `required`/`init` immutability makes the mandatory contract self-documenting and compiler-checked (see [primer §2](00-primer.md#2-architectural-styles-this-codebase-commits-to)); manual DTOs keep the wire contract stable and independent of the aggregate's internals (ADR-001). Reusing the request as the command avoids a redundant wrapper for a create.
- **Where it's used**: bound by the `SessionsController` create endpoint; validated by [`SessionCreateRequestValidator`](#sessioncreaterequestvalidator), mapped by [`SessionCreateRequestMapper`](#sessioncreaterequestmapper), and handled by [`CreateSessionHandler`](#createsessionhandler).
...
- **What it is**: the immutable command that carries the intent "change the text of one existing answer on a session's questions." A three-value payload: the owning session, the answer to change, and the new answer text (`UpdateSessionQuestionAnswerCommand.cs:10-13`).
- **Depends on**: [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating), the `SessionIdentifierType` and `SessionQuestionAnswerIdentifierType` module aliases, and [`Session`](group-17-conference-domain.md#session) (for the cache key only).
- **Concept reinforced: cache-invalidating command with a payload.** The only structural difference from the remove commands is the extra `AnswerValue` string carried alongside the two ids. Its `CachePrefix` (`.cs:16`) evicts the whole `Session` cache namespace whenever an answer is edited, because otherwise stale answer text would survive the cache TTL ([Rubric §12, Performance & Scalability]). [Rubric §6, CQRS & Event-Driven] applies as for the sibling commands.
- **Walkthrough**: a `sealed record` with a positional constructor (`SessionId`, `SessionQuestionAnswerId`, `AnswerValue`) and a single computed `CachePrefix` expression member (`.cs:16`). No behavior, no validation here; the ownership check and the mutation live in [`UpdateSessionQuestionAnswerHandler`](#updatesessionquestionanswerhandler).
- **Why it's built this way**: a pure data contract routed through the [decorator pipeline](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult); making cache invalidation a marker interface keeps the eviction policy declarative and testable.
...
> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Sessions.UseCases.RemoveSessionCategoryItem` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/RemoveSessionCategoryItem/RemoveSessionCategoryItemHandler.cs:13` · Level 8 · class (sealed partial)

- **What it is**: the command handler that applies a [`RemoveSessionCategoryItemCommand`](#removesessioncategoryitemcommand): it loads the session with its category items and delegates the removal to the aggregate.
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`Session`](group-17-conference-domain.md#session), [`Result`](group-01-result-error-handling.md#result)/[`Error`](group-01-result-error-handling.md#error), and `ILogger`.
- **Concept introduced: the load-aggregate-then-delegate handler shape.** [Rubric §4, DDD] assesses whether mutations go through the aggregate rather than mutating rows directly. This is the canonical shape shared by the remove/update session handlers: get the typed repository, load the aggregate tracked with just the child collection it needs (via `includes: [nameof(Session.SessionCategoryItems)]`), and call an aggregate method that owns the invariant. The removal decision (does this join exist, is it deletable) lives in `Session.RemoveSessionCategoryItem`, not here.
- **Walkthrough**: resolves the `Session` repository and loads the aggregate with its category items, tracked (`RemoveSessionCategoryItemHandler.cs:22-27`); returns [`Error.NotFound`](group-01-result-error-handling.md#error) if absent (`.cs:28-29`); calls `entity.RemoveSessionCategoryItem(command.SessionCategoryItemId)` (`.cs:31`); on success saves once and logs via the source-generated `LogCategoryItemRemovedFromSession` (`.cs:32-36`, `.cs:41-42`). The command's [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating) marker means the caching decorator evicts the `Session` cache after success.
- **Why it's built this way**: keeping the mutation inside the aggregate centralizes validation (existence, deletability) and keeps the handler thin. There is no ownership check here because a category association has no author, unlike an answer.
- **Where it's used**: registered as `ICommandHandler<RemoveSessionCategoryItemCommand, Result>`; invoked by the `SessionsController` remove-category-item endpoint through the [decorator pipeline](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult).

### RemoveSessionQuestionAnswerHandler

> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Sessions.UseCases.RemoveSessionQuestionAnswer` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/RemoveSessionQuestionAnswer/RemoveSessionQuestionAnswerHandler.cs:14` · Level 8 · class (sealed partial)

- **What it is**: the command handler that applies a [`RemoveSessionQuestionAnswerCommand`](#removesessionquestionanswercommand): it enforces answer ownership, then removes the answer through the [`Session`](group-17-conference-domain.md#session) aggregate.
...
- **Concept introduced: application-layer authorization inside a handler (owner-or-role).** [Rubric §11, Security] assesses whether authorization is enforced at the right layer with the right data. Field-level "you can only delete your own answer" cannot be expressed by a route policy alone, it needs the loaded row's `CreatedBy`. The handler loads the session with its answers, finds the target answer, and if the caller is not an [`RoleNames.Organizer`](group-08-auth.md#rolenames) and is not the answer's creator, returns [`Error.Forbidden`](group-01-result-error-handling.md#error) with code `SessionQuestionAnswer.NotOwner` (BR-52/BR-53) (`RemoveSessionQuestionAnswerHandler.cs:34-42`). This is the only structural difference from the two behavior-free remove handlers.
- **Walkthrough**: resolves the `Session` repository and loads the aggregate with its answers, tracked (`.cs:24-29`); `Error.NotFound` if absent (`.cs:30-31`). It finds the non-deleted answer by id (`.cs:34`), runs the owner-or-Organizer check (`.cs:35-42`), then calls `entity.RemoveSessionQuestionAnswer(...)` (`.cs:44`); on success it saves once and logs via `LogQuestionAnswerRemovedFromSession` (`.cs:45-48`).
- **Why it's built this way**: ownership authorization needs the persisted `CreatedBy`, so it belongs in the handler after the load, not in a route filter. Routing the mutation through the aggregate keeps validation centralized. The command's [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating) marker evicts the `Session` cache after success.
- **Where it's used**: registered as `ICommandHandler<RemoveSessionQuestionAnswerCommand, Result>`; invoked by the `SessionsController` remove-answer endpoint through the [decorator pipeline](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult).

### RemoveSessionSpeakerHandler

> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Sessions.UseCases.RemoveSessionSpeaker` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/RemoveSessionSpeaker/RemoveSessionSpeakerHandler.cs:13` · Level 8 · class (sealed partial)

- **What it is**: the command handler that applies a [`RemoveSessionSpeakerCommand`](#removesessionspeakercommand): it removes a speaker association through the [`Session`](group-17-conference-domain.md#session) aggregate, resolving the owning session even when the caller did not supply its id.
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`Session`](group-17-conference-domain.md#session), [`Result`](group-01-result-error-handling.md#result)/[`Error`](group-01-result-error-handling.md#error), and `ILogger`.
- **Concept introduced: a handler tolerating a missing parent id from a generic UI delete.** [Rubric §9, API & Contract Design] assesses robustness of the write contract against realistic clients. The DELETE endpoint takes the session id as an optional query parameter, but the UI's generic delete sends only the join-entity id, so `SessionId` arrives as the default `0`. When `command.SessionId == default`, the handler locates the owning session by querying for the session whose `SessionSpeakers` contains the target join id (`RemoveSessionSpeakerHandler.cs:28-36`); otherwise it loads the session directly by id (`.cs:37-44`). Both paths converge on the same aggregate delegation. Contrast with the category-item and answer handlers, which always receive a real `SessionId`.
- **Walkthrough**: resolves the `Session` repository (`.cs:22`); branches on whether `SessionId` is set (`.cs:28-44`), returning [`Error.NotFound`](group-01-result-error-handling.md#error) if no owner is found (`.cs:46-47`); calls `entity.RemoveSessionSpeaker(command.SessionSpeakerId)` (`.cs:49`); on success saves once and logs via `LogSpeakerRemovedFromSession` (`.cs:50-53`).
- **Why it's built this way**: the fallback lookup keeps the generic client working without a bespoke UI change, while still routing every mutation through the aggregate. Like the category-item handler, there is no ownership check because a speaker association has no author. The command's [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating) marker evicts the `Session` cache after success.
- **Where it's used**: registered as `ICommandHandler<RemoveSessionSpeakerCommand, Result>`; invoked by the `SessionsController` remove-speaker endpoint through the [decorator pipeline](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult).

### SessionCreateRequestMapper

> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Sessions.UseCases.Create` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/Create/SessionCreateRequestMapper.cs:11` · Level 8 · class (sealed)

- **What it is**: the request-to-entity mapper that turns a validated [`SessionCreateRequest`](#sessioncreaterequest) into a [`Session`](group-17-conference-domain.md#session) by delegating to the domain factory `Session.Create`.
- **Depends on**: [`IEntityRequestMapper<TEntity, TCreateRequest, TIdentifierType>`](group-12-api-hosting-mapping.md#ientityrequestmappertentity-tcreaterequest-tidentifiertype) (the generic contract it implements), [`Session`](group-17-conference-domain.md#session), the `SessionIdentifierType` alias, and [`Result`](group-01-result-error-handling.md#result).
- **Concept introduced: the request mapper as the single translation point between wire DTO and aggregate.** [Rubric §3, Clean Architecture] assesses whether the application layer, not the domain, owns request translation. Rather than newing up a `Session`, the mapper calls the aggregate's `Create` factory (`SessionCreateRequestMapper.cs:19-33`), which returns a [`Result<Session>`](group-01-result-error-handling.md#result) so any domain-level construction failure surfaces as a first-class error, not an exception. Because `Session.Create` is the guarded entry point, the mapper never bypasses invariants. [Rubric §2, Design Patterns]: this is manual mapping (ADR-001), keeping the DTO-to-entity mapping explicit and analyzer-friendly.
- **Walkthrough**: implements `CreateEntityAsync` (`.cs:15`); guards the request with `ArgumentNullException.ThrowIfNull` (`.cs:17`); calls `Session.Create(...)` passing the request fields positionally, then wraps the synchronous result in `Task.FromResult` (`.cs:19-33`). Note that `Duration`, `IsInformed`, and `IsConfirmed` from the request are not forwarded to `Session.Create`; only the fields the factory accepts are mapped.
- **Why it's built this way**: routing creation through the factory keeps every invariant in one place and returns errors as values; making the mapper `async`-shaped (returning `Task`) satisfies the generic [`IEntityRequestMapper`](group-12-api-hosting-mapping.md#ientityrequestmappertentity-tcreaterequest-tidentifiertype) contract even though the work is synchronous.
...
- **Caveats / not-in-source**: the exact parameter list and validation performed by `Session.Create` is defined in the Conference domain, not here; this mapper only forwards the request fields it passes.

### SessionCreateRequestValidator

> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Sessions.UseCases.Create` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/Create/SessionCreateRequestValidator.cs:7` · Level 8 · class (sealed)

- **What it is**: the FluentValidation validator for [`SessionCreateRequest`](#sessioncreaterequest), assembled purely by composing reusable, per-field rule sets rather than writing rules inline.
- **Depends on**: `AbstractValidator<SessionCreateRequest>` (FluentValidation, NuGet), and the reusable session rule sets [`SessionTitleRules<T>`](#sessiontitlerulest), [`SessionEventIdRules<T>`](#sessioneventidrulest), and the sibling `SessionDescriptionRules`, `SessionStatusRules`, `SessionLiveUrlRules`, `SessionRecordingUrlRules`, `SessionAccessibilityInfoRules`, `SessionResourceLinksRules`.
- **Concept introduced: composing a validator from shared rule sets via `Include`.** [Rubric §24, Forms/Validation/UX Safety] assesses whether validation rules are consistent and reused across write paths. FluentValidation's `Include(...)` merges another validator's rules into this one; the create validator does nothing but include eight generic rule sets, each parameterized by a lambda selecting the field it validates (for example `new SessionTitleRules<SessionCreateRequest>(p => p.Title)` at `SessionCreateRequestValidator.cs:11`). The same rule sets are reused by the session update validator, so title/status/URL constraints stay identical across create and update. [Rubric §1, SOLID]: each rule set is a single-responsibility, open-for-reuse unit, keyed by a property selector rather than a fixed request type.
- **Walkthrough**: the constructor (`.cs:9`) issues eight `Include(...)` calls, one per validated field (`.cs:11-18`); no rules are declared inline. Fields absent from this list (schedule, flags, duration, room) carry no create-time validation here.
- **Why it's built this way**: extracting field rules into generic, selector-driven sets removes duplication between the create and update validators and keeps each constraint defined once (see [primer §2](00-primer.md#2-architectural-styles-this-codebase-commits-to)).
- **Where it's used**: resolved by the validation decorator in the [CQRS pipeline](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult) before [`CreateSessionHandler`](#createsessionhandler) runs.
...
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`ICurrentUserService`](group-08-auth.md#icurrentuserservice), [`RoleNames`](group-08-auth.md#rolenames), [`Session`](group-17-conference-domain.md#session) / [`SessionQuestionAnswer`](group-17-conference-domain.md#sessionquestionanswer), [`Result`](group-01-result-error-handling.md#result)/[`Error`](group-01-result-error-handling.md#error), and `ILogger`.
- **Concept reinforced: owner-or-role authorization inside a handler.** This is the update-side twin of [`RemoveSessionQuestionAnswerHandler`](#removesessionquestionanswerhandler): the same field-level check (BR-52/BR-53) that field-level authorization needs the loaded row's `CreatedBy`, so it lives in the handler. If the caller is not an [`RoleNames.Organizer`](group-08-auth.md#rolenames) and is not the answer's creator, it returns [`Error.Forbidden`](group-01-result-error-handling.md#error) with code `SessionQuestionAnswer.NotOwner` (`UpdateSessionQuestionAnswerHandler.cs:34-42`). [Rubric §11, Security] as above; [Rubric §4, DDD]: the actual text change is delegated to the aggregate's `UpdateSessionQuestionAnswer` method (`.cs:44`), keeping the invariant inside the model.
- **Walkthrough**: resolves the `Session` repository and loads the aggregate with its answers, tracked (`.cs:24-29`); `Error.NotFound` if absent (`.cs:30-31`). It finds the non-deleted answer by id (`.cs:34`), runs the owner-or-Organizer check (`.cs:35-42`), then calls `entity.UpdateSessionQuestionAnswer(...)` (`.cs:44`); on success it saves once and logs via `LogSessionQuestionAnswerUpdated` (`.cs:45-48`).
- **Why it's built this way**: ownership authorization needs the persisted `CreatedBy`, so it belongs in the handler after the load; routing the mutation through the aggregate keeps validation (empty text, existence of the answer) centralized. The command's [`ICacheInvalidating`](group-05-cqrs-pipeline.md#icacheinvalidating) marker evicts the `Session` cache after success.
...
> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Sessions.UseCases.Create` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Sessions/UseCases/Create/CreateSessionHandler.cs:20` · Level 9 · class (sealed partial)

- **What it is**: the command handler that creates a [`Session`](group-17-conference-domain.md#session) from a [`SessionCreateRequest`](#sessioncreaterequest): it assigns a manual id when none is supplied, guards against room double-booking, maps through the domain factory, persists, and returns the mapped [`SessionDTO`](group-17-conference-domain.md#sessiondto), retrying on an id collision.
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), `IServiceScopeFactory` (BCL DI), [`IEntityRequestMapper<Session, SessionCreateRequest, SessionIdentifierType>`](group-12-api-hosting-mapping.md#ientityrequestmappertentity-tcreaterequest-tidentifiertype) (satisfied by [`SessionCreateRequestMapper`](#sessioncreaterequestmapper)), [`SessionDTOMapper`](#sessiondtomapper), `SessionInvariants`/`SessionRoomScheduling` (Conference domain and application helpers), [`Result`](group-01-result-error-handling.md#result)/[`Error`](group-01-result-error-handling.md#error), and `ILogger`.
- **Concept introduced: application-assigned ids in a reserved range with optimistic collision retry.** [Rubric §8, Data Architecture] assesses how identity is assigned when the primary key is meaningful. Session ids are app-assigned because the int PK IS the Sessionize id; an organizer-created session (request id defaults to `0`) must be given the next free id in a reserved manual range so it never collides with Sessionize-assigned ids. The handler queries the max existing id in `[ManualIdRangeStart, ManualIdRangeEnd]` (ignoring query filters so soft-deleted rows still reserve their id) and picks `max + 1`, failing with `Error.Failure` when the range is exhausted (`CreateSessionHandler.cs:77-93`). Because two concurrent organizer creates can compute the same next id, the outer loop retries up to `MaxManualIdAttempts` (3) on a duplicate-key violation, each retry in a fresh DI scope because the ambient `DbContext` still tracks the failed insert (`.cs:40-60`, `.cs:28`). [Rubric §12, Performance & Scalability]: it also enforces a room double-booking guard, rejecting an overlapping `[StartsAt, EndsAt)` in the same room via `SessionRoomScheduling.BuildOverlapPredicate` (`.cs:96-103`).
- **Walkthrough**
  - `HandleAsync` (`.cs:31`): an explicit caller id (for example a Sessionize import) is respected as-is with a single attempt (`.cs:37-38`); otherwise the retry loop runs, using the ambient unit of work on attempt one and a fresh scope's unit of work thereafter (`.cs:40-60`), catching only unique-key violations under the attempt bound (`.cs:55-59`).
  - `CreateCoreAsync` (`.cs:67`): resolves the repository, auto-assigns the manual id when unset (`.cs:77-93`), applies the double-booking guard (`.cs:96-103`), maps via `requestMapper.CreateEntityAsync` and short-circuits on failure (`.cs:105-107`), then adds, saves once, logs `LogSessionCreated`, and returns `dtoMapper.MapToDTO(entity)` (`.cs:111-116`).
  - `IsUniqueKeyViolation` (`.cs:124-133`): walks the exception chain and matches "duplicate key" case-insensitively, because the application layer cannot reference EF Core types (SQL Server error 2601 and 2627 both report it).
- **Why it's built this way**: the reserved-range scheme keeps organizer-created ids from ever clashing with the externally-owned Sessionize id space, and the scope-per-retry is required because a tracked failed insert poisons the ambient context. Message-based violation detection preserves Clean Architecture: the application layer stays free of the EF Core dependency (ADR-001 boundary discipline).
- **Where it's used**: registered as `ICommandHandler<SessionCreateRequest, Result<SessionDTO>>`; invoked by the `SessionsController` create endpoint through the [decorator pipeline](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult), after [`SessionCreateRequestValidator`](#sessioncreaterequestvalidator) has run.
- **Caveats / not-in-source**: `SessionInvariants.ManualIdRangeStart`/`ManualIdRangeEnd`, `SessionRoomScheduling.BuildOverlapPredicate`/`DoubleBookedError`, and `SessionDTOMapper.MapToDTO` are defined elsewhere in the Conference module; this handler only orchestrates them.

### EventNavigationPopulator

> MMCA.ADC.Conference.Application · `MMCA.ADC.Conference.Application.Events` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Application/Events/EventNavigationPopulator.cs:11` · Level 10 · class (sealed)

- **What it is**: the entity-specific navigation populator for the [`Event`](group-17-conference-domain.md#event) aggregate; it declares how to bulk-load an event's three child collections (`Rooms`, `EventSpeakers`, `EventQuestionAnswers`) that EF Core cannot eager-load via `.Include()` here.
- **Depends on**: [`DeclarativeNavigationPopulator<TEntity>`](group-11-navigation-populators.md#declarativenavigationpopulatortentity) (the base), [`ChildNavigationDescriptor<TEntity, TParentId, TChild, TChildId>`](group-11-navigation-populators.md#childnavigationdescriptortentity-tparentid-tchild-tchildid), [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), and the [`Event`](group-17-conference-domain.md#event), `Room`, `EventSpeaker`, `EventQuestionAnswer` domain types with their identifier aliases.
...
- **Walkthrough**: the primary constructor takes an [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork) and passes it plus an array of three descriptors to the base (`EventNavigationPopulator.cs:11-36`). Each [`ChildNavigationDescriptor`](group-11-navigation-populators.md#childnavigationdescriptortentity-tparentid-tchild-tchildid) supplies four members:
  - `PropertyName`, the property-name string (`nameof(Event.Rooms)`, etc.) the base uses to build navigation metadata and mark the property populated (`.cs:17`, `.cs:24`, `.cs:31`).
  - `ParentKeySelector`, `e => e.Id`, the parent aggregate's key used to group fetched child rows.
  - `ChildForeignKeySelector`, `child => child.EventId`, the child's foreign-key column matching children to parents.
  - `AssignAction`, `(e, rooms) => e.SetRooms(rooms)` and siblings (`.cs:20`, `.cs:27`, `.cs:34`), handing the loaded children back through the aggregate's own setter so the domain model (not infrastructure) owns the assignment.
- **Why it's built this way**: ADR-002 mandates that the application layer control eager-loading without an EF dependency. The base provides the generic machinery; this class is the `Event`-specific binding, colocated with the aggregate's use-cases folder so it is small and locatable by convention.
- **Where it's used**: registered as `INavigationPopulator<Event>` in the Conference Application DI; injected into the Event query service and any handler that reads an `Event` with its children.
...
- **Why it's built this way** : ADR-002 requires the application layer to control eager loading without an EF dependency; the generic machinery lives in the base and each aggregate gets a small, locatable configuration file in its own feature folder.
...
- **Why it's built this way** : one populator file per aggregate keeps the configuration small and co-located with the session use cases (ADR-002).
...
- **Concept reinforced** : declarative child loading (ADR-002); see [`ConferenceCategoryNavigationPopulator`](#conferencecategorynavigationpopulator) for the mechanism.
...
- **Why it's built this way** : one small populator per aggregate, co-located with the speaker feature (ADR-002).
