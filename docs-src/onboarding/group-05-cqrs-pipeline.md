...
and validation; reads get result caching), and it is the seam `[Rubric §1, SOLID]` rewards: each
...
per-type sections: [`IQueryCacheable`](#iquerycacheable) is wired and unit-tested but **no production
query opts in**, read caching is currently done at the HTTP `OutputCache` layer instead.)
...
events feed the outbox ([G04](group-04-events-outbox.md), ADR-003) on save. And because the *only*
...
- **Where it's used**: ⚠️ **currently unused in production (a wired-but-unadopted seam).** Verified by source search across all three repos: the *only* type implementing `IQueryCacheable` anywhere is the test `CacheableTestQuery` (`CachingQueryDecoratorTests.cs`); **no production query opts in.** The decorator is registered and ready, but every query falls through its early-return (no-cache) path. Production read-side caching is instead done at the HTTP layer via **ASP.NET Core OutputCache**, the Conference read controllers (`EventsController`, `SessionsController`, `SessionSpeakersController`, …) use named output-cache policies configured by `AddOutputCache` in the service `Program.cs` files. Contrast the write-side sibling [ICacheInvalidating](#icacheinvalidating), which **is** broadly adopted.
- **Caveats / not-in-source**: the read-cache *mechanism* is fully functional and unit-tested but unadopted by real queries. If you add a hot, non-user-specific list query you can opt in by implementing this; do **not** assume existing queries are cached through this path (they aren't).
...
- **Why it's built this way**: most deletes are identical (load → `Delete()` → save), so the framework supplies the handler once via Common's `Delete()` + soft-delete convention (ADR-005); a module overrides only when there is a cascade or extra rule. Returning the entity's own `Delete()` `Result` lets a domain-level refusal (e.g. "cannot delete a published event") propagate without an exception.
...
- **Caveats / not-in-source**: `DeleteEntityHandler` is **not** marked [ITransactional](#itransactional), so a delete that raises domain events relies on `SaveChangesAsync` writing the data + outbox rows atomically rather than on an explicit handler-level transaction; the outbox is the durability mechanism (ADR-003).
...
- **Where it's used**: wraps every query handler, but is a no-op for all of them today. As verified for [IQueryCacheable](#iquerycacheable), no production query opts in (the only implementor is a unit test), and production read caching is done at the HTTP layer with ASP.NET Core OutputCache instead. This decorator is a ready, tested seam rather than an in-use one.
