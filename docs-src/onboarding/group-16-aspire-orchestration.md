...
The two E2E helpers are the CI-shaped siblings, and both are no-ops outside CI. `WithE2eRsaKeys()` (`Extensions.cs:137`) forwards an ephemeral RSA keypair from `E2E_JWT_PRIVATE_KEY_PEM` / `E2E_JWT_PUBLIC_KEY_PEM` onto Identity's `Jwt__RsaPrivateKeyPem` / `Jwt__RsaPublicKeyPem` / `Jwks__RsaPublicKeyPem` (`:148-151`), returning untouched when either variable is absent (`:143-146`), so locally and in production user-secrets or Key Vault supply the keys (`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:195`). `WithE2eRegistrationThrottleLift(alsoLiftWhen)` (`:176`) raises Identity's per-IP registration cap to the `E2eRegistrationsPerIpPerHour` constant of 1000 (`:35`, injected as `LoginProtection__MaxRegistrationsPerIpPerHour` at `:187-189`) when the `E2E_LIFT_REGISTRATION_THROTTLE` env var is set or the caller passes its own trigger (`:180-184`); a Playwright suite registers far more than ten accounts from one localhost IP, so without the lift the anti-abuse control refuses every register test past the tenth and the failures look like broken registration. ADC calls it with its forced-WASM flag as the extra trigger (`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:324`, with the two independently gated E2E render-mode switches immediately above at `:307-322`). The Cosmos and SQLite siblings (`Extensions.cs:245,275`) inject the equivalent per-engine connection-string env vars ([ADR-018](https://ivanball.github.io/docs/adr/018-polyglot-persistence.html)) and are layered on top of, not instead of, the SQL Server source; no ADC or Store service wires them today.
...
- **Depends on**: `Microsoft.AspNetCore.DataProtection` plus its Azure Blob Storage and Key Vault key-ring providers, and `Azure.Identity` (`DefaultAzureCredential`). No first-party types: it is deliberately a thin adapter over the BCL/Azure APIs.
...
- **Where it's used**: The two ADC hosts that mint cookies or antiforgery tokens: `MMCA.ADC/Source/Hosts/UI/MMCA.ADC.UI.Web/Program.cs:31` and `MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/Program.cs:118`. The matching blob and key-vault URIs are injected by the Container Apps Bicep (`MMCA.ADC/infra/main.bicep:1106` for Identity, `:1713` for the UI). Covered by [DataProtectionExtensionsTests](group-27-testing-infrastructure.md#dataprotectionextensionstests), which asserts both gates on a built service provider without any Azure credential (the blob client is constructed lazily by the repository, never at registration time).
- **Caveats / not-in-source**: No Store host calls `AddCommonDataProtection()` today; the method is framework plumbing that ADC alone wires. Nothing in this file validates that the configured URI points at a reachable blob: a wrong URI surfaces at first key-ring access, not at startup.
...
- **Where it's used**: AppHost `Program.cs` in `MMCA.ADC.AppHost` (and `MMCA.Store.AppHost`). ADC wires the broker once (`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:60`), attaches all four services to it (`Program.cs:95,118,158,183`), calls `WithSQLServerDataSource` once per service (`Program.cs:93,116,156,181`), wires JWKS discovery for the three non-Identity services through the gateway (`Program.cs:273-275`), calls `WithE2eRsaKeys()` on Identity (`Program.cs:195`), and lifts the registration throttle with `identityService.WithE2eRegistrationThrottleLift(alsoLiftWhen: forceWasm)` (`Program.cs:324`). The `WithCosmosDataSource` / `WithSqliteDataSource` helpers are available framework plumbing ([ADR-018](https://ivanball.github.io/docs/adr/018-polyglot-persistence.html)) but no consumer wires them today: every ADC and Store service currently runs on SQL Server.
- **Caveats / not-in-source**: For all three `With*DataSource` helpers the `logicalName` must match the key the owning entity's configuration derives (module namespace or `[UseDatabase]`); source shows no validation of that string, so a mismatch routes that entity to the `Default` source rather than failing at startup. Separately, when reading the ADC AppHost note that its comment at `MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:82-86` ("No WithBroker() yet") is stale: the Notification resource does call `.WithBroker(rabbit)` at `:95`. Trust the call, not the comment.
...
- **Where it's used**: The YARP Gateway hosts: `MMCA.ADC/Source/Hosts/MMCA.ADC.Gateway/Program.cs:42` and `MMCA.Store/Source/Hosts/MMCA.Store.Gateway/Program.cs:48`, each followed by a bare `app.UseCors()` (ADC `Program.cs:53`, Store `Program.cs:64`).
...
- **Where it's used**: The [Common.Aspire `Extensions`](#extensions-1): the warm-up check is registered with `[HealthCheckTags.Ready]` (`MMCA.Common/Source/Hosting/MMCA.Common.Aspire/Extensions.cs:109`), the `"self"` check with `[HealthCheckTags.Live]` (`:200`), and the conditional Redis and RabbitMQ checks with `[HealthCheckTags.Optional]` (`:246`, `:258`). The endpoint predicates read them back: `/alive` requires `Live` (`:335`) and `/health/ready` excludes both `Live` and `Optional` (`:351`). The tagging is asserted by [InfrastructureHealthChecksTests](group-27-testing-infrastructure.md#infrastructurehealthcheckstests) (`MMCA.Common/Tests/Hosting/MMCA.Common.Aspire.Tests/Health/InfrastructureHealthChecksTests.cs:89,93`).
...
- **Why it's built this way**: Sealed. Clearing `Recorded` (rather than trying to drop the activity) is what makes the batch export processors skip it, which is why the registration order matters: this processor is added *before* the exporters so its `OnEnd` runs first (comment at `MMCA.Common/Source/Hosting/MMCA.Common.Aspire/Extensions.cs:170-174`). The parent-chain walk, rather than a direct-parent check, catches deeply nested children (the auto-instrumented `SqlClient` span is a grandchild of the poll span). The literal-string duplication is the documented price of keeping this package off `MMCA.Common.Infrastructure`, whose dependency set (EF Core, MassTransit, SignalR/Redis) every `AddServiceDefaults` caller would otherwise inherit.
- **Where it's used**: Registered on the tracing pipeline via `.AddProcessor(new Telemetry.OutboxPollFilterProcessor())` in the [Common.Aspire `Extensions`](#extensions-1) `ConfigureOpenTelemetry` (`Extensions.cs:175`), before `AddOpenTelemetryExporters()` (`:186`). Covered by [OutboxPollFilterProcessorTests](group-27-testing-infrastructure.md#outboxpollfilterprocessortests).
...
- **Why it's built this way**: The decision is separated from the binding so it can be asserted as data (see [KestrelListenerSpec](#kestrellistenerspec)). The probe port is configuration-gated rather than always-on so local and test runs keep Aspire's dynamic ports. Fail-fast on a bad value rather than fail-quiet follows the same contract as **[ADR-070](https://ivanball.github.io/docs/adr/070-fail-fast-configuration-contract.html)**: a misconfiguration that would produce a silently unprobeable revision is worth a startup crash. And the helper lives in the shared framework package rather than in each service, which is what let six service hosts across two apps collapse onto one implementation.
- **Where it's used**: All four ADC services, three on the h2c profile (`MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/Program.cs:81`, `MMCA.ADC/Source/Services/MMCA.ADC.Conference.Service/Program.cs:85`, `MMCA.ADC/Source/Services/MMCA.ADC.Engagement.Service/Program.cs:61`) and Notification on the mixed profile (`MMCA.ADC/Source/Services/MMCA.ADC.Notification.Service/Program.cs:71`, `HttpProtocols.Http1AndHttp2, redeclareCleartextEndpoint: false`); plus both Store services (`MMCA.Store/Source/Services/MMCA.Store.Identity.Service/Program.cs:52`, `MMCA.Store/Source/Services/MMCA.Store.Catalog.Service/Program.cs:47`). The port is supplied by the Container Apps Bicep: `HealthProbe__Port` is `8081` for the ADC REST/gRPC services (`MMCA.ADC/infra/main.bicep:1056`, `:1232`, `:1343`) and `8082` for Notification (`:1477`), and `8081` for both Store services (`MMCA.Store/infra/main.bicep:810`, `:903`). Covered by [KestrelEndpointExtensionsTests](group-27-testing-infrastructure.md#kestrelendpointextensionstests), whose seven cases pin the empty-plan, blank-value, fail-fast, both-profile, always-Http1, and custom-cleartext-port behaviors.
...
- **Walkthrough**:
  ...
  - `AddWarmupReadiness()` (`Extensions.cs:101`): registers the singleton [WarmupReadinessGate](#warmupreadinessgate) (`:103`), the [WarmupHostedService](#warmuphostedservice) runner (`:104`), the built-in [OpenIdConnectMetadataWarmupTask](#openidconnectmetadatawarmuptask) (`:106`), and the [WarmupReadinessHealthCheck](#warmupreadinesshealthcheck) as the `"warmup"` check tagged `HealthCheckTags.Ready` (`:108-109`). `AddWarmupTask<TTask>()` (`:306`, in an `extension(IServiceCollection services)` block at `:297`) lets a consumer add service-specific warm-ups by registering `AddSingleton<IWarmupTask, TTask>()` (`:309`).
  - `ConfigureOpenTelemetry()` (`Extensions.cs:121`): logging with `IncludeFormattedMessage` and `IncludeScopes` (`:125-126`); metrics always add ASP.NET Core instrumentation (`:132`), then add `HttpClient` (`:143`) and .NET runtime (`:152`) instrumentation **only when their cost knobs are not set** (`:141`, `:150`), and subscribe the three MMCA meters by literal name (`:159-161`). Tracing adds the application's own source plus `MMCA.Common.Outbox` (`:165-166`), ASP.NET Core and `HttpClient` instrumentation (`:167-168`), and `.AddProcessor(new Telemetry.OutboxPollFilterProcessor())` (`:175`), which the comment above it (`:170-174`) requires to be registered before the exporters so its `OnEnd` clears `Recorded` first. When `TryGetTraceSampleRatio` succeeds it installs `new ParentBasedSampler(new TraceIdRatioBasedSampler(ratio))` (`:182-183`), parent-based so a sampled-in request keeps its whole trace intact across service boundaries. Finally it calls the private `AddOpenTelemetryExporters` (`:186`).
  - `TryGetTraceSampleRatio(configuration, out ratio)` (`Extensions.cs:364`, `internal static`): reads the optional `Telemetry:TracesSampleRatio` knob and returns `true` only for a value that parses invariantly and falls in the open interval (0,1); absent, unparseable, or out-of-range input returns `false` and leaves `ratio` at `1.0` (`:366-376`). Sampling therefore fails toward keeping everything, so a typo can never silently drop *all* telemetry.
  - `IsInstrumentationDisabled(configuration, configKey)` (`Extensions.cs:387`, `internal static`): the same fail-safe shape for the two metric-family knobs `Telemetry:DisableHttpClientMetrics` and `Telemetry:DisableRuntimeMetrics`, expressed as one line, `bool.TryParse(configuration[configKey], out var disabled) && disabled` (`:388`). It returns `true` (drop the family) only when the value parses as boolean `true`; absent, blank, or unparseable keeps the instrumentation. The comments at `:134-140` and `:146-149` name why those two families are the targets: they are the highest-volume AppMetrics contributors on a low-traffic multi-service deployment and carry no end-user-visible signal.
  - `AddDefaultHealthChecks()` (`Extensions.cs:197`) registers a `"self"` check tagged `HealthCheckTags.Live` (`:200`). `AddInfrastructureHealthChecks(requireSqlServer = false)` (`:228`) conditionally adds SQL Server (`:238-241`), Redis (`:243-247`), and RabbitMQ (`:249-259`) checks only when their connection strings are present, so the same binary runs unchanged where those containers are absent. The `requireSqlServer` flag throws at startup when the SQL connection string is missing (`:233-236`); the asymmetry is deliberate and documented (`:211-222`): Redis and RabbitMQ are optional per host, but a service that cannot resolve its own database is misconfigured and must not report healthy. Redis and RabbitMQ get the `Optional` tag; SQL Server deliberately does not.
  - `MapDefaultEndpoints()` (`Extensions.cs:327`, in an `extension(WebApplication app)` block at `:314`): `/health` maps every check (`:329`); `/alive` filters to `HealthCheckTags.Live` (`:333-336`) so a downstream SQL outage cannot get the container restarted; `/health/ready` maps everything tagged neither `Live` nor `Optional` (`:349-352`). The comment above it (`:338-348`) is the canonical explanation of why `optional` is excluded.
  - `AddOpenTelemetryExporters()` (`Extensions.cs:277`, private): enables **OTLP** when `OTEL_EXPORTER_OTLP_ENDPOINT` is set (`:279-285`, the local Aspire dashboard sets it) and **Azure Monitor** when `APPLICATIONINSIGHTS_CONNECTION_STRING` is set (`:287-293`, set by the Container Apps Bicep). Both can be active simultaneously.
...
- **Where it's used**: `builder.AddServiceDefaults()` early in each framework-consuming service host's `Program.cs` (for example `MMCA.ADC/Source/Hosts/MMCA.ADC.Gateway/Program.cs:31`, `MMCA.ADC/Source/Services/MMCA.ADC.Conference.Service/Program.cs:115`), with `app.MapDefaultEndpoints()` after `Build()` (ADC Gateway `Program.cs:52`, Conference `Program.cs:346`). The two internal cost-knob helpers are covered by [TracesSampleRatioTests](group-27-testing-infrastructure.md#tracessampleratiotests) and [MetricsInstrumentationToggleTests](group-27-testing-infrastructure.md#metricsinstrumentationtoggletests); the conditional infrastructure checks by [InfrastructureHealthChecksTests](group-27-testing-infrastructure.md#infrastructurehealthcheckstests).
