...
roots side by side (`MMCA.ADC.UI.Web/Program.cs:28-100`, `MMCA.ADC.UI.Web.Client/Program.cs:21-80`,
`MMCA.ADC.UI/MauiProgram.cs:51-196`) and the family resemblance is obvious: the same MudBlazor
...
[`ADCHomePageContent`](#adchomepagecontent) (web `MMCA.ADC.UI.Web/Program.cs:60` and
`MMCA.ADC.UI.Web.Client/Program.cs:49`, MAUI `MMCA.ADC.UI/MauiProgram.cs:122`). **Token storage**:
...
(`MMCA.ADC.UI/MauiProgram.cs:161`, backed by the framework's
[`MauiTokenStorageService`](group-26-device-capability-layer.md#mauitokenstorageservice)),
`AddCommonServerTokenStorage()` on the Server head (`MMCA.ADC.UI.Web/Program.cs:73`, backed by
...
(`MMCA.ADC.UI.Web/Program.cs:74`, `MMCA.ADC.UI.Web.Client/Program.cs:53`) while MAUI, which has no
same-origin proxy to lean on, uses
[`DirectApiTokenRefresher`](group-15-common-ui-framework.md#directapitokenrefresher)
(`MMCA.ADC.UI/MauiProgram.cs:162`). **Form factor** is the same story in three registration lines:
`AddCommonWebFormFactor()`, `AddWasmFormFactor()`, and `AddMauiFormFactor()`
(`MMCA.ADC.UI.Web/Program.cs:93`, `MMCA.ADC.UI.Web.Client/Program.cs:80`,
`MMCA.ADC.UI/MauiProgram.cs:167`), all satisfying the same
...
(`MMCA.ADC.UI/MauiProgram.cs:76-91`). Direction one:
...
(`MMCA.ADC.UI/MauiProgram.cs:97,99`, `MMCA.ADC.UI.Web/Program.cs:52-53`,
`MMCA.ADC.UI.Web.Client/Program.cs:42-43`). Direction two: everything that overrides a shared
...
(`MMCA.ADC.UI.Web/Program.cs:58`, `MMCA.ADC.UI.Web.Client/Program.cs:47`). Direction three: a module
...
(`MMCA.ADC.UI.Web/Program.cs:99-100`), feeding the framework's
...
(`MMCA.ADC.UI/MauiProgram.cs:125-135`, `MMCA.ADC.UI.Web/Program.cs:79-89`,
`MMCA.ADC.UI.Web.Client/Program.cs:59-69`), reading the `Modules` section (all four enabled in the
...
(`MMCA.ADC.UI.Web/Program.cs:195-209`). This is the group's cleanest
...
(`MMCA.ADC.UI/MauiProgram.cs:51`) builds the entire DI and configuration graph. Because MAUI does
...
(`MMCA.ADC.UI.Web/Program.cs:177-188`), and the applinks components mirror the same Blazor routes
...
fitness test, and the file says so (`MMCA.ADC.UI/MauiProgram.cs:88-91`). The response is a rule
...
same-origin proxy refresher (`MMCA.ADC.UI.Web/Program.cs:74`,
...
and open-in-new-tab (`MMCA.ADC.UI.Web/Program.cs:66-71,138-141`). On MAUI, which has no DOM and
...
endpoint (`MMCA.ADC.UI.Web/Program.cs:124,169`); the WASM client mirrors the same cookie into the
...
(`MMCA.ADC.UI/MauiProgram.cs:44-45,210-211`). `[Rubric §27, Internationalization & Localization]`
...
(`MMCA.ADC.UI.Web/Program.cs:145-164`, `MMCA.ADC.UI.Web.Client/Program.cs:29-35`), with exactly one
retry on a cold start and a loud failure after that (`MMCA.ADC.UI.Web.Client/Program.cs:125-137`)
and a discarded token-hydration warm-up overlapping first render
(`MMCA.ADC.UI.Web.Client/Program.cs:93,102-119`). On MAUI the same component tree runs inside a
...
- **Where it's used**: registered as a singleton `IHomePageContent` by all three heads: the WebAssembly client (`MMCA.ADC.UI.Web.Client/Program.cs:49`), the Blazor Server host (`MMCA.ADC.UI.Web/Program.cs:60`), and [MauiProgram](#mauiprogram) (`MMCA.ADC.UI/MauiProgram.cs:122`, resolving the `MMCA.ADC.UI.Pages` class imported at `:20`).
...
- **Where it's used**: registered as a singleton `IUIModule` in [MauiProgram](#mauiprogram) (`MMCA.ADC.UI/MauiProgram.cs:157`); its components render inside the shared `MMCA.Common.UI` layout.
...
- **Where it's used**: named as the app type in `builder.UseMauiApp<App>()` (`MMCA.ADC.UI/MauiProgram.cs:55`); the MAUI framework instantiates it after each platform head calls `CreateMauiApp()`.
...
- **Where it's used**: called by `MauiProgram.HandleAppAction` on activation (`MMCA.ADC.UI/MauiProgram.cs:215`), and its id constants are re-exported by [AppActionsInitializer](#appactionsinitializer) (`MMCA.ADC.UI/Services/AppActionsInitializer.cs:20-22`) so the registration side and the activation side cannot drift apart.
...
- **Where it's used**: invoked by the Android OS during the OAuth flow enabled by the [IOAuthUISettings](group-15-common-ui-framework.md#ioauthuisettings) registration in [MauiProgram](#mauiprogram) (`MMCA.ADC.UI/MauiProgram.cs:97`); it is never called from managed code.
...
> MMCA.ADC.UI · `MMCA.ADC.UI` · `MMCA.ADC.UI/MauiProgram.cs:35` · Level 11 · class (static)
...
- **Walkthrough**
  ...
  - `StartupResources` (`:44-45`): a static `ResourceManager` over `MMCA.ADC.UI.MauiProgram`, needed because some startup strings must resolve **before** `Build()`, when no service provider and therefore no `IStringLocalizer` exists yet (doc comment at `:37-43`). It is the same co-located resx convention every other localized string in this head uses (`MMCA.ADC.UI/MauiProgram.resx` and its `.es.resx` sibling), read through the resource manager instead of the localizer.
  - Builder chain (`:53-63`): `UseMauiApp<App>()` (`:55`), `UseMauiCommunityToolkit()` (`:58`, required by the speech-to-text capability, [ADR-042](https://ivanball.github.io/docs/adr/042-device-capability-abstraction.html) Wave 4, and the toolkit analyzer insists the call sits in the app's own chain, `:56-57`), a font registration (`:59`), and `ConfigureEssentials(essentials => essentials.OnAppAction(HandleAppAction))` for home-screen quick actions (`:60-63`).
  - Configuration (`:65-74`): MAUI does not auto-load config files, so the executing assembly's `MMCA.ADC.UI.appsettings.json` manifest resource stream is read (`:66-67`) and added through `AddJsonStream` (`:71`), guarded by a null check on the stream (`:68`).
  - Core UI services: `AddMauiBlazorWebView()` and `AddMudServices()` (`:92-93`), then the pre-shared `IOAuthUISettings` override (`:97`, whose comment at `:95-96` notes it enables the social login buttons that the [ADR-043](https://ivanball.github.io/docs/adr/043-mobile-deep-links-and-native-oauth-callback.html) broker then routes through the system browser), `AddUIShared(builder.Configuration)` (`:99`), and `UseMauiDeviceCapabilities()` (`:104`, which also wires `Plugin.LocalNotification` and the notification-tap deep-link bridge).
  - `UseMmcaMauiErrorHandling()` (`:111`) installs last-chance handlers for `AppDomain.UnhandledException` and `TaskScheduler.UnobservedTaskException` so a managed exception nobody caught is logged instead of silently killing the app. Its comment (`:106-110`) flags that, unlike its neighbours, this call is ordering-insensitive: the handlers are installed when the app is *built*, so a logging provider registered after this line is still picked up. [Rubric §13, Observability & Operability] is the category this call serves.
  - `AddMauiPushDeviceTokenProvider()` (`:120`) registers the credentialed push token providers ([ADR-044](https://ivanball.github.io/docs/adr/044-native-push-delivery.html)): the framework picks FCM on Android and APNs on iOS/MacCatalyst, and its plain `Add` beats the `TryAdd` null default. The comment (`:113-119`) records that the registration is config-gated (with `Push:Fcm` blank and `Push:Apns:Enabled` false, both providers yield null and the pipeline stays inert) and that the platform wiring stays app-side: the `POST_NOTIFICATIONS` declaration in `AndroidManifest.xml` and the two iOS callbacks on [AppDelegate](#appdelegate).
  - `IHomePageContent` landing content (`:122`), then module registration (`:124-135`): four `if` blocks gated on `UIModuleConfiguration.IsModuleEnabled(builder.Configuration, "...")` add the Identity (`:125-126`), Conference (`:128-129`), Engagement (`:131-132`), and Notification (`:134-135`) UI packages.
  - `AddCommonMauiPublicLinkBuilder()` (`:141`) overrides the browser-origin [IPublicLinkBuilder](group-15-common-ui-framework.md#ipubliclinkbuilder) default so shared and copied links carry the public web URL rather than the WebView's internal origin; the framework builder resolves against `PublicSite:BaseUrl` from the embedded appsettings (`:137-140`), the same key that must match [MainActivity](#mainactivity)'s `PublicWebHost`. It stays *after* the module registrations by ordering-contract clause 3.
  - `UseCommonBarcodeScanner(...)` (`:151-153`) enables camera QR scanning for badge check-in, passing **deferred** `cancelText`/`cameraDescription` delegates that call `StartupText`. The comment (`:143-150`) separates the two reasons for its position: it must sit after `AddUIShared` (a real DI requirement, since that is the call which `TryAdd`s the null `IBarcodeScannerService`) but sitting below `AddEngagementUI` is convention rather than a requirement, because Engagement owns the check-in surface but registers no scanner of its own. The same comment explains why the call stays opt-in: it brings in the ZXing handlers, so a head that never scans ships neither the handler nor the camera permission.
  - MAUI-only composition and services: the [DeviceUIModule](#deviceuimodule) singleton (`:157`) and the [AppActionsInitializer](#appactionsinitializer) `IMauiInitializeService` (`:158`); the auth stack of `AddCommonMauiTokenStorage()` (`:161`, the framework helper that binds [MauiTokenStorageService](group-26-device-capability-layer.md#mauitokenstorageservice) to the platform secure store), [DirectApiTokenRefresher](group-15-common-ui-framework.md#directapitokenrefresher) (`:162`), [JwtAuthenticationStateProvider](group-15-common-ui-framework.md#jwtauthenticationstateprovider) (`:163`) and `AddAuthorizationCore()` (`:164`); and `AddMauiFormFactor()` (`:167`).
  - The `#if DEBUG` block (`:169-196`) adds Blazor WebView developer tools (`:170`) and debug logging (`:171`), then appends a `SocketsHttpHandler` primary handler to the `APIClient` whose `RemoteCertificateValidationCallback` always returns true (`:187-194`), so the app can reach the API over LAN using the localhost dev cert. The `S4830`/`CA5359` suppression is scoped to Debug and explained inline (`:173-178`): Android's native SSL layer rejects the dev cert at the Java level, for hostname verification of `localhost` against a LAN IP, before the managed callback would ever fire, so the managed TLS stack is used instead. A second comment (`:180-185`) records that this repeat `AddHttpClient("APIClient")` call only **appends** a primary-handler factory, so the client-configure delegates from `AddUIShared` still run and the 90 second `HttpClient.Timeout` they pin survives; a future edit that replaced rather than added would silently fall back to the BCL's uncoordinated 100 second default.
  - `StartupText(string key, string fallback)` (`:210-211`) reads one string from `StartupResources` under `CultureInfo.CurrentUICulture`, falling back to the supplied literal if the key is ever dropped from the resx. The remarks (`:205-209`) record that since MMCA.Common v1.147.0 the scanner registration takes deferred delegates, so this runs once per scan, after [MauiCultureInitializer](group-26-device-capability-layer.md#mauicultureinitializer) has restored the persisted culture ([ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html)): the scan page follows the in-app language choice, not the device locale at process start.
  - `HandleAppAction(AppAction action)` (`:213-226`) maps the action id to a route via [AppActionRouteMap](#appactionroutemap)`.RouteFor` (`:215`), returns when the id is unknown (`:216-219`), and otherwise publishes the route into [IDeepLinkDispatcher](group-26-device-capability-layer.md#ideeplinkdispatcher) (`:223-225`), which buffers it on cold start and dispatches it live otherwise (`:221-222`).
...
- **Walkthrough**: three lines of authored code, in order.
  ...
  - `protected override MauiApp CreateMauiApp() => MauiProgram.CreateMauiApp();` (`:16`) is the single override and the only application-specific behavior in the file. It returns the fully composed `MauiApp` built at `MMCA.ADC/Source/Hosts/UI/MMCA.ADC.UI/MauiProgram.cs:51`, which is where `UseMauiApp<App>()` (`MauiProgram.cs:55`) names the *cross-platform* [App](#app) as the MAUI application root. So the Windows `App` boots the MAUI runtime, and the MAUI runtime then instantiates the other `App`, which creates the window hosting [MainPage](#mainpage) and the Blazor WebView.
