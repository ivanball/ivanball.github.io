...
>   chapter shifts to slot 27 (`group-28-testing-infrastructure.md`, its file name unchanged from the prior
...
> **Regeneration note (re-verified against current source, 2026-09-05 full drift sweep).** Regenerated
> at MMCA.Common `f994eb1` + MMCA.ADC `977a9776` (both clean; prior pass `5f4d9e2` / `2b77dc07`, five
> Common releases in between: v1.182.0 to v1.186.0). Net change: **+46** distinct nodes (4,211 to
> **4,257**), 68 added and 22 removed by name, **418** line-shifted and **1,278** relocated (same type,
> new namespace and/or path with no diff hunk in its span), 111 body-changed, 43 regrouped;
> `classify.ps1` reports 0 unmapped and the per-group counts sum to 4,257. Individually-sectioned
> types 2,273 to **2,286**, roll-ups 1,932 to **1,971**, `###` sections 2,188 to **2,204**, cycles 36
> to **37**, edges 15,415 by namespace / 589 by unique-name fallback / 42 dropped.
> - **Per-group movement** (25 of 27 groups moved; G01 and G06 are unchanged):
>   - **G02 Domain Building Blocks** (36): `Money` body (`Money.cs:21`, a doc-comment cref only); 10 relocated by the v1.184.0 namespace split.
>   - **G03 Querying** (38), **G05 CQRS pipeline** (57), **G11 Navigation** (12), **G12 API hosting** (81), **G20 Conference API/gRPC** (44), **G21 Conference UI** (146, 126 relocated) and **G27 Device capability layer** (99, 83 relocated): citation-only drift from the namespace split, fixed by `fixcites.ps1` with no author.
>   - **G04 Events + Outbox** (37): `IntegrationEventConsumerExtensions` body (`IntegrationEventConsumerExtensions.cs:12`); `OutboxSettings` regrouped in from G14.
>   - **G07 Persistence & EF Core** (122): `DomainEventSaveChangesInterceptor` body (`DomainEventSaveChangesInterceptor.cs:46`); the 11 persistence/audit-trail/connection-string/data-source/tenancy settings types regrouped in from G14; 13 host-level services (file storage, image processing, native push, `TenantContext`, the upcasting/fault consumers, `PeriodicBackgroundService`) regrouped out to G14.
>   - **G08 Auth** (90): `JwtSettings`, `JwksSettings`, `JwtSigningAlgorithm` regrouped in from G14. **G09 Caching** (9): `CacheSettings` regrouped in from G14.
>   - **G10 Notifications** (57): `NotificationHub` body (`NotificationHub.cs:17`). **G13 gRPC contracts** (6): `DependencyInjection` body (`DependencyInjection.cs:25`).
>   - **G14 Module system** (69): `DeleteUserHandlerBase<TUser, TCommand>` (`DeleteUserHandlerBase.cs:58`), two `DependencyInjection` roots (`:27`, `:53`) and `UserUseCaseLog` (`UserUseCaseLog.cs:11`) body-changed; 16 settings types out to G04/G07/G08/G09, 13 host-level services in from G07 (net 72 to 69).
>   - **G15 Common UI** (117): `AuthenticatedServiceBase` (`AuthenticatedServiceBase.cs:15`; namespace now `MMCA.Common.UI.Services.Api`, tokens via `...Services.Auth.Tokens`) and `ResultUiExtensions` (`ResultUiExtensions.cs:63`) body-changed.
>   - **G16 Aspire** (60): added `HealthEndpointPaths` (`HealthEndpointPaths.cs:8`), `ProbeTelemetryFilter` (`ProbeTelemetryFilter.cs:27`) and `ProbeTelemetryFilterProcessor` (`ProbeTelemetryFilterProcessor.cs:20`), the probe-telemetry suppression; `DownstreamServiceHealthCheck` (`:39`) and `Extensions` (`Extensions.cs:29`) body-changed.
>   - **G17 Conference domain** (99): `Event` (`Event.cs:24`), `SessionAiScore` (`SessionAiScore.cs:13`) and `SessionAiScoreDTO` (`SessionAiScoreDTO.cs:6`) body-changed (model + prompt versioning persisted per score, ADR-111).
>   - **G18 Conference application** (303): `IAiScoringService` (`IAiScoringService.cs:6`, now carries `ModelId` + `PromptVersion`), `ScoreEventSessionsHandler` (`:18`), `GetSessionSelectionDashboardHandler` (`:16`) and `EventDTOMapper` (`EventDTOMapper.cs:15`) body-changed.
>   - **G19 Conference infrastructure** (37): added `AnthropicOutputConfig` (`AnthropicScoringService.cs:452`), `AnthropicJsonSchemaFormat` (`:458`), `AnthropicUsage` (`:488`) and `ScoringInstruments` (`:326`) (schema-constrained output + token metering); `AnthropicScoringService` (`:19`) and its request/response records, `EventConfiguration` (`:12`) and `SessionAiScoreConfiguration` (`:11`) body-changed.
>   - **G22 Engagement module** (193) / **G26 Engagement live layer** (85): 14 types regrouped from the live layer into the module chapter (the `SessionLive`/`LivePoll`/`SessionQuestion`/`LiveEvent` UI services, the `SessionLookupService` family and the three live-poll EF configurations).
>   - **G23 Identity module** (90): `DeleteUserHandler` body (`DeleteUserHandler.cs:28`). **G24 ADC host** (17): `AppDelegate` (`AppDelegate.cs:20`) and `MauiProgram` (`MauiProgram.cs:37`) body-changed.
>   - **G25 Testing** (2,315): +61 / -22 by name (nested fakes and specs re-homed per test class, e.g. `BetaSpecification`, `FakeEntity`, `DriftedTests`; the new 11-type `MMCA.ADC.Conference.Scoring.Evaluation.Tests` project with `GoldenReplayTests`, `PromptContractTests`, `LiveJudgeTests`; per-repo `FolderWidthTests` subclasses, ADR-109; the probe-telemetry tests), 80 body-changed; the per-project rollup counts moved for five projects (Common Infrastructure 380 to 403, Aspire 41 to 44, Shared 44 to 45, ADC Conference Infrastructure 14 to 15, Scoring.Evaluation 0 to 11).
> - **Handled mechanically (no re-authoring):** 6,742 citation remaps over 193 part files (209 flagged for an author: 119 inside a modified range, 80 ambiguous basenames, 10 in deleted files); 624 section blockquotes rewritten for the relocated types that carry their own section (the other relocated types are rolled-up test classes with no blockquote); 867 section repacks (57 flagged); after authoring, 170 cross-links to the 43 regrouped types retargeted by script, one stray leftover part (live-layer p08, three sections already moved) removed, and a duplicate rollup narrative that the unit split had left in testing p08 removed.
> - **Authoring pass:** 66 units re-authored (9 overview, 1 rollup, 56 sections) across 19 groups plus 4 devops chapters, all against real source with `path:line` citations; 1,154 tool calls in total against 15,266 on the 2026-09-02 sweep. Adversarial spot-checks: G04-p00, G07-p00, G08-p00, G09-p00, G14-p00, G16-p00 all CONFIRMED.
> - **Outside the type pipeline:** 4 of 5 `devops-*` chapters refreshed, each because a file it cites has newer commits than the chapter: `devops-cicd`, `devops-testing` and `devops-aspire` for MMCA.ADC `deploy.yml` 4598c679 (2026-09-05: AI scoring evaluation gate TD-22, `scoring` path filter, cross-browser freshness gate), `devops-iac` for MMCA.ADC `infra/main.bicep` 4598c679 (AI-scoring token-ceiling alert) and MMCA.Common `samples/deployment/main.bicep` dd0b6a9 (SQL connection string as a Key Vault `sql-conn` secretRef). The `devops-testing` per-project test-type table was reconciled to this scan (Common 1,420, ADC 770, combined 2,190).
>   - Coverage gap logged as an exception: `MMCA.Common/Tests/Architecture/MMCA.Common.Architecture.Tests/Fixtures/observability-main.bicep` is an architecture-test fixture (input to the observability-convention fitness base), not deployed infrastructure, so no devops chapter covers it.
...
> **Regeneration note (re-verified against current source, 2026-09-15 full drift sweep).** Regenerated
> at MMCA.Common `3a98862` + MMCA.ADC `be87e1fc` (both clean; prior pass `f994eb1` / `977a9776`,
> sixteen Common releases in between: v1.187.0 to v1.202.0). Net change: **+555** distinct nodes
> (4,263 to **4,818**), 595 added and 40 removed by name, **100** relocated (same type, new
> `file:line`), 367 body-only, 0 regrouped; `classify.ps1` reports 0 unmapped and the per-group counts
> sum to 4,818. Individually-sectioned types 2,286 to **2,568**, roll-ups 1,971 to **2,250**, `###`
> sections 2,204 to **2,482**, cycles 37 to **44**, edges 17,598 by namespace / 724 by unique-name
> fallback / 101 dropped.
> - **Per-group movement** (24 of 28 groups moved; G01, G06, G11 and G13 are unchanged):
>   - **G02 Domain Building Blocks** (36): `AuditableAggregateRootEntity<TIdentifierType>` body (`AuditableAggregateRootEntity.cs:13`, a doc-comment change; the author verified no section edit was needed).
>   - **G03 Querying** (42): added `QueryTagScope` and its nested `AmbientScope` (`QueryTagScope.cs:20`, `:61`) plus `QueryFieldContract` (`QueryFieldContract.cs:34`), the v1.192.0 query tags, and `StronglyTypedIdFilterStrategy<TSelf, TValue>` (`StronglyTypedIdFilterStrategy.cs:24`, ADR-115); `EntityQueryService` (`:32`), `EntityQueryPipeline`, `EntityQueryParameters`, `QueryFieldService` and `QueryFilterService` body-changed.
>   - **G04 Events + Outbox** (38): added `OutboxOrigin` (`OutboxOrigin.cs:29`, the v1.195.0 outbox-origin column); `BrokerEventBus` (`:31`), `BrokerMessageBus` (`:43`), `InProcessEventBus`, `IntegrationEventConsumer<TEvent>` (`:38`), `OutboxMessage` and `OutboxProcessor` (`:56`) body-changed.
>   - **G05 CQRS pipeline** (61): added `AuthorizationGate` (`AuthorizationGate.cs:19`, the v1.201.0 permission-claim gate), `IRequiresMfa` (`IRequiresMfa.cs:28`, ADR-116), `ISharedQueryCache` (`ISharedQueryCache.cs:21`) and `UserCacheKey` (`UserCacheKey.cs:26`); the authorization, caching and logging decorators, `IQueryCacheable` and `MutateEntityHandlerCore` (`MutateEntityHandlerBase.cs:52`) body-changed.
>   - **G07 Persistence & EF Core** (161): +40 / -1 by name. The internal-command job queue (`InternalCommandProcessor.cs:47`, `InternalCommandScheduler.cs:39`, `InternalCommandDispatcher.cs:21`, cleanup, metrics, administration and settings, ADR-114); the PostgreSQL engine (`PostgreSQLDbContext.cs:23`, `EntityTypeConfigurationPostgreSQL.cs:22`, ADR-113); the strongly typed id converter, comparer and model configuration (`StronglyTypedIdValueConverter.cs:28`, `:56`, `:80`, `StronglyTypedIdModelConfiguration.cs:21`, ADR-115); `RestrictDeleteByDefaultConvention` (`RestrictDeleteByDefaultConvention.cs:41`, ADR-119); the stored permission grants (`EFPermissionGrantStore.cs:34`, `PermissionGrantCache.cs:35`, `PermissionGrantRefreshService.cs:31`, `PermissionGrantModelGate.cs:19`, ADR-116); `EFRawSqlQueryExecutor` (`EFRawSqlQueryExecutor.cs:22`); `SensitiveDataLoggingGate` (`SensitiveDataLoggingGate.cs:24`, ADR-122); the upload guards `DocumentContentSniffer`, `FileUploadOptions` and `BlobNames` (`DocumentContentSniffer.cs:49`, v1.199.0); `EfCoreConcurrencyConflictDetector`, `UtcDateTimeConverter`, `QueryTags`. `ValReturn<T>` removed. 28 body-changed, among them `ApplicationDbContext` (`:46`), `DbContextFactory` (`:46`), `EFRepository` (`:23`), `KeysetQueryBuilder` (`:22`) and `DesignTimeDbContextOptions` (`:11`).
>   - **G08 Auth** (152): +63 / -1 by name. Two-factor contracts (`ITwoFactorService.cs:16`, `ITwoFactorStore.cs:24`, `ITwoFactorAuthenticator.cs:24`, `RecoveryCodeSet`), email confirmation (`EmailConfirmationTokenService.cs:35`, its requests, validators, errors and settings), stored permission grants and role administration (`PermissionGrant.cs:24`, `IPermissionGrantStore.cs:16`, `IPermissionCatalog.cs:24`, `IRoleAdministrationService.cs:29`, `StoredPermissionRoleAdministrationService.cs:45`, `LayeredPermissionRegistry.cs:30`, `AdministrationPermissions.cs:15`), user administration contracts (`IUserAdministrationService.cs:24`, `IUserAdminDTO.cs:15`, `SetUserRolesRequest`), the feature-flag lifecycle (`FeatureFlagRegistry.cs:35`, `FeatureFlagAttribute.cs:32`, `FeatureFlagLifetime.cs:9`), fallback authorization (`FallbackAuthorizationHandler.cs:15`), `IStronglyTypedId<TSelf, TValue>` and `StronglyTypedId` (`IStronglyTypedId.cs:60`, `StronglyTypedId.cs:19`), `RefreshSessionRevocation`, `MessageHeaders`, `AuthErrorCodes`. `RoleNames` removed from Common (the role vocabulary is app-owned since v1.201.0; it now lives in ADC Identity, G23). 18 body-changed, among them `TokenService` (`TokenService.cs:26`, one `permission` claim per effective permission), `AuthenticationServiceBase<TUser>` (`:74`), `OwnerOrAdminFilterOptions` (`:13`, `BypassRole` required), `PermissionRegistry` and `UnconfiguredPermissionRegistry`.
>   - **G09 Caching** (9): `CacheKeyNamespace` and `CacheKeyPrefixOptions` body (`CacheKeyPrefix.cs:50`, `:31`).
>   - **G10 Notifications** (59): added `LiveChannelJoinAuthorizer` (`LiveChannelJoinAuthorizer.cs:43`) and `NotificationPermissionGrants` (`NotificationPermissionGrants.cs:26`); `NotificationHub` (`:25`), `SmtpEmailSender` (`:15`, transport security per ADR-122), `NotificationFeatures` and `DependencyInjection` body-changed.
>   - **G12 API hosting** (85): added `UsersAdminControllerBase<TUserDto>` (`UsersAdminControllerBase.cs:49`) and `RolesAdminControllerBase` (`RolesAdminControllerBase.cs:61`) (ADR-116) plus `StronglyTypedIdSchemaTransformer` and `StronglyTypedIdParameterTransformer` (`StronglyTypedIdSchemaTransformer.cs:24`, `:74`); 8 body-changed, among them `MiddlewarePipelineBuilder` (`:16`), `OAuthControllerBase` (`:35`), `RateLimitingSettings` (`:21`) and `WebApplicationBuilderExtensions` (`:33`).
>   - **G14 Module system** (86): added the internal-command contracts (`IInternalCommand.cs:38`, `IInternalCommandScheduler.cs:16`, `IInternalCommandAdministration.cs:17`, `InternalCommandDeadLetter`, `InternalCommandNameAttribute.cs:25`), `AmbientOrigin` and `ConsumerOriginRestore` (`AmbientOrigin.cs:19`, `ConsumerOriginRestore.cs:24`), `ApplicationNamespace` (`ApplicationNamespace.cs:28`), the two-factor and email-confirmation handler bases (`BeginTwoFactorEnrollmentHandlerBase.cs:35`, `ConfirmTwoFactorEnrollmentHandlerBase.cs:29`, `DisableTwoFactorHandlerBase.cs:29`, `RegenerateRecoveryCodesHandlerBase.cs:30`, `ConfirmEmailHandlerBase.cs:33`, `SendEmailConfirmationHandlerBase.cs:40`), `ImpersonatingCurrentUserService` and `ScopedUserOverride` (`ImpersonatingCurrentUserService.cs:20`, `ScopedUserOverride.cs:24`), `SmtpTransportSecurity` (`SmtpTransportSecurity.cs:26`, ADR-122); 16 body-changed, among them the user use-case handler bases (`DeleteUserHandlerBase.cs:58`, `ExportUserDataHandlerBase.cs:49`, `ChangePasswordHandlerBase.cs:36`) and both `DependencyInjection` roots (`:27`, `:62`).
>   - **G15 Common UI** (133): added the role and user administration UI (`RoleAdminList.razor.cs:27`, `RoleAdminEdit.razor.cs:35`, `UserAdminList.razor.cs:47`, their `RoleAdminService` / `UserAdminService<TUserDto>` clients, service contracts and resources), `TrustedCallerHandler` (`TrustedCallerHandler.cs:35`, v1.191.0), `OAuthFlowStateStore` (`OAuthFlowStateStore.cs:20`) and `BiometricGate` (`BiometricGate.razor.cs:17`); 12 body-changed, among them `NavItem` (`NavItem.cs:20`, `RequiredPermission`), `NotificationUIModule` (`:15`, gated on the manage permission), `AuthUIService` (`:44`), `MudToastService` (`:19`, live-region toasts, v1.194.0), `MMCATheme` and `LayoutSettings`.
>   - **G16 Aspire** (65): added `CachedHealthReportProvider` and `HealthReportCacheOptions` (`CachedHealthReportProvider.cs:25`, `HealthReportCacheOptions.cs:28`) and ADC's `AdcAppHostFixture` / `AdcAppHostCollection` / `AdcAppHostSmokeTests` (`AdcAppHostFixture.cs:28`, `AdcAppHostSmokeTests.cs:32`, on the new `MMCA.Common.Testing.Aspire` base, ADR-117); `AppHostCompositionSmokeTests` removed; `SecurityHeadersMiddleware` (`SecurityHeaders.cs:145`), `GatewayRateLimitingExtensions` (`:43`), `OutboxPollFilterProcessor` (`:17`) and both `Extensions` roots body-changed.
>   - **G17 Conference domain** (107): added the `SessionAsset` aggregate with its kind, invariants, limits, event and DTO (`SessionAsset.cs:23`, `SessionAssetChanged.cs:13`, `SessionAssetInvariants.cs:13`, ADR-123), `ConferencePermissionGrants` (`ConferencePermissionGrants.cs:27`) and `SessionizeCodeFormat` (`SessionizeCodeFormat.cs:27`); `Session` (`:22`), `SessionDTO`, `ConferencePermissions`, `ConferenceFeatures` and the cascade-deletion domain service body-changed.
>   - **G18 Conference application** (335): +38 / -6 by name. The session-asset use cases (`UploadSessionAssetHandler.cs:35`, `AddSessionAssetLinkHandler.cs:20`, `UpdateSessionAssetHandler.cs:20`, `DeleteSessionAssetHandler.cs:21`, `GetSessionAssetsHandler.cs:27`, the validation rule sets, `SessionAssetAccessService.cs:22`), `ScoreEventSessionsInternalCommand` and its handler (`ScoreEventSessionsInternalCommand.cs:26`, `ScoreEventSessionsInternalCommandHandler.cs:39`: scoring moved from the in-process queue onto an internal command, ADR-114), `DeleteSessionAssetBlobInternalCommand` (`:29`), `SessionScoringRunner` (`SessionScoringRunner.cs:17`), `ISessionScoresCacheEvictor`, and per-aggregate `EntityQueryService` subclasses; removed `ScoreEventSessionsCommand` / `ScoreEventSessionsHandler`, `ISessionScoringQueue` / `SessionScoringQueue`, `SessionScoringWorkItem` and `SessionScoringEnqueueResult`. 11 body-changed, among them `DeleteEventHandler` (`:22`), `DeleteSessionHandler` (`:25`), `RefreshFromSessionizeHandler` (`:20`) and `UserRegisteredHandler` (`:54`).
>   - **G19 Conference infrastructure** (27): +2 / -12 by name. The hand-rolled Anthropic request/response records (`AnthropicRequest`, `AnthropicResponse`, `AnthropicMessage`, `AnthropicContentBlock`, `AnthropicOutputConfig`, `AnthropicJsonSchemaFormat`, `AnthropicUsage`), `ScoringInstruments`, `SessionScoringProcessor`, `SessionScoringSweepJob`, `SessionScoringCandidate` and `SessionScoreStamp` are gone: the model call now goes through `MMCA.Common.AI` (ADR-120) and the sweep through an internal command (ADR-114). Added `SessionAssetConfiguration` (`SessionAssetConfiguration.cs:18`) and `OutputCacheSessionScoresCacheEvictor` (`:17`); `AnthropicScoringService` (`:30`), `ModuleApplicationDbContext` (`:21`), `SessionConfiguration` and `SessionizeService` body-changed.
>   - **G20 Conference API** (45): added `SessionAssetsController` (`SessionAssetsController.cs:51`); `SessionSelectionController` (`:31`), `EventQuestionAnswersController` (`:57`), `SessionQuestionAnswersController` (`:78`) and `DependencyInjection` body-changed.
>   - **G21 Conference UI** (153): added `SessionAssetsPanel` (`SessionAssetsPanel.razor.cs:31`), `SessionAssetsDownloadList` (`:18`), `SessionAssetService` (`SessionAssetService.cs:26`), `ISessionAssetUIService`, `SessionAssetDisplay` and the two asset request records; `SpeakerDetail` (`:24`), `PublicSpeakerDetail` (`:22`) and `DependencyInjection` body-changed.
>   - **G22 Engagement module** (194): added `EngagementPermissionGrants` (`EngagementPermissionGrants.cs:24`); `EngagementFeatures` and `DependencyInjection` body-changed. **G26 Engagement live layer** (85): 9 body-changed (`LivePollsController.cs:45`, `SessionQuestionsController.cs:37`, `SubmitQuestionHandler.cs:31`, `CreateLivePollHandler.cs:20`, `GetPollResultsHandler.cs:23`, `LivePollAuthorization.cs:12`, `SessionLive.razor.cs:27`, ...), the permission-marker adoption.
>   - **G23 Identity module** (113): added `RoleNames` (`RoleNames.cs:20`, moved here from Common Auth), `AdminRolesController` (`AdminRolesController.cs:36`), `UsersAdminController` (`:25`), `UserAdministrationService` (`:38`), `UserAdminDTO`, the `RoleList` / `RoleEdit` / `UserDetail` pages (`RoleList.razor.cs:27`, `RoleEdit.razor.cs:16`, `UserDetail.razor.cs:22`), email confirmation end to end (`ConfirmEmail.razor.cs:23`, `EmailConfirmationController.cs:34`, `EmailConfirmationService.cs:21`, `SendEmailConfirmationHandler.cs:27`, `ConfirmEmailHandler.cs:21`), `DeleteAvatarBlobInternalCommand` and its handler (`:17`, `:21`), `IdentityPermissionGrants`, `TokenPermissionGrants` (`TokenPermissionGrants.cs:34`) and `AuthenticationServiceSettings`; 21 body-changed, among them `User` (`User.cs:34`), `AuthController` (`:33`), `AuthenticationService` (`:46`), `DeleteUserHandler` (`:29`), `SetUserAvatarHandler` (`:26`), `IdentityUIModule` and `UserList`.
>   - **G24 ADC host** (22): added `BoundedCircuitHandler` with `BlazorCircuitLimitSettings` / `BlazorCircuitLimitExtensions` (`BoundedCircuitHandler.cs:37`, `BlazorCircuitLimitSettings.cs:17`) and `UiRateLimitingSettings` / `UiRateLimitingExtensions` (`UiRateLimitingSettings.cs:33`), the v1.188.0 security hardening; `App` (`App.xaml.cs:9`) and `MainActivity` (`:35`) body-changed.
>   - **G27 Device capability layer** (103): added `IAppLifecycleNotifier`, `AppLifecycleNotifier`, `AppResumedEventArgs` and `WindowLifecycleExtensions` (`IAppLifecycleNotifier.cs:13`, `AppLifecycleNotifier.cs:9`, `WindowLifecycleExtensions.cs:22`); the three local-cache stores, `ILocalCacheStore`, `DeepLinkDispatcher`, `MauiExternalAuthBroker` (`:20`) and `DependencyInjection` body-changed.
>   - **G28 Common AI Integration** (8): a new group, all eight types added: `AiSettings` and `AiProvider` (`AiSettings.cs:31`, `:11`), `AiServiceCollectionExtensions` (`DependencyInjection.cs:49`), `BoundedChatClient` (`BoundedChatClient.cs:51`), `UsageRecordingChatClient` (`UsageRecordingChatClient.cs:21`), `AiUsageMeter` (`AiUsageMeter.cs:20`), `IAiTokenEstimator` (`IAiTokenEstimator.cs:14`) and `PromptContract` (`PromptContract.cs:23`), the v1.192.0 `MMCA.Common.AI` package (ADR-120).
>   - **G25 Testing** (2,643): +341 / -19 by name (the new `MMCA.Common.Testing.Aspire` package and its own tests, `MMCA.Common.AI.Tests`, `MMCA.Common.Infrastructure.PostgreSQL.Tests`, and the internal-command, permission-grant, strongly-typed-id, two-factor and email-confirmation test classes: Common Infrastructure +90, Application +54, Architecture +34, API +15, Shared +13, ADC Conference Application +21; removed the ADC scoring queue / processor / sweep tests and their fakes), 181 body-changed, 56 line-shifted; the per-project rollup counts moved for 26 projects (reconciled in `devops-testing`, see below).
> - **Handled mechanically (no re-authoring):** 3,387 citation remaps over 176 part files; 0 relocations rewritten (this delta had none); 854 section repacks with 41 flagged (13 testing-family splits across the renumbered `group-28` parts, 27 orphan headings for the removed Anthropic records, scoring queue types and `ValReturn<T>` plus the testing rollup headings, and the `RoleNames` move into `group-24-identity-module-p02`); 2 emptied part files deleted; links fixed in 96 files by the renumber. Citations flagged for an author: 379 at the gate, 23 then resolved by `fixcites.ps1 -Residual`; a dry-run re-check after the authoring pass still lists **132** across 57 parts (69 old-line-text-not-found, 21 range-end-not-found, 18 ambiguous basenames, the rest baseline-absent, deleted-file or already-correct false positives), left for a per-part follow-up pass like the 2026-09-05 one.
> - **Authoring pass:** 189 units re-authored (22 overview, 1 rollup, 166 sections) across 24 groups, all against real source with `path:line` citations, at concurrency 5 on the sections model. Adversarial spot-checks: G03-p00, G04-p00, G05-p00, G07-p00, G08-p00, G09-p00 all CONFIRMED.
> - **Outside the type pipeline:** 5 of 5 `devops-*` chapters refreshed, each because a file it cites has newer commits than the chapter: `devops-aspire`, `devops-cicd` and `devops-testing` for MMCA.ADC `deploy.yml` 64d6ffee / 29f7997d (2026-09-07 to 2026-09-10: the DataProtection KEK URI, the drift-plan fixes) and `cross-service-tests.yml` c64b9fe2 / d841f798 (the AppHost smoke tier and its ephemeral RS256 keypair); `devops-cicd` also for MMCA.Common `ci.yml` 14435b9 / 960221b (the AppHost integration test base and the PostgreSQL engine) and the 2026-09-07 security-hardening commits on every `claude*.yml`, `cost-guard.yml`, `dr-drill.yml`, `e2e.yml`, `load-test.yml`, `maui-audit.yml` and `release.yml`; `devops-iac` for MMCA.ADC `infra/main.bicep` (eight commits up to d5297706: the session-asset container, the v1.195.0 circuit-open alert) and the Identity service `appsettings.json` 5c87387e (stored permission grants); `devops-runbooks` for `dr-drill.yml` 88a129fe. The `devops-testing` per-project test-type table was reconciled to this scan (Common 1,647, ADC 824, combined 2,471; 26 projects had moved).
>   - Coverage gap: unchanged; `MMCA.Common/Tests/Architecture/MMCA.Common.Architecture.Tests/Fixtures/observability-main.bicep` remains the logged exception (an architecture-test fixture, not deployed infrastructure).
> - **CONCEPT-MAPS.md:** 9 mechanical mismatches corrected: the group count 27 to 28 (the preamble twice, section 3 three times), the section 3 node label `G27` re-pointed from Testing to Common AI Integration (Testing is now `G28`), the package count 17 to 19 in section 1 (three claims, per `MMCA.Common/FACTS.md`), and ADR-052 marked superseded by ADR-121 in section 13.
> - **Verification:** `verify.ps1`: **0 missing**, rubric **34/34**.
> - **Governance events (approved by the user on 2026-09-14):** new group **G28 "Common AI Integration"** (`group-27-common-ai-integration.md`, 8 members, all from `MMCA.Common.AI`); no existing group fit because the package is self-contained (a `FrameworkReference` only, no first-party group dependency) and its concern, a governed language-model boundary, belongs to none of the persistence, pipeline or UI groups. It was inserted above the testing group in `classify.ps1`, so the testing chapter moved from slot 27 to slot 28 (`group-28-testing-infrastructure.md`: 26 part files renamed, the stale `group-27-testing-infrastructure.md` deleted, links fixed in 96 files). Second event: the G25 rule was extended so the assemblies `MMCA.Common.Testing.Aspire` and `MMCA.Common.Testing.Aspire.AppHostTests` route to Testing & Quality Infrastructure (a classifier gap, not a new group). The primer's ADR table gained rows 112 to 123 and struck 052 (superseded by 121).

> **Regeneration note (re-verified against current source, 2026-09-19 full drift sweep).** Regenerated
> at MMCA.Common `90ffa7a` + MMCA.ADC `3f203a03` (both clean; prior pass `3a98862` /
> `be87e1fc`, Common releases v1.203.0, v1.204.0 and v1.205.0 in between). Net change: **+79** distinct nodes (4,826 to **4,905**),
> 87 added and 8 removed by name, **19** relocated (same type, new `file:line`),
> 78 body-only, 0 regrouped; `classify.ps1` reports 0 unmapped and the per-group counts
> sum to 4,905. Individually-sectioned types 2,568 to **2,624**, roll-ups 2,250 to **2,281**,
> `###` sections 2,482 to **2,554**, cycles 44 to **44**,
> edges 17,963 by namespace / 736 by unique-name fallback / 101 dropped.
> The 8 "removed" names are not deletions: four are generic-arity renames whose new arity was already sectioned
> (`UpdateEntityCommand`/`UpdateEntityHandler` lost `TApplier`, `MutateEntityHandlerBase` lost `TEntityDTO`,
> `StronglyTypedIdFilterStrategy` gained `<TSelf, TValue>`) and four are baseline duplicates the prior taxonomy
> rolled into the testing group for lack of a membership row (`Result<T>`, `ResultConverter<T>`, `IEventUpcaster`,
> `AppHostFixtureBase<TAppHost>`); no section was deleted.
> - **Per-group movement** (13 of 28 groups moved; the rest are unchanged):
>   - **G02 Domain Building Blocks** (members 36): body 1, lineShift 3. The `IBaseEntity<TIdentifierType>` hunk is an XML-doc tag change only (`<c>init</c>` to `<see langword>`), so the section was verified and left byte-identical; the three `Enumeration` cites moved with the fitness-rule remark correction (Common #421).
>   - **G04 Domain & Integration Events + Outbox Dual-Dispatch** (members 38): body 1. `OutputCacheEvictionRequested` span touched by the scorecard-lever sweep (Common #412); walkthrough re-verified against current source.
>   - **G12 API Hosting, Middleware, Idempotency & DTO/Contract Mapping** (members 85): body 1. `EntityControllerBase` hunks at `:576` and `:610` are XML-doc tag changes; section verified, no textual change.
>   - **G14 Module System, Composition & Configuration** (members 86): body 2, lineShift 1. `DependencyInjection` and `MessageBusSettings` gained the broker-messaging registration path (Common #412; covered by the new `DependencyInjectionBrokerMessagingTests`).
>   - **G15 Common UI Framework** (members 138): added 5, body 8, lineShift 2. Added `BlazorCspSettings` + `BlazorCspSettingsValidator` (configurable frame-src allow-list for the Blazor CSP, Common #417), `InvariantMudLocalizationInterceptor`, `StringLocalizerPluralExtensions` and `BuiltInStrings` (the MAUI culture-pin fix, ADR-027, Common #411); body changes in `BlazorCspPolicyProvider`, `NotificationSend`, `RoleAdminEdit`, `MoneyExtensions`, `RegisterModel`/`ResetPasswordModel` and both `DependencyInjection` roots.
>   - **G16 Aspire Orchestration & Service Defaults** (members 65): body 1. `Extensions` (ServiceDefaults) touched by Common #412; re-verified.
>   - **G17 ADC Conference - Domain Model & Module Contracts** (members 112): added 5, body 3. The new **Partner aggregate** (`MMCA.ADC.Conference.Domain/Partners/Partner.cs:19`, ADC #207): `Partner`, `PartnerInvariants`, `PartnerType`, `PartnerDTO`, `PartnerChanged`; `ConferencePermissions` gained the Partners permissions and `EventCascadeDeletionDomainService` now cascades partners.
>   - **G18 ADC Conference - Application & Use Cases** (members 353): added 18, body 2. The Partner use-case slice end to end (create/update handlers, request mappers and validators, the per-field `Partner*Rules<T>` family, `PartnerDTOMapper`, `PartnerNavigationPopulator`, `GetPublicPartnerFilterQuery`/`Handler`); `DeleteEventHandler` and the module `DependencyInjection` registered them (ADC #207).
>   - **G19 ADC Conference - Infrastructure & Persistence** (members 28): added 1, body 2. `PartnerConfiguration` (EF config) plus the `ModuleApplicationDbContext` DbSet and `ConferenceModuleDbSeeder` rows for it (ADC #207).
>   - **G20 ADC Conference - API, gRPC Contracts & Service Host** (members 46): added 1. `PartnersController` (`MMCA.ADC.Conference.API/Controllers/Partners/PartnersController.cs:41`) with its 5-minute public output-cache policy (ADC #207).
>   - **G21 ADC Conference - UI** (members 168): added 15, body 16, lineShift 6. The Partner pages and models (`PartnerList`, `PartnerDetail`, `PartnerCreate`, `PartnerFormModel`/`CreateModel`/`EditModel`, `IPartnerUIService`/`PartnerService`), the home-page content alignment (`ADCHome`, `ADCHomeContent`, `ADCPartnerInfo`, `ADCPartnerCollectionResult`, ADC #207), `VenueMapLinks`, `SessionAssetComposer`, `SessionBookmarkButton` and the static category chips / Happening Now links fix (ADC #208); the `*Detail` pages migrated to `DetailPageBase`.
>   - **G28 Common AI Integration** (members 12): added 4, body 4. The guardrail layer (`IChatGuardrail`, `GuardrailVerdict`, `GuardrailChatClient`, `ChatGuardrailException`, Common #412) inserted into the delegating-client pipeline built by `AiServiceCollectionExtensions`; `AiUsageMeter`, `UsageRecordingChatClient` and `PromptContract` re-verified.
>   - **G25 Testing & Quality Infrastructure** (members 2681): added 38, removed 8, body 37, lineShift 7. The Partner test suites across Domain/Application/API/UI/E2E (`PartnerTests`, `PartnerInvariantsTests`, `CreatePartnerHandlerTests`, `PartnersControllerTests`, `PartnerListPage`/`DetailPage`/`CreatePage`, `OrganizerPartnerManagementTests`), the guardrail tests, `SqlClientEntraAuthenticationTests` (Common #415), `SliceCohesionFitnessTests` + the `SliceFixtures` handler bases, `SampleDeploymentObservabilityTests`; the 8 removals are the arity renames and duplicates above.
> - **Handled mechanically (no re-authoring):** citation remaps 645 over 103 part file(s), 78 flagged for a human (41 inside a modified range, 37 ambiguous `DependencyInjection.cs` basenames); 0 relocations; repack moves 502, 18 flagged (12 testing family headings whose members now span parts, kept as family tables in their first part; 6 orphan module-name headings in the testing chapter, removed).
> - **Pipeline fix this pass:** `repack.ps1` recorded the type name of each moved section as its first letter, so the delta could not see where a section had moved and flagged every repack-touched unit as unsectioned (89 units instead of 64). Fixed at harness commit `25c718d` before authoring.
> - **Authoring pass:** 62 units re-authored (8 overview, 1 rollup, 53 sections) across 15 groups, all against real source
>   with `path:line` citations; sections units on sonnet from per-unit briefs, overview/rollup/devops/concept-maps on opus. Struck at the gate as false positives: two Engagement/Identity parts whose only reason was the multi-layer `AssemblyReference`/`ClassReference` headings the coverage check cannot count. Six adversarial spot-checks (the five Conference overviews and the Common UI overview): all **CONFIRMED**.
> - **Outside the type pipeline:** 3 of 5 `devops-*` chapters refreshed, each because a file it cites has newer commits than the chapter;
>   `devops-cicd` was struck because its only newer commit is the `claude-code-action` version bump (Common #407) and the chapter names the action without a version.
>   - `devops-aspire`: `MMCA.ADC/Source/Hosts/MMCA.ADC.Gateway/appsettings.json` (0a77a9fe, 2026-09-17, Partner aggregate, ADC #207).
>   - `devops-iac`: `MMCA.ADC/Source/Services/MMCA.ADC.Identity.Service/appsettings.json` (e3f01c5b, 2026-09-15, NuGet audit sweep, ADC #205) and `MMCA.Common/samples/deployment/main.bicep` (630afc9, 2026-09-15, Common #412).
>   - `devops-testing`: nine per-project test counts reconciled to the inventory (Architecture.Tests 196 to 203, Conference.UI.Tests 56 to 63, E2E.Tests 88 to 92, Conference.Application.Tests 184 to 191, Conference.Domain.Tests 31 to 34, UI.Tests 136 to 140, Infrastructure.Tests 493 to 494, Aspire.Tests 49 to 50, Conference.API.Tests 21 to 22).
>   - Coverage gap: unchanged; `MMCA.Common/Tests/Architecture/MMCA.Common.Architecture.Tests/Fixtures/observability-main.bicep` remains the logged exception (a test fixture, not an operational file).
> - **CONCEPT-MAPS.md:** 294 mechanical checks, 0 mismatches; one relationship-level edit, the section 12 module diagram now lists Partner among the Conference aggregates (`Partner.cs:19`).
> - **Verification:** `verify.ps1`: **0 missing**, rubric **34/34**.
...
- **Cycles kept whole.** **18 of the 19** dependency cycles (SCCs) sit inside a single group, never
  ...
  [group-28](group-28-testing-infrastructure.md): `MidSaveContextCreatingDbContext` /
  ...
  test cycles in [group-28](group-28-testing-infrastructure.md)). Full list:
  [manifest](00-dependency-manifest.md#cycles).
