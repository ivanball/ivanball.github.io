...
That transport is the SignalR **hub-channel** push introduced by **ADR-039**, and it is deliberately
the *opposite* of the durable notification pipeline (ADR-024) that the same hub also carries. A
...
and misses an event, its next fetch still shows the truth. That single design rule (ADR-039's
...
[`BaseDomainEvent`](group-04-events-outbox.md#basedomainevent)s captured by the outbox (ADR-003) for
...
poll's `Options` on query-service paths EF cannot `.Include()` (ADR-002), and the EF configurations
([`LivePollConfiguration`](#livepollconfiguration) and siblings) pin the entities to SQL Server in
the `ADC_Engagement` database under database-per-service (ADR-006). Entity-to-DTO mapping is a
compile-time Mapperly mapper, [`LivePollDTOMapper`](#livepolldtomapper) (ADR-001).
...
`[HasPermission(EngagementPermissions.LiveManage)]` (ADR-020). Crucially, the caller's identity (user
...
- **Concept**: this is the read-side twin of the caller-rights shape introduced by
  ...
  the notable point, per ADR-014 the CQRS pipeline runs no validating/authorization decorator on
...
- **Concept introduced, structural validation in the pipeline.** `[Rubric §24, Forms, Validation & UX
  Safety]` (assesses validation before mutation, with actionable, coded errors). Per ADR-014 the CQRS
...
- **The best-effort guarantee (BR-229).** The publish is wrapped in a `try`/`catch (Exception)` that
  ...
  the Notification hub, ADR-039), *not* the source of truth. If Notification is momentarily
...
- **Why it's built this way**: separating the durable state change (committed transactionally, with a
  ...
  correctness independent of the real-time layer's availability (ADR-039, ADR-024's two-channel model).
...
- **Walkthrough**: load tracked and NotFound-guard (`:32-42`); authorize via
  ...
  (`:107-112`).
- **Why it's built this way**: snapshotting the window end at Open is the ADR-039 live-layer design, it
...
- **Why it's built this way**: `ignoreQueryFilters` is load-bearing, without it the soft-deleted row is
  ...
  `MyVoteOptionId` on broadcast keeps a shared push from carrying anyone's individual vote (ADR-039).
...
- **Concept, an authorization-gated query.** `[Rubric §11, Security]`. Because ADR-014 wraps queries
...
- **Concept, ephemeral broadcast payload carrying only a hint.** `[Rubric §6, CQRS & Event-Driven]` (assesses events that carry just enough context to act on). A close is a structural event, so the payload holds no tally, just the two ids a subscriber needs to reload the affected poll. This is the "channel event as a cache-invalidation hint over fetchable state" rule (ADR-039): the payload is a nudge, and the fresh closed state comes from the next fetch.
...
- **Depends on**: the `LivePollIdentifierType` and `EventIdentifierType` aliases; no first-party types.
- **Concept, universally-visible-only broadcast data.** `[Rubric §11, Security]` (assesses that broadcasts leak no privileged or per-user data). The doc comment (`LivePollOpenedPayload.cs:5`) states the constraint directly: the payload carries only universally visible data (BR-229/ADR-039). The question is safe to broadcast because an open poll is public to everyone in the room; there is nothing per-user here to strip.
...
- **Why it's built this way**: `Options` defaults to `[]` because the generic query-service path materializes the poll without EF `.Include()`, and the [`LivePollNavigationPopulator`](#livepollnavigationpopulator) loads the children afterward (ADR-002); an empty-collection default keeps a not-yet-populated poll safe to render. Mapping from the [`LivePoll`](#livepoll) entity is a compile-time Mapperly mapper (ADR-001).
...
- **Where it's used**: returned by the Q&A read endpoints; produced by the session-question DTO mapper (a compile-time Mapperly mapper, ADR-001) with the per-caller flags projected in against [`ICurrentUserService`](group-08-auth.md#icurrentuserservice); rendered by the session Live and presenter Q&A surfaces.
...
- **Walkthrough**, in teaching order:
  ...
  - **Voting** (`HappeningNow.razor.cs:219-249`): `VoteAsync` fires a haptic click (a no-op off native, ADR-042), casts the vote through [`ILivePollUIService`](#ilivepolluiservice), and patches the returned results into `_polls`.
...
- **Where it's used**: mounted by the Engagement service host; reached by clients through the YARP Gateway (ADR-008).
...
- **Why it's built this way**: committing before publishing, plus the swallow-and-log catch, gives the live layer at-most-once broadcast semantics layered over a durably-committed write, the correct trade for ephemeral UI signals that must never block a moderation (ADR-039 for the channel transport).
...
- **Why it's built this way** - snapshotting the live-window end (rather than calling Conference per upvote) trades a small staleness window for avoiding chatty synchronous coupling on the hot path, the microservice-readiness rationale in ADR-006 / ADR-007. The single-event-plus-state shape (BR-60) keeps the event catalog small and lets one handler branch on `DomainEntityState`.
...
- **Why it's built this way** - Mapperly satisfies the [IEntityDTOMapper](group-12-api-hosting-mapping.md#ientitydtomappertentity-tentitydto-tidentifiertype) contract with generated code, keeping the poll read path fast and drift-free (ADR-001).
...
- **Why it's built this way** - one length constant sourced from the domain (ADR-001-adjacent single-source-of-truth), scalar cross-service references (ADR-006), and an index matching the real query shape.
