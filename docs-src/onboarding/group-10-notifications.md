...
1. **The durable in-app inbox.** Every send writes one [`UserNotification`](#usernotification) row
   ...
   their inbox. This is the persistent half of the two-channel model (ADR-024).
...
lower-traffic, fire-one-message helper rather than part of the broadcast pipeline.
...
[`PushNotificationInvariants`](#pushnotificationinvariants) guards. Both aggregates are
`[IdValueGenerated]`, so the database assigns their ids
(`MMCA.Common.Domain/Notifications/PushNotifications/PushNotification.cs:13`,
`MMCA.Common.Domain/Notifications/UserNotifications/UserNotification.cs:11`). The **Application**
layer defines the four transport contracts and the CQRS use cases; the **Shared** layer carries the
wire types ([`PushNotificationDTO`](#pushnotificationdto),
...
[`NotificationFeatures`](#notificationfeatures) flag constant. The **Infrastructure** layer supplies
the SignalR-backed implementations ([`SignalRPushNotificationSender`](#signalrpushnotificationsender),
...
(`MMCA.Common.Application/Interfaces/Infrastructure/IPushNotificationSender.cs:7`,
...
and API versioning (`MMCA.Common.API/Controllers/Notifications/NotificationsController.cs:28-30`).
The controller wraps the request in a
[`SendPushNotificationCommand`](#sendpushnotificationcommand) carrying the authenticated sender id
and hands it to [`SendPushNotificationHandler`](#sendpushnotificationhandler)
(`MMCA.Common.Application/Notifications/PushNotifications/UseCases/Send/SendPushNotificationHandler.cs:26`).

The handler runs a deliberate ordering
(`SendPushNotificationHandler.cs:31-109`): resolve the recipient user ids through
[`INotificationRecipientProvider`](#inotificationrecipientprovider) (fail early with a validation
error if the set is empty); create the [`PushNotification`](#pushnotification) audit aggregate and
save it; then create one [`UserNotification`](#usernotification) inbox row per recipient and save
again (so a user can retrieve a missed notification even if every real-time channel fails); then
attempt SignalR delivery via [`IPushNotificationSender`](#ipushnotificationsender), catching any
exception and recording `MarkAsSent()` or `MarkAsFailed()` accordingly (delivery failure is
non-fatal, `SendPushNotificationHandler.cs:80-86`); then a best-effort native-push leg through
`INativePushSender` (ADR-044) whose failures are logged but never change the audit status
(`SendPushNotificationHandler.cs:88-105`); and finally map the aggregate to a
[`PushNotificationDTO`](#pushnotificationdto) via [`PushNotificationDTOMapper`](#pushnotificationdtomapper).
The durable inbox write happening **before** the transient channels is the load-bearing choice: the
record of who should have been reached survives even when nobody is connected.
...
framework registers [`NullNotificationRecipientProvider`](#nullnotificationrecipientprovider) as a
safe default, and ADC overrides it with
[`AttendeeNotificationRecipientProvider`](#attendeenotificationrecipientprovider), which bridges the
Identity module's [`IAttendeeQueryService`](group-24-identity-module.md#iattendeequeryservice) (over
gRPC across service boundaries) so a broadcast targets every conference attendee.

## The inbox side

Reading and acknowledging notifications is the query/command counterpart, served by
[`InboxController`](#inboxcontroller) under `[Authorize(RequireAuthenticated)]` so any user reaches
only their own inbox (`MMCA.Common.API/Controllers/Notifications/NotificationInboxController.cs:30`).
It exposes four use cases: the paged inbox
...
content), an unread count ([`GetUnreadNotificationCountQuery`](#getunreadnotificationcountquery),
served `no-store` so the bell badge is never stale), a single mark-read
...
[`UserNotification.MarkAsRead`](#usernotification) is **idempotent** and takes the read timestamp as
a parameter (from an injected `TimeProvider`) so the domain stays free of ambient clock access and
the transition is deterministically testable (`UserNotification.cs:49-67`).

## The SignalR transport, and how it survives extraction

[`NotificationHub`](#notificationhub) is intentionally thin
(`MMCA.Common.Infrastructure/Hubs/NotificationHub.cs:17`): it is `[Authorize]`d, and beyond ASP.NET's
built-in per-user connection mapping it only manages channel (SignalR group) membership through
`JoinChannel`/`LeaveChannel`, validating each channel key against a configured regex with a cached,
timeout-guarded `Regex` so a bad key throws `HubException` rather than opening an injection or ReDoS
hole (`NotificationHub.cs:43-71`) ([Rubric §11, Security]). Actual delivery does not run inside the
hub: [`SignalRPushNotificationSender`](#signalrpushnotificationsender) and
...
(`MMCA.Common.Infrastructure/Services/SignalRPushNotificationSender.cs:15,42-59`)
([Rubric §12, Performance & Scalability]); the live publisher does a single `Clients.Group` send.
When a Redis backplane is configured, both fan out across replicas.
...
depends on [`ILiveChannelPublisher`](#ilivechannelpublisher) as usual but the composition root binds
it to [`LiveChannelPublisherGrpcAdapter`](#livechannelpublishergrpcadapter). That adapter forwards
each event over gRPC (with a tight 2-second deadline) to the Notification service's
[`LiveChannelGrpcService`](#livechannelgrpcservice), which then resolves the local
[`SignalRLiveChannelPublisher`](#signalrlivechannelpublisher), the only host whose `IHubContext` can
reach connected clients. Both the adapter and the whole live path are **best-effort by contract**
(ADR-039): every transport, resolution, or broken-circuit failure is logged and swallowed, never
thrown, so a publishing command can never fail because Notification is slow or down
(`MMCA.ADC.Notification.Contracts/LiveChannelPublisherGrpcAdapter.cs:26,43-48`)
([Rubric §29, Resilience & Business Continuity]). Serving both a WebSocket and an h2c gRPC ingress
from one host is the mixed-endpoint profile of ADR-012.
...
(`MMCA.ADC.Notification.API/NotificationModule.cs:16`), an
[`IModule`](group-14-module-system-composition.md#imodule) that declares a dependency on Identity (it
needs attendee data) and whose `Register` calls `AddNotificationModule` to wire the application
handlers, EF configs, the SignalR push registration, and the native-push channel
(`MMCA.ADC.Notification.API/DependencyInjection.cs:21-34`). It is a deliberately thin module (API +
Application only, no Infrastructure project of its own). The framework's own registration
([`DependencyInjection`](#dependencyinjection) in
`MMCA.Common.Application/Notifications/DependencyInjection.cs:27`) uses `TryAdd` throughout so a
consuming app can override any handler or the recipient provider, and the API-layer
[`DependencyInjection`](#dependencyinjection)
(`MMCA.Common.API/Notifications/DependencyInjection.cs:9`) adds the Common controllers as an MVC
application part (they ship in a NuGet assembly that ASP.NET does not scan by default).
...
*the caller's own* installation (`DevicesController.cs:33-45`). `DeleteAsync`, by contrast, takes no
ownership check at all: it calls `registrar.DeleteAsync(installationId, …)` with only the
route-supplied id (`DevicesController.cs:50-56`), so any authenticated caller who knows an
installation id can remove it. The only thing standing between that and cross-user deletion is that
installation ids are client-generated GUIDs and therefore not enumerable, not an ownership assertion.
The class XML-doc comment (`DevicesController.cs:14-18`) claims ownership "is stamped server-side from
the authenticated user" without qualification; that holds for PUT but overstates DELETE, so the code,
not the comment, is authoritative here (`DevicesController.cs:13-56`) ([Rubric §11, Security]).
The registrar it delegates to lives in
...
requesting user (`MMCA.ADC.Notification.Application/UserNotificationExportService.cs:20-42`). When
the module is disabled, [`NotificationModule`](#notificationmodule) registers
[`DisabledUserNotificationExportService`](#disabledusernotificationexportservice) so the cross-module
interface still resolves ([Rubric §30, Compliance, Privacy & Data Governance]).
...
`ReceiveNotification` / `ReceiveChannelEvent` methods. The ADC Engagement live layer is the busiest
producer of live-channel events, resolving attendees through the
...
- **Depends on**: the solution-wide `UserIdentifierType` alias (`int`, see [primer §2](00-primer.md#2-architectural-styles-this-codebase-commits-to)); BCL only otherwise. Consumed by [`GetMyNotificationsHandler`](#getmynotificationshandler) through the query side of the CQRS pipeline ([`IQueryHandler`](group-05-cqrs-pipeline.md#iqueryhandlerin-tquery-tresult)).
- **Concept introduced**: **the paged-query record shape.** This is the first notification query, so note the convention it shares with every read in the codebase: an immutable positional `record` with defaulted paging (`PageNumber = 1`, `PageSize = 20`, line 9-10) is the message; a matching handler is the behavior; the two are wired by the query dispatcher, never called directly. Tag: `[Rubric §6, CQRS & Event-Driven]` assesses whether reads and writes are modeled as distinct, single-purpose messages, and this record is a pure read with no side effects. `[Rubric §12, Performance & Scalability]` assesses guarding against unbounded work: the XML doc pins `PageSize` at "max 500" (line 6) and the handler enforces that ceiling, so a client cannot request an unbounded page.
...
- **Where it's used**: dispatched from the `InboxController` (Presentation), handled by [`GetMyNotificationsHandler`](#getmynotificationshandler).
...
- **Walkthrough**: two positional members, `PageNumber = 1` (line 7) and `PageSize = 10` (line 8). Note the default page size is 10 here versus 20 for the inbox; both are capped at 500 by their handlers.
- **Where it's used**: dispatched from the `NotificationsController`, handled by [`GetNotificationHistoryHandler`](#getnotificationhistoryhandler).
...
- **What it is**: the tiniest read in the group: "how many unread notifications does this user have?" A single-line `sealed record` wrapping `UserId`. It powers the unread badge on the [`NotificationBell`](group-15-common-ui-framework.md#notificationbell).
...
- **Where it's used**: dispatched by the bell/inbox UI path; handled by [`GetUnreadNotificationCountHandler`](#getunreadnotificationcounthandler).
...
- **Depends on**: BCL only (`Task`, `CancellationToken`). Implemented by [`SmtpEmailSender`](#smtpemailsender) in Infrastructure.
...
- **Walkthrough**: `SendAsync(string to, string subject, string body, bool isHtml = false, CancellationToken)` (line 15): explicit recipient, HTML flag defaulting to plain text. `SendAsync(string subject, string body, bool isHtml = false, CancellationToken)` (line 23): the same message routed to the implementation's configured default/system recipient (e.g. organizer/admin alerts), so callers that always mail the operators do not repeat the address.
- **Why it's built this way**: keeping the interface in Application (and the SMTP dependency in Infrastructure) is what lets a test host register a no-op sender and production register [`SmtpEmailSender`](#smtpemailsender).
- **Where it's used**: notification-triggering handlers and operational alerts. The concrete recipient/host configuration lives with the Infrastructure implementation.
...
- **What it is**: the port for publishing *ephemeral* live events to a channel of currently-connected clients (for example `event:1` or `session:123`). Its defining property, stated in the XML doc (line 4-6): channel events are **not persisted**, so a client that is not connected and subscribed at publish time never sees them.
- **Depends on**: BCL only. Contrast with [`IPushNotificationSender`](#ipushnotificationsender) (which persists a [`PushNotification`](#pushnotification) + per-user [`UserNotification`](#usernotification) inbox rows). Implemented by [`SignalRLiveChannelPublisher`](#signalrlivechannelpublisher) and the [`NullLiveChannelPublisher`](#nulllivechannelpublisher) no-op.
- **Concept introduced**: **ephemeral fan-out versus durable notification.** This is the distinction that splits the whole group in two: live channel events (poll-results-changed, a new session question) are fire-and-forget to whoever is watching *right now*, while push notifications are durable and land in an inbox. The interface deliberately speaks in strings (a `channelKey`, an application-defined `eventName`, a `payloadJson` string) so it stays transport-agnostic. `[Rubric §7, Microservices Readiness]` assesses whether cross-boundary calls go through abstractions that can be re-homed onto a network transport: in MMCA.ADC this exact interface is served over gRPC by the Notification service and called best-effort by Engagement after `SaveChangesAsync`, so the boundary already survives extraction (ADR-007/008).
- **Walkthrough**: one method, `PublishAsync(string channelKey, string eventName, string payloadJson, CancellationToken)` (line 17): publish an event to every client currently subscribed to `channelKey`. No return value beyond the `Task`, because there is no delivery guarantee to report.
- **Why it's built this way**: a JSON-string payload plus a free-form event name keeps the framework out of the business of knowing each live event's schema; the presentation/UI layers agree on the contract. Non-delivery to absent clients is the intended semantics, not a gap.
- **Where it's used**: the conference-day live layer (Engagement) publishes poll/question channel events through this port; the SignalR implementation lands them on connected hub groups.
- **Caveats / not-in-source**: the interface itself makes no delivery or ordering guarantee; those are properties of the concrete SignalR/gRPC wiring, not visible here.
...
- **What it is**: a single-method port returning the set of user IDs that should receive a broadcast push notification. The framework knows *how* to send; the consuming app implements this to answer *who* (all attendees, a role, topic subscribers).
- **Depends on**: the `UserIdentifierType` alias; BCL only otherwise. Works alongside [`IPushNotificationSender`](#ipushnotificationsender). Default framework registration is [`NullNotificationRecipientProvider`](#nullnotificationrecipientprovider); MMCA.ADC supplies [`AttendeeNotificationRecipientProvider`](#attendeenotificationrecipientprovider).
- **Concept introduced**: **separating recipient policy from delivery mechanism.** "Who to notify" is app-specific domain knowledge; "how to notify" is framework infrastructure. Splitting them means the push pipeline never needs to understand ADC's attendee model. `[Rubric §7, Microservices Readiness]`: an inversion point that lets the shared framework host an app-defined audience query. `[Rubric §1, SOLID]` (Interface Segregation / Dependency Inversion): one focused method, and the sender depends on the abstraction rather than on a concrete audience source.
- **Walkthrough**: `Task<IReadOnlyList<UserIdentifierType>> GetRecipientUserIdsAsync(CancellationToken cancellationToken = default)` (line 15): return the eligible recipient IDs. Read-only list, so callers cannot mutate the returned audience.
- **Where it's used**: the send-push handler resolves this to a user-id list, then hands it to [`IPushNotificationSender`](#ipushnotificationsender). Until an app registers its own provider, [`NullNotificationRecipientProvider`](#nullnotificationrecipientprovider) returns an empty audience.
...
- **What it is**: the Application-layer port for real-time push delivery, with three targeting shapes: one user, a set of users, or a broadcast to everyone connected. The XML doc names SignalR or Firebase Cloud Messaging as interchangeable backings (line 4-5).
- **Depends on**: the `UserIdentifierType` alias; BCL `Dictionary` for the optional metadata. Implemented by [`SignalRPushNotificationSender`](#signalrpushnotificationsender) (real delivery) and [`NullPushNotificationSender`](#nullpushnotificationsender) (no-op fallback). Paired with [`INotificationRecipientProvider`](#inotificationrecipientprovider) for audience resolution.
- **Concept introduced**: **the metadata dictionary as an open payload.** All three methods share `title`, `body`, and an optional `Dictionary<string, string>? metadata` (lines 16, 25, 33). That dictionary carries typed extras (a deep-link URL, a notification type) without a bespoke strongly-typed payload per notification kind, so a new notification variety needs no interface change. `[Rubric §10, Cross-Cutting Concerns]` assesses whether a capability like push is factored once and reused broadly, and this single port serves every push-emitting feature. This is the *durable* counterpart to [`ILiveChannelPublisher`](#ilivechannelpublisher)'s ephemeral fan-out.
- **Walkthrough**: `SendToUserAsync(UserIdentifierType userId, string title, string body, Dictionary<string,string>? metadata = null, CancellationToken)` (line 16): one user. `SendToUsersAsync(IEnumerable<UserIdentifierType> userIds, ...)` (line 25): an explicit set. `BroadcastAsync(string title, string body, ...)` (line 33): all connected clients. The three overloads differ only in targeting; body and metadata are identical.
- **Why it's built this way**: three targeting methods rather than one "audience" parameter keeps each call site's intent explicit and lets the SignalR implementation map user-targeting to hub groups directly.
- **Where it's used**: the send-push command handler ([`SendPushNotificationHandler`](#sendpushnotificationhandler)) fans a message out through this port after persisting the [`PushNotification`](#pushnotification) record.
...
- **What it is**: the write request behind the inbox's "mark all read" action. A single-line `sealed record` carrying just the acting `UserId`.
- **Depends on**: the `UserIdentifierType` alias; BCL only otherwise. Handled by [`MarkAllNotificationsReadHandler`](#markallnotificationsreadhandler); the single-item sibling is [`MarkNotificationReadCommand`](#marknotificationreadcommand).
- **Concept introduced**: none new; this is the command (write) counterpart to the query records above. Unlike a query, a command flows through the write half of the decorator pipeline (Validating and Transactional decorators, see [Group 05](group-05-cqrs-pipeline.md)). `[Rubric §6, CQRS & Event-Driven]`: a mutation is modeled as its own explicit message, distinct from the reads it will affect.
- **Walkthrough**: one positional member, `UserId` (line 5): the scope of the bulk mark-read is the caller's own notifications only.
- **Where it's used**: dispatched from the `InboxController`; handled by [`MarkAllNotificationsReadHandler`](#markallnotificationsreadhandler).
...
- **What it is**: one notification-inbox row inside a user's *personal-data export* (the GDPR/CCPA data-subject access artifact, PRIVACY.md §7): the notification id, title, and sent/read timestamps. This is an MMCA.ADC-specific contract, not a framework type.
- **Depends on**: the ADC `UserNotificationIdentifierType` alias; BCL `string`/`DateTime`. Returned by [`IUserNotificationExportService`](#iusernotificationexportservice); consumed by the Identity module's [`ExportUserDataHandler`](group-24-identity-module.md#exportuserdatahandler) and folded into [`UserDataExportNotificationDTO`](group-24-identity-module.md#userdataexportnotificationdto).
- **Concept introduced**: **export DTOs deliberately omit content.** The XML doc (line 5) records that the notification *body* is intentionally left out of the summary; the export carries the metadata a data subject is owed (that they were notified, when, whether they read it) without duplicating message bodies. `[Rubric §30, Compliance/Privacy/Data Governance]` assesses whether the codebase has concrete data-subject access/portability paths, and this DTO is the notification module's contribution to that per-user export.
- **Walkthrough**: a `sealed record class` with `required NotificationId` (line 10), `required Title` (line 13), `required SentOn` (line 16, UTC), plus `IsRead` (line 19) and nullable `ReadOn` (line 22, null when unread). The `required` members force every export row to be fully populated at construction.
- **Why it's built this way**: a flat immutable record is the right shape for a serialized export line: value semantics, no behavior, self-describing timestamps in UTC.
- **Where it's used**: assembled by the in-process notification export service and returned across the export boundary to Identity; the disabled-module path substitutes [`DisabledUserNotificationExportService`](#disabledusernotificationexportservice).
...
- **What it is**: the cross-module service contract for exporting the personal data the Notification module holds for one user: their inbox rows (ids, titles, sent/read dates). It is how the Identity module reaches into Notification-owned data to build a complete cross-service export.
- **Depends on**: [`UserNotificationExportItemDTO`](#usernotificationexportitemdto) (its return element); the ADC `UserIdentifierType` alias. Implemented in-process inside the Notification service and, per the XML doc (line 8-9), by a gRPC adapter (`MMCA.ADC.Notification.Contracts`) everywhere else; the disabled-module stub is [`DisabledUserNotificationExportService`](#disabledusernotificationexportservice).
- **Concept introduced**: **the "one interface, in-process or gRPC" extraction pattern.** The XML doc calls out that this mirrors Engagement's `IUserEngagementExportService`: a single interface the caller depends on, satisfied by an in-process implementation when the module is co-hosted and by a gRPC adapter when it runs as its own service. `[Rubric §7, Microservices Readiness]` assesses exactly this: whether a module boundary is expressed as an interface that can be re-homed onto a network transport without changing the caller (ADR-007 gRPC extraction, ADR-008 topology). `[Rubric §30, Compliance/Privacy]`: it is one leg of the data-subject-access aggregation.
- **Walkthrough**: one method, `GetUserNotificationExportAsync(UserIdentifierType userId, CancellationToken)` (line 20), returning `IReadOnlyList<UserNotificationExportItemDTO>` newest-first. The XML notes the implementation joins the framework [`UserNotification`](#usernotification) rows with their [`PushNotification`](#pushnotification) content.
- **Where it's used**: the Identity module's [`ExportUserDataHandler`](group-24-identity-module.md#exportuserdatahandler) calls it (in-process or over gRPC) and merges the result into the full export.
...
- **Concept introduced**: the **Null Object pattern**. `[Rubric §2, Design Patterns]` assesses idiomatic pattern use; here a benign default lets the push pipeline resolve and run in a host that has not (yet) declared an audience, so there is no null check and no "provider not registered" failure. MMCA.Common ships this as the default; a consuming app overrides it (in MMCA.ADC, [`AttendeeNotificationRecipientProvider`](#attendeenotificationrecipientprovider)).
- **Walkthrough**: `GetRecipientUserIdsAsync` (line 11) returns `Task.FromResult` of an empty array via the collection-expression `[]` (line 13). No allocation of a populated list, no I/O.
- **Where it's used**: registered by `AddApplication()`; a broadcast that resolves recipients through this default simply reaches nobody until an app-specific provider replaces it.
...
- **Depends on**: [`IUserNotificationExportService`](#iusernotificationexportservice), [`UserNotificationExportItemDTO`](#usernotificationexportitemdto); BCL only.
- **Concept introduced**: none new; it is the same Null Object idea as [`NullNotificationRecipientProvider`](#nullnotificationrecipientprovider), applied to the module-disabled path. `[Rubric §7, Microservices Readiness]`: the module system keeps cross-module interfaces resolvable even when a module is switched off (disabled modules register benign stubs), so Identity's export handler still binds `IUserNotificationExportService` and simply gets no notification rows rather than a resolution failure.
- **Walkthrough**: `GetUserNotificationExportAsync` (line 10) returns `Task.FromResult` of an empty array `[]` (line 11), ignoring the `userId`.
- **Where it's used**: registered in the Notification module's disabled-stub path so a host running without the Notification module still produces a valid (empty) notification section in a user data export.
...
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork) (typed repositories) and [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor) (EF terminal ops kept out of Application), injected via primary constructor (lines 16-18). Implements [`IQueryHandler`](group-05-cqrs-pipeline.md#iqueryhandlerin-tquery-tresult)`<GetMyNotificationsQuery, Result<PagedCollectionResult<UserNotificationDTO>>>`. Returns [`Result`](group-01-result-error-handling.md#result) / [`PagedCollectionResult<T>`](group-01-result-error-handling.md#pagedcollectionresultt) / [`PaginationMetadata`](group-01-result-error-handling.md#paginationmetadata).
- **Concept introduced**: **the two-table inbox join and why the model is split.** A push notification is stored once (the [`PushNotification`](#pushnotification): title, body, created time) and fanned out into one lightweight [`UserNotification`](#usernotification) per recipient (which carries only per-user state: `IsRead`, `ReadOn`). The read side rejoins them. `[Rubric §8, Data Architecture]` assesses normalization and read/write model fit: the shared content is not duplicated per recipient, and the handler pays a join at read time to reassemble the inbox view. `[Rubric §3, Clean Architecture]`: the handler expresses the join as a LINQ `IQueryable` but never calls EF's `ToListAsync`/`CountAsync` directly, delegating those to [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor) so Application stays EF-free.
- **Walkthrough**: clamp the page size to a 500 ceiling with `Math.Min(query.PageSize, 500)` (line 25); grab the two `NoTracking` repositories (lines 26-27); build the LINQ join of `UserNotification` to `PushNotification` on `PushNotificationId`, filtered to `query.UserId`, ordered by `PushNotification.CreatedOn` descending, projected into `UserNotificationDTO` (lines 29-42, mapping id, title, body, `IsRead`, `ReadOn`, and `SentOn = pn.CreatedOn`); count the joined set (line 44); page it with `Skip((PageNumber - 1) * pageSize).Take(pageSize)` and materialize (lines 46-48); wrap count/page/number into [`PaginationMetadata`](group-01-result-error-handling.md#paginationmetadata) and return a successful [`PagedCollectionResult<T>`](group-01-result-error-handling.md#pagedcollectionresultt) (lines 50-51).
- **Why it's built this way**: `TableNoTracking` is correct for a read: no change-tracking overhead since nothing is saved. Server-side projection into the DTO means only the needed columns cross the wire, and the count query runs against the same joined expression so the total is consistent with the page.
- **Where it's used**: invoked through the query dispatcher when the `InboxController` serves a user's inbox.
...
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor), and [`PushNotificationDTOMapper`](#pushnotificationdtomapper) (constructor, lines 15-18). Implements [`IQueryHandler`](group-05-cqrs-pipeline.md#iqueryhandlerin-tquery-tresult)`<GetNotificationHistoryQuery, Result<PagedCollectionResult<PushNotificationDTO>>>`.
- **Concept introduced**: none new; note the contrast with the inbox handler. History reads a *single* table (the sent artifacts) with no `UserId` filter, and maps entities to DTOs with an explicit [`PushNotificationDTOMapper`](#pushnotificationdtomapper) rather than an inline LINQ projection. `[Rubric §6, CQRS & Event-Driven]`: a separate read model and handler for the admin history view.
- **Walkthrough**: clamp page size to 500 (line 25); get the [`PushNotification`](#pushnotification) repository (line 26); `CountAsync` for the total (line 28); page `TableNoTracking` ordered by `CreatedOn` descending with `Skip`/`Take` and materialize the entities (lines 30-35); run them through `dtoMapper.MapToDTOs` (line 37); build [`PaginationMetadata`](group-01-result-error-handling.md#paginationmetadata) and return a successful [`PagedCollectionResult<T>`](group-01-result-error-handling.md#pagedcollectionresultt) (lines 38-40).
- **Why it's built this way**: history has no per-user state, so there is nothing to join; a dedicated mapper (versus an inline projection) is used because [`PushNotificationDTO`](#pushnotificationdto) is a richer, reused contract mapped in several places.
- **Where it's used**: the `NotificationsController` history endpoint.
...
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork) and [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor) (constructor, lines 12-14). Implements [`IQueryHandler`](group-05-cqrs-pipeline.md#iqueryhandlerin-tquery-tresult)`<GetUnreadNotificationCountQuery, Result<int>>`.
- **Concept introduced**: none new; the smallest possible read. `[Rubric §12, Performance & Scalability]`: it issues a server-side `COUNT` over a `Where(un => un.UserId == query.UserId && !un.IsRead)` predicate rather than fetching rows, so the bell can poll without materializing the inbox.
- **Walkthrough**: get the [`UserNotification`](#usernotification) repository (line 21); `queryableExecutor.CountAsync` over `TableNoTracking` filtered to the user's unread rows (lines 23-25); return `Result.Success(count)` (line 27).
- **Where it's used**: the [`NotificationBell`](group-15-common-ui-framework.md#notificationbell) badge path in the UI, via the query dispatcher.
...
- **What it is**: the command handler for "mark all read": it loads a user's unread [`UserNotification`](#usernotification) rows and stamps each as read.
- **Depends on**: [`IUnitOfWork`](group-07-persistence-ef-core.md#iunitofwork), [`IQueryableExecutor`](group-07-persistence-ef-core.md#iqueryableexecutor), and BCL `TimeProvider` (constructor, lines 11-14). Implements [`ICommandHandler`](group-05-cqrs-pipeline.md#icommandhandlerin-tcommand-tresult)`<MarkAllNotificationsReadCommand, Result>`.
- **Concept introduced**: **the write path, and testable time.** Unlike the read handlers this one mutates: it uses the *tracked* `Table` (not `TableNoTracking`) so EF observes the state changes it will save. It reads the clock through the injected `TimeProvider` (line 14, `GetUtcNow().UtcDateTime`, line 27) rather than `DateTime.UtcNow`, which is the codebase's standard for deterministic, unit-testable time. `[Rubric §14, Testability]` assesses whether side-effecting time is injectable, and here it is. `[Rubric §4, DDD]`: the read-state change goes through the entity's own `MarkAsRead(readOnUtc)` method (line 30), keeping the invariant inside the aggregate rather than writing `IsRead`/`ReadOn` from the handler.
- **Walkthrough**: get the [`UserNotification`](#usernotification) repository (line 21); materialize the tracked unread rows for the user (`Table.Where(un => un.UserId == command.UserId && !un.IsRead)`, lines 23-25); compute `readOnUtc` from `TimeProvider` (line 27); loop and call `notification.MarkAsRead(readOnUtc)` on each (lines 28-31); persist only when at least one row changed, guarded by `if (unread.Count > 0)` (lines 33-36); return `Result.Success()` (line 38).
- **Why it's built this way**: the `unread.Count > 0` guard avoids opening a transaction and a no-op `SaveChanges` when the inbox is already fully read. Routing every field change through `MarkAsRead` keeps read-state a domain concern, not a handler concern.
- **Where it's used**: dispatched from the `InboxController` "mark all read" action; runs through the command decorator pipeline ([Group 05](group-05-cqrs-pipeline.md)) before reaching this handler.
...
- **Concept, strongly-typed identifier aliases in a DTO.** `[Rubric §4, DDD]` assesses avoiding
  ...
  [primer §2](00-primer.md#2-architectural-styles-this-codebase-commits-to)).
- **Walkthrough**: a `sealed record class` with seven `init` properties. `required` marks the values
  that must always be present, `Id`, `PushNotificationId`, `Title`, `Body`, `IsRead`
  (`UserNotificationDTO.cs:10-22`); plain `init` marks the optional/nullable ones, `ReadOn?` and
  `SentOn` (lines 25, 28). `required` + `init` gives set-once, non-null-where-it-matters immutability
  without a hand-written constructor.
...
- **What it is**: a hand-written adapter that implements the framework's `ILiveChannelPublisher`
  abstraction on top of the generated gRPC `LiveChannelPushService` client. It is how a publishing
  service (Engagement's live layer) reaches the Notification service, the only host whose `IHubContext`
  can fan a channel event out to connected SignalR clients.
- **Depends on**: [ILiveChannelPublisher](#ilivechannelpublisher) (implements it), the generated
  `LiveChannelPushService.LiveChannelPushServiceClient` (gRPC, from the compiled `.proto`), and
  `ILogger<T>` (BCL logging).
- **Concept introduced, the gRPC adapter over a framework abstraction.** This is the extraction
  boundary made concrete (ADR-007/ADR-008): application code keeps depending on the C# interface
  `ILiveChannelPublisher`, while the *transport* (an out-of-process gRPC call) lives only at the edge in
  a `.Contracts` project. Any project whose name ends in `.Contracts` auto-compiles its `Protos/**/*.proto`
  with both server and client stubs, so one contract package serves producer and consumer. `[Rubric §7,
  Microservices Readiness]` assesses exactly this: a module can be lifted out of the monolith without
  rewriting the callers, because the caller only ever sees the interface. `[Rubric §29, Resilience &
  Business Continuity]` is the other half: the doc comment (`LiveChannelPublisherGrpcAdapter.cs:14-18`,
  ADR-039) declares live channel events ephemeral and best-effort.
- **Walkthrough**: a `sealed partial` primary-constructor class taking the generated client and a
  logger (`LiveChannelPublisherGrpcAdapter.cs:20-22`). A `PushDeadline` of 2 seconds
  (`LiveChannelPublisherGrpcAdapter.cs:26`) is set deliberately tighter than the global 30s resilience
  attempt timeout, a stale live event is worthless, so the publishing request is never held hostage.
  `PublishAsync` (lines 29-49) calls `client.PushToChannelAsync` with that deadline, then catches
  *every* exception (the `CA1031` suppression at lines 43-45 is intentional: no failure may escape) and
  routes it to the source-generated `LogPushFailed` (lines 51-52, a `[LoggerMessage]` warning). The
  method swallows transport, resolution, and broken-circuit failures alike so a publishing command can
  never fail because Notification is down or slow.
- **Why it's built this way**: fire-and-forget by contract. Making the publish path best-effort
  decouples Engagement's write path from Notification availability (ADR-039); the AppHost even omits the
  reciprocal `WaitFor` on this edge so startup cannot deadlock.
- **Where it's used**: swapped in for the framework's no-op `NullLiveChannelPublisher` by
  `AddNotificationLiveChannelClient` in the Contracts [DependencyInjection](#dependencyinjection);
  called by Engagement's live layer after `SaveChangesAsync`.
- **Caveats / not-in-source**: the `.proto` service definition itself is not in this file; it is
  compiled from `Protos/**` in the `.Contracts` project.
...
- **Depends on**:
  ...
  (status), the `PushNotificationCreated` domain event, and [Result<T>](group-01-result-error-handling.md#result)
...
- **Walkthrough**: five `private set` properties, `Title`, `Body`, `SentByUserId`, `RecipientCount`,
  ...
  Pending`. The static `Create` factory (lines 56-78) combines the title and body invariants via
  `Result.Combine` (lines 62-64), returns a [Result.Failure](group-01-result-error-handling.md#result)
  ...
  (lines 83, 88) are one-line `void` transitions with no event, they record the delivery outcome for
  audit.
...
- **Concept, ownership enforced in the query, not after it.** `[Rubric §11, Security]` assesses whether
  ...
  `timeProvider.GetUtcNow()` (line 38), not `DateTime.UtcNow`, so a test can pin the clock.
- **Walkthrough**: `HandleAsync` (lines 18-42) gets the `UserNotification` repository from the unit of
  ...
  executor (lines 24-28), returns a `NotFound` failure when there is no match (lines 30-36), otherwise
  calls the aggregate's idempotent `MarkAsRead(...)` with the injected UTC time (line 38), saves through
  the unit of work (line 39), and returns success (line 41). Every await uses `ConfigureAwait(false)`.
...
- **Where it's used**: registered by the Notification module's
  ...
  `IUserNotificationExportService`; when Notification runs as its own service it is instead reached
  cross-process through [UserNotificationExportServiceGrpcAdapter](#usernotificationexportservicegrpcadapter).
...
- **What it is**: the gRPC client-side adapter that implements `IUserNotificationExportService` by
  calling the Notification service's export rpc, so a consumer (Identity's `ExportUserDataHandler`) that
  lives in a different process can still aggregate a user's inbox rows.
- **Depends on**: `IUserNotificationExportService` (implements it), the generated
  `UserNotificationExportService.UserNotificationExportServiceClient` (gRPC), the
  `UserNotificationExportItemDTO` projection, and `System.Globalization` (BCL, for invariant date
  parsing).
- **Concept, the propagating (not swallowing) gRPC adapter.** This is the deliberate counterpart to
  [LiveChannelPublisherGrpcAdapter](#livechannelpublishergrpcadapter): the doc comment
  (`UserNotificationExportServiceGrpcAdapter.cs:12-15`) calls out that here transport failures *do*
  propagate to the caller, which degrades the export's Notifications section to `Available = false`
  rather than returning wrong data. `[Rubric §7, Microservices Readiness]` is the extraction point (the
  consumer keeps depending on the C# interface); `[Rubric §29, Resilience & Business Continuity]` is the
  policy choice, best-effort aggregation at the *caller*, not silent success at the adapter.
- **Walkthrough**: a primary-constructor class taking the generated client
  (`UserNotificationExportServiceGrpcAdapter.cs:17-18`). `GetUserNotificationExportAsync` (lines 21-40)
  calls `client.GetUserNotificationExportAsync` with a `GetUserNotificationExportRequest`
  (lines 25-30), then maps each proto row to a `UserNotificationExportItemDTO` (lines 32-39), parsing
  the wire timestamps with a private `ParseRoundtripUtc` helper (lines 42-43) that uses
  `CultureInfo.InvariantCulture` + `DateTimeStyles.RoundtripKind`, and treating an empty `ReadOn` string
  as `null` (line 38).
- **Why it's built this way**: gRPC carries the timestamps as round-trip strings, so parsing must be
  culture-invariant to be correct across locales (the codebase's headline `[Rubric §27,
  Internationalization]` stance, deliberate culture-invariance where culture would inject bugs). Letting
  failures propagate keeps the resilience decision with the caller that knows the export can tolerate a
  missing section.
- **Where it's used**: registered by the Contracts [DependencyInjection](#dependencyinjection)'s
  `AddNotificationUserExportClient`, replacing the in-process
  [UserNotificationExportService](#usernotificationexportservice) or the disabled-module stub with this
  gRPC-backed one.

### DependencyInjection
> MMCA.ADC.Notification.Contracts · `MMCA.ADC.Notification.Contracts` · `MMCA.ADC/Source/Services/MMCA.ADC.Notification.Contracts/DependencyInjection.cs:16` · Level 10 · class (static)

- **What it is**: the consumer-side DI helpers that wire the two gRPC-backed adapters pointing at the
  Notification service's dedicated h2c ingress: the live-channel publisher swap and the user-export swap.
- **Depends on**: the generated gRPC clients (`LiveChannelPushService`,
  `UserNotificationExportService`), `AddTypedGrpcClient<T>` (from `MMCA.Common.Grpc`), the
  [ILiveChannelPublisher](#ilivechannelpublisher) /
  [LiveChannelPublisherGrpcAdapter](#livechannelpublishergrpcadapter) and
  `IUserNotificationExportService` /
  [UserNotificationExportServiceGrpcAdapter](#usernotificationexportservicegrpcadapter) pairs, plus the
  BCL `ServiceCollectionDescriptorExtensions.Replace`.
- **Concept introduced, `Replace` versus `TryAdd` at the composition root.** Both methods are called
  after a default has already been registered, and both deliberately use `Replace`
  (`DependencyInjection.cs:48,84`): the framework registers a no-op `NullLiveChannelPublisher` (or a
  disabled-module `DisabledUserNotificationExportService` stub), and the gRPC adapter must *win* over
  that default, which `TryAdd` would refuse to do because a registration already exists. `[Rubric §7,
  Microservices Readiness]` shows in the endpoint choice, the mixed-endpoint profile (ADR-012):
  Notification's default Kestrel endpoint stays `Http1AndHttp2` so SignalR WebSockets work, and its
  cleartext gRPC (h2c) rides a dedicated `Http2`-only endpoint resolved by service discovery as the
  named endpoint `_grpc.notification` (documented at `DependencyInjection.cs:26-33`). `[Rubric §11,
  Security]` is present too: the typed client wires the standard JWT-forwarding interceptor so the
  caller's token flows across the process boundary.
- **Walkthrough**: `AddNotificationLiveChannelClient` (lines 42-51) registers a typed gRPC client for
  `LiveChannelPushService` (line 44) then `Replace`s the `ILiveChannelPublisher` registration with the
  scoped [LiveChannelPublisherGrpcAdapter](#livechannelpublishergrpcadapter) (line 48); its doc comment
  says to call it after `AddInfrastructure(...)`. `AddNotificationUserExportClient` (lines 78-87)
  registers a typed client for `UserNotificationExportService` (line 80) then `Replace`s
  `IUserNotificationExportService` with the scoped
  [UserNotificationExportServiceGrpcAdapter](#usernotificationexportservicegrpcadapter) (line 84); its
  doc comment says to call it after `ModuleLoader.DiscoverAndRegister(...)`. Both default the
  `serviceName` parameter to `"_grpc.notification"` (lines 42, 78).
- **Why it's built this way**: the named-endpoint discovery keeps the WebSocket and gRPC transports on
  separate Kestrel endpoints of the same host (ADR-012), and `Replace` makes the wiring order-tolerant,
  the adapter wins whether the prior registration was the framework no-op or a disabled-module stub.
- **Where it's used**: called from the Engagement service host (`AddNotificationLiveChannelClient`) and
  the Identity service host (`AddNotificationUserExportClient`) `Program.cs` composition roots.
...
- **Where it's used**: registered by the framework so `ILiveChannelPublisher` is always resolvable;
  ...
  root `services.Replace(...)` overwrites it with the gRPC adapter (see the Notification.Contracts DI in
  [DependencyInjection](#dependencyinjection)).
...
- **Walkthrough**: three methods, each `=> Task.CompletedTask`: `SendToUserAsync` (line 13),
  ...
  without behavior.
- **Why it's built this way**: the doc comment (lines 5-9) notes downstream apps override this with
...
- **Concept**: the *command-wraps-request* idiom (see
  [ICommandWithRequest](group-05-cqrs-pipeline.md#icommandwithrequestout-trequest), G05). The public
  ...
  which lets the generic `CommandRequestValidator` run
...
- **Where it's used**: dispatched by the push-notification API endpoint (the organizer-only
  `NotificationsController`); handled by [SendPushNotificationHandler](#sendpushnotificationhandler).
...
- **Walkthrough**: the primary constructor copies seven settings into readonly fields (`_host`,
  ...
  - `SendAsync(to, subject, body, isHtml, cancellationToken)` (line 23): guards each string with
    ...
    `NetworkCredential` and `EnableSsl` inside a `using` (lines 29-33), builds a `MailMessage` (also
    `using`, lines 35-38), and awaits `SendMailAsync(message, cancellationToken)` (line 40). The doc
    comment (lines 9-10) is explicit that a new client is created and disposed **per send**, no pooled
    long-lived connection.
  - `SendAsync(subject, body, isHtml, cancellationToken)` (line 51): a convenience overload that sends
    to the default `_toAddress` from settings, for admin/system mail with no explicit recipient.
...
- **What it is**: the ADC Notification module's composition root. Its single `AddNotificationModule`
  ...
  native-push, and controller wiring in one call. (This is distinct from the ADC Notification.Application
  / Notification.Contracts pair and the two Common-side DI classes covered elsewhere in this chapter.)
- **Depends on**: [ApplicationSettings](group-14-module-system-composition.md#applicationsettings)
...
- **Concept, layered DI composition with `extension(IServiceCollection)`.** `[Rubric §5, Vertical
  ...
  Microservices Readiness]`: this class is the seam where the shared push framework becomes ADC's
...
- **Walkthrough**: `AddNotificationModule(ApplicationSettings applicationSettings, IConfiguration
  ...
  1. `AddModuleNotificationApplication(applicationSettings)` (line 23), the ADC Application facade that
     ...
     Common's `AddNotificationApplicationServices`, so the `TryAdd*` defaults leave the ADC recipient
     source in place.
...
- **What it is**: the single SignalR `Hub` that serves **both** transient real-time surfaces, per-user
  push notifications and ephemeral live-channel (group) events. It stays deliberately thin: it maps
  authenticated connections and manages channel membership, while actual delivery happens out-of-band
  through the two adapters below.
- **Depends on**: [PushNotificationSettings](group-14-module-system-composition.md#pushnotificationsettings)
  (Level 1, injected as `IOptions<PushNotificationSettings>`). Externals: `Microsoft.AspNetCore.SignalR`
  (`Hub`, `HubException`, `[HubMethodName]`), `Microsoft.AspNetCore.Authorization` (`[Authorize]`),
  `Microsoft.Extensions.Options`, `System.Text.RegularExpressions`, `System.Collections.Concurrent`.
- **Concept introduced, the SignalR hub as a thin, authorized connection manager.** `[Rubric §11,
  Security]` assesses authentication and input validation at the transport edge: the hub is class-level
  `[Authorize]` (line 16) so only authenticated connections attach, and every channel key is validated
  against a configured regex before a join/leave, so a client cannot subscribe to arbitrary group names.
  `[Rubric §12, Performance & Scalability]` assesses hot-path allocation: the compiled `Regex` is cached
  per pattern in a `ConcurrentDictionary` (line 34) so join/leave do not recompile it per call, and the
  regex carries a 1-second match timeout (line 31) to bound pathological input. `[Rubric §7,
  Microservices Readiness]`: publishing the client-facing method-name **constants** on the hub gives the
  senders and the browser client one shared vocabulary instead of stringly-typed drift.
- **Walkthrough**
  - Four public `const string` method names: `ReceiveNotificationMethod = "ReceiveNotification"`
    (line 20) and `ReceiveChannelEventMethod = "ReceiveChannelEvent"` (line 23) are what clients
    *listen* on; `JoinChannelMethod = "JoinChannel"` (line 26) and `LeaveChannelMethod = "LeaveChannel"`
    (line 29) are what clients *invoke*.
  - `JoinChannelAsync(channelKey)` (line 44), tagged `[HubMethodName(JoinChannelMethod)]` (line 43):
    validates the key then `Groups.AddToGroupAsync(Context.ConnectionId, channelKey)` (line 47), so the
    connection starts receiving events published to that channel.
  - `LeaveChannelAsync(channelKey)` (line 55): the mirror, `Groups.RemoveFromGroupAsync` (line 58).
  - `EnsureValidChannelKey(channelKey)` (line 61): fetches (or compiles once and caches) the regex from
    `settings.Value.ChannelKeyPattern` (lines 63-65) and throws `HubException("Invalid channel key.")`
    (line 69) when the key is empty or does not match.
- **Why it's built this way**: keeping the hub free of delivery logic means it holds no per-connection
  state beyond SignalR's own group membership, and both delivery paths run through `IHubContext` from
  outside a connection. Per-user addressing is handled by SignalR's `IUserIdProvider` (a
  [ClaimBasedUserIdProvider](group-08-auth.md#claimbaseduseridprovider) that `AddPushNotifications`
  registers), so the hub never maps user ids itself.
- **Where it's used**: mapped at a settings-driven path by `MapNotificationHub()` (see
  [group 12](group-12-api-hosting-mapping.md)); addressed out-of-band by
  [SignalRPushNotificationSender](#signalrpushnotificationsender) (via `Clients.User`/`.Users`/`.All`)
  and [SignalRLiveChannelPublisher](#signalrlivechannelpublisher) (via `Clients.Group`). In ADC it is
  mapped by the Notification service host and fronted by the Gateway's `/hubs/*` route.
...
### NotificationModule
> MMCA.ADC.Notification.API · `MMCA.ADC.Notification.API` · `MMCA.ADC/Source/Modules/Notification/MMCA.ADC.Notification.API/NotificationModule.cs:16` · Level 3 · class (sealed)
...
- **Concept**: cross-reference the module system in
  ...
  ["Identity"]` with `RequiresDependencies => true` (lines 22, 25) because it needs the Identity
  ...
  service (the doc comment lines 13-14), so it implements `RegisterDisabledStubs` to keep that interface
  resolvable when the module is turned off.
- **Walkthrough**: three declarative members plus two actions.
  - `Name => "Notification"` (line 19); `Dependencies => ["Identity"]` (line 22);
    `RequiresDependencies => true` (line 25).
  - `Register(services, configuration, applicationSettings)` (line 28), whose whole body delegates to
    `services.AddNotificationModule(applicationSettings, (IConfiguration)configuration)` (line 32). Note
  ...
  - `RegisterDisabledStubs(services)` (line 35) registers
    `AddSingleton<IUserNotificationExportService, DisabledUserNotificationExportService>()` (line 36) so
...
- **Depends on**: [SendPushNotificationRequest](#sendpushnotificationrequest) (Level 0, the validated
  type), `PushNotificationInvariants` (the domain invariant constants, Level 4), and
...
- **Where it's used**: auto-discovered by the notification Application DI's
  `AddValidatorsFromAssemblyContaining` scan; invoked by the validating command decorator in the CQRS
  pipeline (against the embedded `Request`, via
  [ICommandWithRequest](group-05-cqrs-pipeline.md#icommandwithrequestout-trequest)) before
...
- **Walkthrough** (teaching order)
  ...
  2. **create the audit aggregate** (lines 43-47) via `PushNotification.Create(...)` with
     ...
     This is where the aggregate's `PushNotificationCreated` domain event is captured to the outbox
     (ADR-003).
  ...
  5. **best-effort native push** (lines 88-105, ADR-044): `nativePushSender.SendToUsersAsync(...)` in a
     ...
     **purely additive**: the SignalR leg above already decided the audit status, so a native-push
     ...
     default `NullNativePushSender` keeps this a no-op until a notification hub is configured.
...
- **Where it's used**: mapped by the Notification service's `Program.cs`; called by the Identity
  service's export aggregator, which stitches this Notification slice into the full data-subject export.
  When the Notification module is disabled, [DisabledUserNotificationExportService](#disabledusernotificationexportservice)
  (registered by [NotificationModule.RegisterDisabledStubs](#notificationmodule)) stands in for the
  in-process interface this service wraps.
...
- **Where it's used**: registered into the MVC application parts by [`DependencyInjection`](#dependencyinjection)'s `AddNotificationControllers`; called by the native-client login/logout flow.

---

### InboxController
> MMCA.Common.API · `MMCA.Common.API.Controllers.Notifications` · `MMCA.Common/Source/Presentation/MMCA.Common.API/Controllers/Notifications/NotificationInboxController.cs:31` · Level 4 · class (sealed)
...
- **Concept introduced, the feature-gated, handler-injecting controller.** `[Rubric §6, CQRS & Event-Driven]` assesses whether presentation delegates to command/query handlers rather than to service wrappers: this controller injects four handlers directly and only translates HTTP to a query/command and back (`NotificationInboxController.cs:31-36`). `[Rubric §9, API & Contract Design]` (consistent structure: `[ApiVersion("1.0")]`, `[FeatureGate]`, typed `[ProducesResponseType]`). `[Rubric §11, Security]` (the class-level `[Authorize(Policy = AuthorizationPolicies.RequireAuthenticated)]` at `NotificationInboxController.cs:30` enforces authentication; every action re-scopes to `currentUserService.UserId` so a user reads only their own inbox). The class-level `[FeatureGate(NotificationFeatures.PushNotifications)]` (`NotificationInboxController.cs:29`) makes the whole controller 404 when the flag is off, not stubbed.
- **Walkthrough**
  - Primary-constructor DI of two query handlers, two command handlers, and the current-user service (`NotificationInboxController.cs:31-36`).
  - `GetInboxAsync` (`NotificationInboxController.cs:41`): reads `pageNumber`/`pageSize` from `[FromQuery]` with `[Range(1, int.MaxValue)]` validation, guards a null user with `Error.Unauthorized`, builds a `GetMyNotificationsQuery(userId.Value, pageNumber, pageSize)`, and returns 200 with the paged result or `HandleFailure` (`NotificationInboxController.cs:46-56`). Default `pageSize = 20` (`NotificationInboxController.cs:43`).
  - `GetUnreadCountAsync` (`NotificationInboxController.cs:63`): marked `[ResponseCache(NoStore = true)]` (`NotificationInboxController.cs:61`) so the badge count is never cached; returns the raw `int` from `GetUnreadNotificationCountQuery`.
  - `MarkReadAsync` (`NotificationInboxController.cs:81`): route `{id:int}/read`, builds a `MarkNotificationReadCommand(id, userId.Value)`, returns 204 or a 404 `ProblemDetails`.
  - `MarkAllReadAsync` (`NotificationInboxController.cs:100`): no body; issues `MarkAllNotificationsReadCommand(userId.Value)` and returns 204.
...
> MMCA.Common.API · `MMCA.Common.API.Controllers.Notifications` · `MMCA.Common/Source/Presentation/MMCA.Common.API/Controllers/Notifications/NotificationsController.cs:31` · Level 4 · class (sealed)
...
- **Concept**: cross-reference [`InboxController`](#inboxcontroller) for the feature-gated, handler-injecting controller shape. What differs is the authorization boundary: `[Authorize(Policy = AuthorizationPolicies.RequireOrganizer)]` (`NotificationsController.cs:30`) restricts both actions to organizers. `[Rubric §11, Security]` (a distinct, stricter policy for the write side: only organizers may broadcast). `[Rubric §9, API & Contract Design]` (the `SendAsync` action returns `Created(...)` with a `Location` header pointing at the new resource, the REST-correct 201 shape).
- **Walkthrough**
  - Primary-constructor DI of the send command handler, the history query handler, and the current-user service (`NotificationsController.cs:31-34`).
  - `SendAsync` (`NotificationsController.cs:41`): guards a null user with `Error.Unauthorized`, wraps the `[FromBody] SendPushNotificationRequest` plus the caller's `userId` into a `SendPushNotificationCommand`, and on success returns `Created(new Uri($"/notifications/{result.Value!.Id}", UriKind.Relative), result.Value)` built with `CultureInfo.InvariantCulture` (`NotificationsController.cs:51-56`).
  - `GetHistoryAsync` (`NotificationsController.cs:62`): reads `[FromQuery]` paging (default `pageSize = 10`, `NotificationsController.cs:64`), runs `GetNotificationHistoryQuery`, and returns 200 with the paged `PushNotificationDTO` collection.
- **Why it's built this way**: the send path (a broadcast) is an organizer-privileged operation, so its policy is separated from the per-user inbox; keeping the two controllers apart lets each carry exactly its own authorization surface. Both stay behind `[FeatureGate(NotificationFeatures.PushNotifications)]` (`NotificationsController.cs:29`).
...
- **Concept introduced, the application-part registration for controllers shipped in a NuGet assembly.** ASP.NET Core scans only the host's own assembly for controllers by default; the notification controllers live in the `MMCA.Common.API` package, so they are invisible until their assembly is added as an application part. `AddNotificationControllers` calls `builder.AddApplicationPart(typeof(NotificationsController).Assembly)` (`DependencyInjection.cs:20`) to make all three controllers discoverable. `[Rubric §7, Microservices Readiness]` assesses whether a capability packages cleanly for reuse across hosts: shipping the controllers plus their own registration helper means any host adds the whole notification surface with one call, whichever service it is extracted into.
- **Walkthrough**: `AddNotificationControllers(this IMvcBuilder builder)` (`DependencyInjection.cs:18`): adds the application part (`DependencyInjection.cs:20`) and returns the builder for fluent chaining (`DependencyInjection.cs:21`).
- **Why it's built this way**: controllers cannot be auto-discovered from a referenced package without an explicit application part; exposing that as a named extension keeps host composition roots declarative. This is the API counterpart to the Application-layer `DependencyInjection` (`AddNotificationApplicationServices`) below.
...
- **Walkthrough**
  ...
  - `MapToDTOs(IReadOnlyCollection<PushNotification>)` (`PushNotificationDTOMapper.cs:20`): null-guards then projects the collection with `MapToDTO`.
...
### DependencyInjection
> MMCA.Common.Application · `MMCA.Common.Application.Notifications` · `MMCA.Common/Source/Core/MMCA.Common.Application/Notifications/DependencyInjection.cs:27` · Level 9 · class (static, extension)
...
- **Concept introduced, explicit `TryAdd` registration for a module that ships inside the framework assembly.** Most modules are auto-scanned by `ScanModuleApplicationServices<TAssemblyMarker>()`, but the Notification types live in `MMCA.Common.Application` itself, not a separate module assembly, so they are wired by hand here instead (`DependencyInjection.cs:36`). Every registration uses `TryAddScoped` (`DependencyInjection.cs:39-68`), the `TryAdd` semantic assessed under `[Rubric §3, Clean Architecture]`: a consuming app that registers its own handler or recipient provider *before* calling this helper keeps its override, because `TryAdd` never replaces an existing registration. `[Rubric §1, SOLID]` (Dependency Inversion): the default [`INotificationRecipientProvider`](group-10-notifications.md#inotificationrecipientprovider) is the no-op [`NullNotificationRecipientProvider`](group-10-notifications.md#nullnotificationrecipientprovider) (`DependencyInjection.cs:67-68`), documented as the seam a real app overrides to supply its own audience.
- **Walkthrough**: `AddNotificationApplicationServices()` (`DependencyInjection.cs:36`), inside `extension(IServiceCollection services)` (`DependencyInjection.cs:29`), registers in reading order:
  - The [`PushNotification`](group-10-notifications.md#pushnotification) aggregate's navigation populator ([`NullNavigationPopulator<PushNotification>`](group-11-navigation-populators.md#nullnavigationpopulatortentity)) and its [`EntityQueryService<...>`](group-03-querying-specifications.md#entityqueryservicetentity-tentitydto-tidentifiertype) (`DependencyInjection.cs:39-41`).
  - The [`PushNotificationDTOMapper`](#pushnotificationdtomapper), both as itself and as the `IEntityDTOMapper<...>` interface (`DependencyInjection.cs:44-46`).
  - Three command handlers (send, mark-one-read, mark-all-read) at `DependencyInjection.cs:49-54` and three query handlers (history, my-notifications, unread-count) at `DependencyInjection.cs:57-62`.
  - The FluentValidation validators from the [`SendPushNotificationRequestValidator`](group-10-notifications.md#sendpushnotificationrequestvalidator) assembly, `includeInternalTypes: true` (`DependencyInjection.cs:65`).
  - The default no-op recipient provider (`DependencyInjection.cs:68`), then returns the collection for chaining (`DependencyInjection.cs:70`).
- **Why it's built this way**: hand-registration keeps the notification sub-system self-contained inside the framework package while still honoring the framework-wide override contract (`TryAdd`), and the `extension(IServiceCollection)` C# preview syntax lets it read as a first-class `services.AddNotificationApplicationServices()` call (see [primer §4](00-primer.md#c-extensiont-types--read-this-once)).
- **Where it's used**: called from every consuming host's Application composition root, before `AddApplicationDecorators()` so the scanned handlers exist when the decorator pipeline wraps them. Its API counterpart is the `AddNotificationControllers` helper above.
