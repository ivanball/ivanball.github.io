...
### ConferenceTrackInfo
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Home` · `MMCA.ADC.Conference.UI/Pages/Home/ADCHome.razor.cs:273` · Level 0 · record (sealed, private)
...
- **Walkthrough**: a three-property positional record (`:273`). The whole catalogue is a `private static readonly ConferenceTrackInfo[] Tracks` (`:252`) initialized inline as a collection expression (`:253-270`) with eight entries (`:254`, `:256`, `:258`, `:260`, `:262`, `:264`, `:266`, `:268`), each spanning two lines: the track name and its icon constant on the first, the topics blurb on the second. They run from "Foundations (Beginner & Student)" (`:254`) to "Career, Leadership & Community" (`:268`), with the AI, languages, cross-platform, security, and game/XR tracks in between. Every `Icon` is a MudBlazor `Icons.Material.Filled.*` constant picked per track (`School`, `Psychology`, `AutoAwesome`, `Code`, `Devices`, `Security`, `SportsEsports`, `Groups`). Storing the icon as a `string` (rather than a `RenderFragment` or an enum) is what keeps the record a plain data type: the markup passes it straight to `<MudIcon Icon="@track.Icon">` (`ADCHome.razor:138`).
- **Why it's built this way**: the track list changes once per conference cycle and is editorial rather than transactional, so it lives in the assembly instead of behind an API call or a CMS; the `// i18n: allow` marker above the block names "the track catalog" alongside the keynote bio as deliberately English-only editorial content (`:239-240`). The array is `static readonly`, so it is allocated once per process, not per render.
- **Where it's used**: the `Tracks` array on [ADCHome](#adchome) (`:252`), rendered as the track grid in `ADCHome.razor` (`:130-147`): one `MudItem`/`MudCard` per entry, keyed by `track.Name` (`ADCHome.razor:133`), showing the icon (`:138`), the name (`:140`), and the topics line (`:142`).

### EventPhase
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Home` · `MMCA.ADC.Conference.UI/Pages/Home/ADCHome.razor.cs:46` · Level 0 · enum (private)
...
### KeynoteSpeakerInfo
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Home` · `MMCA.ADC.Conference.UI/Pages/Home/ADCHome.razor.cs:272` · Level 0 · record (sealed, private)

- **What it is**: the keynote block's content: the speaker's `Name`, `Title` (their role), the `TalkTitle`, an optional `PhotoFileName`, and `BioParagraphs` as a `string[]` (`:272`).
- **Depends on**: no first-party types. BCL only.
- **Concept introduced: the two-tier content model of a landing page.** The page splits its content into *dynamic* data fetched from the API (dates, venue, name, via [ADCEventInfo](#adceventinfo)) and *editorial* data compiled into the assembly (keynote and tracks, via this record and its sibling [ConferenceTrackInfo](#conferencetrackinfo)). [Rubric §23, Front-End Performance and Rendering] is the payoff: the keynote and the track grid render on the first frame with zero network dependency, so a cold or unreachable backend degrades only the countdown, never the page. [Rubric §27, Internationalization] is the deliberate exception: the block carries an explicit `// i18n: allow` marker with a written reason (`:239-240`) recording that this English-only editorial content is the same copy the API would serve, while the chrome around it is localized. That marker convention is how [ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html) distinguishes "not yet translated" from "intentionally untranslated".
- **Walkthrough**: a five-property positional record (`:272`). The single instance is a `private static readonly KeynoteSpeakerInfo Keynote` initialized inline (`:241-250`) with a three-paragraph biography (`:247-249`). `BioParagraphs` is an array rather than one string so the template can emit each paragraph in its own element instead of relying on whitespace preservation (`ADCHome.razor:103`). The record carries only the portrait's *file name*, not a path, and the current keynote passes `PhotoFileName: null` (`:245`): the usable `src` is composed from the head-specific `ImageBasePath` parameter through `KeynoteImageSrc`, which returns `$"{ImageBasePath}/speakers/{fileName}"` or `null` when no file name is supplied (`:34-35`). That split exists because the Web and MAUI heads serve static assets from different roots, and the `null` case is guarded in the markup so the card renders name and title without a portrait (`ADCHome.razor:83-93`).
...
- **Where it's used**: the `Keynote` field on [ADCHome](#adchome) (`:241`), rendered in the keynote section of `ADCHome.razor` (`:70-111`).

### ADCCollectionResult
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Home` · `MMCA.ADC.Conference.UI/Pages/Home/ADCHome.razor.cs:226` · Level 1 · record (sealed, private)
...
- **What it is**: the four dropdown option lists the organizer's session-selection filters bind to, `Statuses`, `Localities`, `Categories`, and `Levels` (`:11-15`), derived in one pass from a loaded decision-support dashboard. It is a pure projection with no state of its own: hand it the DTO, get four ready-to-bind string lists back.
- **Depends on**: [SessionSelectionDashboardDTO](group-17-conference-domain.md#sessionselectiondashboarddto) (its only input, imported at `:1` and taken at `:20`) and, through it, [SpeakerSessionOverlapDTO](group-17-conference-domain.md#speakersessionoverlapdto) with its [MultiSessionSpeaker](group-17-conference-domain.md#multisessionspeaker) and [SpeakerSessionSummary](group-17-conference-domain.md#speakersessionsummary) rows, [SessionAiScoreDTO](group-17-conference-domain.md#sessionaiscoredto), [SpeakerLocalitySummary](group-17-conference-domain.md#speakerlocalitysummary), and [CategoryDistributionDTO](group-17-conference-domain.md#categorydistributiondto) with its [CategoryGroupDistribution](group-17-conference-domain.md#categorygroupdistribution) and [CategoryItemDistribution](group-17-conference-domain.md#categoryitemdistribution) rows. Nothing else: no service, no MudBlazor, no Blazor type, just LINQ and `StringComparer.OrdinalIgnoreCase`.
- **Concept introduced**: nothing new. This is the third member of the "keep the page orchestration-only, put the rules in a plain type beside it" family, taught under [SessionSelectionDisplay](#sessionselectiondisplay) and [ScorePollTracker](#scorepolltracker); the class doc names both as its models (`:6-9`). [Rubric §19, State Management and Data Flow] assesses whether derived UI state is recomputed from one source of truth rather than accumulated: every option list here is a function of the currently loaded dashboard and is recomputed each time a dashboard is applied, so an option can never outlive the data that produced it, and a filter can never offer a value the current event does not contain. [Rubric §18, UI Architecture and Component Design] is the size argument: pulling this derivation out leaves the page's `ComputeFilterOptions` one factory call and four assignments (`.../SessionSelectionDashboard.razor.cs:191-198`). [Rubric §28, Front-End Testing and Quality] is available rather than exercised: the type is `internal`, and the project's `InternalsVisibleTo` opens it to the bUnit project (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/MMCA.ADC.Conference.UI.csproj:9`), so it could be asserted without rendering anything, but no test names it today.
- **Walkthrough**: a positional record of four `IReadOnlyList<string>` members (`:11-15`), plus a `private static readonly` `Empty` instance holding four empty lists (`:17`). The single entry point is the static factory `From(SessionSelectionDashboardDTO? dashboard)` (`:20`), which returns `Empty` for a null dashboard (`:22-25`), so the page binds to lists rather than nulls before its first load and after the organizer clears the event picker. The four derivations then differ in instructive ways:
  - **Statuses** (`:28-33`): flattened from `SpeakerOverlap.Speakers.SelectMany(s => s.Sessions)`, each `Status ?? "Accepted"`, then `Concat`ed with the same normalization over `AiScores`, and finally `Distinct` plus `Order`, both with `StringComparer.OrdinalIgnoreCase`. The `?? "Accepted"` is the same unset-means-accepted rule [SessionSelectionDisplay](#sessionselectiondisplay)`.SessionMatchesStatus` encodes on the matching side, so the option list and the predicate agree. The comment at `:27` records why the speaker-overlap collection is the base: it covers every multi-session speaker.
  - **Localities** (`:35-37`): a straight projection of `SpeakerLocality` to `LocalityTier`, ordered case-insensitively. No `Distinct`, because each locality summary row is already one tier.
  - **Levels** (`:39-44`): the level category is found by a case-insensitive `CategoryTitle.Contains("Level")` scan over `CategoryDistribution.Categories` (`:39-40`); when it exists, its items' `CategoryItemName`s become the list *unsorted and un-deduplicated* (`:42-43`), which is the one place the server's own ordering of the items is preserved rather than replaced by an alphabetical sort. When no such group exists the list is empty (`:44`).
  - **Categories** (`:46-51`): every *other* category group, excluded by `c.CategoryId != levelGroup.CategoryId` (`:47`), flattened to item names, then `Distinct` plus `Order` case-insensitively.
  The factory returns a new instance from the four lists (`:53`).
- **Why it's built this way**: deriving the options from the payload rather than hard-coding them means the dropdowns can only ever offer values that exist for the selected event, and splitting the "Level" group out of the general category list turns one distribution payload into two independent dropdowns. The lists deliberately carry no "All" entry: each `MudSelect` prepends its own empty-string All item (`.../SessionSelectionDashboard.razor:72`, `:82`, `:92`, `:102`), and empty string is the page's "no filter" sentinel (`.../SessionSelectionDashboard.razor.cs:40-44`). Note also what is *not* here: the fifth filter, score tier, is a fixed nine-entry localized list built on the page itself (`.../SessionSelectionDashboard.razor.cs:52-63`), because its thresholds are policy, not data.
- **Where it's used**: [SessionSelectionDashboard](#sessionselectiondashboard)`.ComputeFilterOptions` (`.../SessionSelectionDashboard.razor.cs:191-198`) is the only caller. It runs after a successful dashboard load (`:154`) and again on every poll that applies a fresh dashboard during AI scoring (`ApplyFreshDashboard`, `:347`), so newly scored sessions can widen the status list mid-run. The four lists populate the four `MudSelect` filters (`.../SessionSelectionDashboard.razor:70-108`), whose selected values flow down as the `FilterStatus` / `FilterLocality` / `FilterCategory` / `FilterLevel` parameters of [SessionSelectionSpeakerOverlap](#sessionselectionspeakeroverlap) and [SessionSelectionAiScores](#sessionselectionaiscores) (`.../SessionSelectionDashboard.razor:221-224`, `:231-234`).
- **Caveats / not-in-source**: the level group is identified by a substring match on the category title, so which category counts as "Level" depends on the conference categories seeded for the event, not on anything in this file; a differently titled level category yields an empty level list, and any other category whose title contains "Level" would be treated as the level group. Likewise, the `Distinct` on category item names merges same-named items belonging to different categories, which is consistent with how the children match (by name against a session's `CategoryItemNames`, `.../SessionSelectionSpeakerOverlap.razor.cs:63-64`) but means the dropdown cannot distinguish them.
...
- **Walkthrough**, in lifecycle order:
  ...
  - **`FormatEventDate`** (`:199-206`): formats the date with a pattern read from a *resource* (`L["Hero.DateFormat"]`) against `CultureInfo.CurrentCulture`, so both the layout and the month names follow the selected language ([ADR-027](https://ivanball.github.io/docs/adr/027-multi-locale-i18n.html)).
...
- **Concept introduced, the cache-bypassing dashboard/aggregate read service.** Unlike the CRUD services,
  ...
  gRPC/event boundary ([ADR-007](https://ivanball.github.io/docs/adr/007-grpc-extraction.html)), but the UI sees only this single typed contract.
...
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.ConferenceCategory` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/ConferenceCategory/ConferenceCategoryList.razor.cs:11` · Level 4 · class (partial component)
...
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Public` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Public/PublicSessionListView.razor.cs:21` · Level 4 · class (partial component)
...
### EventList
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Event` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Event/EventList.razor.cs:16` · Level 5 · class (partial component)
...
### PublicEventList
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Public` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Public/PublicEventList.razor.cs:17` · Level 5 · class (partial component)
...
### PublicSpeakerList
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Public` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Public/PublicSpeakerList.razor.cs:27` · Level 7 · class (partial component)
...
### PublicSessionList
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Public` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Public/PublicSessionList.razor.cs:25` · Level 8 · class (partial component)
...
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Question` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Question/QuestionList.razor.cs:11` · Level 5 · class (Blazor code-behind)
...
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Speaker` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Speaker/SpeakerDetail.razor.cs:19` · Level 6 · class (Blazor code-behind)
...
> MMCA.ADC.Conference.UI · `MMCA.ADC.Conference.UI.Pages.Speaker` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.UI/Pages/Speaker/SpeakerDashboard.razor.cs:19` · Level 7 · class (Blazor code-behind)
