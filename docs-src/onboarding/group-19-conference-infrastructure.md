...
- **Where it's used**: written by the Conference scoring pipeline: [`ScoreEventSessionsInternalCommandHandler`](group-18-conference-application.md#scoreeventsessionsinternalcommandhandler) drives one pass per event, calling [`SessionScoringService`](#sessionscoringservice) (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Infrastructure/Sessions/Scoring/SessionScoringService.cs:13`, the `IChatClient` adapter) which is itself checked by [`SessionScoreResponseGuardrail`](#sessionscoreresponseguardrail) (`MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Infrastructure/Sessions/Scoring/SessionScoreResponseGuardrail.cs:11`, which refuses a refusal, an empty answer, or a malformed score object) before a row lands here.
