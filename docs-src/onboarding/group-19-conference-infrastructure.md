...
and `MMCA.ADC.Conference.Infrastructure/Persistence/EntityConfiguration/ActivityConfiguration.cs:11-12`),
...
`EventInvariants.NameMaxLength` at `EventConfiguration.cs:19-21`, `SponsorInvariants.NameMaxLength` at
...
is indexed only where present (`EventConfiguration.cs:41-42`). Two further quirks are worth knowing:
...
(`MMCA.ADC/Source/Hosting/MMCA.ADC.AppHost/Program.cs:33`) with its own outbox, and cross-module
...
(`MMCA.ADC.Conference.Infrastructure/DependencyInjection.cs:22-24`), so it inherits the standard Aspire
...
registering that meter name (`MMCA.ADC/Source/Services/MMCA.ADC.Conference.Service/Program.cs:134`): that
...
(`MMCA.ADC.Conference.Infrastructure/Services/SessionScoringSweepJob.cs:54`) is the backstop for exactly
...
(`MMCA.ADC.Conference.Infrastructure/DependencyInjection.cs:12`) is a single
...
(`MMCA.ADC/Source/Services/MMCA.ADC.Conference.Service/Program.cs:313`); anywhere else the registration
...
(`MMCA.ADC.Conference.API/Controllers/SessionSelectionController.cs:110-131`),
...
- **Why it's built this way**: nesting it as a private record of the one class that speaks HTTP keeps it an implementation detail; making the score fields nullable turns "the model returned three of seven scores" into a detectable parse failure rather than a plausible-looking but wrong row in [SessionAiScore](group-17-conference-domain.md#sessionaiscore).
- **Where it's used**: [AnthropicScoringService](#anthropicscoringservice)`.ParseSingleScore` (`:87`) deserializes into it, and `BuildResult` (`:92`) converts it into a [SessionScoringResult](group-18-conference-application.md#sessionscoringresult).
...
- **Where it's used**: composed into [AnthropicResponse](#anthropicresponse)`.Content` (`:223-224`).
...
- **Walkthrough**: `required string Role` and `required string Content` (`:213-218`). Both are `required`, the mirror image of the response records' nullability: the adapter controls what it sends, so a half-built message is a compile error, while what comes back must be parsed defensively. Exactly one instance is ever constructed, with `Role = "user"` and the reviewer prompt as `Content` (`:44`).
...
> MMCA.ADC.Conference.Infrastructure · `MMCA.ADC.Conference.Infrastructure.Services` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Infrastructure/Services/SessionScoringSweepJob.cs:213` · Level 0 · record (internal sealed)
...
> MMCA.ADC.Conference.Infrastructure · `MMCA.ADC.Conference.Infrastructure.Services` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Infrastructure/Services/SessionScoringSweepJob.cs:208` · Level 0 · record (internal sealed)
...
- **Where it's used**: [AnthropicScoringService](#anthropicscoringservice)`.ScoreSessionAsync` (`:40-49`), handed to `JsonContent.Create`.
...
- **Where it's used**: [AnthropicScoringService](#anthropicscoringservice)`.ScoreSessionAsync` (`:59`).
...
- **Walkthrough**
  ...
  - `FailedResult` (`:179-192`) is the single shape of failure: all seven scores `0m`, `Reasoning = "Scoring failed"`, `Success = false`. Note that a zero sits outside the 1.0 to 10.0 band by construction, so a failed row is distinguishable from any real score.
...
> MMCA.ADC.Conference.Infrastructure · `MMCA.ADC.Conference.Infrastructure.Services` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Infrastructure/Services/SessionScoringSweepJob.cs:54` · Level 9 · class (sealed partial)
...
- **Why it's built this way**: registering the job in the module's own `DependencyInjection` rather than in a service host means the module carries its own recovery job, the way `AddAuditTrail` carries the framework's retention job (`DependencyInjection.cs:48-53`). The registration is inert in a host that does not turn the scheduler on, so a module can ship a job without forcing every host to run it.
- **Where it's used**: registered by [DependencyInjection](#dependencyinjection) via `services.AddScheduledJob<SessionScoringSweepJob>()` (`DependencyInjection.cs:54`); it only executes in a host that also calls `AddScheduledJobs`, which the Conference service host does.
...
> MMCA.ADC.Conference.Infrastructure · `MMCA.ADC.Conference.Infrastructure` · `MMCA.ADC/Source/Modules/Conference/MMCA.ADC.Conference.Infrastructure/DependencyInjection.cs:12` · Level 10 · class (static)
