...
- **Concept introduced, externalizing a filter's vocabulary through the options pattern.**
  ...
  is fixed in code while its identifiers are configuration) and `[Rubric §15, Best Practices & Code Quality]`
...
- **Concept introduced: a password is stored as a deliberately slow, salted one-way derivation.**
  ...
  carries no information about how close the guess was. `[Rubric §15, Best Practices & Code Quality]` applies to the
...
- **Concept introduced, centralized constants over magic strings.** `[Rubric §11, Security]` assesses
  authorization correctness and `[Rubric §15, Best Practices & Code Quality]` assesses duplication and single points
...
- **Concept introduced, the feature flag as a shared constant.** `[Rubric §30, Compliance / Privacy /
  ...
  surfaces are turned on. `[Rubric §9, API & Contract Design]` assesses whether concerns like feature
