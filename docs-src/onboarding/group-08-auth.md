...
together (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:176-182`).
...
(`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:184-191`), and the mapping
...
`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:170-174`).
...
(`EmailConfirmationTokenService.cs:141`), a constant-time comparison on redemption
(`EmailConfirmationTokenService.cs:103`), and every rejection collapsed into one generic failure
(`EmailConfirmationTokenService.cs:168`). The cached record is
[`EmailConfirmationEntry`](#emailconfirmationentry) (`EmailConfirmationTokenService.cs:180`), JSON
...
(`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:535`), which binds
...
bound at `DependencyInjection.cs:537-540`), registers the EF-backed
[`IPermissionGrantStore`](#ipermissiongrantstore) (`DependencyInjection.cs:545`), and decorates the
compiled registry with [`LayeredPermissionRegistry`](#layeredpermissionregistry)
(`DependencyInjection.cs:566-572`). The layering rule is a union with **no deny row**: a role holds a
...
whose mere registration is the opt-in the context reads (`DependencyInjection.cs:543`).
...
- **Where it's used**: `AddAuthorizationPolicies()` is called at the end of both framework
  ...
  (`MMCA.Common/Source/Presentation/MMCA.Common.API/Startup/WebApplicationBuilderExtensions.cs:570`,
  the call at `WebApplicationBuilderExtensions.cs:621`) and `AddCommonAuthentication`
  (`WebApplicationBuilderExtensions.cs:638`, the call at `WebApplicationBuilderExtensions.cs:672`).
...
- **Concept**: the same cache-DTO rule [PasswordResetEntry](#passwordresetentry) demonstrates: a value round-tripped through `System.Text.Json` in a distributed cache must be a JSON primitive, so the token digest travels as Base64 text (`TokenHashBase64`, `:181`) rather than as `byte[]`, and the expiry as Unix seconds (`ExpiresAtUnixSeconds`, `:184`) rather than as `DateTimeOffset`. `[Rubric §11, Security]` applies through the same member name convention: `TokenHashBase64`, never `Token`, so the record cannot hold the redeemable secret it guards.
- **Walkthrough**: four members, the same shape as [PasswordResetEntry](#passwordresetentry):
  - `string TokenHashBase64` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:181`): the SHA-256 digest of the issued token, Base64-encoded. Validation re-hashes the presented token and compares digests.
  - `UserIdentifierType UserId` (`:182`): the account the token confirms.
  - `int FailedAttempts` (`:183`): wrong tokens presented against this record so far.
  - `long ExpiresAtUnixSeconds` (`:184`): when the record expires; a failed-attempt rewrite recaches with the **remaining** lifetime computed from this field so a wrong guess cannot extend how long the token stays redeemable (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:816-818`).
- **Why it's built this way**: being a `record` gives the `with` expression the attempt-counter rewrite relies on (`entry with { FailedAttempts = attempts }`, `MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:816`). Being `internal` keeps the cache layout out of the package's public API.
- **Where it's used**: written by `IssueAsync` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:722-728`), read by `ValidateAndConsumeAsync` (`:740`), and rewritten by `RecordFailedAttemptAsync` (`:814-818`). It appears nowhere else.
...
- **Where it's used**: registered as `services.TryAddSingleton<IJwksProvider, RsaJwksProvider>()` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:202`) immediately after the `JwksSettings` options binding (`:165-168`); the JWKS minimal-API endpoint calls it, and consuming services fetch the resulting document through `AddForwardedJwtBearer` at startup (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/IJwksProvider.cs:9`).
...
- **Where it's used**: bound with `.ValidateDataAnnotations().ValidateOnStart()` in `AddInfrastructure` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:198-201`), immediately followed by the [IJwksProvider](#ijwksprovider) registration (`:183`). [TokenService](#tokenservice) takes it as an optional constructor dependency and falls back to `new JwksSettings().KeyId` when it is absent (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/TokenService.cs:80`).
...
- **Walkthrough**: `HS256 = 0` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/JwtSigningAlgorithm.cs:24`) and `RS256 = 1` (`:27`), both with explicit ordinals.
  ...
  - The API layer branches on the same value when configuring in-process JWT bearer validation: `BuildValidationParameters` takes the RSA path for RS256 (`MMCA.Common/Source/Presentation/MMCA.Common.API/Startup/WebApplicationBuilderExtensions.cs:728-741`) and, when the public key is absent, throws a message that points the reader at `AddForwardedJwtBearer` for services that fetch the key through JWKS at runtime instead (`:633-636`).
...
- **Concept introduced: `IValidatableObject` for conditional requirements.** Attributes describe a property in isolation, so they cannot say "this one is required only when that one has a particular value". `IValidatableObject` is the options-validation extension point for exactly that case: the type implements a single `Validate` method that yields one `ValidationResult` per failure, and `.ValidateDataAnnotations()` runs it alongside the attribute checks. This class is the framework's canonical example, and says so in its own doc (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/JwtSettings.cs:13-14`).
  ...
  `[Rubric §15, Best Practices & Code Quality]` assesses fail-fast posture. Registration pairs the bind with `.ValidateDataAnnotations().ValidateOnStart()` (`MMCA.Common/Source/Presentation/MMCA.Common.API/Startup/WebApplicationBuilderExtensions.cs:640-643`), so both the attribute checks and `Validate` run at boot, not on the first token issued ([ADR-070](https://ivanball.github.io/docs/adr/070-fail-fast-configuration-contract.html)).
- **Walkthrough**: one static field, eight `init` properties, one method.
  ...
  - The in-process validator enforces the other half at wiring time instead: `BuildValidationParameters` throws when RS256 is selected with no `RsaPublicKeyPem`, and the message points at `AddForwardedJwtBearer` for services that should fetch the key at runtime (`MMCA.Common/Source/Presentation/MMCA.Common.API/Startup/WebApplicationBuilderExtensions.cs:728-741`).
...
- **Where it's used**: bound in `AddCommonAuthentication` (`MMCA.Common/Source/Presentation/MMCA.Common.API/Startup/WebApplicationBuilderExtensions.cs:640-643`), which then re-reads the section eagerly to build the token validation parameters at wiring time (`:549-552`); consumed by [TokenService](#tokenservice) through `IOptions<JwtSettings>`, which branches on the algorithm once in the constructor and caches the credentials (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/TokenService.cs:64-88`).
...
- **Where it's used**: registered as `services.TryAddSingleton<IPasswordHasher, PasswordHasher>()`
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:724`); the type is
...
- **Why it's built this way**:
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:717-723`): the comment
...
- **Where it's used**: registered as `services.TryAddSingleton<ITokenService, TokenService>()`
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:723`), which resolves
...
> MMCA.Common.Infrastructure · `MMCA.Common.Infrastructure.Auth` · `MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:35` · Level 5 · class (sealed)
...
- **Depends on**: `IEmailConfirmationTokenService` (the Application port it implements); [EmailConfirmationSettings](group-08-auth.md#emailconfirmationsettings) via `IOptions<>` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:692`, snapshotted at `:696`); [ICacheService](group-09-caching.md#icacheservice) (`:691`); [EmailConfirmationEntry](#emailconfirmationentry), its cached record; [Result](group-01-result-error-handling.md#result) and [Error](group-01-result-error-handling.md#error); the [Email](group-02-domain-building-blocks.md#email) value object as a normalizer (`:785`); and from the BCL `SHA256`, `RandomNumberGenerator`, `CryptographicOperations`, and `System.Buffers.Text.Base64Url`.
- **Concept introduced: the same cache-backed token pattern, applied a third time.** `[Rubric §11, Security]` assesses credential issuance and redemption; `[Rubric §15, Best Practices & Code Quality]` assesses whether a proven idiom is reused or reinvented per flow. This type is structurally the same shape as [PasswordResetTokenService](#passwordresettokenservice): hash the token at rest, throttle issuance per address, cap validation attempts, and consume both the token and the request counter together on success. The `NormalizeIdentity` doc comment says so directly, citing [PasswordResetTokenService](#passwordresettokenservice) as the precedent for normalizing the cache key the same way [Email](group-02-domain-building-blocks.md#email) normalizes the lookup value (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:773-777`). The same read-modify-write gap [LoginProtectionService](#loginprotectionservice) documents on its own throttle is inherited here too: the comment at the top of `IssueAsync` names it explicitly (`:704-705`).
- **Walkthrough**
  - `TokenByteLength = 32` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/Auth/EmailConfirmationTokenService.cs:694`) is the only constant; `_settings` is the snapshotted `EmailConfirmationSettings` (`:696`).
  - `IssueAsync(string email, UserIdentifierType userId, CancellationToken)` (`:699-731`): increments the per-email request counter with the configured `RequestWindowMinutes` TTL (`:706-709`) and fails with `Authentication.ConfirmationThrottled` once the count exceeds `MaxRequestsPerEmail` (`:711-717`). It then mints 32 CSPRNG bytes rendered with `Base64Url.EncodeToString` (`:719`), builds an [EmailConfirmationEntry](#emailconfirmationentry) holding the Base64 digest of the token, the user id, a zero attempt count, and the absolute expiry as Unix seconds (`:722-726`), caches it under the token key with the configured lifetime (`:728`), and returns the raw token to the caller (`:730`); the raw token is never written anywhere else.
  - `ValidateAndConsumeAsync(string email, string token, CancellationToken)` (`:734-770`): loads the entry and returns `InvalidToken()` when there is none (`:740-744`); a `FormatException` decoding the stored Base64 removes the unreadable record rather than leaving it to expire (`:747-756`); the comparison is `CryptographicOperations.FixedTimeEquals` over the two digests (`:758`), with `token ?? string.Empty` so a null token hashes rather than throwing; a mismatch records a failed attempt and returns the same generic failure (`:760-762`). On a match it removes **both** the token key and the address's request counter (`:766-767`), so a confirmed address is not left throttled out of a later legitimate request, and returns the entry's `UserId` (`:769`).
  - `NormalizeIdentity`, `TokenKey`, `RequestKey`, `HashToken` (`:780-796`): the same [Email](group-02-domain-building-blocks.md#email)`.Create`-then-fallback shape as [PasswordResetTokenService](#passwordresettokenservice), producing `emailconfirm:token:{normalized}` and `emailconfirm:req:{normalized}` keys.
  - `RecordFailedAttemptAsync` (`:798-819`): computes `attempts = entry.FailedAttempts + 1` and the remaining lifetime from `ExpiresAtUnixSeconds` (`:803-804`). At `MaxValidationAttempts`, or once the remaining lifetime is non-positive, it deletes the record (`:806-810`); otherwise it rewrites the entry with `entry with { FailedAttempts = attempts }` at the **remaining** TTL, not a fresh one (`:814-818`).
  - `InvalidToken()` (`:821-823`) is the single failure factory built from `EmailConfirmationErrors.InvalidToken`, so unknown, expired, mismatched and attempt-capped tokens collapse to one error rather than giving an oracle for which addresses have an outstanding confirmation.
...
- **Where it's used**: registered `services.TryAddScoped<IEmailConfirmationTokenService, EmailConfirmationTokenService>()` (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:506`), directly after the `EmailConfirmationSettings` binding (`:500-501`).
...
- **Where it's used**: broadly, and always instead of a hand-rolled claim lookup.
  ...
  (`MMCA.Common/Source/Presentation/MMCA.Common.API/Startup/WebApplicationBuilderExtensions.cs:182`);
...
- **Where it's used**: registered by assembly scan.
  ...
  auto-registered for every such command at `DependencyInjection.cs:260-276`.
...
- **Where it's used**: registered by the assembly scan
  (`MMCA.Common/Source/Core/MMCA.Common.Application/DependencyInjection.cs:51`).
...
- **Where it's used**: registered as
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:846`), in the same block
...
- **Concept introduced: a scoped identity snapshot, computed lazily and parsed invariantly.**
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:716`), the claim walk
...
- **Where it's used**: registered as
  ...
  (`MMCA.Common/Source/Core/MMCA.Common.Infrastructure/DependencyInjection.cs:716`). The
