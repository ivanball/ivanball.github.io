...
| 3 | #26 | Front-End Security | 3 | 8 | the CSP `script-src`/`style-src` gap recorded as the unclosed 8→9 reason (re-verified live 2026-08-23, fifth consecutive hold: `Aspire/Security/SecurityHeaders.cs:43-44` documents the deliberate omission and `:48` is the static policy string, still with neither directive; the only full policy remains the opt-in Blazor provider, unchanged since 2026-07-05) |
...
- [ ] **FR-5 (§8) - Cascade soft-delete semantics.** Soft-deleting an aggregate root leaves its
  ...
  *(Effort M-L, design first.)* *(Re-verified open in code 2026-08-23, twenty-eighth-wave §8
  ...
  `AuditableBaseEntity.cs:47-60`, and a content search across Infrastructure/Persistence returns
  no cascade handling.)*
...
- [ ] **Closed by the #32 pin fix** + add a per-release **CHANGELOG**. *(Left open 2026-08-23: the
  ...
  re-read this run, so no claim is made about it.)*
