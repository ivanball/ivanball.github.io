...
(2026-09-21 thirty-eighth wave, scoped §16 re-score: **membership unchanged, one row moves**: #16 drops from
implPriority 6 to 2 on its Implementation 6→8 lift, so the band reads 13 categories / 29 gap points, and #16 also
leaves the maturity band at M4; no other category was re-scored this cycle; prior thirty-fifth wave, 2026-09-15: **membership unchanged, one row moves**: #16 drops from implPriority 8 to 6 on its
...
implPriority 4 to 2 on its Implementation 7→8 lift): **13 categories, 29 gap points** (33 before the thirty-eighth wave, 35 before the thirty-fifth, 30
...
| 2 | #16 | AI-Native Application Architecture | 2 | 8 | named 2026-09-21 (thirty-eighth wave, after the 6→8 lift): ship an injection defense or a content policy beyond contact-detail redaction behind the guardrail extension point (`Guardrails/PiiRedactionGuardrail.cs:11`; ADR-120 `:330` puts injection handling and response-schema constraints in the consuming feature), the one partial criterion the framework still owns; retrieval is scoped out by ADR-120 `:344`, so a 9 is capped by design unless retrieval enters scope. Superseded lever, named 2026-09-15 (thirty-fifth wave, after the 5→6 lift) and **shipped in v1.207.0**: close the two partial criteria the framework owns, extend `GuardrailChatClient` to inspect the streamed response and not only the request (`Source/Core/MMCA.Common.AI/Chat/GuardrailChatClient.cs:65`) and add per-caller tool authorization with a confirmation path on the bounded client in place of the blanket strip (`Chat/BoundedChatClient.cs:155`), both pinned by tests in the CI-gated AI test project; effort M. The two absent criteria (an evaluation gate in CI, retrieval as data architecture) stay feature-side scope-outs per ADR-120's 2026-09-15 revision (`120-governed-chat-client-boundary.md:226`) and are not scheduled here. Provenance: scored for the first time on 2026-09-11 at I5 with no lever named; the package is still off by default (`DependencyInjection.cs:98`, anchor corrected 2026-09-15 from `:90`), and what is automatically enforced is still only the dependency direction, `Tests/Architecture/MMCA.Common.Architecture.Tests/Layering/AiDependencyIsolationTests.cs:14` over `Source/Build/MMCA.Common.LayerEnforcement.targets:86`. #16 is also in the maturity band, now at priority 2 |
