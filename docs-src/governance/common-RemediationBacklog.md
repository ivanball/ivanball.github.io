...
| 3 | #11 | Security | 3 | 8 | not yet identified (a first-pass lift was refuted on the twenty-first wave; the 2026-08-07 cycle recorded a held-at-prior 8→9 with verdict FLAG; a further 8→9 was refuted 2026-08-23: both stated caps, deployer-owned vault secret binding and RBAC-not-ABAC, verbatim unchanged, and the new CI-gated security fitness work credits Maturity, already 4) |
| 3 | #19 | State Management & Data Flow | 3 | 8 | not yet identified (a first-pass lift was refuted on the twenty-first wave) |
| 3 | #26 | Front-End Security | 3 | 8 | the CSP `script-src`/`style-src` gap recorded as the unclosed 8→9 reason (re-verified live 2026-08-31, sixth consecutive hold, anchors corrected: `Aspire/Security/SecurityHeaders.cs:42-45` documents the deliberate `script-src`/`style-src` omission and the static policy string is the initializer at `:47-48`, the policy at `:48` still carrying neither directive; the only full policy remains the opt-in Blazor provider, unchanged since 2026-07-05) |
...
| 2 | #13 | Observability & Operability | 2 | 8 | SLO-breach alerting with dashboards, plus runbooks for common failures, the two rubric criteria left to the deployer (the recorded reason the 8→9 lift was refuted on the twenty-fourth wave, and again on 2026-08-23, the fifth consecutive refusal: no criterion-closing evidence since 2026-08-14) |
...
| 2 | #25 | Navigation & Information Arch | 2 | 8 | typed and validated deep-link route parameters beyond list-state (framework routes are plain string templates with only the sanitized `?returnUrl=`), the recorded reason the 8→9 lift was refuted on the twenty-fourth wave; on the 2026-08-07 cycle the scorer returned no fresh score (verdict FLAG) and the row is held at prior Implementation 8 by merged-prior policy: the verifier re-confirmed the hold-reason literally true in source (all routes parameterless, `NavigationFlow.md:13-21` at that time), re-adjudicate next re-score; re-verified 2026-08-31: the route table is now **twelve** routes at `NavigationFlow.md:13-24`, all still parameterless string templates with only query-string prefills (`?returnUrl=` at `:14`, `?email=`/`?token=` at `:17`), so the hold reason (no typed or validated deep-link route parameters) is re-confirmed and only the count and the line range drifted |
...
- [ ] **FR-5 (§8) - Cascade soft-delete semantics.** Soft-deleting an aggregate root leaves its
  ...
  *(Effort M-L, design first.)* *(Re-verified open in code 2026-08-23, twenty-eighth-wave §8
  adversarial pass: `Delete()` flips `IsDeleted` on the single entity only,
  `AuditableBaseEntity.cs:47-60`.)* *(Re-verified open 2026-08-31, twenty-ninth-wave §8 pass, and the
  earlier "no cascade soft-delete helper anywhere in `Source/`" wording is corrected here: an opt-in
  `DeleteChildren<TChild,TChildId>` cascade helper now ships (`AuditableAggregateRootEntity.cs:273-292`,
  unit-tested) alongside a `DeleteEntityHandler.Includes`/`AsTracking` eager-load extension point
  (`DeleteEntityHandler.cs:57,63`), but no aggregate under `Source/` calls the helper (a repo-wide grep
  finds usages only in Domain tests) and the default `Delete()` still flips `IsDeleted` on the single
  entity only (`AuditableBaseEntity.cs:67-80`), so the FR-5 gap (soft-deleting a root leaves its
  children live by default) persists and the item stays OPEN.)*
