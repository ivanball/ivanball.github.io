...
Derived from `ArchitectureScorecard.md` (canonical two-axis scoring: **Maturity 96.9% / Implementation 84.8%**, framework v1.135.0; the 2026-08-01 twenty-fifth-wave two-pass re-score at HEAD `f292233`, working tree clean, moved **one score**, ending the four-cycle steady state: **§10 Cross-Cutting Concerns Implementation 8→9** (685 → 687/810), on new shipped evidence: the idempotency guard now acquires an `IDistributedLock` (`RedisDistributedLock`, SET NX PX + compare-and-delete release) instead of the in-memory semaphore. Four first-pass lifts were refuted on adversarial re-verification and held at prior: §2 I9→10 and §15 I9→10 (a live rubric red flag, and a `NoWarn` surface that grew), §17 3/8→4/9 (compile-only Bicep job, not a required context), and §31 I7→8 (third refutation on byte-identical evidence). **Categories still below Maturity 4: #9, #17, #30 (M3) and #31 (M2)**, so 30 of 34 sit at Maturity 4; the maturity band is unchanged (#31 at priority 4, then #9, #17 and #30 at priority 2 each) and the implementation band shrinks to **17 categories, 42 gap points** (#10 left the band at M4/I9). The prior twenty-fourth-wave entry, retained for provenance: the 2026-07-28 re-score at HEAD `2c52aa9` moved no scores, the fourth consecutive cycle at 96.9% / 84.6%, refuting five lifts (§9 3→4, §30 3→4, §13 8→9, §25 8→9, §29 8→9); its only new work was doc hygiene under #34. The twenty-third-wave entry, likewise retained: the 2026-07-25 re-score at HEAD `3dff29b` also moved no scores, refuting four lifts (§17 on both axes, §30, §31, §34) and one downgrade (§23). The twenty-second-wave entry, likewise retained: the 2026-07-23 re-score at HEAD `c911480` also moved no scores, refuting three lifts (§9, §10, §30) and two downgrades (§23, §24). The twenty-first-wave entry, likewise retained: the 2026-07-21 re-score at HEAD `4a4fc05` moved exactly one score, **§12 Performance & Scalability Maturity 3→4**, because the `Performance gate (BenchmarkDotNet Short + baseline verify)` context is present in live `required_status_checks` on `main` (8 required contexts), refuting the twentieth wave's sole basis for holding it at 3; seven first-pass lifts (§4, §9, §11, §19, §20, §26, §29) were refuted and the remaining 26 categories re-confirmed.)
...
(2026-08-01 twenty-fifth wave, one implementation move: #10 8→9, the band's first closure since it
was ranked): **17 categories, 42 gap points**.
