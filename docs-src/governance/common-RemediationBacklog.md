...
and the gap between them (97.0% vs 86.0% today) looked like neglect. Ranked from the current scorecard
(2026-09-15 thirty-fifth wave: **membership unchanged, one row moves**: #16 drops from implPriority 8 to 6 on its
...
implPriority 4 to 2 on its Implementation 7→8 lift): **13 categories, 33 gap points** (35 going into this cycle, 30
...
at the next re-score, never invented here. Four of these categories (#16, #17, #30, #31) also sit in the
...
- *Lever (named 2026-09-15, effort M):* close the two partial criteria the framework owns: extend `GuardrailChatClient` to inspect the streamed response as well as the request (`Chat/GuardrailChatClient.cs:65`), and add per-caller tool authorization with a confirmation path on the bounded client in place of the blanket strip (`Chat/BoundedChatClient.cs:155`), both pinned by tests in the CI-gated AI test project. The evaluation gate and retrieval stay feature-side per ADR-120 (`120-governed-chat-client-boundary.md:226`); M4 needs a prompt-change or evaluation process enforced in-repo, which the ADR scopes out, so no maturity lever is scheduled.
