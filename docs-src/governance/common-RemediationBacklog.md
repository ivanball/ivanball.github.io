...
and the gap between them (96.9% vs 84.8%) looked like neglect. Ranked from the current scorecard
(2026-08-14 twenty-seventh wave: no implementation moves; nine rows (#5, #11, #13, #17, #25, #26, #29,
#30, #34) came back FLAG and are held at prior by merged-prior policy after adversarial refutation of
their proposed lifts, not re-confirmed fresh; the band's only closure to date remains #10's on the
twenty-fifth wave): **17 categories, 42 gap points**.
...
| 3 | #11 | Security | 3 | 8 | not yet identified (a first-pass lift was refuted on the twenty-first wave; the 2026-08-07 cycle recorded a further held-at-prior 8→9 with verdict FLAG: both stated caps still open, no fresh re-confirmation claimed) |
| 3 | #19 | State Management & Data Flow | 3 | 8 | not yet identified (a first-pass lift was refuted on the twenty-first wave) |
| 3 | #26 | Front-End Security | 3 | 8 | the CSP `script-src`/`style-src` gap recorded as the unclosed 8→9 reason (re-verified live 2026-08-07 at `Aspire/Security/SecurityHeaders.cs:42-48`; a proposed 8→9 was held at prior, re-litigating the eighteenth- and twenty-first-wave adjudications on byte-identical evidence) |
...
| 2 | #13 | Observability & Operability | 2 | 8 | SLO-breach alerting with dashboards, plus runbooks for common failures, the two rubric criteria left to the deployer (the recorded reason the 8→9 lift was refuted on the twenty-fourth wave) |
| 2 | #16 | Maintainability & Evolvability | 2 | 8 | not yet identified |
| 2 | #17 | DevOps & Deployment | 2 | 8 | the bicep job is a compile check and is not among the required contexts (the recorded reason the 8→9 lift was refuted on the twenty-third wave, and a 4/9 proposal was refuted for the third consecutive cycle on 2026-08-07, on byte-identical evidence) |
...
| 2 | #25 | Navigation & Information Arch | 2 | 8 | typed and validated deep-link route parameters beyond list-state (framework routes are plain string templates with only the sanitized `?returnUrl=`), the recorded reason the 8→9 lift was refuted on the twenty-fourth wave; on the 2026-08-07 cycle the scorer returned no fresh score (verdict FLAG) and the row is held at prior Implementation 8 by merged-prior policy: the verifier re-confirmed the hold-reason literally true in source (all ten routes parameterless, `NavigationFlow.md:13-21`), re-adjudicate next re-score |
...
- ✅ **FR-1 through FR-7 re-verified open on exact anchors.** The single Infrastructure package
  ...
  DI surface (`Application/DependencyInjection.cs:22`), no cascade soft-delete helper anywhere in
  `Source/`, and `MMCA.Common.UI.Maui` still shipping with no test project of its own. FR-7's anchor
...
- 📌 **Live consumer-state note (not an accepted cap; updated 2026-08-14):** the framework is at
  **v1.152.0** (`FACTS.md:4,14`) and BOTH consumers are re-converged on it:
  `MMCA.Store/Directory.Packages.props:9-10` and `MMCA.ADC/Directory.Packages.props:96` both pin
  1.152.0 (the ADC anchor moved from `:128` to `:96`; the 2026-08-07 in-flight sweep, ADC PR #106,
  is closed). Lockstep is intact. See the refreshed parenthetical under the skipped-versions
  acceptance below.
...
- [ ] **FR-7 (§34) - CS1591 ratchet.** XML doc coverage is enforced by convention, not the
  compiler: `CS1591` sits in `NoWarn` (`Directory.Build.props:22`, anchor corrected 2026-07-28; re-verified open 2026-08-07 and again 2026-08-14, anchor exact: `<NoWarn>$(NoWarn);CS1591;RMG020;S8970</NoWarn>`), so a public member can ship
...
- [ ] **CD-2 (§9/§15) - the lookup projection cannot translate value-object properties and throws
  ...
  *(Re-verified open 2026-08-14, twenty-seventh-wave re-score, with an ANCHOR CORRECTION: the
  ...
  defect itself is unchanged and still open.)*
...
**Fix**
- [x] Add an optional **`Func<bool>?` live-accessor** parameter so the guard reads current dirty state at navigation time. → shipped: `UnsavedChangesGuard.razor:33` (`[Parameter] public Func<bool>? IsDirtyAccessor`), `:35` (`CurrentIsDirty => IsDirtyAccessor?.Invoke() ?? IsDirty`), `:51` (the guard reads `CurrentIsDirty` at navigation time). Verified 2026-08-14.
- [x] Cover with a **bUnit test**. → `Tests/Presentation/MMCA.Common.UI.Tests/Components/UnsavedChangesGuardTests.cs:17,38` covers both accessor states. *(Ticking these sub-items is not a category closure: the #19 heading stays open in the implementation band at M4/I8, implPriority 3.)*
