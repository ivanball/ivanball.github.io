...
Derived from `ArchitectureScorecard.md` (canonical two-axis scoring: **Maturity 96.9% / Implementation 84.6%**, framework v1.131.0; the 2026-07-28 twenty-fourth-wave two-pass re-score at HEAD `2c52aa9`, working tree clean, moved **no scores**: all 34 categories re-confirmed at their twenty-third-wave values, the fourth consecutive cycle at these indices. Five first-pass lifts were refuted on adversarial re-verification and held at prior: two maturity (§9 3→4, §30 3→4) and three implementation (§13 8→9, §25 8→9, §29 8→9). **Categories still below Maturity 4: #9, #17, #30 (M3) and #31 (M2)**, so 30 of 34 sit at Maturity 4, and both rankings are unchanged (maturity band: #31 at priority 4, then #9, #17 and #30 at priority 2 each; implementation band: 18 categories, 44 gap points). The cycle's only new work is doc hygiene under #34: this backlog and the scorecard had drifted three minor versions behind the CI-gated `FACTS.md` again. The prior twenty-third-wave entry, retained for provenance: the 2026-07-25 re-score at HEAD `3dff29b` also moved no scores, refuting four lifts (§17 on both axes, §30, §31, §34) and one downgrade (§23). The twenty-second-wave entry, likewise retained: the 2026-07-23 re-score at HEAD `c911480` also moved no scores, refuting three lifts (§9, §10, §30) and two downgrades (§23, §24). The twenty-first-wave entry, likewise retained: the 2026-07-21 re-score at HEAD `4a4fc05` moved exactly one score, **§12 Performance & Scalability Maturity 3→4**, because the `Performance gate (BenchmarkDotNet Short + baseline verify)` context is present in live `required_status_checks` on `main` (8 required contexts), refuting the twentieth wave's sole basis for holding it at 3; seven first-pass lifts (§4, §9, §11, §19, §20, §26, §29) were refuted and the remaining 26 categories re-confirmed.)
...
Higher priority = bigger weighted gap = more index points per unit of effort. A category leaves each band independently and reaches the protect list only at **maturity 4 AND implementation >= 9**. The indices keep their `× 4` and `× 10` denominators, so the trend line stays comparable across all twenty-four waves; the 9-target governs scheduling only.
...
(2026-07-28 twenty-fourth wave, no score moves): **18 categories, 44 gap points**. Against the
...
| 3 | #29 | Resilience & Business Continuity | 3 | 8 | tested restores, RTO/RPO per service, and measured production SLOs, recorded by the framework's own guide as consumer-IaC work (`common-RESILIENCE.md:3,27`); the 8→9 lift was refuted on the twenty-first and twenty-fourth waves |
...
| 2 | #25 | Navigation & Information Arch | 2 | 8 | typed and validated deep-link route parameters beyond list-state (framework routes are plain string templates with only the sanitized `?returnUrl=`), the recorded reason the 8→9 lift was refuted on the twenty-fourth wave |
...
- [ ] **FR-7 (§34) - CS1591 ratchet.** XML doc coverage is enforced by convention, not the
  compiler: `CS1591` sits in `NoWarn` (`Directory.Build.props:22`, anchor corrected 2026-07-28), so a public member can ship
