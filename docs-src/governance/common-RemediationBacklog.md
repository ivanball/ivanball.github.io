...
(2026-08-31 thirtieth wave: **#8, #19, #25 and #26 close at Implementation 9 and leave the band**, the
band's first closures since #10 on the twenty-fifth wave; #11, #13 and #23 came back FLAG and are held
at prior by merged-prior policy after adversarial refutation of their proposed lifts, not re-confirmed
fresh): **13 categories, 31 gap points**.
...
| 2 | #13 | Observability & Operability | 2 | 8 | SLO-breach alerting with dashboards, plus runbooks for common failures, the two rubric criteria left to the deployer (the recorded reason the 8→9 lift was refuted on the twenty-fourth wave and on every cycle since; held at prior on FLAG on the thirtieth wave, 2026-08-31, the seventh consecutive refusal: Common's own sample IaC declares an action group but no `Microsoft.Insights/metricAlerts` resource despite its own comment, `samples/deployment/main.bicep:155`, and the only runbook remains the #29 restore drill) |
