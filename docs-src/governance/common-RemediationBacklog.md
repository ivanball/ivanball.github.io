...
(2026-08-07 twenty-sixth wave: no implementation moves, six proposed lifts refuted and held at prior;
the band's only closure to date remains #10's on the twenty-fifth wave): **17 categories, 42 gap points**.
...
- ⚠️ **FR-1 through FR-6 and C-1..C-7 not individually re-verified this cycle.** They stay open
  ...
  forward to 2026-08-07 rather than being silently re-confirmed.
...
- 📌 **Live consumer-state note (not an accepted cap):** the framework is at v1.142.0; MMCA.Store
  pins 1.142.0 (`Directory.Packages.props:9`) while MMCA.ADC pins 1.141.0
  (`Directory.Packages.props:128`) with its bump open as **ADC PR #106**, an in-flight sweep. See
  the refreshed parenthetical under the skipped-versions acceptance below.
...
- [ ] **FR-7 (§34) - CS1591 ratchet.** XML doc coverage is enforced by convention, not the
  compiler: `CS1591` sits in `NoWarn` (`Directory.Build.props:22`, anchor corrected 2026-07-28; re-verified open 2026-08-07, anchor exact), so a public member can ship
...
- [x] Ship an optional **EF-backed inbox/dedup filter** keyed on a message id; add a unique **event Id** to base events. → shipped: `Source/Core/MMCA.Common.Infrastructure/Persistence/Inbox/EfInboxStore.cs:18` (ADR-021; verified 2026-08-07). Opt-in and off by default (`MessageBusSettings.EnableInbox`), which is the recorded reason §6's Implementation holds at 9.
- *(Heading left open 2026-08-07: §6 scores M4/I9 and is protect-eligible, but its twenty-sixth-wave verdict was FLAG (a refuted 9→10, not a fresh CONFIRMED), so the protect-list move waits for a CONFIRMED verdict at the next re-score.)*
