...
- **Query grammar** (`parseQuery` in `search.js`): bare words are **OR** (broad by default suits a reference library, where a visitor rarely knows which words the author used), `"quoted"` is an **exact phrase**, and `AND` / `OR` in **uppercase** combine explicitly, with `AND` binding tighter. The query folds into OR-groups of AND-ed items, and a record qualifies when any one group matches in full. Uppercase-only operators keep a search for the word "and" working; smart quotes are accepted because that is what a paste carries; an unterminated quote runs to the end rather than failing, since it is nearly always a query mid-typing.
- **Both sides of a comparison are normalized** so punctuation collapses to a single space: without it a phrase search for `"soft delete"` would miss **Soft-Delete**, and this corpus is full of hyphenated terms (`database-per-service`, `dual-dispatch`). Dots survive, so `MMCA.Common.API` stays one token. Highlighting uses the same tolerance, or a matched phrase would fail to light up the text that matched it.
- Coverage dominates the ranking, so a record matching two of the query's words always outranks one matching a single word however strongly. The status line **echoes the parsed reading back** (`7 results · "soft delete" AND erasure`), which is the only documentation the operators get and the moment it is useful.
