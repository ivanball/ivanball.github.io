...
- **Bare words are OR, a quoted query is AND.** Broad by default suits a reference library, where a visitor rarely knows which words the author used; wrapping the query in double quotes (smart quotes included, since that is what a paste carries) requires every word. Coverage dominates the ranking, so in OR mode a record matching two words always outranks one matching a single word however strongly. The status line names the reading it used, which is how quoting stays discoverable.
