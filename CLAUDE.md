...
- **One record per H2 section**, not per page: a hit lands on the section that answers the question rather than on a 500 KB chapter. `MAX_SECTIONS_PER_DOC` still caps the long tail (`00-inventory.md` alone has hundreds of H2s), but capped sections **no longer vanish**: `foldSectionsIntoDocument()` folds their plain text and their identifiers into the DOCUMENT record, so the words stay findable and the reader lands on the document that contains them.
